...
## Current frontier — Step 40.0

Physical 0.0.170 closed Step 39 at 4/4: the exact real NGame hierarchy is admitted to the live SceneTree, automatic `_EnterTree`/`_Ready` established singleton/window authority, rendering was synchronously frozen, and final in-tree/state/native confinement passed. The active candidate is **0.0.171 / Step 40.0**.

Step 40 keeps the Step-39.1 compatibility image and inert GameStartupWrapper unchanged. It requires the same-process Step-39 4/4 retained hierarchy with rendering stopped, statically audits actual in-tree `_Process`, `_PhysicsProcess`, `_Draw`, and input callbacks using deferred/rejecting Cecil, then authorizes one short `StartRendering()` pulse targeted at 100 ms. The first continuation immediately invokes `StopRendering()`; observed elapsed must be <=500 ms, rendering must be inactive again, and NGame/state plus initializer/rejected/native confinement must remain intact. No in-process pulse retry is allowed. GameStartup/platform initialization, main-menu launch, ExecuteDeferred, Steam/native extensions, explicit `_ExitTree`, RemoveChild/Free, gameplay, and leaving rendering active remain later boundaries.
