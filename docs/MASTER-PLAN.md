## Current frontier — Steps 43–52 / 0.0.180

The active candidate is **0.0.180 / sequential startup ladder Steps 43–52**. Physical 0.0.175 closed Step 42 at 4/4. Physical 0.0.178 closed Step 45 4/4. Physical **0.0.179 then closed Steps 46.1 and 47 4/4, and proved exact real `NMainMenu` PackedScene instantiation succeeds once off-tree**. Step 48 stopped safely before SceneTree admission because its static lifecycle audit over-expanded conditional `_Ready()` branches into command-line multiplayer hosting, SaveManager fallback construction, and platform rich-presence paths. No AddChild, original LaunchMainMenu, Steam/native execution, or rendering occurred.

0.0.180 preserves the batched but independently gated cadence. Step 48.1 replaces that over-broad static assumption with concrete off-tree runtime guard rehearsal: empty Godot command line, exact already-created production SaveManager singleton/getter identity, exact Null-platform routing, no-drift `SetRichPresence`, and no-drift off-tree `CheckCommandLineArgs`. Only those exact rehearsed calls may be pruned from the lifecycle frontier; every other forbidden boundary remains fail-closed. Step 49 then attempts frozen direct menu admission, Step 50 runs the short bounded render pulse/refreeze, Step 51 maps the single-player button/open-submenu frontier without invocation, and Step 52 runs a longer 1.5-second bounded menu render residency/refreeze. Any failure stops the ladder and later rungs remain locked.
...
Physical 0.0.170 closed Step 39 at 4/4. Physical 0.0.171 then closed Step 40 at 4/4 on a successful second run: the real in-tree hierarchy passed its frame/input audit, the Godot renderer started, stopped again at 103.3 ms, and frozen NGame/state/native confinement held. That result advanced the project to Step 41; Step 41 and Step 42 have since physically closed 4/4, and the current frontier is the 0.0.180 Steps 43–52 guarded direct-main-menu ladder described at the top of this plan.
