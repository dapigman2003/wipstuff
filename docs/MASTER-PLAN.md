...
Physical Step 28.0.2 / 0.0.111 closed deterministic ahead-of-load managed transformation positively at 5/5: immutable source bytes were cloned, one exact Cecil semantic change was applied before CLR admission, source/transformed images were reopened and hash-verified, only transformed bytes entered the private context, both reflection and an in-fixture direct managed IL call observed 1000 / 1041 / 1041, and final OfflineReady/isolation passed. The active frontier therefore advances to exact real-StS2 target selection and then one narrowly audited semantic transformation based on current receipt-backed ARM64 IL evidence. Step 29 is a read-only evidence boundary: it regenerates the exact primary sts2.dll method/token/IL/target fingerprint first because the old physical Step-17 samples are not preserved in the repository; it performs no real-game write or CLR admission. Runtime Harmony detours are not to be revived merely to improve mod compatibility; Workshop/Harmony-mod compatibility is a later separate design problem and must not block getting the base game running. Keep real StS2 transformation, real member invocation, Godot/game startup, native game loading, and broad mod compatibility separately gated.
