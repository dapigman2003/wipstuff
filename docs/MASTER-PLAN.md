...
Physical 0.0.132 proved the game hard-kill interval begins when `NullPlatformUtilStrategy..ctor` triggers `CommandLineHelper.TryGetValue`; the same-run map exposed a +1 NP ordinal defect. Physical 0.0.133 corrected NP002 but the new CommandLine cctor instrumentation itself was CLR-invalid: no cctor marker executed, the returned Task faulted with nested `InvalidProgramException`, and the launcher reached normal `RUN_END`. Candidate 0.0.134 fixes this diagnostic MaxStack defect, adds stack-neutral critical markers around dictionary setup and `Godot.OS.GetCmdlineArgs`, and retains the full CL/CLTV sweep so one expensive device run can distinguish the suspected Godot boundary from later parser/dictionary work without broadening runtime authority. A diagnostic success still leaves Step 35 OPEN.
