...
Physical 0.0.132 proved the current diagnostic hard kill occurs after entering `NullPlatformUtilStrategy..ctor` and immediately after PRE for `CommandLineHelper.TryGetValue`, while the same-run exact-source map exposed a +1 dynamic ordinal defect. Candidate 0.0.133 repairs that defect and expands output-only localization into `CommandLineHelper..cctor` and `TryGetValue`, requiring physical coverage of the `Godot.OS.GetCmdlineArgs` call. The goal is to obtain enough evidence in one expensive device run to choose the next compatibility candidate without broadening runtime authority. A diagnostic success still leaves Step 35 OPEN.
