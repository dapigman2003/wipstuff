...
## Current frontier — Step 38.0.1

Physical 0.0.159 closes unchanged `ExecuteEssential()` at 4/4, and physical 0.0.161 closes controlled `game.tscn` admission at Step 37.0.1 4/4: the exact FMOD-neutral derivative loads as PackedScene and the real managed NGame hierarchy instantiates off-tree with zero resolver/native escape. The active frontier is 0.0.163 Step 38.0.1: read-only Cecil mapping of the exact NGame lifecycle, followed only if safe by one direct `NGame._EnterTree()` invocation on a fresh off-tree NGame instance. SceneTree insertion, `_Ready`, `_ExitTree`, GameStartup/platform initialization, main-menu launch, ExecuteDeferred, Steam/native extension loading, and gameplay remain later gates. See `docs/CURRENT-STATUS.md` and `docs/history/steps/STEP-38.0-CONTROLLED-NGAME-ENTERTREE.md`.
