...
Physical 0.0.136 entered `CommandLineHelper..cctor`, emitted `INMETHOD_CL_CRITICAL_001_PRE` immediately before `_args` dictionary construction, and hard-terminated before the matching POST. This is the first physical run to execute the stack-neutral CommandLine cctor instrumentation. The final `System.Collections.Concurrent 8 -> 9` binding remains contextual; the PRE/no-POST pair localizes the measured interval to `Godot.Collections.Dictionary<string,string>` construction before assignment.

0.0.137 preserves exact-source maps and every existing runtime prohibition, but the diagnostic derivative rewrites only the private `CommandLineHelper` string dictionary contract to `System.Collections.Generic.Dictionary<string,string>`: field type, constructor, `set_Item`, and `TryGetValue`. `Godot.OS.GetCmdlineArgs()` is intentionally left untouched so the next physical run can prove whether the frontier advances to that natural Godot API. The rewrite reuses the existing `System.Collections` AssemblyRef, preserves generic VAR MemberRef signatures, and is verified after serialization under rejecting resolution.

Acceptance remains evidence-only. A 4/4 result cannot close exact Step 35 because Gate B/C execute a diagnostic compatibility derivative. No Godot bootstrap, native game loading, resolver broadening, later initialization phase, game entry point, or runtime Harmony/MonoMod patching is authorized.
