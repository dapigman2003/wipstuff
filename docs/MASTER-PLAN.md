...
## Current frontier — Step 37.0.1

Physical 0.0.159 closes the full unchanged `ExecuteEssential()` boundary at 4/4. The next active frontier is 0.0.161 Step 37.0.1: sealed `game.tscn` extraction from the receipt-backed PCK, exact three-edit FMOD-neutral private derivative, PackedScene load, and off-tree NGame instantiation. SceneTree insertion, GameStartup/main-menu launch, ExecuteDeferred, Steam/native extension loading, and gameplay remain later gates. See `docs/CURRENT-STATUS.md` and `docs/history/steps/STEP-37.0-CONTROLLED-GAME-SCENE-ADMISSION.md`.
