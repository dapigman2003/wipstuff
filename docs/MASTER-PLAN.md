## Current frontier — Steps 43–52 / 0.0.181

The active candidate is **0.0.181 / sequential startup ladder Steps 43–52**, with current engineering changes intentionally limited to Steps 48–52. Physical 0.0.175 closed Step 42 at 4/4; 0.0.178 closed Step 45; 0.0.179 closed Steps 46.1 and 47 and proved exact real `NMainMenu` off-tree instantiation. Physical **0.0.180 then passed Step 48 Gate A/B, instantiated exact real `NMainMenu` once off-tree, and stopped safely during runtime guard rehearsal because `NGame.RootSceneContainer` was null**. No AddChild, original LaunchMainMenu, Steam/native execution, or rendering occurred.

0.0.181 corrects only this Steps-48–52 portion. Step 48.2 resolves exact retained `/Game/RootSceneContainer` child authority directly from the real NGame hierarchy and no longer requires the nullable property for off-tree tree-shape guards. Step 49 statically/token-audits exact `set_RootSceneContainer`; if and only if the property is null, its already one-shot frozen Gate C assigns the exact retained child, verifies zero drift, then performs the audited `AddChild(NMainMenu)`. A non-null mismatched property fails closed. Steps 50–52 retain the same short render pulse, non-invoking single-player frontier map, and 1.5-second sustained render residency/refreeze. Any failure stops the ladder and later rungs remain locked.
...
Physical 0.0.170 closed Step 39 at 4/4. Physical 0.0.171 then closed Step 40 at 4/4 on a successful second run: the real in-tree hierarchy passed its frame/input audit, the Godot renderer started, stopped again at 103.3 ms, and frozen NGame/state/native confinement held. That result advanced the project to Step 41; Step 41 and Step 42 have since physically closed 4/4, and the current frontier is the 0.0.181 Steps 43–52 guarded direct-main-menu ladder described at the top of this plan.
