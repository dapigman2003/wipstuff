...
The active candidate is **0.0.167 / Step 39.0.1**. Physical Codemagic 0.0.166 passed 1051/1051 static checks, 233/233 host tests, and Step-15 native-link preflight, then failed before publish/device execution because the new Step-39 UI partial omitted the `StS2Launcher.iOS.Platform` import required for `GodotStep15NativeBridge`. 0.0.167 is a compile-only correction: the runtime experiment remains one audited real `SceneTree.Root.AddChild(NGame)`, followed immediately by a render-loop freeze and frozen in-tree confinement. The verified inert GameStartupWrapper remains in force. `RemoveChild`/`Free`/explicit `_ExitTree`, render restart, GameStartup/platform initialization, main-menu launch, ExecuteDeferred, Steam/native extension loading, and gameplay remain later boundaries. See `docs/CURRENT-STATUS.md` and `docs/history/steps/STEP-39.0.1-IOS-NAMESPACE-COMPILE-CORRECTION.md`.
