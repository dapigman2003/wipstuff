## Current frontier — Steps 53–57 / 0.0.185

The active candidate is **0.0.185 / single-player submenu continuation through Step 57**. Physical runtime authority is closed through Step 52. The existing fresh-process one-button route remains capped at Step 52 and reconstructs that known same-process authority without changing any closed gate.

0.0.185 opens only the next narrow UI layer. Step 53 isolates exact zero-arg `NMainMenu.OpenSingleplayerSubmenu()` returning `NSingleplayerSubmenu` and requires its immediate execution frontier admissible without invocation. Step 54 token-matches and invokes only that exact method once while frozen, requiring the real retained `NSingleplayerSubmenu` to become visible/in-tree with zero drift. Step 55 audits that actual subtree and runs one bounded 750 ms render residency with synchronous refreeze. Steps 56–57 keep character select non-invoking: they map exact `NSingleplayerSubmenu.OpenCharacterSelect()`, discover its resource literals, and read-only-preflight one exact character-select TSCN entry from the trusted PCK. Step 58 and character-select execution remain unopened.
...
Physical 0.0.170 closed Step 39 at 4/4. Physical 0.0.171 then closed Step 40 at 4/4 on a successful second run: the real in-tree hierarchy passed its frame/input audit, the Godot renderer started, stopped again at 103.3 ms, and frozen NGame/state/native confinement held. That result advanced the project to Step 41; Step 41 and Step 42 have since physically closed 4/4, and physical authority has since advanced through Step 52, and the active 0.0.185 Steps 53–57 single-player submenu continuation is described at the top of this plan.
