...
## Current frontier — Step 39.0

Physical 0.0.159 closes unchanged `ExecuteEssential()` at 4/4; physical 0.0.161 closes controlled `game.tscn` admission at Step 37.0.1 4/4; and physical 0.0.165 closes the iOS/off-tree-compatible `NGame._EnterTree()` boundary at Step 38.2 4/4 with state/resolver/native confinement intact. A targeted PCK preflight then showed the audio-manager GDScript proxy has no automatic lifecycle callback identifier and the reaction-wheel/timeout nested scenes introduce no FMOD/Spine/Sentry/Steam/GDExtension node.

The active candidate is **0.0.170 / Step 39.2**. Physical 0.0.169 passed the actual-hierarchy Gate-B lifecycle audit with zero forbidden/unresolved references and then physically completed the first real `SceneTree.Root.AddChild(NGame)`. Gate C returned with the real game hierarchy in-tree, exact `NGame.Instance`, non-null `_window`, exact SceneTree-root parent, OneTimeInitialization state 2, and zero rejected/initializer/native escape; the launcher then synchronously stopped rendering. Gate D failed only in launcher reflection bookkeeping because `Single(...)` matched more than one zero-argument `GetParent` method shape. 0.0.170 changes only that Gate-D selector to require the non-generic, closed, root-compatible overload. The verified Step-39.1 compatibility image, inert GameStartupWrapper, Gate-B fail-closed policy, one-AddChild boundary, and immediate render freeze remain unchanged. `RemoveChild`/`Free`/explicit `_ExitTree`, render restart, GameStartup/platform initialization, main-menu launch, ExecuteDeferred, Steam/native extension loading, and gameplay remain later boundaries.