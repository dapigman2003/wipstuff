## Current frontier — Steps 53–57 / 0.0.187

The active candidate is **0.0.187 / single-player submenu continuation through Step 57**. Physical runtime authority is now closed through Step 55; the existing fresh-process one-button route intentionally remains capped at Step 52 and reconstructs the stable baseline before Steps 53–55 are rerun manually in the same process.

Physical 0.0.186 closed Steps 53–55 in sequence and then reached Step 56 Gate B, where the old zero-argument signature assumption failed safely before any character-select invocation. 0.0.187 keeps Steps 53–55 unchanged and corrects Step 56 to the exact `void NSingleplayerSubmenu.OpenCharacterSelect(MegaCrit.Sts2.Core.Nodes.GodotExtensions.NButton)` callback signature. Step 56 still maps only IL, the immediate/deferred frontier, and resource literals; Step 57 still read-only-preflights one exact character-select TSCN entry from the trusted PCK. Step 58 and character-select execution remain unopened.
...
Physical 0.0.170 closed Step 39 at 4/4. Physical 0.0.171 then closed Step 40 at 4/4 on a successful second run: the real in-tree hierarchy passed its frame/input audit, the Godot renderer started, stopped again at 103.3 ms, and frozen NGame/state/native confinement held. That result advanced the project to Step 41; Step 41 and Step 42 have since physically closed 4/4, and physical authority has since advanced through Step 52, and the active 0.0.187 Steps 53–57 single-player submenu continuation is described at the top of this plan.
