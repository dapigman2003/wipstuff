## Current frontier — Steps 58–62 / 0.0.194

The active candidate is **0.0.194 / game-owned submenu-stack repair + real character-select ownership and visible-render trial through Step 62**. Physical runtime authority is closed through Step 57 4/4/frozen. Physical 0.0.191 further proved that the real cached `NCharacterSelectScreen` can already be inside the SceneTree before Step 58 performs a new operation. The fresh-process convenience route remains capped at Step 52; Steps 53–57 must be reconstructed manually before the new block.

Physical 0.0.192 proved SceneTree attachment can precede logical stack binding. Physical 0.0.193 then reached the original character-select handler and exposed the skipped logical navigation lifecycle: the hidden/in-tree cache was present, but the earlier directly opened `NSingleplayerSubmenu` had not been proven pushed/bound to the retained stack.

The architecture still avoids reproducing the game's character-select internals. Step 58 audits actual runtime ownership. Step 59 conditionally restores the missing game-owned single-player navigation state using exact audited `NSubmenuStack.Push(NSubmenu)`—never a direct `_stack` write—then runs original `OpenCharacterSelect(NButton)` with the retained real `_standardButton`. Step 60 audits the resulting live frame/input surface. Steps 61 and 62 visibly render the real character-select screen for 2 seconds and 10 seconds respectively, synchronously refreezing after each trial. Character choice/embark/run-start and Step 63+ remain unopened.
