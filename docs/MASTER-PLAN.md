...
## Current frontier — Step 38.2

Physical 0.0.159 closes unchanged `ExecuteEssential()` at 4/4, and physical 0.0.161 closes controlled `game.tscn` admission at Step 37.0.1 4/4: the exact FMOD-neutral derivative loads as PackedScene and the real managed NGame hierarchy instantiates off-tree with zero resolver/native escape. Physical 0.0.163 proves exact `NGame._EnterTree()` directly reaches `GameStartupWrapper`; 0.0.164 then verifies the inert `Task.CompletedTask` wrapper, re-instantiates the real NGame off-tree, physically enters `_EnterTree`, and fails with a managed NullReferenceException while `IsInsideTree=false`. The exact map isolates two remaining environment-sensitive edges: SentryService.Initialize and the GetWindow/FilesDropped/Connect block. The active frontier is 0.0.165 Step 38.2: preserve the ModelDb + inert-wrapper derivative, NOP only those two exact off-tree/iOS edges under serialized instruction-count/NOP-delta verification, then invoke the remaining `_EnterTree` body once. SceneTree insertion, `_Ready`, `_ExitTree`, GameStartup/platform initialization, main-menu launch, ExecuteDeferred, Steam/native extension loading, and gameplay remain later gates. See `docs/CURRENT-STATUS.md` and `docs/history/steps/STEP-38.2-IOS-OFFTREE-ENTERTREE-COMPATIBILITY.md`.
