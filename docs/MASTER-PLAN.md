...
Physical 0.0.133 corrected NP002 but the new CommandLine cctor instrumentation itself was CLR-invalid: no cctor marker executed, the returned Task faulted with nested `InvalidProgramException`, and the launcher reached normal `RUN_END`. Candidate 0.0.135 fixes this diagnostic MaxStack defect, adds stack-neutral critical markers around dictionary setup and `Godot.OS.GetCmdlineArgs`, and retains the full CL/CLTV sweep so one expensive device run can distinguish the suspected Godot boundary from later parser/dictionary work without broadening runtime authority. A diagnostic success still leaves Step 35 OPEN.
