...
Physical 0.0.152 closes the exact Step-35 **core/runtime authority** positively under the explicitly defined source-built Godot 4.5.1 bridge prerequisite: exact transformed sts2 + exact prepared GodotSharp, exact ExecuteVeryEarly RanToCompletion, clean resolver/native confinement, OfflineReady 428/428, and final Gate-D result constructed as passed=true/exactAuthority=true. Physical 0.0.153 further proves the outer worker returns the already-passed Gate-D result at `D_WORKER_RETURN`; the remaining defect is the captured UIKit continuation itself. 0.0.154 bypasses that capture with `ConfigureAwait(false)` and explicit main-queue finalization without changing the core boundary.
