## Current frontier — Steps 53–57 / 0.0.188

The active candidate is **0.0.188 / single-player submenu continuation through Step 57**. Physical runtime authority is now closed through Step 56; the existing fresh-process one-button route intentionally remains capped at Step 52 and reconstructs the stable baseline before Steps 53–56 are rerun manually in the same process.

Physical 0.0.187 closed Step 56 non-invoking and entered Step 57, where Gate A failed safely with `observed=none` before PCK extraction because 0.0.187 retained only full `res://...*.tscn` literals. 0.0.188 keeps Steps 53–56 otherwise unchanged, retains the exact observed managed hint `screens/character_select_screen`, canonicalizes only that supported identity to `res://scenes/screens/character_select_screen.tscn`, and requires exactly one matching receipt-backed PCK directory entry before Step 57 reads bytes. Step 58 and character-select execution remain unopened.
