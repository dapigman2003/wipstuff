...
## Current frontier — Step 38.1

Physical 0.0.159 closes unchanged `ExecuteEssential()` at 4/4, and physical 0.0.161 closes controlled `game.tscn` admission at Step 37.0.1 4/4: the exact FMOD-neutral derivative loads as PackedScene and the real managed NGame hierarchy instantiates off-tree with zero resolver/native escape. Physical 0.0.163 then proves exact `NGame._EnterTree()` directly reaches `GameStartupWrapper` and stops safely before lifecycle execution. The active frontier is 0.0.164 Step 38.1: extend the existing ahead-of-load selected derivative by making only `GameStartupWrapper` return `Task.CompletedTask`, verify the serialized two-instruction body and unchanged `_EnterTree` edge, then invoke exact `_EnterTree` once on a fresh off-tree NGame. SceneTree insertion, `_Ready`, `_ExitTree`, GameStartup/platform initialization, main-menu launch, ExecuteDeferred, Steam/native extension loading, and gameplay remain later gates. See `docs/CURRENT-STATUS.md` and `docs/history/steps/STEP-38.1-INERT-GAMESTARTUPWRAPPER-COMPATIBILITY.md`.
