# 0.0.204 active frontier

**Active candidate — 0.0.204 current ownership + PropertyTweener managed-wrapper compatibility.** Physical history through Step 57 remains retained, while Step 58 starts directly from Step 52 and legacy Steps 53–57 remain optional. Physical 0.0.203 proves 59U `UpdateControllerButton` returns, 59T reaches `Tween.TweenProperty` after successful visual/tween setup and observes a null return, and 59E reproduces the same `NConfirmButton.OnEnable` NRE outside `OpenCharacterSelect`. 0.0.204 preserves the proven `PackedScene.Instantiate(GenEditState)` unique-name compatibility and specializes only the one-callsite GodotSharp NativeCalls helper used by `TweenProperty`: normal managed returns pass through, native null stays null, and managed-null/nonzero-native-pointer constructs the exact internal `PropertyTweener(IntPtr)` wrapper with runtime fallback telemetry. The same IPA retains 59D/59R/59H/59U/59T/59E and the real Step 59 transition. Step 63 remains unopened.
...
The numbered ladder remains the development/diagnostic harness, but it is not the intended product runtime. The long-term launcher should preserve original StS2 lifecycle ownership and use deterministic ahead-of-load compatibility transforms only at genuinely incompatible iOS/desktop boundaries. The preferred product direction is a verified compatibility profile plus a compatibility-transformed normal `GameStartup` / `LaunchMainMenu` path, followed by continuous Godot rendering/input ownership.

The current Step-59 fault analysis is being used to determine exactly where the current piecemeal startup diverges from normal StS2 lifecycle. The next broader startup experiment must run as an alternative fresh-process startup mode, not on top of already-executed one-time initialization/save steps. Original startup operations are classified as KEEP, SUBSTITUTE, DEFER, or BLOCK; in particular, normal menu ownership and `RootSceneContainer.SetCurrentScene(...)` should be restored when evidence shows they are needed, while desktop-native Steam/stat/platform work stays substituted or deferred. Runtime Harmony/MonoMod replacement remains retired; deterministic ahead-of-load Mono.Cecil transformation remains the behavior-changing mechanism.

Resume from `docs/CURRENT-STATUS.md`, enforce `docs/REGRESSION-CONTRACTS.md`, and use `docs/history/INDEX.md` for provenance.
...
Physical 0.0.152 closes the exact Step-35 **core/runtime authority** positively under the explicitly defined source-built Godot 4.5.1 bridge prerequisite: exact transformed sts2 + exact prepared GodotSharp, exact ExecuteVeryEarly RanToCompletion, clean resolver/native confinement, OfflineReady 428/428, and final Gate-D result constructed as passed=true/exactAuthority=true. Physical 0.0.153 further proves the outer worker returns the already-passed Gate-D result at `D_WORKER_RETURN`; the remaining defect is the captured UIKit continuation itself. 0.0.155 bypasses that capture with `ConfigureAwait(false)` and explicit main-queue finalization without changing the core boundary.
...
Physical 0.0.170 closed Step 39 at 4/4. Physical 0.0.171 then closed Step 40 at 4/4 on a successful second run: the real in-tree hierarchy passed its frame/input audit, the Godot renderer started, stopped again at 103.3 ms, and frozen NGame/state/native confinement held. That result advanced the project to Step 41; Step 41 and Step 42 have since physically closed 4/4, and physical authority has since advanced through Step 52, and the active 0.0.189 Steps 53–57 single-player submenu continuation is described at the top of this plan.
