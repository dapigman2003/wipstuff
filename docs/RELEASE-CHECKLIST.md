# Release Checklist — Step 27.0.13
...
- Codemagic 0.0.96 compiled successfully and ran 211 host tests: 209 passed, 2 failed because production-only HarmonySharedState normalization was applied to randomized minimal synthetic fixtures before their intended A–N replay.
- The public constructor must remain pinned to exact `0Harmony` / `2.4.2.0`; only that canonical target may enter the production HarmonySharedState normalization path.
- Internal non-canonical synthetic targets must retain byte-identical runtime bytes and must not be forced through the full exact real-Harmony patch-engine fingerprint audit.
- The canonical production path must still require the exact original Harmony 2.4.2 patch-engine metadata fingerprint and a byte-distinct runtime image.
...
- Gate B may use retained memory bytes only for the exact admitted target identity; the production target must be the normalized image while internal synthetic replay remains byte-identical.
...
- top launcher banner: **Step 27.0.13**, bundle-derived **Version 0.0.97**, current iOS HarmonySharedState normalization status.
...
- Interpret the final checkpoint conservatively: T5a means the normalized production image was reverified; T5b means its cctor was entered; T6 means that cctor returned and direct-state validation began; T7 means public `PatchProcessor.Patch()` was entered; T8/T9 mean Patch returned / Gate-T validation advanced.
