...
Current candidate: **Step 35.0.31 / Step 36.0 — 0.0.155 (155)**.

Physical 0.0.153 proves the exact Step-35 Gate-D PASS result also returns through the outer worker at `D_WORKER_RETURN`. The remaining defect is the captured UIKit continuation itself; 0.0.155 bypasses it with `ConfigureAwait(false)` and explicit main-thread finalization without changing core Step-35 behavior.
