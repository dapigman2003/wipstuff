...
Current candidate: **Step 35.0.13 / 0.0.136 (136)**. Step 35 remains OPEN.
...
Physical 0.0.133 corrected the ordinal to NP002 but its new CommandLine cctor sweep was rejected with managed `InvalidProgramException` before any cctor marker and the launcher reached normal `RUN_END`; that is a diagnostic MaxStack defect. 0.0.136 reserves/verifies MaxStack and adds stack-neutral critical boundaries around dictionary setup and `Godot.OS.GetCmdlineArgs`.
