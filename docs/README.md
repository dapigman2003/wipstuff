...
Current candidate: **Step 35.0.19 / 0.0.142 (142)**. Step 35 remains OPEN.
...
Step 35.0.19 / 0.0.142 preserves NATURAL / OS-RECON / FORWARD and adds a separately gated CORE-HANDOFF mode. CORE-HANDOFF requires the already-live Step-15 smoke engine, rejects `dotnet` feature / competing Godot-managed-runtime state, obtains the exact source-built Godot 4.5.1 runtime callback table, initializes only the verified private GodotSharp derivative with it, and then measures the natural ExecuteVeryEarly path.
