# Testing — Steps 43–47 startup ladder / 0.0.177

Active candidate: `0.0.177 (177)`, IPA `StS2-Launcher-Steps-43-47.ipa`, workflow `ios-canonical`.
...
Step 45 requirements: Step44 4/4/no legacy data; require exact static `_mockInstance` and `_instance` fields; verify serialized `get_Instance()` uses `_mockInstance -> _instance -> ConstructDefault` with the expected 2/2/1/1 load/store/call counts; require runtime `_mockInstance == null` and existing `_instance != null` without invoking the getter/fallback; map `InitProfileId(Nullable<int>)`, `InitProgressData()`, `InitPrefsData()` and write the map before Gate C; one-shot invoke `InitProfileId(null) -> InitProgressData -> InitPrefsData`; never retry after arm; require SaveManager `SettingsSave`, `PrefsSave`, `Progress` non-null plus state/context/native confinement.
