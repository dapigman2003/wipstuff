...
2. **59** — if the exact screen is already visible/in-tree, adopt it with no handler call. Otherwise arm and invoke the exact real `OpenCharacterSelect(NButton)` once while frozen, then require the exact cached screen visible/in-tree under the retained stack.
...
Never retry Step **59** after its handler invocation is armed, or Steps **61/62** after their render boundary is armed. Step 58/60 keep rendering frozen. Steps 61/62 must call `StopRendering()` before post-stop telemetry/file I/O.

The original real `OpenCharacterSelect(NButton)` handler is permitted **only in Step 59 and only when the observed screen is not already visible/in-tree**. Its selected IL must prove the `NButton` parameter is unused before the launcher may pass null. No character choice, confirm, embark, run-start, or Step 63 behavior is authorized.
