...
For Step 28.0.1 / `0.0.110 (110)`, validation preserves all protected earlier runtime/build policies while retaining the Step-28 architecture invariants:
...
Expected local static result for 0.0.110: **850/850 PASS**.
...
0.0.110 makes only the established callback-adapter correction and adds the static regression guard. Gate ordering and semantic acceptance are unchanged.
...
Codemagic must pass, in order: canonical static validation, Core/test compilation, the complete host suite, iOS publish, and IPA verification. Then install `0.0.110 (110)` from a fresh process.
