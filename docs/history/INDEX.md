...
- `reports/STEP-35.0.6-PHYSICAL-DIAGNOSTIC-BRIDGE-MEMBERREF-FAILURE.txt` — physical 0.0.129 proves the deferred Cecil writer fix through Gate A/B and reaches armed Gate C, then returns managed MissingMethodException on the synthetic Action<string>.Invoke(string) bridge MemberRef before any INMETHOD marker.
- `steps/STEP-35.0.7-GENERIC-DELEGATE-MEMBERREF-CORRECTION.md` — 0.0.130 corrects only the diagnostic bridge MemberRef to Action<string>::Invoke(!0) and requires post-write rejecting-resolver verification of the VAR(0) signature.
- `reports/STEP-35.0.7-PHYSICAL-SAVEMANAGER-LOCALIZATION-0.0.130.txt` — physical 0.0.130 proves the `Action<string>::Invoke(!0)` bridge correction and localizes the hard termination under `SaveManager.get_Instance`, before settings initialization; no matching `.ips` is available.
...
- `reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-CHECKPOINT.txt` — physical 0.0.179 Step 48 run: Steps 46.1/47 retained, exact off-tree NMainMenu instantiated, lifecycle audit stopped before AddChild/rendering on conditional platform/Steam branches.
