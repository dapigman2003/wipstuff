...
Physical 0.0.86 stopped at Gate O, 14/26, before AccessTools execution or patching. The candidate's 56-instruction model was one instruction short. The phone showed 57 instructions and exactly one additional `ldc.i4.1`. Step 27.0.3 initially attributed that constant to the first `Type.GetType("System.Runtime.InteropServices.RuntimeInformation", bool)` probe; physical 0.0.87 later disproved that attribution and established that both probes use `false`, while the `ldc.i4.1` belongs to `ReaderWriterLockSlim(LockRecursionPolicy.SupportsRecursion)`. The raw 0.0.86 result remains preserved at `docs/history/reports/STEP-27.0.2-PHYSICAL-GATE-O-REPORT.txt`.
