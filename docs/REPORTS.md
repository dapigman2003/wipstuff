...
Physical 0.0.198 proves SceneState=true -> fresh off-tree instanced-subscene root UniqueNameInOwner=false while ordinary nodes preserve true. 0.0.199 moves the correction to a global private GodotSharp PackedScene.Instantiate compatibility hook; 0.0.200 physically proves the corrected roots remain true and localizes the next one-shot failure to `NConfirmButton.OnEnable()`. Physical 0.0.201 proves the retained embark Ready products remain populated and `_isEnabled` flips true while the real transition binds the exact submenu stack and creates a one-player lobby before the same failure. 0.0.202 adds internal `STEP59TRACE_*` callsite breadcrumbs plus reusable 59R/59U/59E branches so multiple isolating phone runs can reuse one compilation.
