...
Physical 0.0.198 proves SceneState=true -> fresh off-tree instanced-subscene root UniqueNameInOwner=false while ordinary nodes preserve true. 0.0.199 moves the correction to a global private GodotSharp PackedScene.Instantiate compatibility hook; 0.0.200 physically proves the corrected roots remain true and then localizes the next one-shot transition failure to `NConfirmButton.OnEnable()`. 0.0.201 keeps the PackedScene runtime behavior unchanged and adds a compilation-efficient retained-embark `NConfirmButton` diagnostic platform: Step 59D captures Gates A+B only with durable IL/full-field/peer/null-candidate/frontier/node/SceneState/TSCN evidence and no handler arm; the separate real-handler run from the same IPA automatically refreshes the static map with a post-failure runtime deck if the original transition still throws.

## 0.0.203 current-ownership / Step-59 micro-probe evidence contract

0.0.203 starts the active character-select continuation from Step 52, not from mandatory replay of legacy Steps 53–57. Step 58 Gate A records whether it adopted an existing `NSingleplayerSubmenu` or invoked exact game-owned `OpenSingleplayerSubmenu()` once to materialize/activate ownership. Rendering remains frozen and `OpenCharacterSelect` is not invoked in Step 58.
