...
## Active candidate — Step 38.0.1 / 0.0.163 (163)
...
Physical **0.0.162** reached Step 38 Gate A and stopped safely before lifecycle execution: Cecil's immediate/default metadata reader attempted to resolve `GodotSharp, Version=4.5.1.0`. The run ended normally. This is a static-audit reader defect, not an `_EnterTree` runtime result.

**0.0.163 / Step 38.0.1** corrects only that reader to `ReadingMode.Deferred` with the existing fail-closed `RejectingAssemblyResolver` and an explicitly bound `MetadataResolver`; Gate A now also requires zero external resolution requests.

**0.0.163 / Step 38.0.1** advances only `NGame._EnterTree`. Before execution, Gate A Cecil-maps `_EnterTree`, `_Ready`, `GameStartupWrapper`, `GameStartup`, `InitializePlatform`, `LaunchMainMenu`, and `LoadDeferredStartupAssetsAsync` from the exact selected compatibility image and rejects a same-`NGame` `_EnterTree` call closure that reaches later startup/deferred or OneTimeInitialization re-entry.
...
## Physical sequence for 0.0.163

1. Fresh process → Step 15 A-C.
2. Step 35.0.32 MODEL-BOOTSTRAP → 4/4.
3. Step 36.0.5 → 4/4.
4. Step 37.0.1 → 4/4.
5. Step 38.0.1 A-D once. Do not retry in-process after Gate C starts.
