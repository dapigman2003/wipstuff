...
Codemagic is now split for the individual free M2 tier: run `step32-fast` first (static + complete host suite only), then `ios-step-32` **only on the exact same commit** if fast passes. The device workflow does not repeat the full host suite; it performs static validation, iOS workload/publish and IPA verification. Both workflows report phase timings/cache sizes.

Physical close remains Gates A–D **4/4 PASS**. Preserve `Step32-RealStS2PrepareMethodRewrite.txt`.
