# Current Status — Step 32.0.5 Stable Transformed Method Verification

## Active candidate — Step 32.0.5 / 0.0.120 (120)

Physical baseline summary: Steps 01–26 closed; Step 27 CLOSED NEGATIVE; Step 28 CLOSED POSITIVE 5/5; Step 29 CLOSED POSITIVE 4/4; Step 30 CLOSED POSITIVE 4/4; Step 31 CLOSED POSITIVE 4/4. Step 32 remains **OPEN**, with the physical frontier now **2/4**.

Physical Step 31.0 / 0.0.114 is **CLOSED POSITIVE — 4/4**. The exact receipt-backed `MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::PrewarmJit()` method, source token `0x06007D05`, body SHA-256 `7f25b7bd955c407fc69306cf26af2162223353f5606560458066aed085e72ab9`, and all ten `RuntimeHelpers.PrepareMethod` sites were rebound without writes or CLR admission.
...
Physical 0.0.116 exposed exact `System.Runtime 9.0.0.0` Cecil Constant-table write resolution. Physical **0.0.117** then re-proved Gate A and failed closed before mutation on exact `Sentry 5.0.0.0`. The exact receipt-backed DLL was subsequently audited statically; `docs/history/reports/STEP-32-STATIC-STS2-CONSTANT-METADATA-AUDIT.txt` proves 3,059 Constant rows and exactly three non-null external type/storage requirements under the active scanner rule: exact System.Runtime 9.0.0.0 / `System.Reflection.BindingFlags` / `Int32`; exact Sentry 5.0.0.0 / `Sentry.BreadcrumbLevel` / `Int32`; exact Sentry 5.0.0.0 / `Sentry.SentryLevel` / `Int16`. Null-only GodotSharp/System.Collections scopes remain unauthorized.

Step 32.0.3 / **0.0.118** was maintenance-only and removed retired Step-25–27 Harmony active weight. The user has confirmed 0.0.118 works in Codemagic, establishing the proven lean baseline. Step 32.0.4 / **0.0.119** then added only the exact audited multi-scope in-memory Cecil writer surrogates.

Physical **0.0.119** advanced Step 32 to **2/4**. Gate A passed with OfflineReady **428/428**, exact source SHA-1 `e424ace9399a82edea4dd7e0fa5761635dfd6c5d`, SHA-256 `e7ceb80669bfaf5c8fccabaa126ae2bb283aba514be5b5b55612579cfd285f18`, MVID `518e4758-52d7-47c2-b776-471a0e29e49d`, source token `0x06007D05`, 10/10 sites, zero read-time resolution, zero CLR admission, and no trusted-install modification. Gate B **PASS** then completed the first real-StS2 private semantic Cecil write: 6/6 + 4/4 replacements, expected transformed semantic fingerprint `47fadf2a46eda098f310b7d0ee54e37d1e952ac272fc966d16d557ed46a0b74a`, constant-metadata semantic fingerprint `945f87ca177cabe587a3c8d6eef3b0ef419cbe4327349c0ffc7c541a4652ad37`, exactly 3 audited synthetic metadata types, **9** write-time resolution requests confined to exact System.Runtime/Sentry, zero external framework/game assembly bytes opened, transformed SHA-256 `39c0a89ad0d5c6eb1553e23dd8537a7b7ab8278fad4115d186db5751570211ef`, and no source/trusted mutation or CLR load. Preserve `docs/history/reports/STEP-32.0.4-PHYSICAL-GATE-C-TRANSFORMED-METHOD-IDENTITY-FAILURE.txt`.

Gate C on 0.0.119 failed immediately after transformed-module reopen at `Step-32 transformed PrewarmJit method identity/body drifted.` The old verifier looked up the transformed method by the physical Step-31 **source MethodDef token** `0x06007D05` and only then checked its full name/body. Because that condition failed before PrepareMethod count, semantic fingerprint, Constant-table fingerprint, instruction/EH shape, or Pop-delta verification, the physical report does **not** show that the rewrite semantics drifted; it shows that source-token identity is not a proven post-Cecil-write locator.

Step 32.0.5 / **0.0.120** changes only Gate C's post-write locator. Gate A/B still bind the exact source token/body/sites. Gate C now requires exactly one reopened method matching the exact declaring type `MegaCrit.Sts2.Core.Helpers.OneTimeInitialization` plus full signature `System.Void MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::PrewarmJit()`, then runs the existing stronger semantic/metadata checks. It reports the transformed MethodDef token, whether the old source token survived serialization, and the old-token occupant diagnostically. Token preservation is not treated as transformed semantic identity. The 6+4 rewrite, exact System.Runtime/Sentry resolver authority, no-CLR-load boundary, and Gate-D isolation contract are unchanged.

Physical close condition remains Gates A–D **4/4 PASS** on one exact candidate. First require Codemagic static validation, full host suite, iOS publish, and IPA verification for 0.0.120. Then perform a fresh-process physical run and preserve `Step32-RealStS2PrepareMethodRewrite.txt`. Step 33 remains unauthorized until Step 32 closes 4/4.
