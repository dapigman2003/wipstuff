...
**Current correction:** Step 18.2 / `0.0.49 (49)` replaces filename-derived dependency lookup with a SHA-1-verified workspace assembly-identity catalog and explicitly binds Cecil assembly + metadata resolution to that catalog after the repeated physical Gate B `GodotSharp` failure.
...
**Current source candidate:** Step 18.2 — Real Assembly Rewrite Workspace identity-resolver correction.

- App version: `0.0.49 (49)`
- Codemagic workflow: `ios-step-18-2`
...
**Step 18.2 does not apply a behaviorally significant compatibility fix and does not load or execute StS2 assemblies. Cecil writer-required resolution is restricted to the SHA-1-verified launcher-private Step 18 workspace; runtime/system/live-install/network fallback is forbidden.**
