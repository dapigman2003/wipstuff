# Current Status — Step 35.0.1 Very-Early B→C Hard-Termination Crash Localization
...
Physical Step 32.0.5 / **0.0.120** closed the first real-StS2 semantic rewrite at 4/4. The exact transformed image is 9,304,576 bytes, SHA-256 `39c0a89ad0d5c6eb1553e23dd8537a7b7ab8278fad4115d186db5751570211ef`, MVID `518e4758-52d7-47c2-b776-471a0e29e49d`, transformed PrewarmJit semantic fingerprint `47fadf2a46eda098f310b7d0ee54e37d1e952ac272fc966d16d557ed46a0b74a`, transformed MethodDef token `0x0600AFEA`, and zero remaining `RuntimeHelpers.PrepareMethod` references.

Physical Step 33.0 / **0.0.121** closed transformed-primary CLR admission at 4/4. Only exact transformed primary bytes entered `StS2Launcher-Step33-TransformedGame`; admission produced zero managed/private/native resolution and no game-member reflection/invocation.

Physical Step 34.0 / **0.0.122** closed exact transformed-site execution at **4/4 PASS**. `MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::PrewarmJit()` token `0x0600AFEA` was invoked exactly once and returned normally. Binding/invocation produced 8 managed resolver requests: 6 exact host-framework resolutions and 2 hash-pinned initializer-free private dependency loads. There were zero initializer-bearing requests, zero unplanned managed loads, and zero native resolutions. Preserve `docs/history/reports/STEP-34.0-PHYSICAL-CLOSURE-4OF4.txt`.

### Physical Step 35.0 / 0.0.123 observation

The first physical Step-35 candidate was run on iPhone. The app opened, advanced to the visible Gate-B region, then hard-terminated. No `Step35-TransformedRealStS2VeryEarlyInitialization.txt` survived. The supplied matching iOS `.ips` identifies app version/build **0.0.123 (123)** and records `EXC_BAD_ACCESS / SIGKILL`, faulting **main thread**, program counter **0x0**, with `CODESIGNING / Invalid Page` termination text. The raw `.ips` is not stored in source because it can contain device identifiers; a sanitized summary is preserved in `docs/history/reports/STEP-35.0-PHYSICAL-HARD-TERMINATION-SUMMARY.txt`.

This observation does **not** yet prove Gate B itself crashed. Gate B runs on `Task.Run`, whereas the crash report faults the main thread. After the Gate-B await resumes, UIKit can still visibly show “Gate B” while Gate C has already synchronously entered `Assembly.GetType`, `Type.GetMethod`, signature/token/MVID checks, or `MethodInfo.Invoke` before the first incomplete async await allows a repaint. Step 35 therefore freezes the compatibility boundary and localizes B→C before changing behavior.

A separate physical check—run Step 34 and then Step 35 without force-quitting—causes Step 35 Gate A to fail normally rather than hard-crash. That is **expected**: Step 34 leaves `sts2` resident in a non-collectible private ALC, and Step 35 intentionally rejects any non-fresh process.

### Step 35.0.1 / 0.0.124 diagnostic candidate

Step 35.0.1 preserves the exact 0.0.123 compatibility experiment and adds **output-only, synchronously flushed crash checkpoints**. It does not change the target, transformed bytes, resolver authority, invocation count, Task timeout, or forbidden boundaries.

Gate A still re-runs Step 32 A–D, pins source/transformed hashes and MVID, binds source `ExecuteVeryEarly` token `0x06007D02` and source async `MoveNext` token `0x0600BC71`, proves source/transformed `ExecuteVeryEarly` wrapper and async `MoveNext` semantic fingerprints match, requires zero direct calls from that `MoveNext` to `ExecuteEssential`, `ExecuteDeferred`, or `PrewarmJit`, requires zero direct Harmony method references, and requalifies the prepared binding plan.

Gate B still admits only the exact transformed primary into `StS2Launcher-Step35-VeryEarly` and requires Step-33 zero-resolution admission behavior. New checkpoints bracket fresh-process validation, hash rechecks, ALC construction, primary re-hash, `LoadFromStream`, context ownership, identity, MVID, resolver-state checks, global residency, and private-context enumeration.

Gate C still reflects exact transformed `ExecuteVeryEarly()`, requires static/parameterless/Task + preflight token/MVID, invokes it exactly once, and awaits that exact Task for at most 60 seconds. New checkpoints bracket `Assembly.GetType`, `Type.GetMethod`, signature/token/MVID validation, `C_INVOKE_START`, `C_INVOKE_RETURNED`, Task confirmation, Task wait, post-await confinement, and managed/private/native resolver callbacks.

`Documents/StS2Launcher/Reports/Step35-CrashCheckpoint.txt` carries source-pinned release provenance, UTC, process ID, managed thread ID, and the durable frontier. A checkpoint write failure is diagnostic-only and must not alter resolver or compatibility decisions. If the process hard-terminates, preserve the file **before rerunning**.

Cancellation semantics are now explicit: **CANCELLED = INCONCLUSIVE**, not PASS or compatibility FAIL. Once Gate B starts, the process is spent and must be force-quit before retry. Once Gate C invocation begins, cancellation cannot undo any code already executed.

Step 35 still does **not** authorize CLR admission of the receipt-backed/prepared original `sts2.dll`, intentional launcher invocation of `ExecuteEssential`, `ExecuteDeferred`, `PrewarmJit`, the game entry point, `0Harmony` initialization, Harmony/MonoMod patching, Godot/game startup, native game loading, arbitrary resolver fallback, or broad startup sequencing. Physical close condition remains ordered Gates A–D **4/4 PASS** after Codemagic static validation, full host suite, iOS publish, and IPA verification.
