...
- strict managed-plan resolution and native-load refusal remain in force while crossing controlled initialization; Step 25 physically proved the exact measured Harmony API/type-initializer surface and one inert Harmony object; Step 26 may advance only through the exact `CreateProcessor(MethodBase)` / `PatchProcessor` surface, the measured PatchProcessor type initializer, one launcher-owned inert target `MethodInfo`, and one empty processor object; `Patch()`, Harmony patch APIs, StS2 member invocation, Godot/game startup, and native game loading remain later boundaries.
