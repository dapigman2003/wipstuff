...
Physical Step 35.0 / 0.0.123 hard-terminated while the UI still appeared near Gate B. Step 35.0.1 / 0.0.124 added output-only crash checkpoints and physically resolved that ambiguity: Gate B passed completely; Gate C bound exact transformed `ExecuteVeryEarly()`, entered its first and only `MethodInfo.Invoke`, successfully serviced planned `GodotSharp`, `Steamworks.NET` and host-framework requests, and then hard-terminated before `C_INVOKE_RETURNED`. The matching `.ips` again faults the main thread at PC=`0x0` and shows effectively the same runtime-heavy stack path as 0.0.123. Step 35.0.2 / 0.0.125 therefore keeps execution authority frozen and adds only a pre-CLR output-only static IL/callsite map of the exact verified transformed wrapper + async MoveNext. This map is used to correlate the invoke-time resolver frontier to exact IL before designing a smaller execution discriminator; it is never trusted runtime input.
