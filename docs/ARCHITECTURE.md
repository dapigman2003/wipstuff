...
Physical 0.0.132 placed the game hard-kill interval inside type initialization triggered by `CommandLineHelper.TryGetValue`; 0.0.133 corrected the NP ordinal but physically returned managed `InvalidProgramException` before any CommandLine cctor marker, then reached normal `RUN_END`. That identifies a diagnostic IL header defect: live-stack PRE/POST markers raised transient stack depth without increasing `MaxStack`. Step 35.0.11 keeps exact-source/runtime authority unchanged, reserves one MaxStack slot for live-stack sweeps, post-write verifies the CommandLine cctor header, retains corrected NP/CL/CLTV exact-source ordinals, and adds four stack-neutral critical markers around dictionary assignment and `Godot.OS.GetCmdlineArgs` result storage. Exact-source maps are generated before CLR admission and the cctor diagnostic plan must still include `Godot.OS.GetCmdlineArgs` or Gate A fails closed. This is localization architecture, not startup authorization.
