...
## Step 35.0.24 reverse-binding readiness boundary

The Step-15 source-built Godot native module exposes two interop directions. The physically proven 225-pointer runtime interop table supplies managed->native GodotSharp callbacks. Physical 0.0.145 then proved CSharpLanguage exists while the native->managed `GDMonoCache::ManagedCallbacks` cache is absent. Physical 0.0.146 proved the generated GodotPlugins managed bootstrap in the existing launcher CLR: the full 37-pointer ManagedCallbacks struct was created, adopted through `GDMonoCache::update_godot_api_cache`, reverse binding became ready, and `GD_OnCoreApiAssemblyLoaded` returned. 0.0.147 preserves that bridge and treats its exact eight framework resolutions as a bounded bootstrap phase, then freezes a post-bootstrap resolver baseline before Gate C. The launcher deliberately does not claim/set `GDMono::runtime_initialized`.


## Step 36.0 exact essential-initialization boundary

Step 36 reuses the exact non-collectible `StS2Launcher-Step35-VeryEarly` authority context rather than loading a second sts2 assembly. The same exact closed Step-32 transformed sts2 and exact prepared GodotSharp remain resident after Step-35 exact closure. Gate A performs read-only Cecil source/transformed semantic verification for source token `0x06007D03` (`System.Void MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::ExecuteEssential()`); Gate B proves authority continuity and state 1; Gate C invokes that exact MethodInfo once and requires state 2; Gate D re-proves receipt/hash/plan/dependency/resolver/context isolation. No Step-36 path loads the game native executable or authorizes deferred initialization or entry-point execution.
