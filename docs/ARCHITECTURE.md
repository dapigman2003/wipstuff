...
Physical Step 31.0 / 0.0.114 closed at 4/4 and established the exact `OneTimeInitialization::PrewarmJit()` / ten-`RuntimeHelpers.PrepareMethod` family as eligible for explicit rewrite design. Step 32.0 / 0.0.115 introduced the first real-game semantic write; Codemagic exposed a Gate-C pre-serialization offset-fingerprint verifier defect, corrected in 0.0.116 without changing rewrite semantics. Physical 0.0.116 then showed whole-module Cecil serialization resolving unrelated `System.Runtime` Constant-table enum metadata. Physical 0.0.117 deliberately bounded that path and failed closed before writing when the complete constant pre-scan found another unrelated `Sentry 5.0.0.0` scope. Step 32.0.3 / 0.0.118 therefore keeps Cecil as exact metadata/IL binding and reopen-verification authority but removes `ModuleDefinition.Write` from this candidate: the verified five-byte direct-call windows are replaced only on the launcher-private image with equal-length `Pop/Nop` bytes, and whole-image comparison rejects any change outside those fifty bytes. This preserves later IL offsets, branch displacements, EH boundaries, metadata tables, RVA/section layout and file length by construction while avoiding dependency-resolution expansion. The receipt-backed install remains immutable, and transformed-real-StS2 CLR admission/execution, game startup, Godot bridging, and native game loading remain later separately gated boundaries.
