...
IPA="$ROOT/artifacts/StS2-Launcher-Step-08.ipa"
...
PUBLISH_LOG="artifacts/logs/step08-publish.log"
PATCH_LOG="artifacts/logs/step08-steamkit-patch.log"
...
echo "Restoring Step 08 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step08-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step08-patcher-restore.log
...
{
  echo "=== Step 08 SteamKit iOS compatibility patch ==="
...
echo "Publishing Step 08 depot-manifest-discovery build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step08-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 08 publish failed: focused scan ===" \
    | tee artifacts/logs/step08-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step08-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-08.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-08.ipa Payload
)
