...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-21.1.ipa}"
...
[[ "$VERSION" == "0.0.57" ]] || { echo "ERROR: wrong Step 21.1 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "57" ]] || { echo "ERROR: wrong Step 21.1 build version: $BUILD_VERSION" >&2; exit 4; }
[[ "$FILE_SHARING" == "true" ]] || { echo "ERROR: Step 21.1 final IPA does not enable UIFileSharingEnabled: $FILE_SHARING" >&2; exit 4; }
[[ "$OPEN_IN_PLACE" == "true" ]] || { echo "ERROR: Step 21.1 final IPA does not enable LSSupportsOpeningDocumentsInPlace: $OPEN_IN_PLACE" >&2; exit 4; }
...
cp "$DEPENDENCIES" artifacts/logs/step21-1-final-native-dependencies.log
...
{
  echo "Step 21.1 IPA verification passed."
  ...
  echo "  Expected device UI: STEP 21.1 — BINDING DIAGNOSTIC EXPORT"
} | tee artifacts/logs/step21-1-ipa-verification-summary.log
