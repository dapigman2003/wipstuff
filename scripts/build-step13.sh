...
bash scripts/validate-step13.sh
...
IPA="$ROOT/artifacts/StS2-Launcher-Step-13.ipa"
...
PUBLISH_LOG="artifacts/logs/step13-publish.log"
PATCH_LOG="artifacts/logs/step13-steamkit-patch.log"
...
echo "Restoring Step 13 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step13-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step13-patcher-restore.log
...
{
  echo "=== Step 13 SteamKit iOS compatibility patch ==="
...
echo "Publishing Step 13 offline-launcher-state build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step13-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 13 publish failed: focused scan ===" \
    | tee artifacts/logs/step13-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step13-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-13.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-13.ipa Payload
)
