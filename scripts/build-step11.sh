...
bash scripts/validate-step11.sh
...
IPA="$ROOT/artifacts/StS2-Launcher-Step-11.ipa"
...
PUBLISH_LOG="artifacts/logs/step11-publish.log"
PATCH_LOG="artifacts/logs/step11-steamkit-patch.log"
...
echo "Restoring Step 11 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step11-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step11-patcher-restore.log
...
{
  echo "=== Step 11 SteamKit iOS compatibility patch ==="
...
echo "Publishing Step 11 interrupted-download-resume build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step11-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 11 publish failed: focused scan ===" \
    | tee artifacts/logs/step11-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step11-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-11.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-11.ipa Payload
)
