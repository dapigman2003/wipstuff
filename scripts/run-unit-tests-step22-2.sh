...
dotnet build "$DYNAMIC_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step22-2-dynamic-fixture-host-build.log
dotnet build "$DEPENDENCY_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step22-2-dependency-fixture-host-build.log
dotnet build "$ROOT_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step22-2-root-fixture-host-build.log
...
dotnet test "$TEST_PROJECT" \
  ...
  --logger "trx;LogFileName=step22-2.trx" \
  2>&1 | tee artifacts/logs/step22-2-unit-tests.log
