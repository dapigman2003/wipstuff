...
dotnet build "$DYNAMIC_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step21-dynamic-fixture-host-build.log
dotnet build "$DEPENDENCY_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step21-dependency-fixture-host-build.log
dotnet build "$ROOT_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step21-root-fixture-host-build.log
...
echo "=== Step 21 host unit tests (Steps 01-20 regressions + prepared runtime/framework binding) ==="
dotnet test "$TEST_PROJECT" \
  -c Release \
  --nologo \
  --results-directory "$RESULTS_DIR" \
  --logger "trx;LogFileName=step21.trx" \
  2>&1 | tee artifacts/logs/step21-unit-tests.log
