...
{
  echo "=== StS2 Launcher Step 21.1 Binding Diagnostic Export environment ==="
  date -u
  uname -a
  xcodebuild -version
  ...
  python3 --version
  git --version
} | tee artifacts/logs/step21-1-environment.log
...
bash scripts/validate-step21-1.sh | tee artifacts/logs/step21-1-validation.log
...
) | tee artifacts/logs/step21-1-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step21-1.sh 2>&1 | tee artifacts/logs/step21-1-wrapper.log
bash scripts/verify-step21-1-ipa.sh artifacts/StS2-Launcher-Step-21.1.ipa \
  2>&1 | tee artifacts/logs/step21-1-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 21.1 Binding Diagnostic Export"
  ...
  echo "Step 21 A-D implementation is hash-protected and unchanged in this reporting-only hotfix"
  ...
  echo "Step 21.1 export: reads persisted runtime-binding-plan.json only; writes complete share-safe plain-text blocker report"
  echo "Step 21.1 Files integration: UIFileSharingEnabled=true + LSSupportsOpeningDocumentsInPlace=true"
  echo "Step 21.1 report: Documents/StS2Launcher/Step21.1-RuntimeBindingDiagnostics.txt"
  ...
  echo "IPA: artifacts/StS2-Launcher-Step-21.1.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-21.1.ipa | awk '{print $1}')"
  fi
} > artifacts/step21-1-build-summary.txt

cat artifacts/step21-1-build-summary.txt
