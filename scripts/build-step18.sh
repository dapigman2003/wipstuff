...
IPA="$ROOT/artifacts/StS2-Launcher-Step-18.ipa"
...
PUBLISH_LOG="artifacts/logs/step18-publish.log"
PATCH_LOG="artifacts/logs/step18-steamkit-patch.log"
...
echo "Restoring Step 18 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step18-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step18-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step18-fixture-restore.log
...
{
  echo "=== Step 18 SteamKit iOS compatibility patch ==="
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step18-fixture-build.log
...
echo "Publishing Step 18 Real Assembly Rewrite Workspace build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step18-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 18 publish failed: focused scan ===" \
    | tee artifacts/logs/step18-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step18-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-18.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-18.ipa Payload
)
