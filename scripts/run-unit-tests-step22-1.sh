...
dotnet build "$DYNAMIC_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step22-1-dynamic-fixture-host-build.log
dotnet build "$DEPENDENCY_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step22-1-dependency-fixture-host-build.log
dotnet build "$ROOT_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step22-1-root-fixture-host-build.log
...
echo "=== Step 22 host unit tests (Steps 01-20 regressions + protected Step 21/21.1 binding diagnostics + Step 22 host framework closure) ==="
dotnet test "$TEST_PROJECT" \
  -c Release \
  --nologo \
  --results-directory "$RESULTS_DIR" \
  --logger "trx;LogFileName=step22-1.trx" \
  2>&1 | tee artifacts/logs/step22-1-unit-tests.log
