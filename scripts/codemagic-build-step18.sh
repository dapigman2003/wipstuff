...
{
  echo "=== StS2 Launcher Step 18 Real Assembly Rewrite Workspace environment ==="
  date -u
  uname -a
  xcodebuild -version
  ...
  python3 --version
  git --version
} | tee artifacts/logs/step18-environment.log
...
bash scripts/validate-step18.sh | tee artifacts/logs/step18-validation.log
bash scripts/run-unit-tests-step18.sh
...
) | tee artifacts/logs/step18-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step18.sh 2>&1 | tee artifacts/logs/step18-wrapper.log
bash scripts/verify-step18-ipa.sh artifacts/StS2-Launcher-Step-18.ipa \
  2>&1 | tee artifacts/logs/step18-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 18 Real Assembly Rewrite Workspace"
  ...
  echo "Foundation + Steps 01-17 regression boundaries: retained"
  ...
  echo "Step 18 gate A: OfflineReady + receipt-backed macOS arm64/shared workspace clone"
  echo "Step 18 gate B: Cecil write/reopen of the real copied primary arm64 sts2.dll"
  echo "Step 18 gate C: semantics-neutral one-NOP IL rewrite on the primary copy only"
  echo "Step 18 gate D: full workspace-source + original-install SHA-1 isolation audit"
  echo "Game-data policy: no StS2 game files/assemblies or proprietary FMOD/Spine binaries in IPA"
  echo "Still absent: behaviorally significant StS2 compatibility rewrite/execution, FMOD/Spine runtime integration, Cloud, Workshop"
  echo "IPA: artifacts/StS2-Launcher-Step-18.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-18.ipa | awk '{print $1}')"
  fi
} > artifacts/step18-build-summary.txt

cat artifacts/step18-build-summary.txt
