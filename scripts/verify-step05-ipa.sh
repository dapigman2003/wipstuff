...
IPA="${1:-artifacts/StS2-Launcher-Step-05.ipa}"
...
[[ "$VERSION" == "0.0.6" ]] || {
  echo "ERROR: wrong Step 05 version: $VERSION" >&2
  exit 4
}
...
if find "$APP" -type f | grep -Ei \
  '(godot|mono\.cecil|libsts2godothost|slay.*spire.*2)' >/dev/null; then
  echo "ERROR: Step 05 IPA contains a later-stage/game component." >&2
  exit 5
fi
...
echo "Step 05 IPA verification passed."
...
echo "  IPA size: $SIZE"
echo "  Expected device UI: white screen + STEP 05 — STEAM NETWORK PROBE"
