...
[[ "$VERSION" == "0.0.12" ]] || {
  echo "ERROR: wrong Step 05.6 version: $VERSION" >&2
  exit 4
}
...
if find "$APP" -type f | grep -Ei \
  ...
  echo "ERROR: Step 05.6 IPA contains a forbidden build/later-stage/game component." >&2
  exit 5
fi
...
echo "Step 05.6 IPA verification passed."
...
echo "  IPA size: $SIZE"
echo "  Expected device UI: STEP 05.6 — STEAMKIT INTERNAL BOUNDARY"
