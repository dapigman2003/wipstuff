...
IPA="${1:-artifacts/StS2-Launcher-Step-05.10.ipa}"
...
[[ "$VERSION" == "0.0.16" ]] || {
  echo "ERROR: wrong Step 05.10 version: $VERSION" >&2
  exit 4
}
...
if find "$APP" -type f | grep -Ei \
  ...
  echo "ERROR: Step 05.10 IPA contains a forbidden build/later-stage/game component." >&2
  exit 5
fi
...
echo "Step 05.10 IPA verification passed."
...
echo "  IPA size: $SIZE"
echo "  Expected device UI: STEP 05.10 — CLIENTHELLO AOT DIAGNOSTICS"
