...
IPA="$ROOT/artifacts/StS2-Launcher-Step-17.ipa"
...
PUBLISH_LOG="artifacts/logs/step17-publish.log"
PATCH_LOG="artifacts/logs/step17-steamkit-patch.log"
...
echo "Restoring Step 17 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step17-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step17-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step17-fixture-restore.log
...
{
  echo "=== Step 17 SteamKit iOS compatibility patch ==="
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step17-fixture-build.log
...
echo "Publishing Step 17 Compatibility Call-Site Analysis build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step17-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 17 publish failed: focused scan ===" \
    | tee artifacts/logs/step17-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step17-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-17.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-17.ipa Payload
)
