...
required = [
    ...
    Path('scripts/build-step22-1.sh'),
    Path('scripts/run-unit-tests-step22-1.sh'),
    Path('scripts/codemagic-build-step22-1.sh'),
    Path('scripts/verify-step22-1-ipa.sh'),
    Path('docs/STEP-22-HOST-FRAMEWORK-CLOSURE.md'),
    Path('docs/STEP-22-TEST.md'),
    Path('docs/STEP-22.1-HOST-AVAILABILITY-DIAGNOSTICS.md'),
]
for path in required:
    if not path.exists():
        raise SystemExit(f'ERROR: Step 22 artifact missing: {path}')

# Step 22.1 is diagnostic-only: the measured Step 22 root set, gate enum and host-test contract remain byte-for-byte unchanged.
step22_protected = {
    Path('src/StS2Launcher.Core/HostFrameworkClosureRootSet.cs'): '08807fcda8f80acadadf6cfdec57e149fa4860313925c0dcb652631619bf38f1',
    Path('src/StS2Launcher.Core/HostFrameworkClosureGate.cs'): '061977397924b2bb4f5c150ce5beefe6a94ce1892aebdf889596079a2eb0fc57',
    Path('tests/StS2Launcher.Core.Tests/HostFrameworkClosureFoundationTests.cs'): '2c143bee997118be44569a86cc3062e16840273de398fdf365af8f339392c95c',
}
for path, expected in step22_protected.items():
    actual = hashlib.sha256(path.read_bytes()).hexdigest()
    if actual != expected:
        raise SystemExit(f'ERROR: Step 22.1 diagnostic hotfix changed protected Step 22 root/gate/test source: {path}\nexpected {expected}\nactual   {actual}')
...
if plist.get('CFBundleShortVersionString') != '0.0.59' or str(plist.get('CFBundleVersion')) != '59':
    raise SystemExit('ERROR: Step 22 must be version 0.0.59 (59).')
...
for marker in (
    ...
    '<ApplicationDisplayVersion>0.0.59</ApplicationDisplayVersion>',
...
for marker in (
    ...
    'Step22.1-HostFrameworkAvailabilityDiagnostics.txt',
    'simple-name fallback probe',
    'WriteHostAvailabilityReport',
...
for marker in (
    'STEP 22.1 — HOST FRAMEWORK AVAILABILITY DIAGNOSTICS', 'Version 0.0.59',
...
for marker in ('ios-step-22-1:', 'Step 22.1 - Host Framework Availability Diagnostics', 'bash scripts/codemagic-build-step22-1.sh', 'artifacts/StS2-Launcher-Step-22.1.ipa'):
...
print('Step 22.1 Host Framework Availability Diagnostics source validation: PASS')
...
print('  Physical Gate A probes all 44 names and exports complete Files-visible success/failure diagnostics')
