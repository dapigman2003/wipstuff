...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-20.ipa}"
...
[[ "$VERSION" == "0.0.55" ]] || { echo "ERROR: wrong Step 20 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "55" ]] || { echo "ERROR: wrong Step 20 build version: $BUILD_VERSION" >&2; exit 4; }
...
cp "$DEPENDENCIES" artifacts/logs/step20-final-native-dependencies.log
...
{
  echo "Step 20 IPA verification passed."
  ...
  echo "  Build-time launcher assemblies: remain AOT-targeted; MtouchInterpreter=-all links interpreter for runtime/dynamic code"
  echo "  Real StS2/proprietary payload in IPA: none"
  echo "  Dynamic dependency audit: system or bundled only"
  echo "  Expected device UI: STEP 20 — DYNAMIC MANAGED EXECUTION FOUNDATION"
} | tee artifacts/logs/step20-ipa-verification-summary.log
