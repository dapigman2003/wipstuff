...
{
  echo "StS2 Launcher — Step 35.0.26 Gate D Progress + Warm Toolchain Cache build environment"
  date -u
  uname -a
  xcodebuild -version
...
{
  echo "StS2 Launcher iOS — Step 35.0.26 Gate D Progress + Warm Toolchain Cache"
  echo "UTC: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
