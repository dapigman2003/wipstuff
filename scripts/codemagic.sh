...
{
  echo "StS2 Launcher — Codemagic cache state before Steps 58–62 real character-select ownership / visible-render trial build"
...
{
  echo "StS2 Launcher — Steps 58–62 real character-select ownership / visible-render trial build environment"
  date -u
  uname -a
  xcodebuild -version
...
{
  echo "StS2 Launcher iOS — Steps 58–62 real character-select ownership / visible-render trial"
  echo "UTC: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
