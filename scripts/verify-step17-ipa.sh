...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-17.ipa}"
...
[[ "$VERSION" == "0.0.46" ]] || { echo "ERROR: wrong Step 17 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "46" ]] || { echo "ERROR: wrong Step 17 build version: $BUILD_VERSION" >&2; exit 4; }
...
# Step 15 remains available as a regression boundary in Step 17.
...
cp "$DEPENDENCIES" artifacts/logs/step17-final-native-dependencies.log
...
# physical Steps 16–17 Cecil gates are the authoritative managed-runtime checks.
...
{
  echo "Step 17 IPA verification passed."
  ...
  echo "  Expected device UI: STEP 17 — COMPATIBILITY CALL-SITE ANALYSIS"
} | tee artifacts/logs/step17-ipa-verification-summary.log
