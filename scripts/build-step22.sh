...
bash scripts/validate-step22.sh
...
IPA="$ROOT/artifacts/StS2-Launcher-Step-22.ipa"
...
PUBLISH_LOG="artifacts/logs/step22-publish.log"
PATCH_LOG="artifacts/logs/step22-steamkit-patch.log"
...
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step22-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step22-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step22-fixture-restore.log
dotnet restore "$STEP20_DYNAMIC_PROJECT" 2>&1 | tee artifacts/logs/step22-dynamic-fixture-restore.log
dotnet restore "$STEP20_DEPENDENCY_PROJECT" 2>&1 | tee artifacts/logs/step22-dependency-fixture-restore.log
dotnet restore "$STEP20_ROOT_PROJECT" 2>&1 | tee artifacts/logs/step22-root-fixture-restore.log
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-fixture-build.log
...
dotnet build "$STEP20_DYNAMIC_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-dynamic-fixture-build.log
dotnet build "$STEP20_DEPENDENCY_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-dependency-fixture-build.log
dotnet build "$STEP20_ROOT_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-root-fixture-build.log
...
echo "Publishing Step 22 Host Framework Closure Foundation build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step22-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
...
if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 22 publish failed: focused scan ===" \
    | tee artifacts/logs/step22-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step22-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-22.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-22.ipa Payload
)
