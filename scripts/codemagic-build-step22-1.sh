...
{
  echo "=== StS2 Launcher Step 22.1 Host Framework Availability Diagnostics environment ==="
  date -u
  uname -a
  xcodebuild -version
  ...
  python3 --version
  git --version
} | tee artifacts/logs/step22-1-environment.log
...
bash scripts/validate-step22-1.sh | tee artifacts/logs/step22-1-validation.log
bash scripts/run-unit-tests-step22-1.sh
...
) | tee artifacts/logs/step22-1-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step22-1.sh 2>&1 | tee artifacts/logs/step22-1-wrapper.log
bash scripts/verify-step22-1-ipa.sh artifacts/StS2-Launcher-Step-22.1.ipa \
  2>&1 | tee artifacts/logs/step22-1-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 22.1 Host Framework Availability Diagnostics"
  ...
  echo "Step 21/21.1 binding engine and diagnostic exporter are hash-protected; Step 22 changes only host framework rooting plus a wrapper closure gate"
  ...
  echo "Step 22 framework rooting: 22 measured TrimmerRootAssembly seeds; physical Gate A requires the full 44-name observed framework closure"
  ...
  echo "IPA: artifacts/StS2-Launcher-Step-22.1.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-22.1.ipa | awk '{print $1}')"
  fi
} > artifacts/step22-1-build-summary.txt

cat artifacts/step22-1-build-summary.txt
