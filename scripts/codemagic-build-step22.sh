...
{
  echo "=== StS2 Launcher Step 22 Host Framework Closure Foundation environment ==="
  date -u
  uname -a
  xcodebuild -version
  ...
  python3 --version
  git --version
} | tee artifacts/logs/step22-environment.log
...
bash scripts/validate-step22.sh | tee artifacts/logs/step22-validation.log
bash scripts/run-unit-tests-step22.sh
...
) | tee artifacts/logs/step22-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step22.sh 2>&1 | tee artifacts/logs/step22-wrapper.log
bash scripts/verify-step22-ipa.sh artifacts/StS2-Launcher-Step-22.ipa \
  2>&1 | tee artifacts/logs/step22-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 22 Host Framework Closure Foundation"
  ...
  echo "IPA: artifacts/StS2-Launcher-Step-22.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-22.ipa | awk '{print $1}')"
  fi
} > artifacts/step22-build-summary.txt

cat artifacts/step22-build-summary.txt
