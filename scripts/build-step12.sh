...
IPA="$ROOT/artifacts/StS2-Launcher-Step-12.4.1.ipa"
...
PATCH_LOG="artifacts/logs/step12-steamkit-patch.log"
...
echo "Restoring Step 12.4.1 projects into isolated NuGet package root..."
...
{
  echo "=== Step 12.4.1 SteamKit iOS compatibility patch ==="
...
echo "Publishing Step 12.4.1 download-cache test-control build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step12-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 12.4.1 publish failed: focused scan ===" \
  ...
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step12-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-12.4.1.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-12.4.1.ipa Payload
)
