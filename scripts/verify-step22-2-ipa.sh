...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-22.2.ipa}"
...
[[ "$VERSION" == "0.0.60" ]] || { echo "ERROR: wrong Step 22 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "60" ]] || { echo "ERROR: wrong Step 22 build version: $BUILD_VERSION" >&2; exit 4; }
...
cp "$DEPENDENCIES" artifacts/logs/step22-2-final-native-dependencies.log
...

{
  ...
  echo "  Expected device UI: STEP 22.2 — HOST BINDING FRONTIER CORRECTION"
} | tee artifacts/logs/step22-2-ipa-verification-summary.log
