...
PROJECT="$ROOT/src/StS2Launcher.Step05.iOS/StS2Launcher.Step05.iOS.csproj"
...
if [[ -f "$CACHE_LIB" && -f "$CACHE_FINGERPRINT" ]] && \
  ...
  if bash scripts/preflight-godot-link-step15.sh "$OUT_LIB"; then
...
bash scripts/preflight-godot-link-step15.sh "$OUT_LIB"
