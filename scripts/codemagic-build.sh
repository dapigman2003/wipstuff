...
{
  echo "=== StS2 Launcher Step 07 environment ==="
  date -u
  uname -a
  xcodebuild -version
  xcrun --sdk iphoneos --show-sdk-version
  sw_vers
} | tee artifacts/logs/step07-environment.log
...
bash scripts/validate-step07.sh | tee artifacts/logs/step07-validation.log
...
) | tee artifacts/logs/step07-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step07.sh 2>&1 | tee artifacts/logs/step07-wrapper.log
bash scripts/verify-step07-ipa.sh artifacts/StS2-Launcher-Step-07.ipa \
  2>&1 | tee artifacts/logs/step07-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 07 ownership verification"
  ...
  echo "Fix boundary: ownership ticket verification for App ID 2868840"
  ...
  echo "Ownership: App ID 2868840 ticket verification only; download not implemented"
  echo "IPA: artifacts/StS2-Launcher-Step-07.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-07.ipa | awk '{print $1}')"
  fi
} > artifacts/step07-build-summary.txt

cat artifacts/step07-build-summary.txt
