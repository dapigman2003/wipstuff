...
{
  echo "=== StS2 Launcher Step 06.1 environment ==="
  date -u
  uname -a
  xcodebuild -version
  xcrun --sdk iphoneos --show-sdk-version
  sw_vers
} | tee artifacts/logs/step06.1-environment.log
...
bash scripts/validate-step06-1.sh | tee artifacts/logs/step06.1-validation.log
...
) | tee artifacts/logs/step06.1-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step06-1.sh 2>&1 | tee artifacts/logs/step06.1-wrapper.log
bash scripts/verify-step06-1-ipa.sh artifacts/StS2-Launcher-Step-06.1.ipa \
  2>&1 | tee artifacts/logs/step06.1-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 06.1 Steam Guard mobile approval"
  ...
  echo "New capability: same-session Steam Guard mobile approval"
  echo "Steam Guard: mobile-app approval polling only; no manual code entry"
  echo "Credential/token persistence: none"
  echo "Ownership/download: not implemented"
  echo "IPA: artifacts/StS2-Launcher-Step-06.1.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-06.1.ipa | awk '{print $1}')"
  fi
} > artifacts/step06.1-build-summary.txt

cat artifacts/step06.1-build-summary.txt
