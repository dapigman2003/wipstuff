...
{
  echo "=== StS2 Launcher Step 06.2 environment ==="
  date -u
  uname -a
  xcodebuild -version
  xcrun --sdk iphoneos --show-sdk-version
  sw_vers
} | tee artifacts/logs/step06.2-environment.log
...
bash scripts/validate-step06-2.sh | tee artifacts/logs/step06.2-validation.log
...
) | tee artifacts/logs/step06.2-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step06-2.sh 2>&1 | tee artifacts/logs/step06.2-wrapper.log
bash scripts/verify-step06-2-ipa.sh artifacts/StS2-Launcher-Step-06.2.ipa \
  2>&1 | tee artifacts/logs/step06.2-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 06.2 Keychain session persistence"
  ...
  echo "New capability: persistent refresh-token Keychain storage + password-free saved-session resume"
  ...
  echo "IPA: artifacts/StS2-Launcher-Step-06.2.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-06.2.ipa | awk '{print $1}')"
  fi
} > artifacts/step06.2-build-summary.txt

cat artifacts/step06.2-build-summary.txt
