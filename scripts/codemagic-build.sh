...
{
  echo "=== StS2 Launcher Step 12.1 environment ==="
  date -u
  uname -a
  xcodebuild -version
...
bash scripts/verify-step12-ipa.sh artifacts/StS2-Launcher-Step-12.1.ipa \
  2>&1 | tee artifacts/logs/step12-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 12.1 AOT receipt serialization hotfix"
  ...
  echo "Content access: Step 11 remains the verified source-acquisition engine; Step 12.1 keeps the same stable one-depot manager boundary"
  echo "Integrity: Step 12.1 receipt JSON uses compile-time System.Text.Json metadata; receipt still stores only path/length/SHA-1 and staging is verified before rollback-safe commit"
  echo "Still absent: multi-depot app composition, compatibility inventory, Godot/runtime execution, Cloud, Workshop"
  echo "IPA: artifacts/StS2-Launcher-Step-12.1.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-12.1.ipa | awk '{print $1}')"
  fi
} > artifacts/step12.1-build-summary.txt

cat artifacts/step12.1-build-summary.txt
