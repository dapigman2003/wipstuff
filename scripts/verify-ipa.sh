...
[[ "$VERSION" == "$STS2_DISPLAY_VERSION" ]] || { echo "ERROR: wrong Step 25 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "$STS2_BUILD_VERSION" ]] || { echo "ERROR: wrong Step 25 build version: $BUILD_VERSION" >&2; exit 4; }
[[ "$FILE_SHARING" == "true" ]] || { echo "ERROR: Step 25 final IPA does not enable UIFileSharingEnabled: $FILE_SHARING" >&2; exit 4; }
[[ "$OPEN_IN_PLACE" == "true" ]] || { echo "ERROR: Step 25 final IPA does not enable LSSupportsOpeningDocumentsInPlace: $OPEN_IN_PLACE" >&2; exit 4; }
...
cp "$DEPENDENCIES" artifacts/logs/step25-final-native-dependencies.log
...
{
  echo "Step 25 IPA verification passed."
  ...
  echo "  Real StS2/proprietary payload in IPA: none (Step 25 still loads only the user-owned prepared copy from Documents at runtime)"
...
} | tee artifacts/logs/step25-ipa-verification-summary.log
