...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-22.3.ipa}"
...
[[ "$VERSION" == "0.0.61" ]] || { echo "ERROR: wrong Step 22.3 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "61" ]] || { echo "ERROR: wrong Step 22.3 build version: $BUILD_VERSION" >&2; exit 4; }
...
cp "$DEPENDENCIES" artifacts/logs/step22-3-final-native-dependencies.log
...
{
  echo "Step 22.3 IPA verification passed."
  ...
  echo "  Expected device UI: STEP 22.3 — FOUNDATION CONSOLIDATION"
} | tee artifacts/logs/step22-3-ipa-verification-summary.log
