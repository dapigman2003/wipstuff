...
[[ "$VERSION" == "$STS2_DISPLAY_VERSION" ]] || { echo "ERROR: wrong Step 26 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "$STS2_BUILD_VERSION" ]] || { echo "ERROR: wrong Step 26 build version: $BUILD_VERSION" >&2; exit 4; }
[[ "$FILE_SHARING" == "true" ]] || { echo "ERROR: Step 26 final IPA does not enable UIFileSharingEnabled: $FILE_SHARING" >&2; exit 4; }
[[ "$OPEN_IN_PLACE" == "true" ]] || { echo "ERROR: Step 26 final IPA does not enable LSSupportsOpeningDocumentsInPlace: $OPEN_IN_PLACE" >&2; exit 4; }
...
cp "$DEPENDENCIES" artifacts/logs/step26-final-native-dependencies.log
...
{
  echo "Step 26 IPA verification passed."
  ...
  echo "  Real StS2/proprietary payload in IPA: none (Step 26 still loads only the user-owned prepared copy from Documents at runtime)"
...
} | tee artifacts/logs/step26-ipa-verification-summary.log
