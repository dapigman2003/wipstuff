...
IPA="$ROOT/artifacts/StS2-Launcher-Step-05.5.ipa"
...
PUBLISH_LOG="artifacts/logs/step05-5-publish.log"
PATCH_LOG="artifacts/logs/step05-5-steamkit-patch.log"
FRAMEWORK_LOG="artifacts/logs/step05-5-framework-filter.log"
...
echo "iPhoneOS SDK: $SDK_ROOT" \
  | tee -a artifacts/logs/step05-5-native-preflight.log
...
Step 05.5 policy:
  ...
  - then rerun SteamKit WebSocket-only and TCP-only;
...
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step05-5-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
...
if [[ "$PUBLISH_STATUS" != "0" ]]; then
  {
    ...
    grep -E 'STEP05\.5|Assembly:|Patched method:|Replacement|Strong-name' "$PATCH_LOG" || true
    echo
    ...
    grep -E -m 100 \
      ...
      "$PUBLISH_LOG" || true
  } | tee artifacts/logs/step05-5-failure-scan.log
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-05.5.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-05.5.ipa Payload
)
