...
IPA="$ROOT/artifacts/StS2-Launcher-Step-05.ipa"

echo "Publishing Step 05..."
dotnet publish src/StS2Launcher.Step05.iOS/StS2Launcher.Step05.iOS.csproj \
  -c Release \
  -f net9.0-ios \
  -r ios-arm64 \
  -p:BuildIpa=false \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step05-dotnet-ios.binlog
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-05.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-05.ipa Payload
)
