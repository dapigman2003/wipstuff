...
IPA="$ROOT/artifacts/StS2-Launcher-Step-19.ipa"
...
PUBLISH_LOG="artifacts/logs/step19-publish.log"
PATCH_LOG="artifacts/logs/step19-steamkit-patch.log"
...
echo "Restoring Step 19 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step19-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step19-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step19-fixture-restore.log
...
{
  echo "=== Step 19 SteamKit iOS compatibility patch ==="
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step19-fixture-build.log
...
echo "Publishing Step 19 Expression Interpreter Compatibility build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step19-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 19 publish failed: focused scan ===" \
    | tee artifacts/logs/step19-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step19-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-19.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-19.ipa Payload
)
