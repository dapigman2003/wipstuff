...
IPA="$ROOT/artifacts/StS2-Launcher-Step-05.16.ipa"
...
PUBLISH_LOG="artifacts/logs/step05-final-publish.log"
PATCH_LOG="artifacts/logs/step05-final-steamkit-patch.log"

# Patch only a disposable repository-local SteamKit package copy. Never mutate
# the developer/Codemagic global NuGet cache.
...
echo "Restoring final Step 05 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step05-final-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step05-final-patcher-restore.log
...
{
  echo "=== Step 05.16 SteamKit iOS compatibility patch ==="
...
echo "Publishing Step 05.16 final foundation build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step05-final-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 05.16 publish failed: focused scan ===" \
    | tee artifacts/logs/step05-final-failure-scan.log
  grep -E -m 120 \
    '(^|: )(error|fatal error)|undefined symbol|Undefined symbols|framework .+ not found|DiskArbitration|PlatformNotSupported' \
    "$PUBLISH_LOG" | tee -a artifacts/logs/step05-final-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi

# The linker workaround is a proven compatibility requirement. A successful
# publish is accepted only if the generated framework list shows the filter ran.
...
if [[ "$BEFORE_LINE" != *"DiskArbitration"* ]]; then
  echo "ERROR: expected DiskArbitration before filtering." >&2
  echo "$BEFORE_LINE" >&2
  exit 8
fi
if [[ "$AFTER_LINE" == *"DiskArbitration"* ]]; then
  echo "ERROR: DiskArbitration survived the generated-framework filter." >&2
  echo "$AFTER_LINE" >&2
  exit 8
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-05.16.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-05.16.ipa Payload
)
