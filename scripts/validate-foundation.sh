...
# MSTest 4 removed the legacy ThrowsException API and deprecates DataTestMethod.
# Keep the test source on the supported assertion/parameterized-test surface.
if 'ThrowsException<' in test_text:
    raise SystemExit('ERROR: MSTest 4-incompatible Assert.ThrowsException remains in tests.')
if '[DataTestMethod]' in test_text:
    raise SystemExit('ERROR: obsolete DataTestMethod remains in tests; use TestMethod + DataRow.')
if 'ThrowsExactly<ArgumentException>' not in test_text:
    raise SystemExit('ERROR: invalid-key exception regression test is missing ThrowsExactly.')
