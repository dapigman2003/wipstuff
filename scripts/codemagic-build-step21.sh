...
{
  echo "=== StS2 Launcher Step 21 Prepared Runtime / Framework Binding environment ==="
  date -u
  uname -a
  xcodebuild -version
  ...
  python3 --version
  git --version
} | tee artifacts/logs/step21-environment.log
...
bash scripts/validate-step21.sh | tee artifacts/logs/step21-validation.log
...
) | tee artifacts/logs/step21-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step21.sh 2>&1 | tee artifacts/logs/step21-wrapper.log
bash scripts/verify-step21-ipa.sh artifacts/StS2-Launcher-Step-21.ipa \
  2>&1 | tee artifacts/logs/step21-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 21 Prepared Runtime / Framework Binding"
  ...
  echo "Step 21 gate A: OfflineReady + receipt-backed ARM64/shared managed workspace + real identity/IL-only classification"
  echo "Step 21 gate B: real sts2.dll AssemblyRef graph; iOS-host framework binding preferred; exact/controlled-version private workspace resolution; blockers explicit"
  echo "Step 21 gate C: zero-Cecil-write byte-identical execution-oriented private/game IL set + deterministic runtime-binding-plan.json"
  echo "Step 21 gate D: source/prepared/live/plan hash + identity isolation audit; Runtime closure ready is reported separately from gate pass"
  ...
  echo "IPA: artifacts/StS2-Launcher-Step-21.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-21.ipa | awk '{print $1}')"
  fi
} > artifacts/step21-build-summary.txt

cat artifacts/step21-build-summary.txt
