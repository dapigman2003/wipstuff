...
dotnet build "$DYNAMIC_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step21-1-dynamic-fixture-host-build.log
dotnet build "$DEPENDENCY_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step21-1-dependency-fixture-host-build.log
dotnet build "$ROOT_PROJECT" -c Release --nologo 2>&1 | tee artifacts/logs/step21-1-root-fixture-host-build.log
...
echo "=== Step 21.1 host unit tests (Steps 01-20 regressions + physically passed Step 21 binding + Step 21.1 diagnostic exporter) ==="
dotnet test "$TEST_PROJECT" \
  -c Release \
  --nologo \
  --results-directory "$RESULTS_DIR" \
  --logger "trx;LogFileName=step21-1.trx" \
  2>&1 | tee artifacts/logs/step21-1-unit-tests.log
