...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-22.1.ipa}"
...
[[ "$VERSION" == "0.0.59" ]] || { echo "ERROR: wrong Step 22 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "59" ]] || { echo "ERROR: wrong Step 22 build version: $BUILD_VERSION" >&2; exit 4; }
[[ "$FILE_SHARING" == "true" ]] || { echo "ERROR: Step 22 final IPA does not enable UIFileSharingEnabled: $FILE_SHARING" >&2; exit 4; }
[[ "$OPEN_IN_PLACE" == "true" ]] || { echo "ERROR: Step 22 final IPA does not enable LSSupportsOpeningDocumentsInPlace: $OPEN_IN_PLACE" >&2; exit 4; }
...
# Step 15 remains available as a regression boundary in Step 20.
...
cp "$DEPENDENCIES" artifacts/logs/step22-1-final-native-dependencies.log
...
{
  echo "Step 22 IPA verification passed."
  ...
  echo "  Real StS2/proprietary payload in IPA: none (Step 21 reads only the user-owned installed copy at runtime)"
  ...
  echo "  Expected device UI: STEP 22.1 — HOST FRAMEWORK AVAILABILITY DIAGNOSTICS"
} | tee artifacts/logs/step22-1-ipa-verification-summary.log
