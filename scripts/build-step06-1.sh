...
bash scripts/validate-step06-1.sh
...
IPA="$ROOT/artifacts/StS2-Launcher-Step-06.1.ipa"
...
PUBLISH_LOG="artifacts/logs/step06.1-publish.log"
PATCH_LOG="artifacts/logs/step06.1-steamkit-patch.log"
...
echo "Restoring Step 06.1 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step06.1-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step06.1-patcher-restore.log
...
{
  echo "=== Step 06.1 SteamKit iOS compatibility patch ==="
...
echo "Publishing Step 06.1 mobile-approval build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step06.1-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 06.1 publish failed: focused scan ===" \
    | tee artifacts/logs/step06.1-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step06.1-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-06.1.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-06.1.ipa Payload
)
