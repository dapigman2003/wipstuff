...
IPA="${1:-artifacts/StS2-Launcher-Step-04.ipa}"
...
[[ "$VERSION" == "0.0.5" ]] || {
  echo "ERROR: wrong Step 04 version: $VERSION" >&2
  exit 4
}
...
if find "$APP" -type f | grep -Ei \
  ...
  echo "ERROR: Step 04 IPA contains a later-stage/game component." >&2
  exit 5
fi
...
echo "Step 04 IPA verification passed."
...
echo "  IPA size: $SIZE"
echo "  Expected device UI: white screen + STEP 04 — KEYCHAIN PROBE"
