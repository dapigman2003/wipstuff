...
IPA="${1:-artifacts/StS2-Launcher-Step-03.ipa}"
...
[[ "$VERSION" == "0.0.4" ]] || {
  echo "ERROR: wrong Step 03 version: $VERSION" >&2
  exit 4
}
...
if find "$APP" -type f | grep -Ei \
  '(godot|steamkit|mono\.cecil|libsts2godothost|slay.*spire.*2)' >/dev/null; then
  echo "ERROR: Step 03 IPA contains a later-stage/game component." >&2
  exit 5
fi
...
echo "Step 03 IPA verification passed."
...
echo "  IPA size: $SIZE"
echo "  Expected device UI: white screen + STEP 03 — CORE STATE MACHINE"
