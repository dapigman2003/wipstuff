...
echo "=== Step 19 host unit tests (Steps 01-18 regressions + expression-interpreter compatibility gates) ==="
dotnet test "$TEST_PROJECT" \
  -c Release \
  --nologo \
  --results-directory "$RESULTS_DIR" \
  --logger "trx;LogFileName=step19.trx" \
  2>&1 | tee artifacts/logs/step19-unit-tests.log
