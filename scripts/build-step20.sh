...
IPA="$ROOT/artifacts/StS2-Launcher-Step-20.ipa"
...
PUBLISH_LOG="artifacts/logs/step20-publish.log"
PATCH_LOG="artifacts/logs/step20-steamkit-patch.log"
...
echo "Restoring Step 20 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step20-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step20-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step20-fixture-restore.log
dotnet restore "$STEP20_DYNAMIC_PROJECT" 2>&1 | tee artifacts/logs/step20-dynamic-fixture-restore.log
dotnet restore "$STEP20_DEPENDENCY_PROJECT" 2>&1 | tee artifacts/logs/step20-dependency-fixture-restore.log
dotnet restore "$STEP20_ROOT_PROJECT" 2>&1 | tee artifacts/logs/step20-root-fixture-restore.log
...
{
  echo "=== Step 20 SteamKit iOS compatibility patch ==="
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step20-fixture-build.log
...
dotnet build "$STEP20_DYNAMIC_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step20-dynamic-fixture-build.log
dotnet build "$STEP20_DEPENDENCY_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step20-dependency-fixture-build.log
dotnet build "$STEP20_ROOT_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step20-root-fixture-build.log
...
echo "Publishing Step 20 Dynamic Managed Execution Foundation build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step20-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 20 publish failed: focused scan ===" \
    | tee artifacts/logs/step20-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step20-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi

for required in \
  'STEP20 INTERPRETER POLICY: MtouchInterpreter=-all' \
  'UseInterpreter='; do
  if ! grep -Fq "$required" "$PUBLISH_LOG"; then
    echo "ERROR: Step 20 interpreter-policy telemetry missing from publish log: $required" >&2
    exit 15
  fi
done
if grep -F 'STEP20 INTERPRETER POLICY:' "$PUBLISH_LOG" | grep -Fq 'UseInterpreter=true'; then
  echo "ERROR: Step 20 publish unexpectedly resolved the broad UseInterpreter=true policy." >&2
  exit 15
fi
if grep -F 'STEP20 INTERPRETER POLICY:' "$PUBLISH_LOG" | grep -Fq 'PublishAot=true'; then
  echo "ERROR: Step 20 publish unexpectedly enabled NativeAOT." >&2
  exit 15
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-20.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-20.ipa Payload
)
