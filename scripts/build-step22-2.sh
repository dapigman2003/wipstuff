...
bash scripts/validate-step22-2.sh
...
IPA="$ROOT/artifacts/StS2-Launcher-Step-22.2.ipa"
...
PUBLISH_LOG="artifacts/logs/step22-2-publish.log"
PATCH_LOG="artifacts/logs/step22-2-steamkit-patch.log"
...
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step22-2-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step22-2-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step22-2-fixture-restore.log
dotnet restore "$STEP20_DYNAMIC_PROJECT" 2>&1 | tee artifacts/logs/step22-2-dynamic-fixture-restore.log
dotnet restore "$STEP20_DEPENDENCY_PROJECT" 2>&1 | tee artifacts/logs/step22-2-dependency-fixture-restore.log
dotnet restore "$STEP20_ROOT_PROJECT" 2>&1 | tee artifacts/logs/step22-2-root-fixture-restore.log
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-2-fixture-build.log
...
dotnet build "$STEP20_DYNAMIC_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-2-dynamic-fixture-build.log
dotnet build "$STEP20_DEPENDENCY_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-2-dependency-fixture-build.log
dotnet build "$STEP20_ROOT_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-2-root-fixture-build.log
...
echo "Publishing Step 22.2 Host Binding Frontier Correction build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step22-2-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
...
if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 22 publish failed: focused scan ===" \
    | tee artifacts/logs/step22-2-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step22-2-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-22.2.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-22.2.ipa Payload
)
