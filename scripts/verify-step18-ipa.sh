...
cd "$ROOT"
IPA="${1:-artifacts/StS2-Launcher-Step-18.ipa}"
...
[[ "$VERSION" == "0.0.48" ]] || { echo "ERROR: wrong Step 18 version: $VERSION" >&2; exit 4; }
[[ "$BUILD_VERSION" == "48" ]] || { echo "ERROR: wrong Step 18 build version: $BUILD_VERSION" >&2; exit 4; }
...
# Step 15 remains available as a regression boundary in Step 18.
...
cp "$DEPENDENCIES" artifacts/logs/step18-final-native-dependencies.log
...
# physical Steps 16–18 Cecil gates are the authoritative managed-runtime checks.
...
{
  echo "Step 18 IPA verification passed."
  echo "  Bundle ID: $BUNDLE_ID"
  echo "  Version: $VERSION ($BUILD_VERSION)"
  echo "  Architecture: arm64"
  echo "  Godot 4.5.1 Step 15 regression host: retained"
  echo "  Mono.Cecil 0.11.6 rewrite-workspace dependency: linked/AOT input"
  ...
  echo "  Expected device UI: STEP 18.1 — REAL ASSEMBLY REWRITE WORKSPACE"
