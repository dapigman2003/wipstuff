...
IPA="$ROOT/artifacts/StS2-Launcher-Step-22.3.ipa"
...
echo "Publishing Step 22.3 Foundation Consolidation..."
set +e
...
grep -Fq 'STEP22.3 RUNTIME POLICY: MtouchInterpreter=-all' "$PUBLISH_LOG" || { echo "ERROR: runtime-policy telemetry missing." >&2; exit 15; }
if grep -F 'STEP22.3 RUNTIME POLICY:' "$PUBLISH_LOG" | grep -Fq 'UseInterpreter=true'; then echo "ERROR: broad UseInterpreter=true policy resolved." >&2; exit 15; fi
if grep -F 'STEP22.3 RUNTIME POLICY:' "$PUBLISH_LOG" | grep -Fq 'PublishAot=true'; then echo "ERROR: NativeAOT unexpectedly enabled." >&2; exit 15; fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-22.3.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-22.3.ipa Payload
)
echo "Created $IPA"
