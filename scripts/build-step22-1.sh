...
bash scripts/validate-step22-1.sh
...
IPA="$ROOT/artifacts/StS2-Launcher-Step-22.1.ipa"
...
PUBLISH_LOG="artifacts/logs/step22-1-publish.log"
PATCH_LOG="artifacts/logs/step22-1-steamkit-patch.log"
...
echo "Restoring Step 22 projects into isolated NuGet package root..."
dotnet restore "$PROJECT" 2>&1 | tee artifacts/logs/step22-1-restore.log
dotnet restore "$PATCHER" 2>&1 | tee artifacts/logs/step22-1-patcher-restore.log
dotnet restore "$FIXTURE_PROJECT" 2>&1 | tee artifacts/logs/step22-1-fixture-restore.log
dotnet restore "$STEP20_DYNAMIC_PROJECT" 2>&1 | tee artifacts/logs/step22-1-dynamic-fixture-restore.log
dotnet restore "$STEP20_DEPENDENCY_PROJECT" 2>&1 | tee artifacts/logs/step22-1-dependency-fixture-restore.log
dotnet restore "$STEP20_ROOT_PROJECT" 2>&1 | tee artifacts/logs/step22-1-root-fixture-restore.log
...
{
  echo "=== Step 22 SteamKit iOS compatibility patch ==="
...
dotnet build "$FIXTURE_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-1-fixture-build.log
...
dotnet build "$STEP20_DYNAMIC_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-1-dynamic-fixture-build.log
dotnet build "$STEP20_DEPENDENCY_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-1-dependency-fixture-build.log
dotnet build "$STEP20_ROOT_PROJECT" -c Release --no-restore 2>&1 | tee artifacts/logs/step22-1-root-fixture-build.log
...
echo "Publishing Step 22.1 Host Framework Availability Diagnostics build..."
set +e
dotnet publish "$PROJECT" \
  ...
  -p:CodesignProvision="" \
  -p:AppBundleDir="$APP" \
  -bl:artifacts/logs/step22-1-dotnet-ios.binlog \
  2>&1 | tee "$PUBLISH_LOG"
PUBLISH_STATUS=${PIPESTATUS[0]}
set -e

if [[ "$PUBLISH_STATUS" != "0" ]]; then
  echo "=== Step 22 publish failed: focused scan ===" \
    | tee artifacts/logs/step22-1-failure-scan.log
  grep -E -m 160 \
    ...
    "$PUBLISH_LOG" | tee -a artifacts/logs/step22-1-failure-scan.log || true
  exit "$PUBLISH_STATUS"
fi

for required in \
  'STEP22 INTERPRETER POLICY: MtouchInterpreter=-all' \
  'UseInterpreter='; do
  if ! grep -Fq "$required" "$PUBLISH_LOG"; then
    echo "ERROR: Step 22 interpreter-policy telemetry missing from publish log: $required" >&2
    exit 15
  fi
done
if grep -F 'STEP22 INTERPRETER POLICY:' "$PUBLISH_LOG" | grep -Fq 'UseInterpreter=true'; then
  echo "ERROR: Step 22 publish unexpectedly resolved the broad UseInterpreter=true policy." >&2
  exit 15
fi
if grep -F 'STEP22 INTERPRETER POLICY:' "$PUBLISH_LOG" | grep -Fq 'PublishAot=true'; then
  echo "ERROR: Step 22 publish unexpectedly enabled NativeAOT." >&2
  exit 15
fi
...
(
  cd artifacts
  rm -f StS2-Launcher-Step-22.1.ipa
  /usr/bin/zip -qry StS2-Launcher-Step-22.1.ipa Payload
)
