...
{
  echo "=== StS2 Launcher Step 20 Dynamic Managed Execution Foundation environment ==="
  date -u
  uname -a
  xcodebuild -version
  ...
  python3 --version
  git --version
} | tee artifacts/logs/step20-environment.log
...
bash scripts/validate-step20.sh | tee artifacts/logs/step20-validation.log
...
) | tee artifacts/logs/step20-workload.log
rm -rf "$WORKLOAD_CWD"
trap - EXIT

bash scripts/build-step20.sh 2>&1 | tee artifacts/logs/step20-wrapper.log
bash scripts/verify-step20-ipa.sh artifacts/StS2-Launcher-Step-20.ipa \
  2>&1 | tee artifacts/logs/step20-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 20 Dynamic Managed Execution Foundation"
  ...
  echo "Foundation + Steps 01-19 physically closed regression boundaries: retained"
  ...
  echo "Step 20 build policy: MtouchInterpreter=-all keeps build-time launcher assemblies AOT-compiled while retaining the interpreter for runtime/dynamic code; fixture DLLs are copied after publish as data-only, never AOT/link inputs"
  echo "Step 20 gate A: OfflineReady + SHA-256/identity verification of three project-owned external managed fixtures"
  echo "Step 20 gate B: dedicated AssemblyLoadContext loads and executes non-AOT project-owned IL to deterministic result 42"
  echo "Step 20 gate C: one exact SHA-256-verified private managed dependency resolution hop + transitive execution to 42"
  echo "Step 20 gate D: fixture rehash + post-execution OfflineReady + no StS2 CLR load/isolation audit"
  echo "Game-data policy: no StS2 game files/assemblies or proprietary FMOD/Spine binaries in IPA"
  echo "Still absent: StS2 CLR load/execution, GodotSharp runtime binding, Harmony/MonoMod detours, FMOD/Spine runtime integration, Cloud, Workshop"
  echo "IPA: artifacts/StS2-Launcher-Step-20.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-20.ipa | awk '{print $1}')"
  fi
} > artifacts/step20-build-summary.txt

cat artifacts/step20-build-summary.txt
