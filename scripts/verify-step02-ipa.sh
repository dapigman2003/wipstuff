...
IPA="${1:-artifacts/StS2-Launcher-Step-02.ipa}"
...
[[ "$VERSION" == "0.0.3" ]] || {
  echo "ERROR: wrong Step 02 version: $VERSION" >&2
  exit 4
}
...
if find "$APP" -type f | grep -Ei \
  ...
  echo "ERROR: Step 02 IPA contains a later-stage/game component." >&2
  exit 5
fi
...
echo "Step 02 IPA verification passed."
...
echo "  IPA size: $SIZE"
echo "  Expected device UI: white screen + STEP 02 — LAUNCHER UI SHELL"
