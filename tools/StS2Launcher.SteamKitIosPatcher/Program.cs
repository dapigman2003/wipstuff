...
try
{
    ...
    if (!version.StartsWith(ExpectedVersionPrefix, StringComparison.Ordinal))
    {
        Console.Error.WriteLine(
            $"ERROR: Step 05.16 final patcher expects SteamKit2 {ExpectedVersionPrefix}; got {version}.");
        return 5;
    }
    ...
    if (matches.Length == 0)
    {
        ...
        // This is acceptable in the final build: verify the unsupported call is
        // absent and leave the publisher-signed assembly intact.
        Console.WriteLine("STEP05.16 STEAMKIT IOS PATCH: PASS");
