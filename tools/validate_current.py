...
require("<ApplicationVersion>103</ApplicationVersion>" in project_text, "build version is 103")
require("<ApplicationDisplayVersion>0.0.103</ApplicationDisplayVersion>" in project_text, "display version is 0.0.103")
require(plist.get("CFBundleVersion") == "103", "Info.plist build version is 103")
require(plist.get("CFBundleShortVersionString") == "0.0.103", "Info.plist display version is 0.0.103")
...
require("STEP 27.0.19 — NET9 SURROGATE REFERENCE-GRAPH ASSERTION FIX" in release_presentation, "top launcher banner identifies active Step 27.0.19 candidate")
require("0.0.102" in release_presentation and "212 host tests" in release_presentation and "211 passed" in release_presentation and "netstandard AssemblyRef" in release_presentation and "System.Runtime 9.0" in release_presentation and "production Deferred-Cecil normalizer" in release_presentation and "No StS2 member" in release_presentation, "top launcher banner records the 0.0.102 real-surrogate test result and scopes the 0.0.103 assertion-only correction")
...
require('ExpectedDisplayVersion = "0.0.103"' in release_presentation and 'ExpectedBuildVersion = "103"' in release_presentation, "Step 27.0.19 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.103",
    "STS2_BUILD_VERSION": "103",
...
require("module.Write(output)" in step27_source and "new MemoryStream(runtimeBytes, writable: false)" in step27_source and "instructions.Count != 11" in step27_source, "Step 27.0.14 writes/reopens the compatibility image in memory and pins the exact 11-instruction cctor")
require("using CecilOpCodes = Mono.Cecil.Cil.OpCodes;" in step27_source, "Step 27.0.14 explicitly aliases Cecil OpCodes alongside System.Reflection.Emit")
require(step27_source.count("Instruction.Create(CecilOpCodes.") == 11 and "Instruction.Create(OpCodes." not in step27_source, "Step 27.0.14 uses CecilOpCodes for all eleven generated cctor instructions and forbids bare ambiguous OpCodes")
require(step27_source.count("ReadingMode = ReadingMode.Deferred") >= 8 and "ReadingMode = ReadingMode.Immediate" not in step27_source, "Step 27.0.14 keeps all Step-27 Cecil reads deferred and forbids the physical 0.0.97 Immediate-reader regression")
require("Cecil's writer completes a deferred read" in step27_source and "custom-attribute resolution disabled" in step27_source, "Step 27.0.14 documents why Deferred mode preserves unrelated custom-attribute blobs during normalization")
...
require('PackageDownload Include="Lib.Harmony"' not in step27_test_project and 'PackageReference Include="Lib.Harmony"' not in step27_test_project and "CopyStep27RealHarmonyNormalizerFixture" not in step27_test_project and "NuGetPackageRoot" not in step27_test_project, "Step 27.0.19 keeps Harmony package assets and brittle NuGet/MSBuild fixture-path assumptions out of the test project")
require('STEP27_HARMONY_RELEASE_URL="https://github.com/pardeike/Harmony/releases/download/v2.4.2.0/Harmony-Fat.2.4.2.0.zip"' in step27_test_script and 'STEP27_HARMONY_ARCHIVE_RELATIVE="net9.0/0Harmony.dll"' in step27_test_script, "Step 27.0.19 pins the exact official Harmony-Fat 2.4.2 release URL and strict net9.0 implementation relative path")
require("host-only structural surrogate" in step27_test_script and "production normalizer remains pinned" in step27_test_script and "EditorBrowsable metadata" in step27_test_script, "Step 27.0.19 explicitly scopes the official net9.0 binary to CI structural regression only")
require("curl --fail --location --silent --show-error" in step27_test_script and "--retry 3" in step27_test_script and "--proto '=https' --tlsv1.2" in step27_test_script and 'unzip -Z1 "$STEP27_HARMONY_ARCHIVE" > "$STEP27_HARMONY_MEMBER_LIST"' in step27_test_script and 'STEP27_HARMONY_MEMBER_COUNT=' in step27_test_script and "awk 'END { print NR + 0 }'" in step27_test_script, "Step 27.0.19 fixture acquisition is HTTPS-only, retried, lists the archive once, and requires exactly one root-or-wrapped relative-path match")
require(r'gsub(/\\/, "/", normalized)' in step27_test_script and "normalized == target" in step27_test_script and 'wrapped="/" target' in step27_test_script and 'STEP27_HARMONY_ARCHIVE_MEMBER="$(cat "$STEP27_HARMONY_MATCH_LIST")"' in step27_test_script and "Discovered 0Harmony.dll archive members:" in step27_test_script and 'unzip -p "$STEP27_HARMONY_ARCHIVE" "$STEP27_HARMONY_ARCHIVE_MEMBER"' in step27_test_script, "Step 27.0.19 accepts the exact root member or one wrapped relative path, normalizes separators only for matching, retains the original archive member, and emits drift diagnostics")
require('artifacts/host-step27-fixtures' in step27_test_script and 'export STS2_STEP27_REAL_HARMONY_FIXTURE="$ROOT/$STEP27_HARMONY_FIXTURE"' in step27_test_script and 'export STS2_STEP27_REAL_HARMONY_ARCHIVE_MEMBER="$STEP27_HARMONY_ARCHIVE_MEMBER"' in step27_test_script and "Harmony archive SHA-256:" in step27_test_script and "Harmony fixture SHA-256:" in step27_test_script, "Step 27.0.19 quarantines the official Harmony fixture, exports its absolute path plus exact selected archive member, and records content hashes")
require('Environment.GetEnvironmentVariable("STS2_STEP27_REAL_HARMONY_FIXTURE")' in step27_tests and 'Environment.GetEnvironmentVariable("STS2_STEP27_REAL_HARMONY_ARCHIVE_MEMBER")' in step27_tests and "Path.GetFullPath(fixturePath!)" in step27_tests, "Step 27.0.19 real-Harmony regression consumes only the canonical host-script fixture path and selected archive-member evidence")
require("OfficialHarmony242Net9FatNormalizerUsesDeferredMetadataAndPreservesSourceBytes" in step27_tests and "HasEditorBrowsableAttributeSurface" in step27_tests and "CreateIosNormalizedHarmonyRuntimeImage" in step27_tests and "SourcePreparedSha1" in step27_tests and "RuntimeImageSha1" in step27_tests and "instructions=11" in step27_tests, "Step 27.0.19 host tests exercise the official merged Harmony 2.4.2 net9 structural surrogate through the production normalizer and preserve source bytes")
real_harmony_test_start = step27_tests.index("OfficialHarmony242Net9FatNormalizerUsesDeferredMetadataAndPreservesSourceBytes")
...
require("ConstructorArguments" not in step27_tests[real_harmony_test_start:real_harmony_test_end], "Step 27.0.19 real-Harmony regression detects the EditorBrowsable surface without resolving attribute argument values")
require('STS2_STEP27_REAL_HARMONY_ARCHIVE_MEMBER' in step27_tests and 'normalizedArchiveMember.Equals("net9.0/0Harmony.dll"' in step27_tests and 'EndsWith("/net9.0/0Harmony.dll"' in step27_tests and 'reference.Name == "System.Runtime"' in step27_tests and 'new Version(9, 0, 0, 0)' in step27_tests and "legitimately retains a netstandard compatibility reference" in step27_tests, "Step 27.0.19 positively pins net9 from the selected release member and System.Runtime 9.0 without the invalid no-netstandard-reference inference")
require('Assert.IsFalse(module.AssemblyReferences.Any(reference => reference.Name == "netstandard")' not in step27_tests, "Step 27.0.19 forbids the false assumption that a net9 implementation cannot retain a netstandard AssemblyRef")
...
require("Steps 01–26" in current_status and "0.0.89 (89)" in current_status and "0.0.90 (90)" in current_status and "0.0.93 (93)" in current_status and "0.0.94 (94)" in current_status and "0.0.95 (95)" in current_status and "0.0.96 (96)" in current_status and "209 passed" in current_status and "0.0.97 (97)" in current_status and "EditorBrowsableState" in current_status and "ReadingMode.Immediate" in current_status and "ReadingMode.Deferred" in current_status and "0.0.98 (98)" in current_status and "ICustomAttributeProvider" in current_status and "CS0104" in current_status and "0.0.99 (99)" in current_status and "NuGetPackageRoot" in current_status and "0.0.100 (100)" in current_status and "0.0.101 (101)" in current_status and "no `netstandard2.0` implementation" in current_status and "0.0.102 (102)" in current_status and "211 passed" in current_status and "exactly one failed" in current_status and "Step 27.0.19 / 0.0.103 (103)" in current_status and "net9.0/0Harmony.dll" in current_status and "netstandard compatibility reference" in current_status and "STS2_STEP27_REAL_HARMONY_FIXTURE" in current_status and "STS2_STEP27_REAL_HARMONY_ARCHIVE_MEMBER" in current_status and "post-publish interpreted fixture" in current_status and "ahead-of-load Cecil" in current_status and "T6" in current_status and "T7/T8" in current_status and "Step27-CrashCheckpoint.txt" in current_status and "Force-quit/relaunch before every Step-27 retry" in current_status and "26/26 PASS" in current_status and "Step 28" in current_status, "current status preserves Step 27 evidence and advances to Step 27.0.19 / 0.0.103 net9 surrogate reference-graph assertion fix")
