...
print("StS2 Launcher — Step 35.0.13 Stack-Neutral Command-Line / Godot boundary localization validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>136</ApplicationVersion>" in project_text, "build version is 136")
require("<ApplicationDisplayVersion>0.0.136</ApplicationDisplayVersion>" in project_text, "display version is 0.0.136")
require(plist.get("CFBundleVersion") == "136", "Info.plist build version is 136")
require(plist.get("CFBundleShortVersionString") == "0.0.136", "Info.plist display version is 0.0.136")
...
require("STEP 35.0.13 — STACK-NEUTRAL COMMAND-LINE/GODOT BOUNDARY LOCALIZATION" in release_presentation, "top launcher banner identifies active Step 35.0.13 stack-neutral Command-Line/Godot boundary-localization candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.129", "MissingMethodException", "Action<string>.Invoke(string)", "Action<string>::Invoke(!0)", "0.0.130", "0.0.131", "0.0.132", "NP003", "CALLSITE#002", "0.0.133", "NP002", "InvalidProgramException", "RUN_END", "MaxStack", "Godot.OS.GetCmdlineArgs", "0.0.136", "diagnostic"]), "top launcher banner preserves physical 0.0.129-0.0.133 history and identifies active 0.0.136 stack-neutral diagnostic")
...
require('ExpectedDisplayVersion = "0.0.136"' in release_presentation and 'ExpectedBuildVersion = "136"' in release_presentation, "Step 35.0.13 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.136",
    "STS2_BUILD_VERSION": "136",
...
require("Step 35.0.13 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.13 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.13 — Command-Line / Godot boundary localization with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.13 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.13 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.13 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
...
require(all(marker in step35_source for marker in ["BuildStaticInstructionMap", "CALLSITE#", "AWAIT-CANDIDATE", "DescribeMetadataScope", "GetVerifiedVeryEarlyStaticInstructionMap", "[NULL PLATFORM CTOR IL]", "[COMMAND LINE HELPER CCTOR IL]", "[COMMAND LINE HELPER TRYGETVALUE IL]", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName"]), "Step 35.0.13 static map covers the exact NullPlatform constructor plus CommandLineHelper cctor/TryGetValue callsite ordinals without resolving dependencies")
require(all(marker in step35_source for marker in ["CreateInstrumentedDiagnosticClone", "GetDiagnosticMarkerTargets", "HasInjectedEntryMarkerAtStart", "InsertNullPlatformConstructorCallsiteMarkers", "InsertCommandLineHelperCriticalBoundaryMarkers", "HasCommandLineHelperCriticalBoundaryMarkers", "CommandLineCctorDictionaryBeforeMarker", "CommandLineCctorDictionaryAfterMarker", "CommandLineCctorGetCmdlineArgsBeforeMarker", "CommandLineCctorGetCmdlineArgsAfterMarker", "CommandLineHelper cctor MaxStack exact-source/diagnostic", "NullPlatformTypeFullName", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName", "INMETHOD_NP", "INMETHOD_CL_CRITICAL", "INMETHOD_027", "DiagnosticBridgeTypeFullName", "INMETHOD_001", "INMETHOD_021", "INMETHOD_022", "INMETHOD_023", "INMETHOD_024", "INMETHOD_025", "INMETHOD_026", "INMETHOD_031", "INMETHOD_050", "INMETHOD_100", "INMETHOD_180", "INMETHOD_181", "INMETHOD_182", "INMETHOD_183", "C_DIAGNOSTIC_BRIDGE_ARMED", "preflight.DiagnosticPath", "preflight.DiagnosticSha256"]), "Step 35.0.13 preserves prior diagnostics and uses stack-neutral-only CommandLine localization")
...
require(sweep_definition >= 0 and sweep_skip > sweep_definition and sweep_increment > sweep_skip, "Step 35.0.13 excludes injected diagnostic bridge calls before exact-source callsite ordinal accounting")
require("InsertCommandLineHelperCriticalBoundaryMarkers(commandLineCctor, emitReference)" in step35_source and "could not locate Godot.OS.GetCmdlineArgs" in step35_source, "Step 35.0.13 fails Gate A closed unless stack-neutral critical localization contains Godot.OS.GetCmdlineArgs")
require(all(marker in step35_source for marker in ["commandLineCctorOriginalMaxStack", "commandLineCctorDiagnosticMaxStack", "commandLineCctorDiagnosticMaxStack != commandLineCctorOriginalMaxStack", "verifiedCommandLineCctor.Body.MaxStackSize != commandLineCctorOriginalMaxStack"]), "Step 35.0.13 preserves and post-write verifies unchanged CommandLine cctor MaxStack")
require("commandLineCctorCallsitePlan = InsertCommandLineHelperCctorCallsiteMarkers(commandLineCctor" not in step35_source and "commandLineTryGetValueCallsitePlan = InsertCommandLineHelperTryGetValueCallsiteMarkers(commandLineTryGetValue" not in step35_source, "Step 35.0.13 production clone forbids live-stack CL/CLTV sweep insertion")
require("if (cctor is not null && verifiedCctorTypes.Add(item.TypeName))\n            if (cctor is not null && verifiedCctorTypes.Add(item.TypeName))" not in step35_source, "Step 35.0.13 has no duplicated cctor-verification guard that can suppress serialized entry-marker checks")
require(all(marker in step35_source for marker in ["CommandLineCctorDictionaryBeforeMarker", "CommandLineCctorDictionaryAfterMarker", "CommandLineCctorGetCmdlineArgsBeforeMarker", "CommandLineCctorGetCmdlineArgsAfterMarker", "InsertMarkerBefore", "InsertMarkerAfter"]), "Step 35.0.13 carries redundant stack-neutral critical markers around dictionary and Godot command-line boundaries")
require(all(marker in step35_source for marker in ["module.Write(diagnosticPath", "verifyModule.Mvid != TransformedRealStS2AssemblyAdmission.ClosedStep32Mvid", "transformedSha256AfterDiagnosticEmission", "ComputeSha256Hex(preflight.TransformedPath)", "markerCountVerified != expectedMarkerCount"]), "Step 35.0.13 preserves/rechecks the exact transformed source and verifies diagnostic-clone MVID/hash/marker provenance")
require(all(marker in step35_source for marker in ["DiagnosticConstantMetadataWriteResolver", "DiagnosticCecilWriteSystemRuntimeIdentity", "DiagnosticCecilWriteSentryIdentity", "System.Reflection.BindingFlags", "Sentry.BreadcrumbLevel", "Sentry.SentryLevel", "CollectDiagnosticExternalConstantTypeRequirements", "resolver.Configure(module)", "resolver.ValidateWriteRequests()", "RealStS2PrepareMethodRewrite.ComputeConstantMetadataFingerprint", "verifiedConstantMetadataSha256.Equals(expectedConstantMetadataSha256", "External assembly bytes opened by the writer-only surrogate resolver: 0"]), "Step 35.0.13 reuses the Step-32 audited constant-metadata requirement set locally for Cecil serialization only")
require("new DiagnosticConstantMetadataWriteResolver()" in step35_source and "new RejectingAssemblyResolver()" in step35_source, "Step 35.0.13 separates bounded writer-only Cecil resolution from rejecting post-write verification")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.13 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.13 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.13 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.13 host regressions protect the Action<string>::Invoke(!0) encoding")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.13 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.13 regressions protect exact ordinals, critical boundaries and executable MaxStack safety")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.13 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.13 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.13 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.13 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.13 active source/UI reject stale exact-closure wording")
...
require("Step35ImplementationMarker" in release_presentation and "0x06007D02" in release_presentation and "0x0600BC71" in release_presentation and "ExecuteVeryEarly" in release_presentation and "0.0.136" in release_presentation, "release presentation pins the Step-35.0.13 exact-source/instrumented-clone boundary")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.13 / 0.0.136 (136)", "0.0.126", "0.0.129", "0.0.130", "0.0.131", "0.0.132", "0.0.133", "MissingMethodException", "Action<string>::Invoke(!0)", "NullPlatformUtilStrategy..ctor", "GodotFileIo..ctor", "INMETHOD_NP003_PRE", "INMETHOD_NP002_PRE", "InvalidProgramException", "RUN_END", "CALLSITE#002", "Godot.OS.GetCmdlineArgs", "MaxStack", "INMETHOD_CL_CRITICAL", "INMETHOD_CL", "INMETHOD_CLTV", "[NULL PLATFORM CTOR IL]", "[COMMAND LINE HELPER CCTOR IL]", "[COMMAND LINE HELPER TRYGETVALUE IL]", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact authority, 0.0.132 game frontier, 0.0.133 diagnostic defect and active 0.0.136 stack-neutral localization")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.136 (136)" in testing_doc and "0.0.136 (136)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.136 (136) release identity")
require("0.0.136 (132)" not in testing_doc and "0.0.136 (131)" not in testing_doc and "0.0.136 (132)" not in release_checklist_doc and "0.0.136 (131)" not in release_checklist_doc, "release docs reject display/build-number drift like the prior 0.0.130 (129) documentation bug")
...
require((ROOT / "docs/history/steps/STEP-35.0.13-STACK-NEUTRAL-COMMANDLINE-GODOT-LOCALIZATION.md").is_file(), "Step 35.0.13 stack-neutral CommandLine/Godot localization design record is present")
...
require("Step 35.0.13 Stack-Neutral Command-Line / Godot Boundary Localization build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.13 stack-neutral candidate")
...
require("Expected device UI: STEP 35.0.13 — STACK-NEUTRAL COMMAND-LINE/GODOT BOUNDARY LOCALIZATION" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.13 stack-neutral device boundary")
...
if step35_manifest.is_file():
    ...
    required_step35_manifest_paths = [
        ...
        "docs/history/steps/STEP-35.0.13-STACK-NEUTRAL-COMMANDLINE-GODOT-LOCALIZATION.md",
    ]
    require(
        all(f"  {relative}" in step35_manifest_text for relative in required_step35_manifest_paths),
        "Step 35.0.13 candidate manifest includes active implementation/tests and the 0.0.133 evidence + 0.0.136 design record",
    )
    mismatches=[]
