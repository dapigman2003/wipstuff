...
require("<ApplicationVersion>207</ApplicationVersion>" in project_text, "build version is 207")
require("<ApplicationDisplayVersion>0.0.207</ApplicationDisplayVersion>" in project_text, "display version is 0.0.207")
require(plist.get("CFBundleVersion") == "207", "Info.plist build version is 207")
require(plist.get("CFBundleShortVersionString") == "0.0.207", "Info.plist display version is 0.0.207")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.207 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.207 (207)**" in read("README.md") and "Active candidate: **0.0.207 (207)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.207 (207) release identity")
require(all(v in read("docs/MASTER-PLAN.md") for v in ["0.0.207", "Core runtime is frozen byte-for-byte", "59T CONTROL", "59W WRAPPER-ONLY", "59X FULL REPAIR", "Step 63"]), "master plan points to the active 0.0.207 viability-hardened compilation-efficient PropertyTweener frontier")
...
require('ExpectedDisplayVersion = "0.0.207"' in release_presentation and 'ExpectedBuildVersion = "207"' in release_presentation, "startup-ladder source pins expected 0.0.207 bundle release identity")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs") == "f32eef56c8120b30a453237cfbcb63c89eb39e0a3dae6946a5a780f07002e342", "0.0.207 freezes the Codemagic-compiled 0.0.206 VeryEarly Core runtime bytes")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2CharacterSelectContinuation.cs") == "2e9665de425a0ce2e0478ea755a05e3b22a4909f9b9c5cee41d2b19cf4ce734c", "0.0.207 freezes the Codemagic-compiled 0.0.206 character-select Core runtime bytes")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadder.cs") == "7968570ec64d710ba80d9f8a3ba98d8fd037269651e9348a08d51475cb8d7057", "0.0.207 freezes the Codemagic-compiled 0.0.206 startup-ladder Core runtime bytes")
...
require(all(value in closed_path for value in ["_physicallyClosedPathStarted", "IsFreshProcessForPhysicallyClosedPath", "_physicallyClosedPathButton.Enabled = false", "RunClosedPhysicalStageAsync", "if (!passed())", "No later stage was invoked", "PhysicallyClosedPath-ToStep52.txt"]), "closed-path runner is fresh-process-only, one-shot, fail-closed, and has a convenience report")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.207",
    "STS2_BUILD_VERSION": "207",
...
require(used_assert_methods <= known_mstest_v4_assert_methods, "0.0.207 test sources use only the repository's known compiling MSTest-v4 Assert API surface", "unexpected=" + ",".join(sorted(used_assert_methods - known_mstest_v4_assert_methods)))
require("Assert.Greater(" not in all_test_text and "Assert.IsTrue(fluentRepairCalls > 0, fluentName);" in step35_tests, "0.0.207 replaces the 0.0.206 unsupported Assert.Greater host-test compile stop with an established Assert.IsTrue form")
...
require("0.0.201" in testing_doc and "0.0.201" in release_checklist_doc and "0.0.201 NConfirmButton OnEnable-preflight expectations" in testing_doc and "0.0.201 NConfirmButton OnEnable-preflight requirements" in release_checklist_doc and "M59_B_CONFIRM_BUTTON_PREFLIGHT" in testing_doc, "testing and release-checklist docs pin the 0.0.201 NConfirmButton OnEnable-preflight PackedScene compatibility contract")
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.207\"", "ExpectedBuildVersion = \"207\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL"]), "active release presentation pins Step-52 current architecture and Steps-58–62 / 0.0.206 identity")
...
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.207 UI unlocks active Step 58 directly from same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.207 active ownership path contains no obsolete Step-57 prerequisite gate")
...
require(hashlib.sha256(packed_slice_bytes).hexdigest() == "36bbdd3f8bee48c4a71b29bbb4ac34a3e7ac4b7718f8abe25006535cff0f9395", "0.0.207 preserves the exact physically proven PackedScene compatibility implementation bytes")
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(marker not in very_early_core for marker in ["STEP59TRACE_", "ConfirmButtonCheckpointBridge", "Step59RuntimeTraceBridge", "ArmStep59RuntimeTrace", "DisarmStep59RuntimeTrace"]), "0.0.207 keeps the rejected 0.0.202 runtime-wide Step-59 trace bridge absent")
require(all(v in very_early_core for v in ["GodotSharp.step59.7.property-tweener-observe-repair.dll", "GodotSharpPropertyTweenerRepairEnabledFieldName", "GodotSharpPropertyTweenerFluentRepairEnabledFieldName", "GodotSharpPropertyTweenerObservationCountFieldName", "GodotSharpPropertyTweenerNativeNullCountFieldName", "GodotSharpPropertyTweenerManagedNullCountFieldName", "GodotSharpPropertyTweenerWrongTypeCountFieldName", "GodotSharpPropertyTweenerLastManagedObjectFieldName", "GodotSharpPropertyTweenerFluentFallbackMethodName", "PATCHED_TYPED_RETURN_EPILOGUE_OBSERVE_REPAIR"]), "0.0.207 retains the Codemagic-compiled independently switchable TweenProperty-wrapper and typed fluent-return observation/repair telemetry")
require("could not find PropertyTweener cast after native return" not in very_early_core and "followingIsinst" not in very_early_core, "0.0.207 retains the 0.0.206 removal of the 0.0.205 speculative fluent post-native isinst-shape assumption")
require(all(v in character_core for v in ["RunStep59ConfirmButtonTailWrapperRepairProbe", "RunStep59ConfirmButtonTailRepairProbe", "SetStep59PropertyTweenerRepairMode", "M59E_PROPERTY_TWEENER_REPAIR_MODE", "M59_C_PROPERTY_TWEENER_REPAIR_MODE", "PROPERTYTWEENER MANAGED-WRAPPER OBSERVE/REPAIR", "wrapperRepairEnabled=", "fluentRepairEnabled=", "wrongTypes=", "lastManagedType=", "fluentRepairs="]), "0.0.207 retains three-mode control/wrapper/full-repair state across 59T/59W/59X/59E/real Step59")
require(all(v in step35_tests for v in ["godot_icall_4_1441", "GodotSharpPropertyTweenerRepairEnabledFieldName", "GodotSharpPropertyTweenerFluentRepairEnabledFieldName", "GodotSharpPropertyTweenerWrongTypeCountFieldName", "GodotSharpPropertyTweenerFluentFallbackMethodName", "PATCHED_DEDICATED_HELPER_OBSERVE_REPAIR", "PATCHED_TYPED_RETURN_EPILOGUE_OBSERVE_REPAIR", "AddFluentTypedReturn"]), "0.0.207 host fixture retains serialization/reopen coverage for dedicated TweenProperty repair while deliberately varying fluent IL shapes")
...
require((ROOT / "docs/history/steps/STEP-59.13-VIABILITY-HARDENING-0.0.207.md").is_file(), "0.0.207 history records the 0.0.206 host-test compile stop, runtime freeze, and validation correction")
require((ROOT / "docs/history/reports/STEP-59.13-CODEMAGIC-0.0.206-HOST-TEST-COMPILE-FAILURE.txt").is_file(), "0.0.207 preserves the supplied 0.0.206 Codemagic host-test compile evidence")
require(all(v in read("docs/history/reports/STEP-59.13-CODEMAGIC-0.0.206-HOST-TEST-COMPILE-FAILURE.txt") for v in ["VALIDATION PASS: 1437 checks", "StS2Launcher.Core: compiled successfully", "Assert' does not contain a definition for 'Greater'", "No iOS build/device result exists for 0.0.206"]), "0.0.207 history accurately classifies 0.0.206 as a host-test construction failure after successful Core compilation")
require(all(v in character_core for v in ["RunStep59ControllerSingletonRehearsal", "RunStep59RegisterHotkeysProbe", "RunStep59UpdateControllerButtonProbe", "RunStep59ConfirmButtonTailProbe", "RunStep59ConfirmButtonTailWrapperRepairProbe", "RunStep59ConfirmButtonTailRepairProbe", "RunStep59IsolatedEmbarkEnableProbe", "M59H_INVOKE_ARMED", "M59U_INVOKE_ARMED", "M59E_ENABLE_ARMED"]), "0.0.207 retains controller, hotkey, controller-update, control-tail, wrapper-only, full-repair and full-Enable branches in one compilation")
require(all(v in character_ui for v in ["Run Step 59R — CONTROLLER REHEARSAL", "Run Step 59H — REGISTER HOTKEYS ONLY", "Run Step 59U — UPDATE CONTROLLER ONLY", "Run Step 59T — CONTROL / OBSERVE", "Run Step 59W — WRAPPER REPAIR ONLY", "Run Step 59X — FULL REPAIR / FULL TAIL", "Run Step 59E — ISOLATED EMBARK ENABLE"]), "0.0.207 exposes all reusable Step-59 isolation branches in the same IPA")
...
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(x in very_early_core for x in ["GodotSharp.step59.7.property-tweener-observe-repair.dll", "GodotSharpPackedSceneCompatibilityCallbackFieldName", "PackedSceneInstantiateCompatibilityCallback", "ApplyPackedSceneInstancedRootUniqueNameCompatibility", "GetNodeInstance", "unique_name_in_owner", "GODOT_PACKEDSCENE_UNIQUE_NAME_COMPAT_ARMED", "CreateDiagnosticActionObjectObjectInvokeReference"]), "0.0.199 implements a central private-GodotSharp PackedScene instanced-root unique-name compatibility hook")
