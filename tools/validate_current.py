...
require("STEP 35.0.14 — COMPREHENSIVE GODOTSHARP / NATIVE RECON + DUAL MODE" in release_presentation, "top launcher banner identifies active Step 35.0.14 comprehensive GodotSharp/native dual-mode candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.129", "0.0.132", "0.0.133", "0.0.135", "0.0.136", "CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "Dictionary<string,string>", "Godot.OS.GetCmdlineArgs", "NATURAL", "COMPAT", "Mach-O", "GodotSharp", "entry-only", "0.0.137", "diagnostic"]), "top launcher banner preserves physical Step-35 history and identifies active 0.0.137 comprehensive dual-mode reconnaissance derivative")
...
# ---------------------------------------------------------------------------
# Step 35.0.14 — comprehensive GodotSharp/native reconnaissance + dual mode with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_mode for marker in ["NaturalGodotDictionaryRecon", "ManagedDictionaryCompatibility"]), "Step 35.0.14 exposes NATURAL and COMPAT modes in one build")
require(all(marker in step35_recon for marker in ["[GODOTSHARP MANAGED / NATIVE-BOUNDARY MAP]", "GodotSharp P/Invoke methods", "GodotSharp calli sites", "NativeFuncs/UnmanagedCallbacks field-use sites", "[MACH-O / NATIVE INVENTORY]", "sha256=", "uuid=", "dylib=", "rpath=", "symbol=", "string=", "MH_MAGIC_64", "FAT_MAGIC", "0x0100000C"]), "Step 35.0.14 reconnaissance records managed/native boundaries and read-only arm64 Mach-O metadata")
require("LoadFromStream" not in step35_recon and "NativeLibrary.Load" not in step35_recon and "DllImport" not in step35_recon, "Step 35.0.14 reconnaissance source performs no CLR/native image execution")
...
require((ROOT / "docs/history/steps/STEP-35.0.14-COMPREHENSIVE-GODOT-NATIVE-RECONNAISSANCE.md").is_file(), "active Step 35.0.14 comprehensive Godot/native reconnaissance design record is present")
...
require("Step 35.0.14 Comprehensive GodotSharp / Native Recon + Dual Mode build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.14 comprehensive dual-mode candidate")
...
require("Expected device UI: STEP 35.0.14 — COMPREHENSIVE GODOTSHARP / NATIVE RECON + DUAL MODE" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.14 comprehensive dual-mode candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.14 candidate manifest includes comprehensive implementation/tests, physical 0.0.136 evidence, and both superseded + active 0.0.137 design records",
    )
    mismatches=[]
