...
print("StS2 Launcher — Step 35.0.24 Post-Bootstrap Resolver Baseline validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>147</ApplicationVersion>" in project_text, "build version is 147")
require("<ApplicationDisplayVersion>0.0.147</ApplicationDisplayVersion>" in project_text, "display version is 0.0.147")
require(plist.get("CFBundleVersion") == "147", "Info.plist build version is 147")
require(plist.get("CFBundleShortVersionString") == "0.0.147", "Info.plist display version is 0.0.147")
...
require("STEP 35.0.24 — POST-BOOTSTRAP RESOLVER BASELINE CORRECTION" in release_presentation, "top launcher banner identifies active Step 35.0.24 post-bootstrap resolver baseline candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.140", "0.0.141", "210/211", "GS031", "GS024", "GodotFileIo.CreateDirectory", "NATURAL", "OS-RECON", "FORWARD", "CORE-HANDOFF", "NativeFuncs.Initialize(IntPtr,int)", "0.0.143", "1,800-byte", "225-pointer", "initialized=true", "GetCmdlineArgs", "OS.get_Singleton", "EngineGetSingleton", "0.0.144", "GS035", "0.0.145", "godotApiCacheUpdated=false", "0.0.146", "37-pointer", "ManagedCallbacks", "GD_OnCoreApiAssemblyLoaded", "eight-request", "0.0.147", "post-bootstrap"]), "top launcher banner preserves prior CI/physical provenance, records the positive 0.0.146 managed-plugin bridge bootstrap, and identifies the 0.0.147 post-bootstrap resolver-baseline correction")
...
require('ExpectedDisplayVersion = "0.0.147"' in release_presentation and 'ExpectedBuildVersion = "147"' in release_presentation, "Step 35.0.24 source pins expected bundle release identity")
...
require(not unexpected_platform_changed, "platform/native behavior remains protected except the explicit Step-35.0.24 callback-handoff bridge delta", ", ".join(unexpected_platform_changed))
require(step35_callback_platform_delta.issubset(set(platform_changed)), "Step-35.0.24 callback-handoff bridge delta is explicit rather than silently matching the old no-callback baseline")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.147",
    "STS2_BUILD_VERSION": "147",
...
require("Step 35.0.24 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.24 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.24 — post-bootstrap resolver baseline correction with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
for path, label in [(step35_source_path,"Step 35 very-early core"),(step35_gate_path,"Step 35 gate enum"),(step35_summary_path,"Step 35 summary"),(step35_tests_path,"Step 35 host regressions"),(step35_ui_path,"Step 35 iOS report surface"),(step35_bootstrap_ui_path,"Step 35 managed-plugin bootstrap iOS partial"),(step35_mode_path,"Step 35 diagnostic mode"),(step35_recon_path,"Step 35 Godot/native reconnaissance")]:
    require(path.is_file(), f"{label} exists")
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.24 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.24 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.24 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
require("STEP 35.0.24 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.24 host gate-summary assertion matches the active production summary identity")
require("STEP 35.0.15 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" not in step35_tests, "Step 35.0.24 static validation rejects the stale 0.0.139 Step-35.0.15 gate-summary assertion")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.24 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.24 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.24 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.24 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.24 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.24 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require(all(marker in project_text for marker in new_native_exports), "iOS ReferenceNativeSymbol roots preserve every Step-35.0.24 managed-plugin bootstrap export")
...
require(all(marker in step35_ui for marker in ["CORE-HANDOFF", "GodotCoreCallbackHandoff", "Run Step 15 Gates A-C first", "IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "CB_NATIVE_TABLE_REQUEST_RETURNED", "RunGodotCoreCallbackHandoffInitialization"]), "Step 35.0.24 iOS surface gates CORE-HANDOFF on an already-live Step-15 engine, rejects competing dotnet state, obtains the exact native table, and records the managed handoff")
require("using StS2Launcher.iOS.Platform;" in step35_ui and "GodotStep15NativeBridge" in step35_ui, "Step 35.0.24 Step-35 iOS partial explicitly imports the Platform namespace required to resolve GodotStep15NativeBridge and protects the 0.0.142 CS0103 compile regression")
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.24 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.24 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.24 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.24 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.24 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.24 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.24 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.24 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.24 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.24 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.24 active source/UI reject stale exact-closure wording")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.24 / 0.0.147 (147)", "0.0.137", "208/209", "0.0.139", "209", "210", "0.0.141", "210 passed / 1 failed", "0.0.142", "855/855", "211/211", "CS0103", "StS2Launcher.iOS.Platform", "0.0.143", "1,800 bytes / 225 pointers", "initialized=true", "CL_CRITICAL_001_POST", "Godot.OS.GetCmdlineArgs", "GS039", "Godot.OS::get_Singleton()", "InteropUtils.EngineGetSingleton", "UnmanagedGetManaged", "godotsharp_engine_get_singleton", "0.0.145", "Physical 0.0.146", "CB_MANAGED_PLUGIN_BOOTSTRAP_PASS", "managed=10; host=9; private=1", "CB_POST_BOOTSTRAP_RESOLVER_BASELINE_PASS", "GodotCoreCallbackHandoff", "godotsharp::get_runtime_interop_funcs", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact authority and CI provenance, records the physical 0.0.146 managed-plugin bridge success/stale-guard stop, and documents the active 0.0.147 post-bootstrap resolver-baseline correction")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.147 (147)" in testing_doc and "0.0.147 (147)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.147 (147) release identity")
...
require((ROOT / "docs/history/steps/STEP-35.0.23-GODOT-MANAGED-PLUGIN-BRIDGE-BOOTSTRAP.md").is_file(), "Step 35.0.24 managed-plugin bridge bootstrap design record is present")
...
require("Step 35.0.24 Post-Bootstrap Resolver Baseline build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.24 post-bootstrap resolver baseline candidate")
...
require("Expected device UI: STEP 35.0.24 — POST-BOOTSTRAP RESOLVER BASELINE CORRECTION" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.24 post-bootstrap resolver baseline candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.24 candidate manifest includes implementation/tests, native/build bridge, prior CI/physical provenance, the physical 0.0.146 managed-plugin bridge success/stale-guard result, and the active 0.0.147 post-bootstrap resolver-baseline correction",
    )
    mismatches=[]
