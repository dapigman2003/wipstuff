...
print("StS2 Launcher — Step 35.0.15 Comprehensive GodotSharp / Native Recon + Dual Mode validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>138</ApplicationVersion>" in project_text, "build version is 138")
require("<ApplicationDisplayVersion>0.0.138</ApplicationDisplayVersion>" in project_text, "display version is 0.0.138")
require(plist.get("CFBundleVersion") == "138", "Info.plist build version is 138")
require(plist.get("CFBundleShortVersionString") == "0.0.138", "Info.plist display version is 0.0.138")
...
require("STEP 35.0.15 — COMPREHENSIVE GODOTSHARP / NATIVE RECON + DUAL MODE" in release_presentation, "top launcher banner identifies active Step 35.0.15 comprehensive GodotSharp/native dual-mode candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.129", "0.0.132", "0.0.133", "0.0.135", "0.0.136", "CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "Dictionary<string,string>", "Godot.OS.GetCmdlineArgs", "NATURAL", "COMPAT", "Mach-O", "GodotSharp", "entry-only", "0.0.137", "0.0.138", "208/209", "diagnostic"]), "top launcher banner preserves physical/CI Step-35 history and identifies active 0.0.138 comprehensive dual-mode reconnaissance derivative")
...
require('ExpectedDisplayVersion = "0.0.138"' in release_presentation and 'ExpectedBuildVersion = "138"' in release_presentation, "Step 35.0.15 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.138",
    "STS2_BUILD_VERSION": "138",
...
require("Step 35.0.15 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.15 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.15 — comprehensive GodotSharp/native reconnaissance + dual mode with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.15 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.15 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.15 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.15 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.15 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.15 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.15 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.15 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.15 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require("string expectedBridgeTypeFullName = DiagnosticBridgeTypeFullName" in step35_source and "HasInjectedEntryMarkerAtStart(method, item.Marker, GodotSharpDiagnosticBridgeTypeFullName)" in godot_clone_block and "call.DeclaringType.FullName == expectedBridgeTypeFullName" in step35_source, "Step 35.0.15 verifies serialized entry markers against the derivative-specific bridge type and protects the 0.0.137 208/209 regression")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.15 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.15 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require(all(marker in step35_source for marker in ["B_LOADFROMSTREAM_START", "B_LOADFROMSTREAM_PASS", "C_BIND_TYPE_START", "C_BIND_METHOD_START", "C_INVOKE_START", "C_INVOKE_RETURNED", "C_TASK_CONFIRMED", "C_WAIT_START", "C_WAIT_COMPLETED", "RESOLVE_MANAGED_START", "RESOLVE_PRIVATE_PASS", "RESOLVE_HOST_PASS", "RESOLVE_NATIVE_REJECT"]), "Step 35.0.15 core retains durable B→C/invocation/resolver crash-localization markers")
require(all(marker in step35_ui for marker in ["Step35-CurrentRun.txt", "Step35-LastCheckpoint.txt", "Step35-CrashCheckpoint-", "Step35-ExecuteVeryEarly-StaticMap-", "Step35-GodotNativeReconnaissance-", "TryInitializeStep35RunTelemetry", "WriteStep35StaticMap", "WriteStep35GodotReconnaissance", "Flush(flushToDisk: true)", "Environment.CurrentManagedThreadId", "CANCELLED / INCONCLUSIVE", "Output-only diagnostic", "0.0.136", "CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "System.Collections.Generic.Dictionary<string,string>", "NaturalGodotDictionaryRecon", "ManagedDictionaryCompatibility", "entry-only GodotSharp", "Mach-O/native"]), "Step 35.0.15 iOS surface preserves the physical 0.0.136 frontier and exposes dual-mode comprehensive GodotSharp/native reconnaissance")
require("crashCheckpoints.Any" in step35_tests and "B_LOADFROMSTREAM_START" in step35_tests and "RESOLVE_INITIALIZER_BEARING_REJECT" in step35_tests and "StaticInstructionMapCapturesCallsitesAndAwaitCandidatesWithoutResolution" in step35_tests, "Step 35.0.15 host regressions retain crash-checkpoint and static-map coverage")
require("OrderedDiagnosticLocalizationGatesReachFourOfFourWithoutClaimingClosure" in step35_tests and "VeryEarlyInitializationStopsAfterFirstFailure" in step35_tests and "VeryEarlyContextLoadsInitializerFreePrivateDependencyAndRejectsInitializerBearingDependency" in step35_tests and "Step35PinsTheExactVeryEarlyManagedInitializationTarget" in step35_tests, "Step 35.0.15 host regressions protect diagnostic gate order, exact source target and initializer-bearing refusal")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.15 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.15 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.15 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.15 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.15 active source/UI reject stale exact-closure wording")
...
require("Step35ImplementationMarker" in release_presentation and "0x06007D02" in release_presentation and "0x0600BC71" in release_presentation and "ExecuteVeryEarly" in release_presentation and "0.0.137" in release_presentation and "0.0.138" in release_presentation, "release presentation pins the Step-35.0.15 exact-source/instrumented-clone boundary and 0.0.137 CI provenance")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.15 / 0.0.138 (138)", "0.0.126", "0.0.129", "0.0.130", "0.0.131", "0.0.132", "0.0.133", "0.0.135", "0.0.136", "Action<string>::Invoke(!0)", "NullPlatformUtilStrategy..ctor", "INMETHOD_NP003_PRE", "INMETHOD_CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "System.Collections.Generic.Dictionary<string,string>", "Godot.OS.GetCmdlineArgs", "NATURAL", "COMPAT", "Mach-O", "entry-only", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact authority, physical 0.0.136 dictionary frontier, 0.0.137 CI failure, and active 0.0.138 comprehensive dual-mode reconnaissance design")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.138 (138)" in testing_doc and "0.0.138 (138)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.138 (138) release identity")
require("0.0.138 (137)" not in testing_doc and "0.0.138 (136)" not in testing_doc and "0.0.138 (137)" not in release_checklist_doc and "0.0.138 (136)" not in release_checklist_doc, "release docs reject display/build-number drift like the prior 0.0.130 (129) documentation bug")
...
require("Step 35.0.15 Comprehensive GodotSharp / Native Recon + Dual Mode build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.15 comprehensive dual-mode candidate")
...
require("Expected device UI: STEP 35.0.15 — COMPREHENSIVE GODOTSHARP / NATIVE RECON + DUAL MODE" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.15 comprehensive dual-mode candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.15 candidate manifest includes implementation/tests, physical 0.0.136 evidence, 0.0.137 CI failure provenance, and the 0.0.138 verifier correction record",
    )
    mismatches=[]
