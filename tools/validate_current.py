...
require("<ApplicationVersion>205</ApplicationVersion>" in project_text, "build version is 205")
require("<ApplicationDisplayVersion>0.0.205</ApplicationDisplayVersion>" in project_text, "display version is 0.0.205")
require(plist.get("CFBundleVersion") == "205", "Info.plist build version is 205")
require(plist.get("CFBundleShortVersionString") == "0.0.205", "Info.plist display version is 0.0.205")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.205 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.205 (205)**" in read("README.md") and "Active candidate: **0.0.205 (205)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.205 (205) release identity")
require(all(v in read("docs/MASTER-PLAN.md") for v in ["0.0.205", "maximizes information/optionality per compilation", "59T CONTROL", "59X REPAIR", "Step 63"]), "master plan points to the active 0.0.205 compilation-efficient PropertyTweener observe/repair frontier")
...
require('ExpectedDisplayVersion = "0.0.205"' in release_presentation and 'ExpectedBuildVersion = "205"' in release_presentation, "startup-ladder source pins expected 0.0.205 bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.205",
    "STS2_BUILD_VERSION": "205",
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.205\"", "ExpectedBuildVersion = \"205\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL"]), "active release presentation pins Step-52 current architecture and Steps-58–62 / 0.0.205 identity")
...
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.205 UI unlocks active Step 58 directly from same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.205 active ownership path contains no obsolete Step-57 prerequisite gate")
...
require(hashlib.sha256(packed_slice_bytes).hexdigest() == "36bbdd3f8bee48c4a71b29bbb4ac34a3e7ac4b7718f8abe25006535cff0f9395", "0.0.205 preserves the exact physically proven PackedScene compatibility implementation bytes")
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(marker not in very_early_core for marker in ["STEP59TRACE_", "ConfirmButtonCheckpointBridge", "Step59RuntimeTraceBridge", "ArmStep59RuntimeTrace", "DisarmStep59RuntimeTrace"]), "0.0.205 keeps the rejected 0.0.202 runtime-wide Step-59 trace bridge absent")
require(all(v in very_early_core for v in ["GodotSharp.step59.7.property-tweener-observe-repair.dll", "GodotSharpPropertyTweenerRepairEnabledFieldName", "GodotSharpPropertyTweenerObservationCountFieldName", "GodotSharpPropertyTweenerNativeNullCountFieldName", "GodotSharpPropertyTweenerManagedNullCountFieldName", "GodotSharpPropertyTweenerWrongTypeCountFieldName", "GodotSharpPropertyTweenerLastManagedObjectFieldName", "GodotSharpPropertyTweenerFluentFallbackMethodName", "PATCHED_METHOD_LOCAL_OBSERVE_REPAIR"]), "0.0.205 contains switchable full-chain PropertyTweener observation and repair telemetry")
require(all(v in character_core for v in ["RunStep59ConfirmButtonTailRepairProbe", "SetStep59PropertyTweenerRepairEnabled", "M59E_PROPERTY_TWEENER_REPAIR_MODE", "M59_C_PROPERTY_TWEENER_REPAIR_MODE", "PROPERTYTWEENER MANAGED-WRAPPER OBSERVE/REPAIR", "wrongTypes=", "lastManagedType=", "fluentRepairs="]), "0.0.205 exposes control/repair state across 59T/59X/59E/real Step59")
require(all(v in step35_tests for v in ["godot_icall_4_1441", "GodotSharpPropertyTweenerRepairEnabledFieldName", "GodotSharpPropertyTweenerWrongTypeCountFieldName", "GodotSharpPropertyTweenerFluentFallbackMethodName", "PATCHED_DEDICATED_HELPER_OBSERVE_REPAIR", "PATCHED_METHOD_LOCAL_OBSERVE_REPAIR"]), "0.0.205 host fixture serializes and reopens dedicated TweenProperty plus method-local fluent observe/repair hooks")
...
require(all(v in character_core for v in ["RunStep59ControllerSingletonRehearsal", "RunStep59RegisterHotkeysProbe", "RunStep59UpdateControllerButtonProbe", "RunStep59ConfirmButtonTailProbe", "RunStep59ConfirmButtonTailRepairProbe", "RunStep59IsolatedEmbarkEnableProbe", "M59H_INVOKE_ARMED", "M59U_INVOKE_ARMED", "M59E_ENABLE_ARMED"]), "0.0.205 retains controller, hotkey, controller-update, control-tail, repair-tail and full-Enable branches in one compilation")
require(all(v in character_ui for v in ["Run Step 59R — CONTROLLER REHEARSAL", "Run Step 59H — REGISTER HOTKEYS ONLY", "Run Step 59U — UPDATE CONTROLLER ONLY", "Run Step 59T — CONTROL / OBSERVE", "Run Step 59X — REPAIR / FULL TAIL", "Run Step 59E — ISOLATED EMBARK ENABLE"]), "0.0.205 exposes all reusable Step-59 isolation branches in the same IPA")
...
require("_Ready().Invoke" not in confirm_diag_slice and "OnEnable().Invoke" not in confirm_diag_slice and "_ascensionPanel" not in confirm_diag_slice, "0.0.201 confirm-button preflight remains free of direct game-field/Ready/OnEnable repair; 0.0.205 may toggle only its GodotSharp bridge field")
...
require("submenuStackField.SetValue" not in step59_core_slice and "_ascensionPanel" not in step59_core_slice, "real Step 59 binding/transition never repairs submenu/game ownership by directly writing game fields; 0.0.205 may toggle only its GodotSharp repair bridge field")
