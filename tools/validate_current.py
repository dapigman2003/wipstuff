...
require("<ApplicationVersion>204</ApplicationVersion>" in project_text, "build version is 204")
require("<ApplicationDisplayVersion>0.0.204</ApplicationDisplayVersion>" in project_text, "display version is 0.0.204")
require(plist.get("CFBundleVersion") == "204", "Info.plist build version is 204")
require(plist.get("CFBundleShortVersionString") == "0.0.204", "Info.plist display version is 0.0.204")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.204 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.204 (204)**" in read("README.md") and "Active candidate: **0.0.204 (204)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.204 (204) release identity")
require("Active candidate — 0.0.204 current ownership + PropertyTweener managed-wrapper compatibility" in read("docs/MASTER-PLAN.md") and "PackedScene.Instantiate(GenEditState)" in read("docs/MASTER-PLAN.md") and "legacy Steps 53–57" in read("docs/MASTER-PLAN.md"), "master plan points to the active 0.0.204 PropertyTweener compatibility frontier while retaining Step52-to-58 ownership and PackedScene compatibility")
...
require('ExpectedDisplayVersion = "0.0.204"' in release_presentation and 'ExpectedBuildVersion = "204"' in release_presentation, "startup-ladder source pins expected 0.0.204 bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.204",
    "STS2_BUILD_VERSION": "204",
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.204\"", "ExpectedBuildVersion = \"204\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL"]), "active release presentation pins Step-52 current architecture and Steps-58–62 / 0.0.204 identity")
...
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.204 UI unlocks active Step 58 directly from same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.204 active ownership path contains no obsolete Step-57 prerequisite gate")
...
require(hashlib.sha256(packed_slice_bytes).hexdigest() == "36bbdd3f8bee48c4a71b29bbb4ac34a3e7ac4b7718f8abe25006535cff0f9395", "0.0.204 preserves the exact physically proven PackedScene compatibility implementation bytes")
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(marker not in very_early_core for marker in ["STEP59TRACE_", "ConfirmButtonCheckpointBridge", "Step59RuntimeTraceBridge", "ArmStep59RuntimeTrace", "DisarmStep59RuntimeTrace"]), "0.0.204 removes the rejected 0.0.202 runtime-wide Step-59 trace bridge and shared-method markers")
require(all(v in very_early_core for v in ["GodotSharp.step59.6.property-tweener-compat.dll", "GodotSharpPropertyTweenerFallbackCountFieldName", "GodotSharpPropertyTweenerLastNativePtrFieldName", "GodotSharpPropertyTweenerFallbackMethodName", "ApplyPropertyTweenerManagedWrapperCompatibility", "Godot.Tween", "TweenProperty", "Godot.NativeCalls", "UnmanagedGetManaged", "Godot.PropertyTweener", "System.IntPtr", "managed=null", "nativePtr!=0"]), "0.0.204 contains the bounded TweenProperty PropertyTweener managed-wrapper compatibility and telemetry")
require(all(v in character_core for v in ["BuildStep59PropertyTweenerCompatibilityRuntimeState", "M59T_PROPERTY_TWEENER_COMPAT", "M59E_PROPERTY_TWEENER_COMPAT", "PROPERTYTWEENER MANAGED-WRAPPER COMPATIBILITY"]), "0.0.204 exposes PropertyTweener compatibility state in Step-59 probes and diagnostic deck")
require(all(v in step35_tests for v in ["godot_icall_4_1441", "Godot.PropertyTweener", "GodotSharpPropertyTweenerFallbackMethodName", "PATCHED_DEDICATED_HELPER", "UNCHANGED_SHARED_HELPER"]), "0.0.204 host fixture serializes and reopens the dedicated TweenProperty helper compatibility while preserving shared fluent helpers")
require((ROOT / "docs/history/steps/STEP-59.10-PROPERTYTWEENER-COMPAT-0.0.204.md").is_file() and all(v in read("docs/history/steps/STEP-59.10-PROPERTYTWEENER-COMPAT-0.0.204.md") for v in ["59U", "Tween.TweenProperty", "returned `null`", "59E", "one callsite", "PropertyTweener(IntPtr)", "59T", "Step 63"]), "0.0.204 history note records physical localization, narrow compatibility, and compilation-efficient test sequence")
require(all(v in character_core for v in ["RunStep59ControllerSingletonRehearsal", "RunStep59RegisterHotkeysProbe", "RunStep59UpdateControllerButtonProbe", "RunStep59ConfirmButtonTailProbe", "RunStep59IsolatedEmbarkEnableProbe", "M59H_INVOKE_ARMED", "M59U_INVOKE_ARMED", "M59T_STAGE_PRE", "M59T_STAGE_POST", "M59T_STAGE_FAIL", "M59E_ENABLE_ARMED"]), "0.0.204 retains controller, hotkey, controller-update, confirm-tail and full-Enable Step-59 probes in one compilation")
require(all(v in character_ui for v in ["Run Step 59R — CONTROLLER REHEARSAL", "Run Step 59H — REGISTER HOTKEYS ONLY", "Run Step 59U — UPDATE CONTROLLER ONLY", "Run Step 59T — CONFIRM TAIL ONLY", "Run Step 59E — ISOLATED EMBARK ENABLE"]), "0.0.204 exposes all reusable Step-59 isolation branches in the same IPA")
...
require("SetValue(" not in confirm_diag_slice and "_Ready().Invoke" not in confirm_diag_slice and "OnEnable().Invoke" not in confirm_diag_slice, "0.0.201 confirm-button preflight is observational and performs no direct field/Ready/OnEnable repair")
...
require("SetValue(" not in step59_core_slice, "real Step 59 binding/transition never repairs submenu ownership by directly writing a runtime field; isolated 59T may set only Godot visual properties")
...
require(all(v not in deck_slice for v in ["SetValue(", 'GetMethod("_Ready")', 'GetMethod("OnEnable")']), "0.0.201 compilation-efficient diagnostic deck performs no direct field write or reflective Ready/OnEnable replay")
