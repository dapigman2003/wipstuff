...
print("StS2 Launcher — Step 35.0.19 Comprehensive GodotSharp / Native Recon + Dual Mode validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>142</ApplicationVersion>" in project_text, "build version is 142")
require("<ApplicationDisplayVersion>0.0.142</ApplicationDisplayVersion>" in project_text, "display version is 0.0.142")
require(plist.get("CFBundleVersion") == "142", "Info.plist build version is 142")
require(plist.get("CFBundleShortVersionString") == "0.0.142", "Info.plist display version is 0.0.142")
...
require("STEP 35.0.19 — GODOT CORE CALLBACK-HANDOFF PROBE" in release_presentation, "top launcher banner identifies active Step 35.0.19 Godot core callback-handoff candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.140", "0.0.141", "210/211", "CB_INITIALIZE_MANAGED_FAIL", "GS031", "GS024", "GodotFileIo.CreateDirectory", "Godot.DirAccess.DirExistsAbsolute", "NATURAL", "OS-RECON", "FORWARD", "CORE-HANDOFF", "NativeFuncs.Initialize(IntPtr,int)", "0.0.142"]), "top launcher banner preserves the physical 0.0.140 three-mode proof, 0.0.141 CI telemetry-contract failure, and active 0.0.142 correction")
...
require('ExpectedDisplayVersion = "0.0.142"' in release_presentation and 'ExpectedBuildVersion = "142"' in release_presentation, "Step 35.0.19 source pins expected bundle release identity")
...
require(not unexpected_platform_changed, "platform/native behavior remains protected except the explicit Step-35.0.19 callback-handoff bridge delta", ", ".join(unexpected_platform_changed))
require(step35_callback_platform_delta.issubset(set(platform_changed)), "Step-35.0.19 callback-handoff bridge delta is explicit rather than silently matching the old no-callback baseline")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.142",
    "STS2_BUILD_VERSION": "142",
...
require("Step 35.0.19 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.19 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.19 — comprehensive GodotSharp/native reconnaissance + dual mode with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.19 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.19 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.19 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
require("STEP 35.0.19 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.19 host gate-summary assertion matches the active production summary identity")
require("STEP 35.0.15 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" not in step35_tests, "Step 35.0.19 static validation rejects the stale 0.0.139 Step-35.0.15 gate-summary assertion")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.19 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.19 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.19 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.19 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.19 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.19 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require("string expectedBridgeTypeFullName = DiagnosticBridgeTypeFullName" in step35_source and "HasInjectedEntryMarkerAtStart(method, item.Marker, GodotSharpDiagnosticBridgeTypeFullName)" in godot_clone_block and "call.DeclaringType.FullName == expectedBridgeTypeFullName" in step35_source, "Step 35.0.19 verifies serialized entry markers against the derivative-specific bridge type and protects the 0.0.137 208/209 regression")
require(all(marker in step35_mode for marker in ["NaturalGodotDictionaryRecon = 0", "ManagedDictionaryCompatibility = 1", "ManagedCommandLineCompatibility = 2", "GodotCoreCallbackHandoff = 3"]), "Step 35.0.19 exposes the NATURAL, OS-RECON, FORWARD, and Step15-live CORE-HANDOFF mode identities")
require(all(marker in step35_source for marker in ["RunGodotCoreCallbackHandoffInitialization", "CB_INIT_ENTRY", "CB_GODOTSHARP_LOAD_START", "CB_NATIVEFUNCS_BIND_PASS", "CB_INITIALIZE_INVOKE_START", "CB_INITIALIZE_INVOKE_RETURNED", "CB_INITIALIZE_PASS", "Godot.NativeInterop.NativeFuncs", "Initialize(IntPtr,int)", "initializedField", "CallbackHandoffSnapshot", "GodotCoreCallbackHandoff"]), "Step 35.0.19 CORE-HANDOFF loads only the verified private GodotSharp derivative, binds exact NativeFuncs.Initialize(IntPtr,int), requires initialized=false, invokes once, and preserves a resolver baseline before Gate C")
require(all(marker in step35_tests for marker in ["GodotCoreCallbackHandoffRejectsMissingTableBeforeAnyPreflightOrClrWork", "IntPtr.Zero", "Assert.AreEqual(1, checkpoints.Count", "CB_INITIALIZE_MANAGED_FAIL", "stage=initialization", "System.ArgumentException", "CB_INIT_ENTRY"]), "Step 35.0.19 host regression rejects invalid native callback metadata before preflight/CLR work while preserving exactly one durable managed-failure checkpoint and proving CB_INIT_ENTRY was not reached")
require(all(marker in step15_objc_bridge for marker in ["modules/mono/glue/runtime_interop.h", "CSharpLanguage::get_singleton()", "GDMono::get_singleton()", "godotsharp::get_runtime_interop_funcs", "sts2_step15_is_runtime_interop_ready", "sts2_step15_has_dotnet_feature", "sts2_step15_is_dotnet_runtime_initialized", "sts2_step15_get_runtime_interop_funcs", 'has_feature("dotnet")']), "Step 35.0.19 native Step-15 bridge exposes only the source-built Godot runtime interop table after C# native scaffolding readiness and reports competing dotnet state")
require(all(marker in step15_native_bridge for marker in ["IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "sts2_step15_get_runtime_interop_funcs"]), "Step 35.0.19 managed native bridge exposes callback-table readiness/state/pointer without loading a game native image")
...
require(all(marker in step35_ui for marker in ["CORE-HANDOFF", "GodotCoreCallbackHandoff", "Run Step 15 Gates A-C first", "IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "CB_NATIVE_TABLE_REQUEST_RETURNED", "RunGodotCoreCallbackHandoffInitialization"]), "Step 35.0.19 iOS surface gates CORE-HANDOFF on an already-live Step-15 engine, rejects competing dotnet state, obtains the exact native table, and records the managed handoff")
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.19 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.19 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.19 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.19 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.19 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.19 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.19 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.19 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.19 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.19 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.19 active source/UI reject stale exact-closure wording")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.19 / 0.0.142 (142)", "0.0.137", "208/209", "0.0.138", "0.0.139", "209", "210", "0.0.140", "0.0.141", "211", "210 passed / 1 failed", "CB_INITIALIZE_MANAGED_FAIL", "GS031", "GS024", "CL_CRITICAL_002_POST", "NP002_POST", "GodotFileIo.CreateDirectory", "Godot.DirAccess.DirExistsAbsolute", "NativeFuncs._unmanagedCallbacks", "ManagedCommandLineCompatibility", "FORWARD", "GodotCoreCallbackHandoff", "godotsharp::get_runtime_interop_funcs", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact authority and prior CI provenance, records the physical 0.0.140 general callback-boundary proof plus 0.0.141 210/211 telemetry-contract failure, and documents the active 0.0.142 correction")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.142 (142)" in testing_doc and "0.0.142 (142)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.142 (142) release identity")
...
require("Step 35.0.19 Godot Core Callback-Handoff Probe build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.19 Godot core callback-handoff candidate")
...
require("Expected device UI: STEP 35.0.19 — GODOT CORE CALLBACK-HANDOFF PROBE" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.19 Godot core callback-handoff candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.19 candidate manifest includes implementation/tests, callback-handoff native/build bridge, prior CI/physical provenance, the physical 0.0.140 three-mode proof, and the active 0.0.142 design",
    )
    mismatches=[]
