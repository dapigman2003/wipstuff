...
print("StS2 Launcher — Steps 43–52 guarded direct main-menu startup ladder validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>210</ApplicationVersion>" in project_text, "build version is 210")
require("<ApplicationDisplayVersion>0.0.210</ApplicationDisplayVersion>" in project_text, "display version is 0.0.210")
require(plist.get("CFBundleVersion") == "210", "Info.plist build version is 210")
require(plist.get("CFBundleShortVersionString") == "0.0.210", "Info.plist display version is 0.0.210")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.210 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.210 (210)**" in read("README.md") and "Active candidate: **0.0.210 (210)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.210 (210) release identity")
require(all(v in read("docs/MASTER-PLAN.md") for v in ["0.0.210", "scripted-object generic property bridge", "59P", "59I", "59Q", "Step 63"]), "master plan points to the active 0.0.210 scripted-object property bridge frontier")
require(all(marker in release_presentation for marker in ["STEP 52 CURRENT-ARCHITECTURE BASELINE", "legacy Steps 53–57", "Step37ImplementationMarker", "Step38ImplementationMarker", "Step39ImplementationMarker", "Step40ImplementationMarker", "Step41ImplementationMarker", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "LaunchMainMenu", "Steam", "Step 57", "Steps 58", "Step 62", "Step 63"]), "top launcher banner preserves historical authority while identifying Step52->58 as the active architecture")
...
require('ExpectedDisplayVersion = "0.0.210"' in release_presentation and 'ExpectedBuildVersion = "210"' in release_presentation, "startup-ladder source pins expected 0.0.210 bundle release identity")
require("SelectedPropertyTweenerProfile" in read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs"), "0.0.208 VeryEarly Core carries explicit profile isolation instead of freezing the coupled 0.0.206 experiment surface")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs") == "c86c10e2de4097ef2232ab1b7a93fe4eae9be74cce4b8a6e5e59f8edf47d117e", "0.0.210 leaves the physically viable 0.0.208/0.0.209 GodotSharp derivative builder byte-for-byte unchanged")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/PropertyTweenerExperimentProfile.cs") == "48d62538fcd495eeba0f3028e6e10b7fea75dfa577fca80598d9f6ec45f181d9", "0.0.210 leaves the 0.0.208/0.0.209 Baseline/Wrapper/Full profile enum byte-for-byte unchanged")
character_core_210 = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2CharacterSelectContinuation.cs")
scripted_property_ui_210 = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2ScriptedPropertyDiagnostics.cs")
character_ui_210 = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2CharacterSelectContinuation.cs")
require(all(marker in character_core_210 for marker in ["RunStep59GenericPropertyResolutionMatrix", "STEP 59P GENERIC PROPERTY RESOLUTION MATRIX", "GetIndexed", "SetIndexed", "VariantConversion", "EMBARK_SCRIPTED", "ACT_DROPDOWN_SCRIPTED", "OUTLINE_NATIVE"]), "0.0.210 Core carries the scripted/native 59P generic property-resolution matrix")
require(all(marker in character_core_210 for marker in ["RunStep59ManagedNativeIdentityMatrix", "UnmanagedGetManaged", "referenceEquals=", "NATIVE_CALLBACK_SIGNATURES", "godotsharp_internal_unmanaged_get_script_instance_managed", "godotsharp_internal_unmanaged_get_instance_binding_managed"]), "0.0.210 Core carries the bounded 59I retained/native identity matrix")
require(all(marker in character_core_210 for marker in ["RunStep59ManagedPropertyBypassPreparation", "_isEnabled=true", "NButton.RegisterHotkeys", "NButton.UpdateControllerButton", "_moveTween=null", "Position=_showPos direct managed setter", "RefreshFocus", "optionalFailures", "requiredFailure", "STEP 59Q MANAGED-PROPERTY BYPASS PREPARATION"]), "0.0.210 Core carries the direct-managed 59Q embark preconditioning stages")
require(all(marker in scripted_property_ui_210 for marker in ["RunStep59GenericPropertyMatrixAsync", "RunStep59ManagedIdentityMatrixAsync", "RunStep59ManagedPropertyBypassTransitionAsync", "RunStep59RealHandlerTransition", "RunStep59FrozenPostTransitionConfinement", "RUN_STEP59Q_4OF4", "continue directly to Step 60"]), "0.0.210 UI carries independently selectable 59P/59I/59Q and same-IPA 59Q→60 progression")
require(all(marker in character_ui_210 for marker in ["Step 59P", "Step 59I", "Step 59Q"]), "0.0.210 character-select UI exposes all three new high-value Step-59 branches")
physical_0209 = read("docs/history/reports/STEP-59.16-PHYSICAL-0.0.209-NATIVE-TWEEN-MATRIX.txt")
require(all(marker in physical_0209 for marker in ["TweenInterval returned a valid IntervalTweener", "NConfirmButton", "position", "scale", "modulate", "Outline", "workingAlternativeFound=False", "ended normally"]), "0.0.210 history preserves the decisive physical 0.0.209 native-Tween target-specific matrix")
require("STEP-59.17-SCRIPTED-PROPERTY-BRIDGE-0.0.210.md" in read("docs/history/INDEX.md") and "59P" in read("docs/history/steps/STEP-59.17-SCRIPTED-PROPERTY-BRIDGE-0.0.210.md") and "59I" in read("docs/history/steps/STEP-59.17-SCRIPTED-PROPERTY-BRIDGE-0.0.210.md") and "59Q" in read("docs/history/steps/STEP-59.17-SCRIPTED-PROPERTY-BRIDGE-0.0.210.md"), "0.0.210 history index and step record preserve the scripted-property frontier and experiment deck")
require("TransformedRealStS2VeryEarlyInitialization.cs" not in scripted_property_ui_210 and "BuildGodotSharp" not in character_core_210, "0.0.210 scripted-property experiments do not introduce a new bootstrap-critical GodotSharp transform")
require("RequireStep59PropertyTweenerProfile" in read("src/StS2Launcher.Core/Runtime/TransformedRealStS2CharacterSelectContinuation.cs"), "0.0.208 character-select Core gates each Step-59 experiment to its selected PropertyTweener profile")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadder.cs") == "7968570ec64d710ba80d9f8a3ba98d8fd037269651e9348a08d51475cb8d7057", "0.0.208 freezes the Codemagic-compiled 0.0.206 startup-ladder Core runtime bytes")
...
closed_path_file = ROOT / "src/StS2Launcher.iOS/UI/RootViewController.PhysicallyClosedPath.cs"
require(closed_path_file.is_file(), "physically closed-path runner lives in a dedicated RootViewController partial")
closed_path = closed_path_file.read_text() if closed_path_file.is_file() else ""
require("AddPhysicallyClosedPathControls(content);" in root_ui_text, "RootViewController exposes the physically closed-path runner adjacent to Step 15")
closed_order = [
    ...
    *[f"RunStep{step}StartupLadderAsync" for step in range(43, 53)],
]
positions = [closed_path.find(value) for value in closed_order]
require(all(pos >= 0 for pos in positions) and positions == sorted(positions), "closed-path runner calls Step15 A-C, 35-37, 39-52 in exact physical order")
require("RunTransformedRealStS2GameLifecycleEntryAsync" not in closed_path and "Step 38 SKIPPED by contract" in closed_path, "closed-path runner explicitly skips Step 38")
require("VerifyGodotFoundationGateDAsync" not in closed_path and "Step 15 Gate D" in closed_path, "closed-path runner never auto-runs Step 15 Gate D")
require(all(value in closed_path for value in ["_physicallyClosedPathStarted", "IsFreshProcessForPhysicallyClosedPath", "_physicallyClosedPathSelectedProfile", "DisablePhysicallyClosedPathProfileButtons", "_physicallyClosedPathBaselineButton", "_physicallyClosedPathWrapperButton", "_physicallyClosedPathFullButton", "RunClosedPhysicalStageAsync", "if (!passed())", "No later stage was invoked", "PhysicallyClosedPath-ToStep52.txt"]), "closed-path runner seals one of three profiles before Step35, remains fresh-process-only/one-shot/fail-closed, and has a convenience report")
require("DisableClosedPhysicalPathManualControls" in closed_path and "_step38LifecycleButton.Enabled = false" in closed_path and closed_path.count("DisableClosedPhysicalPathManualControls();") >= 2, "closed-path runner prevents manual-rung races and explicitly disables Step 38 while armed")
for step in list(range(36, 38)) + list(range(39, 53)):
    require(f"ExactStep{step}ClosurePassed" in closed_path, f"closed-path runner requires exact Step {step} Core closure before advancing")
require("EssentialCompatibilityAuthorityPassed" in closed_path and "GodotCoreModelBootstrapCompatibility" in closed_path, "closed-path runner requires the physically closed Step35 MODEL-BOOTSTRAP authority")
require("GodotStep15NativeBridge.IsRenderingActive" in closed_path and "final renderer state to be frozen" in closed_path, "closed-path runner requires rendering frozen after Step 52")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.210",
    "STS2_BUILD_VERSION": "210",
...
require("Steps 58–62 real character-select ownership / visible-render trial IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Steps-58–62 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.32 — physically closed exact-authority core + UI-return correction; Step 36.0 follows below.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.32 retains exactly four ordered A-D gates shared by diagnostic and exact-authority modes")
require("STEP 35.0.32 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.32 diagnostic summary keeps diagnostic 4/4 distinct from exact closure")
require("STEP 35.0.32 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.32 host gate-summary assertion matches the active production diagnostic summary identity")
...
require(all(marker in step35_source for marker in ["BuildStaticInstructionMap", "CALLSITE#", "AWAIT-CANDIDATE", "DescribeMetadataScope", "GetVerifiedVeryEarlyStaticInstructionMap", "[NULL PLATFORM CTOR IL]", "[COMMAND LINE HELPER CCTOR IL]", "[COMMAND LINE HELPER TRYGETVALUE IL]", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName"]), "Step 35.0.32 static map covers the exact NullPlatform constructor plus CommandLineHelper cctor/TryGetValue callsite ordinals without resolving dependencies")
require(all(marker in step35_source for marker in ["CreateInstrumentedDiagnosticClone", "CreateInstrumentedGodotSharpDiagnosticClone", "GetDiagnosticMarkerTargets", "BuildGodotSharpDiagnosticMarkerPlan", "HasInjectedEntryMarkerAtStart", "InsertNullPlatformConstructorCallsiteMarkers", "InsertCommandLineHelperCriticalBoundaryMarkers", "HasCommandLineHelperCriticalBoundaryMarkers", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperNaturalGodotDictionaryPreserved", "ManagedStringDictionaryFullName", "CommandLineManagedDictionarySubstitutionCount", "GodotSharpDiagnosticCloneFileName", "GodotSharpDiagnosticBridgeTypeFullName", "GODOT_DIAGNOSTIC_BRIDGE_ARMED", "PrivateDiagnosticOverride", "CommandLineCctorDictionaryBeforeMarker", "CommandLineCctorDictionaryAfterMarker", "CommandLineCctorGetCmdlineArgsBeforeMarker", "CommandLineCctorGetCmdlineArgsAfterMarker", "NullPlatformTypeFullName", "CommandLineHelperTypeFullName", "INMETHOD_NP", "INMETHOD_CL_CRITICAL", "INMETHOD_027", "C_DIAGNOSTIC_BRIDGE_ARMED", "preflight.DiagnosticPath", "preflight.DiagnosticSha256"]), "Step 35.0.32 preserves prior sts2 diagnostics, supports natural/compat modes, and adds a separately verified entry-only GodotSharp diagnostic derivative")
require(all(marker in step35_tests for marker in ["var packedScene = new TypeDefinition(\"Godot\", \"PackedScene\"", "var instantiate = new MethodDefinition(\"Instantiate\"", "GodotSharpPackedSceneCompatibilityCallbackFieldName", "System.Action`2<System.Object,System.Object>"]), "0.0.201 host fixture models PackedScene.Instantiate and asserts the serialized Step-59.5 compatibility hook")
require(all(marker in step35_ui_main for marker in ["verified GodotSharp PackedScene compatibility derivative", "verified GodotSharp compatibility derivative is the selected bridge dependency", "exact prepared GodotSharp remains immutable source authority"]), "0.0.201 live MODEL-BOOTSTRAP evidence names the compatibility derivative as selected bridge authority without rewriting historical exact-authority claims")
require("MODELDB-BOOTSTRAP COMPATIBILITY + VERIFIED PACKEDSCENE-COMPAT GODOTSHARP" in step35_telemetry_ui and "MODELDB-BOOTSTRAP COMPATIBILITY + EXACT GODOTSHARP" not in step35_telemetry_ui, "0.0.201 Step-35 run telemetry identifies the selected PackedScene-compatible GodotSharp derivative instead of the exact source bytes")
require("CLR-LOADED — VERIFIED GODOTSHARP PACKEDSCENE COMPATIBILITY DERIVATIVE" in step35_source and "NOT CLR-LOADED — EXACT PREPARED GODOTSHARP USED" not in step35_source, "0.0.201 Gate-D evidence records the GodotSharp compatibility derivative as the MODEL-BOOTSTRAP CLR authority")
require(all(marker in step35_source for marker in ["Godot.NativeInterop.InteropUtils", "EngineGetSingleton", "UnmanagedGetManaged", "ConvertStringToNative", "godotsharp_engine_get_singleton", "godotsharp_internal_unmanaged_get_script_instance_managed", "godotsharp_internal_unmanaged_get_instance_binding_managed", "godotsharp_internal_unmanaged_instance_binding_create_managed", "singleton localization requires"]), "Step 35.0.32 GodotSharp derivative fails closed on the exact singleton acquisition/native-to-managed localization targets")
require(all(marker in step35_tests for marker in ["Godot.NativeInterop.InteropUtils::EngineGetSingleton", "Godot.NativeInterop.InteropUtils::UnmanagedGetManaged", "godotsharp_engine_get_singleton", "godotsharp_internal_unmanaged_instance_binding_create_managed"]), "Step 35.0.32 host regression protects singleton-acquisition marker coverage")
require(all(marker in step35_recon for marker in ["Godot.NativeInterop.InteropUtils", "EngineGetSingleton", "UnmanagedGetManaged", "ConvertStringToNative", "godotsharp_engine_get_singleton", "godotsharp_internal_unmanaged_get_script_instance_managed", "godotsharp_internal_unmanaged_get_instance_binding_managed", "godotsharp_internal_unmanaged_instance_binding_create_managed"]), "Step 35.0.32 read-only reconnaissance mirrors singleton acquisition/native-to-managed callback coverage")
require(all(marker in step35_mode for marker in ["NaturalGodotDictionaryRecon", "ManagedDictionaryCompatibility"]), "Step 35.0.32 exposes NATURAL and COMPAT modes in one build")
require(all(marker in step35_recon for marker in ["[GODOTSHARP MANAGED / NATIVE-BOUNDARY MAP]", "GodotSharp P/Invoke methods", "GodotSharp calli sites", "NativeFuncs/UnmanagedCallbacks field-use sites", "[MACH-O / NATIVE INVENTORY]", "sha256=", "uuid=", "dylib=", "rpath=", "symbol=", "string=", "MH_MAGIC_64", "FAT_MAGIC", "0x0100000C"]), "Step 35.0.32 reconnaissance records managed/native boundaries and read-only arm64 Mach-O metadata")
require("LoadFromStream" not in step35_recon and "NativeLibrary.Load" not in step35_recon and "DllImport" not in step35_recon, "Step 35.0.32 reconnaissance source performs no CLR/native image execution")
...
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.32 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.32 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.32 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require(all(marker in step35_source for marker in ["PrepareGodotManagedPluginReverseBridge", "GodotPlugins.Game.Main", "InitializeFromGameProject", "godotsharp_game_main_init", "Godot.Bridge.ManagedCallbacks", "callbackFields.Length != 37", "ManagedCallbacks.Create", "ScriptManagerBridge", "LookupScriptsInAssembly", "CB_REVERSE_PREP_PASS"]), "Step 35.0.32 core reproduces and verifies the generated game-plugin managed bootstrap contract instead of invoking the UnmanagedCallersOnly entry point directly")
...
require(all(marker in step35_ui for marker in ["CORE-HANDOFF", "GodotCoreCallbackHandoff", "Run Step 15 Gates A-C first", "IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "CB_NATIVE_TABLE_REQUEST_RETURNED", "RunGodotCoreCallbackHandoffInitialization"]), "Step 35.0.32 iOS surface gates CORE-HANDOFF on an already-live Step-15 engine, rejects competing dotnet state, obtains the exact native table, and records the managed handoff")
...
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.32 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.32 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.32 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.32 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.32 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.32 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
all_test_text = "\n".join(path.read_text() for path in (ROOT / "tests").rglob("*.cs"))
used_assert_methods = set(re.findall(r"\bAssert\.([A-Za-z_][A-Za-z0-9_]*)\s*\(", all_test_text))
known_mstest_v4_assert_methods = {"AreEqual", "AreEquivalent", "AreNotEqual", "AreSame", "Contains", "Fail", "IsFalse", "IsInstanceOfType", "IsNotNull", "IsNull", "IsTrue", "ThrowsExactly", "ThrowsExactlyAsync"}
require(used_assert_methods <= known_mstest_v4_assert_methods, "0.0.210 test sources use only the repository's known compiling MSTest-v4 Assert API surface", "unexpected=" + ",".join(sorted(used_assert_methods - known_mstest_v4_assert_methods)))
require("Assert.Greater(" not in all_test_text and "Assert.IsTrue(fluentRepairCalls > 0, fluentName);" in step35_tests, "0.0.210 retains the 0.0.206 unsupported Assert.Greater correction with an established Assert.IsTrue form")
require(all(marker in step35_ui for marker in ["DIAGNOSTIC COMPLETE: STEP 35.0.32 — 4/4", "EXACT STEP 35 CLOSURE CANDIDATE: COMPLETE — 4/4", "RUN_EXACT_STEP35_4OF4", "STEP 36 MODEL-BOOTSTRAP AUTHORITY: READY — 4/4", "RUN_MODEL_BOOTSTRAP_4OF4", "GodotCoreExactClosure", "GodotCoreModelBootstrapCompatibility"]), "Step 35.0.32 iOS surface separates historical diagnostic/exact closure from the new ModelDb-bootstrap forward authority")
step35_bootstrap_ui = read("src/StS2Launcher.iOS/UI/RootViewController.Step35ManagedPluginBootstrap.cs")
require("using StS2Launcher.Core;" in step35_bootstrap_ui and "Step35DiagnosticMode.GodotCoreExactClosure" in step35_bootstrap_ui, "Step 35.0.32 managed-plugin bootstrap partial imports the declared StS2Launcher.Core namespace for Step35DiagnosticMode exact-closure compile integration")
require(all(marker in step35_source for marker in ["Gate D OfflineReady", "CallbackProgress<SteamOfflineInstallProgress>", "value.CompletedBytes", "value.TotalBytes"]), "Step 35.0.32 Gate D forwards protected OfflineReady file/byte checkpoints into the Step-35 progress contract")
step35_gate_d_ui = read("src/StS2Launcher.iOS/UI/RootViewController.Step35GateDProgress.cs")
require(all(marker in step35_gate_d_ui for marker in ["UIProgressView", "Gate D receipt hash", "ProcessedBytes", "TotalBytes", "Latest verifier file:", "GiB", "/s", "NSTimer.CreateRepeatingScheduledTimer", "last verifier progress"]), "Step 35.0.32 iOS surface exposes dedicated Gate-D file/byte progress plus a live heartbeat during long single-file hashes")
require("SteamOfflineInstallInspection.cs" not in ["changed-by-step35-gated-progress"], "Step 35.0.32 leaves the physically protected Step-13 OfflineReady verifier implementation unchanged")
require("STEP-35 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source and "EXACT CLOSED STEP-32 TRANSFORMED EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY" in step35_source, "Step 35.0.32 Gate-C PASS wording distinguishes diagnostic from exact authority")
require(all(marker in step35_mode for marker in ["GodotCoreExactClosure = 4", "GodotCoreModelBootstrapCompatibility = 5", "ModelDb bootstrap-order compatibility rewrite"]) and all(marker in step35_source for marker in ["IsExactAuthorityMode", "IsModelBootstrapCompatibilityMode", "CreateModelBootstrapCompatibilityClone", "EssentialCompatibilityAuthorityPassed", "preflight.TransformedPath", "C_EXACT_AUTHORITY_PASS", "D_EXACT_PRIMARY_REPROOF_PASS", "D_MODEL_BOOTSTRAP_REPROOF_PASS"]) and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.32 retains exact-authority closure and adds ModelDb-bootstrap authority inside the same four-gate contract")
require(all(marker in step35_source for marker in ["CreateModelBootstrapCompatibilityClone", "AbstractModelSubtypes", "BowlbugsNormal", "BowlbugEgg", "GenericInstanceMethod", "backwardTargets", "AddDependencyClosure", "preinjectNames", "StS2LauncherBootstrapInjectOrReorder", "ModelBootstrapCompatibilityReport", "same instance", "module.Write(compatibilityPath", "new RejectingAssemblyResolver()", "serialized pre-inject count drifted", "expectedCanonicalTypeCount = 1624", "expectedDependencyEdgeCount = 56", "expectedBackwardDependencyCount = 47", "expectedPreinjectTypeCount = 41", "expectedBowlbugsOriginalIndex = 658", "expectedBowlbugEggOriginalIndex = 846", "independently audited v0.107.1 authority"]), "Step 36.0.5 ModelDb bootstrap transform derives canonical/static dependencies, pins the independently audited 1624/56/47/41 graph and physical indices, closes backward edges generally, preserves same-instance canonical order, and verifies the serialized derivative fail-closed")
require(all(marker in step35_source for marker in ["RequireModelDbMethod", "DictionaryMethod", "ContainsKey", "dictionaryOpen", "GenericParameter(\"TKey\"", "GenericParameter(\"TValue\"", "VAR(0)/VAR(1)", "Physical 0.0.158", "RequireTypeVar"]) and 'new MethodReference(name, returnType, dictionaryType)' not in step35_source, "Step 36.0.5 uses ECMA-correct declaring-type VAR generic signatures for the ModelDb Dictionary MemberRefs and verifies them after serialization")
require(all(marker in step35_tests for marker in ["GodotCoreExactClosureRejectsMissingTableBeforeAnyPreflightOrClrWork", "GodotCoreModelBootstrapCompatibilityRejectsMissingTableBeforeAnyPreflightOrClrWork", "Assert.AreEqual(4, (int)Step35DiagnosticMode.GodotCoreExactClosure)", "Assert.AreEqual(5, (int)Step35DiagnosticMode.GodotCoreModelBootstrapCompatibility)"]), "Step 35.0.32 host regressions pin exact/model-bootstrap mode values and fail-closed callback metadata")
require(all(marker in step35_source for marker in ["D_AUDIT_ENTRY", "D_OFFLINE_READY_RETURNED", "D_FINAL_CHECKS_PASS", "D_RESULT_CONSTRUCT_START", "D_RESULT_CONSTRUCT_RETURNED", "D_FINAL_PROGRESS_EMITTED", "D_TASK_RETURN_START"]), "Step 35.0.32 core durably localizes Gate-D finalization around result construction/progress/return")
require(all(marker in step35_ui for marker in ["D_THREADPOOL_CONTINUATION", "D_UI_DISPATCH_ENTER", "D_RESULT_RECORD_PASS", "D_UI_DISPATCH_RETURN", "CompleteStep35GateDProgress", ").ConfigureAwait(false);", "InvokeOnMainThread(FinalizeGateDOnMainThread)"]), "Step 35.0.32 bypasses captured UIKit await continuation and explicitly finalizes Gate D on the main thread")
...
require(all(marker in step36_source for marker in ["RequireExactStep35CoreClosure", "IsExactAuthorityMode", "IsModelBootstrapCompatibilityMode", "RequireStep36BaselineUnchanged", "_managedPluginReverseBridgePrepared"]) and "EssentialCompatibilityAuthorityPassed" in step35_source, "Step 36.0.5 requires same-process verified Step-35 compatibility authority plus the proven bridge before any new boundary")
...
require(all(marker in step36_source for marker in ["E_C_INVOKE_START", "binding.Method.Invoke(null, null)", "E_C_INVOKE_RETURNED", "stateAfter != ExpectedStateAfterEssential", "InitializerBearingRequests", "RejectedManagedRequests", "NativeLoadAttempts", "E_C_PASS"]), "Step 36 Gate C invokes unchanged ExecuteEssential once from the selected verified authority, requires state 2, and fails closed on resolver/native escape")
require(all(marker in step36_source for marker in ["FormatExceptionDiagnostic", "Exception depth", "GetBaseException()", "ReflectionTypeLoadException", "LoaderExceptions", "E_C_EXCEPTION_CAPTURED", "E_C_EXCEPTION_DEPTH", "E_C_BASE_EXCEPTION", "E_C_POST_FAILURE_CONTEXT", "stateAfterFailure", "resolverCountBefore", "hostLoadCountBefore", "privateLoadCountBefore", "sts2ContextSame", "godotSharpContextSame"]), "Step 36.0.5 captures complete nested reflection/base/loader exceptions plus post-failure state, resolver/load deltas, and authority-context continuity")
require("binding.Method.Invoke(null, null)" in step36_source and step36_source.count("binding.Method.Invoke(null, null)") == 1 and "state reset" not in step36_source.lower(), "Step 36.0.5 preserves exactly one launcher ExecuteEssential invocation and adds no state-reset retry path")
...
require(all(marker in step36_ui for marker in ["Step 36.0.5 — ModelDb Bootstrap Compatibility + Full ExecuteEssential Attempt", "EssentialCompatibilityAuthorityPassed", "GodotCoreModelBootstrapCompatibility", "MODEL-BOOTSTRAP", "RunEssentialStaticPreflight", "RunEssentialAuthorityBinding", "RunExactExecuteEssentialInvocation", "RunEssentialFinalIsolationAuditAsync", "ModelIdSerializationCache.Init", "ModelDb.InitIds", "MessageTypes.Initialize", "ActionTypes.Initialize"]), "Step 36 iOS surface requires the new MODEL-BOOTSTRAP authority and attempts the full unchanged ExecuteEssential sequence once")
require("selected same-process GodotSharp bridge authority" in step36_ui and "through exact GodotSharp" not in step36_ui, "0.0.201 Step-36 live status follows the selected MODEL-BOOTSTRAP GodotSharp bridge authority")
require("_transformedRealStS2VeryEarlyInitializationGates.Snapshot().Passed" not in step36_ui and "EssentialCompatibilityAuthorityPassed" in step36_ui and "GodotCoreModelBootstrapCompatibility" in step36_ui, "Step 36 prerequisite keys off durable MODEL-BOOTSTRAP core authority rather than the historical UIKit gate snapshot")
...
require(all(marker in step36_tests for marker in ["GateSequenceCompletesFourOfFourInOrder", "GateSequenceRejectsOutOfOrderAdvance", "ExecuteEssentialAuthorityConstantsArePinned", "ExecuteEssentialFailureFormatterPreservesNestedReflectionEvidence", "ReflectionTypeLoadException", "LoaderExceptions: 1", "Step36GateOrdinalsAreStable", "0x06007D03u"]), "Step 36 host regressions pin gate order and exact ExecuteEssential authority constants")
...
require("Step36ImplementationMarker" in release_presentation and all(marker in release_presentation for marker in ["BowlbugsNormal", "BowlbugEgg", "ModelDb", "full unchanged ExecuteEssential one-shot", "ExecuteDeferred/PrewarmJit/entry remain forbidden"]), "release presentation pins the Step-36 ModelDb-bootstrap compatibility boundary and unchanged full ExecuteEssential objective")

# ---------------------------------------------------------------------------
# Step 37.0.1 — controlled sealed game.tscn admission, load, and off-tree instantiate.
step37_source_text = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmission.cs")
require('string.Join(",", observedChildren)' in step37_source_text, "Step 37.0.1 uses valid C# string.Join syntax in Gate-D checkpoint interpolation")
require('string.Join(\\",\\", observedChildren)' not in step37_source_text, "Step 37.0.1 removes the 0.0.160 escaped-quote interpolation compile failure")
# ---------------------------------------------------------------------------
step37_source_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmission.cs"
step37_gate_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionGate.cs"
step37_result_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionGateResult.cs"
step37_progress_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionProgress.cs"
step37_summary_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionSummary.cs"
step37_sequence_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionGateSequence.cs"
step37_tests_path = ROOT / "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameSceneAdmissionTests.cs"
step37_ui_path = ROOT / "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameSceneAdmission.cs"
for path, label in [
    (step37_source_path, "Step 37 game-scene core"),
    (step37_gate_path, "Step 37 gate enum"),
    (step37_result_path, "Step 37 gate result"),
    (step37_progress_path, "Step 37 progress model"),
    (step37_summary_path, "Step 37 summary"),
    (step37_sequence_path, "Step 37 gate sequence"),
    (step37_tests_path, "Step 37 host regressions"),
    (step37_ui_path, "Step 37 iOS surface"),
]:
    require(path.is_file(), f"{label} exists")

step37_source = step37_source_path.read_text() if step37_source_path.is_file() else ""
step37_gate = step37_gate_path.read_text() if step37_gate_path.is_file() else ""
step37_summary = step37_summary_path.read_text() if step37_summary_path.is_file() else ""
step37_tests = step37_tests_path.read_text() if step37_tests_path.is_file() else ""
step37_ui = step37_ui_path.read_text() if step37_ui_path.is_file() else ""

require(all(marker in step37_source for marker in [
    'GameSceneResourcePath = "res://scenes/game.tscn"',
    'ClosedGameSceneBytes = 10_414',
    'aaec1e04f689122fd812b83fee09ea6e30e2991cf5851dc599b01b7802320ad8',
    'dc9a89798eb1bb38e23563866e746ac5',
    'ClosedPckFormat = 3', 'ClosedPckEngineMajor = 4', 'ClosedPckEngineMinor = 5', 'ClosedPckEnginePatch = 1',
    'ClosedPckFlags = 0x00000002', 'ClosedPckDirectoryEntries = 12_328',
    'ExtractSealedGameSceneFromPck', 'MD5.HashData', 'SHA256.HashData',
]), "Step 37 pins and re-verifies the exact receipt-backed game.tscn/PCK authority")
require(all(marker in step37_source for marker in [
    r'[node name=\"FmodBankLoader\" type=\"FmodBankLoader\" parent=\".\"]',
    r'[node name=\"FmodBankLoader\" type=\"Node\" parent=\".\"]',
    r'bank_paths = [\"res://banks/desktop/Master.strings.bank\"',
    r'[node name=\"FmodListener2D\" type=\"FmodListener2D\" parent=\"AudioManager\"]',
    r'[node name=\"FmodListener2D\" type=\"Node\" parent=\"AudioManager\"]',
    'ReplaceExactlyOnce', 'RemoveExactLineOnce', 'original PCK/install untouched',
]), "Step 37 derivative is bounded to the three exact FMOD-neutral copied-scene edits")
require(all(marker in step37_source for marker in [
    'ResourceLoader', 'Load(string,string,CacheMode)', 'Enum.ToObject(load.GetParameters()[2].ParameterType, 0)',
    'Godot.PackedScene', 'CanInstantiate', 'scene still uninstantiated',
]), "Step 37 Gate C performs only cache-ignore PackedScene loading before instantiation")
require(all(marker in step37_source for marker in [
    'Instantiate(GenEditState)', 'Enum.ToObject(instantiate.GetParameters()[0].ParameterType, 0)',
    'MegaCrit.Sts2.Core.Nodes.NGame', 'IsInsideTree', 'insideTree=False',
    '("FmodBankLoader", "Godot.Node")', '("FmodListener2D", "Godot.Node")',
    'MegaCrit.Sts2.Core.Nodes.Audio.NAudioManager', 'MegaCrit.Sts2.Core.Nodes.NSceneContainer', 'MegaCrit.Sts2.Core.Nodes.NAssetLoader',
    'RequireNoForbiddenStep37Escape', 'ExpectedStateAfterEssential',
]), "Step 37 Gate D instantiates only off-tree and audits the exact managed hierarchy plus inert FMOD nodes")
require('AddChild(' not in step37_source and '.GameStartup(' not in step37_source and '.LaunchMainMenu(' not in step37_source and '.ExecuteDeferred(' not in step37_source, "Step 37 core contains no direct SceneTree/startup/main-menu/deferred invocation")
require(all(marker in step37_source for marker in ['AddChild/GameStartup/main-menu/ExecuteDeferred not invoked', 'Steam init', 'native GDExtension loading']), "Step 37 telemetry explicitly keeps later startup/native boundaries forbidden")
require('MarkExactStep36ClosurePassed(context)' in step36_source and 'E_D_STEP37_AUTHORITY_READY' in step36_source, "successful Step-36 Gate D explicitly hands same-process authority to Step 37")
require(step36_source.count('ResetStep37State();') >= 2 and 'ResetStep36State();' in step35_source, "Step-35/36 resets invalidate Step-37 authority rather than allowing stale reuse")
require(all(marker in step37_gate for marker in ['ClosedEssentialAndSealedScenePreflight = 1', 'FmodNeutralDerivativePreparation = 2', 'PackedSceneLoad = 3', 'OffTreeInstantiationAudit = 4']), "Step 37 defines exactly four ordered A-D gates")
require('STEP 37.0.1 CONTROLLED GAME-SCENE ADMISSION COMPLETE — 4/4' in step37_summary, "Step 37 summary closes only at four-of-four")
require(all(marker in step37_ui for marker in ['ExactStep36ClosurePassed', 'RunGameSceneSealedPreflight', 'RunGameSceneDerivativePreparation', 'RunGameScenePackedLoad', 'RunGameSceneOffTreeInstantiationAudit', 'Step37-CrashCheckpoint-', 'Step37-GameScene-StaticMap-', 'Step37-LastCheckpoint.txt', 'Step37-TransformedRealStS2GameSceneAdmission.txt', 'RUN_STEP37_4OF4']), "Step 37 iOS surface requires same-process Step-36 closure and emits all four-gate telemetry artifacts")
require("selected same-process GodotSharp bridge authority" in step37_ui and "through exact GodotSharp" not in step37_ui and "exact GodotSharp loaded it as PackedScene" not in step37_ui, "0.0.201 Step-37 live evidence follows the selected same-process GodotSharp bridge authority")
require(all(marker in step37_tests for marker in ['GateSequenceCompletesFourOfFourInOrder', 'GateSequenceRejectsOutOfOrderAdvance', 'SealedGameSceneAuthorityConstantsArePinned', 'Step37GateOrdinalsAreStable', 'Assert.ThrowsExactly<InvalidOperationException>']), "Step 37 host regressions pin gate order and exact sealed scene/PCK constants using MSTest v4 APIs")
require('AddTransformedRealStS2GameSceneAdmissionControls(content)' in root_ui_text, "RootViewController wires Step 37 controls into the active device surface")
require('MarkExactStep37ClosurePassed();' in step37_source and 'G_D_STEP38_AUTHORITY_READY' in step37_source, "successful Step-37 Gate D explicitly hands same-process authority to Step 38")
require('ResetStep38State();' in step37_source, "Step-37 reset invalidates Step-38 authority rather than allowing stale reuse")
require('Step37ImplementationMarker' in release_presentation and all(marker in release_presentation for marker in ['physical 0.0.161 Step-37.0.1 4/4 closure', 'FMOD-neutral', 'off-tree Instantiate', 'IsInsideTree=false']), "release presentation pins the physical Step-37 closure and bounded scene-admission result")

# ---------------------------------------------------------------------------
# Step 38.2 — verified iOS/off-tree compatibility + one-shot off-tree _EnterTree.
# ---------------------------------------------------------------------------
step38_source_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntry.cs"
step38_gate_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryGate.cs"
step38_result_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryGateResult.cs"
step38_progress_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryProgress.cs"
step38_summary_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntrySummary.cs"
step38_sequence_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryGateSequence.cs"
step38_tests_path = ROOT / "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameLifecycleEntryTests.cs"
step38_ui_path = ROOT / "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameLifecycleEntry.cs"
for path, label in [
    (step38_source_path, "Step 38 lifecycle core"),
    (step38_gate_path, "Step 38 gate enum"),
    (step38_result_path, "Step 38 gate result"),
    (step38_progress_path, "Step 38 progress model"),
    (step38_summary_path, "Step 38 summary"),
    (step38_sequence_path, "Step 38 gate sequence"),
    (step38_tests_path, "Step 38 host regressions"),
    (step38_ui_path, "Step 38 iOS surface"),
]:
    require(path.is_file(), f"{label} exists")

step38_source = step38_source_path.read_text() if step38_source_path.is_file() else ""
step38_gate = step38_gate_path.read_text() if step38_gate_path.is_file() else ""
step38_summary = step38_summary_path.read_text() if step38_summary_path.is_file() else ""
step38_tests = step38_tests_path.read_text() if step38_tests_path.is_file() else ""
step38_ui = step38_ui_path.read_text() if step38_ui_path.is_file() else ""

require(all(marker in step38_source for marker in [
    'NGameEnterTreeMethodName = "_EnterTree"', 'NGameReadyMethodName = "_Ready"',
    'NGameGameStartupWrapperMethodName = "GameStartupWrapper"', 'NGameGameStartupMethodName = "GameStartup"',
    'NGameInitializePlatformMethodName = "InitializePlatform"', 'NGameLaunchMainMenuMethodName = "LaunchMainMenu"',
    'NGameLoadDeferredStartupAssetsMethodName = "LoadDeferredStartupAssetsAsync"',
]), "Step 38 pins the exact NGame lifecycle/startup method names under audit")
require(all(marker in step38_source for marker in [
    'ModuleDefinition.ReadModule', 'ReadingMode = ReadingMode.Deferred', 'RejectingAssemblyResolver', 'MetadataResolver = new MetadataResolver(resolver)', 'preflight.DiagnosticPath', 'preflight.DiagnosticSha256',
    'RequireInertGameStartupWrapper(gameStartupWrapper)', 'RequireStep38OffTreeCompatibility(enterTree)', 'get_CompletedTask', 'enterTreeWrapperCalls != 1',
    'ComputeSameNGameReachableMethods', 'forbiddenRefs', 'oneTimeForbidden', 'AppendInstructionMap',
    'H_A_PASS', 'externalResolutionRequests=0', 'verifiedInertStartupWrapper=YES', 'sentryInitializeCalls=0', 'getWindowCalls=0', 'Same-NGame methods reachable from _EnterTree',
]), "Step 38 Gate A requires the inert wrapper plus exact Sentry/window suppressions, reads the selected compatibility image with deferred rejecting Cecil, and fail-closes on later lifecycle/OneTimeInitialization reachability")
...
require(all(marker in step35_source for marker in [
    ...
    'serializedEnterTreeWrapperCalls != 1',
    'Physical 0.0.164',
    'MegaCrit.Sts2.Core.Debug.SentryService',
    'method.Name == "GetWindow"',
    'field.Name == "FilesDropped"',
    'method.Name == "Connect"',
    'externalBranchIntoWindowBlock',
    'expectedEnterTreeNopCount',
    'serializedSentryInitializeCalls != 0',
    'serializedGetWindowCalls != 0',
...
]), "Step 38 Gate B recreates the physically proven NGame only off-tree and binds the exact _EnterTree token without invoking it")
require(all(marker in step38_source for marker in [
    'enterTree.Invoke(instance, null)', '_step38EnterTreeInvocationStarted', 'H_C_INVOKE_START', 'H_C_PASS',
    'FormatExceptionDiagnostic', 'ExpectedStateAfterEssential', 'InitializerBearingRequests', 'RejectedManagedRequests', 'NativeLoadAttempts',
]), "Step 38 Gate C invokes the verified compatibility NGame._EnterTree once and captures nested failures while requiring state/native confinement")
require(step38_source.count('enterTree.Invoke(instance, null)') == 1, "Step 38 contains exactly one launcher-authorized _EnterTree invocation")
require('.AddChild(' not in step38_source and '.GameStartup(' not in step38_source and '.InitializePlatform(' not in step38_source and '.LaunchMainMenu(' not in step38_source and '.ExecuteDeferred(' not in step38_source, "Step 38 core contains no direct SceneTree/later-startup/deferred invocation")
require('GetMethod("_ExitTree"' not in step38_source and '_ExitTree.Invoke' not in step38_source, "Step 38 release path does not synthesize an _ExitTree lifecycle call")
require(all(marker in step38_source for marker in [
    'TryReleaseStep38Instance', 'post-_EnterTree confinement', '_ExitTree/_Ready/GameStartup', 'Steam',
    'temporary instance released', 'H_D_PASS',
]), "Step 38 Gate D audits confinement and releases the off-tree instance without crossing later lifecycle/native boundaries")
require(all(marker in step38_gate for marker in [
    'LifecycleStaticAudit = 1', 'OffTreeNGameReinstantiation = 2',
    'DirectEnterTreeInvocation = 3', 'PostEnterTreeConfinementAndRelease = 4',
]), "Step 38 defines exactly four ordered A-D gates")
require('STEP 38.2 IOS/OFFTREE-COMPAT NGAME _ENTERTREE ENTRY COMPLETE — 4/4' in step38_summary, "Step 38 summary closes only at four-of-four")
require(all(marker in step38_tests for marker in [
    'GateSequenceCompletesFourOfFourInOrder', 'GateSequenceRejectsOutOfOrderAdvance', 'LifecycleBoundaryNamesArePinned',
    'Step38GateOrdinalsAreStable', 'Assert.ThrowsExactly<InvalidOperationException>',
]), "Step 38 host regressions pin gate order and lifecycle boundary names using MSTest v4 APIs")
require(all(marker in step38_ui for marker in [
    'ExactStep37ClosurePassed', 'RunNGameLifecycleStaticAudit', 'RunNGameOffTreeReinstantiation',
    'RunNGameDirectEnterTreeInvocation', 'RunNGamePostEnterTreeConfinementAndRelease',
    'Step38-CrashCheckpoint-', 'Step38-NGameLifecycle-StaticMap-', 'Step38-LastCheckpoint.txt',
    'Step38-TransformedRealStS2NGameLifecycleEntry.txt', 'RUN_STEP38_4OF4',
...
for native_name in [
    ...
    'libGodotFmod.macos.template_release', 'libspine_godot.macos.template_release',
]:
    require(not any(path.name == native_name for path in ROOT.rglob('*') if path.is_file()), f"analysis-only native reference is absent from project: {native_name}")
...
required_docs = [
    ...
    "docs/history/reports/STEP-36.0.1-PHYSICAL-EXECUTEESSENTIAL-NESTED-EXCEPTION-0.0.155.txt",
    "docs/history/steps/STEP-36.0.2-EXECUTEESSENTIAL-FAILURE-CHAIN-CAPTURE.md",
    "docs/history/reports/STEP-36.0.2-PHYSICAL-CRASH-CHECKPOINT-0.0.156.txt",
    "docs/history/reports/STEP-36.0.2-PHYSICAL-MODELDB-BOOTSTRAP-FAILURE-0.0.156.txt",
    "docs/history/reports/STEP-36.0.2-PHYSICAL-STATIC-MAP-0.0.156.txt",
    "docs/history/reports/STEP-36.0.2-PHYSICAL-LAST-CHECKPOINT-0.0.156.txt",
    "docs/history/steps/STEP-36.0.3-MODELDB-BOOTSTRAP-COMPATIBILITY.md",
    "docs/history/reports/STEP-36.0.5-PHYSICAL-COMPLETE-0.0.159-REPORT.txt",
    "docs/history/steps/STEP-37.0-CONTROLLED-GAME-SCENE-ADMISSION.md",
    "docs/history/reports/STEP-37.0-NATIVE-REFERENCE-ABI-AUDIT.md",
    "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-CHECKPOINT.txt",
    "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-STATIC-MAP.txt",
    "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-LAST-CHECKPOINT.txt",
    "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-REPORT.txt",
    "docs/history/steps/STEP-38.0-CONTROLLED-NGAME-ENTERTREE.md",
...
require(all(marker in current_status for marker in ["Active candidate — Steps 58–62 NConfirmButton OnEnable preflight / visible-render trial / 0.0.201 (201)", "capped at Step 52", "Step 57 4/4/frozen", "What physical 0.0.198 changed", "What physical 0.0.200 changed", "SceneState", "PackedScene.Instantiate", "unique_name_in_owner", "NConfirmButton.OnEnable()", "_outline", "_buttonImage", "_viewport", "_hotkeys", "Step 59.0 — forensic prerequisites + one-shot real OpenCharacterSelect transition", "Step 60.0 — actual active character-select surface audit", "Step 61.0 — short visible character-select render residency", "Step 62.0 — sustained visible character-select render residency", "Step 63", "skip Step 38", "Step 15 Gate D"]), "current status records stable Step-57 baseline, physical 0.0.200 NConfirmButton localization, and active 0.0.201 fail-closed preflight design")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.201 NConfirmButton OnEnable-preflight expectations" in testing_doc and "M59_B_CONFIRM_BUTTON_PREFLIGHT" in testing_doc and "PackedScene unique-name compatibility remains present in every profile" in release_checklist_doc and "0.0.210 scripted-object property bridge release checklist" in release_checklist_doc and "59P" in release_checklist_doc and "59I" in release_checklist_doc and "59Q" in release_checklist_doc, "active testing/release docs retain the proven 0.0.201 preflight regression while pinning the 0.0.210 scripted-object property bridge contract")
...
require("instance_type: mac_mini_m2" in codemagic and "ios-canonical:" in codemagic, "Codemagic keeps the cache-bearing ios-canonical workflow and M2 machine stable")
require("bin/Release/net9.0-ios/ios-arm64/AOTCompileInputs.cache" in codemagic and "bin/Release/net9.0-ios/ios-arm64/AOTCompileInputs.cache.uptodate" in codemagic, "Codemagic caches the two missing AOT dependency sentinels")
require("- $CM_BUILD_DIR/src/StS2Launcher.iOS/bin/Release/net9.0-ios/ios-arm64\n" not in codemagic, "Codemagic does not cache the entire iOS bin tree")
...
require("Steps 58–62 real character-select ownership / visible-render trial host regression tests" in read("scripts/test.sh"), "host-test report heading identifies the Steps 58–62 candidate")
require("LogFileName=steps43-52.trx" in read("scripts/test.sh") and "artifacts/test-results/steps43-52.trx" in read("scripts/test.sh"), "host-test TRX artifact identifies Steps 43–50")
require("Steps 58–62 real character-select ownership / visible-render trial build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies the Steps 58–62 candidate")
...
require(all(marker in read("scripts/codemagic.sh") for marker in ["AOT_INPUT_CACHE", "AOT_UPTODATE_MARKER", "cache_file_line", "sha256=", "LLVM_OPT_COUNT", "LLVM_LLC_COUNT", "AOT_UPTODATE_COUNT", "AOT_SENTINEL_MISSING_COUNT", "Executing opt:", "Executing llc:"]), "Codemagic reports AOT sentinel identity and LLVM incremental-work counters")
...
require("Publishing Steps 58–62 real character-select ownership / visible-render trial..." in build_ios, "iOS publish presentation identifies the active Steps 58–62 candidate")
require(all(value in codemagic for value in ["name: Steps 58-62 - real character-select ownership / visible-render trial", "name: Validate, test, build, and verify Steps 58-62 real character-select ownership / visible-render trial"]), "Codemagic workflow presentation identifies the active Steps 58–62 candidate")
codemagic_script = read("scripts/codemagic.sh")
require(all(value in codemagic_script for value in ["Codemagic cache state before Steps 58–62 real character-select ownership / visible-render trial build", "StS2 Launcher iOS — Steps 58–62 real character-select ownership / visible-render trial"]), "Codemagic report headings identify the active Steps 58–62 candidate")
require("The active candidate is **0.0.173 / Step 41.0**" not in read("docs/MASTER-PLAN.md"), "master plan contains no stale Step-41 active-candidate claim")
...
require("Expected device UI: STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL" in verify_ipa and "steps58-62-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Steps-58–62 candidate")
...
if step35_manifest.is_file():
    ...
    "docs/history/steps/STEP-35.0.24-POST-BOOTSTRAP-RESOLVER-BASELINE-CORRECTION.md",
        ...
        "docs/history/reports/STEP-35.0.32-PHYSICAL-MODEL-BOOTSTRAP-GATE-A-0.0.157-CHECKPOINT.txt",
        "docs/history/reports/STEP-35.0.32-PHYSICAL-MODEL-BOOTSTRAP-GATE-A-0.0.157-REPORT.txt",
    ]
    require(
        all(f"  {relative}" in step35_manifest_text for relative in required_step35_manifest_paths),
        "Step 35.0.32 candidate manifest includes implementation/tests, native/build bridge, prior provenance, physical 0.0.153 worker-return evidence, and active explicit-main-queue correction",
    )
    mismatches=[]
...
if step36_manifest.is_file():
    ...
    required_step36_manifest_paths = [
        ...
        "docs/history/steps/STEP-36.0.1-EXACT-GAME-RESOURCE-PACK-HANDOFF.md",
        "docs/history/reports/STEP-36.0.1-PHYSICAL-EXECUTEESSENTIAL-NESTED-EXCEPTION-0.0.155.txt",
        "docs/history/reports/STEP-36.0.1-PHYSICAL-CHECKPOINT-0.0.155.txt",
        "docs/history/reports/STEP-36.0.1-PHYSICAL-STATIC-MAP-0.0.155.txt",
        "docs/history/steps/STEP-36.0.2-EXECUTEESSENTIAL-FAILURE-CHAIN-CAPTURE.md",
        "docs/history/reports/STEP-36.0.2-PHYSICAL-CRASH-CHECKPOINT-0.0.156.txt",
        "docs/history/reports/STEP-36.0.2-PHYSICAL-MODELDB-BOOTSTRAP-FAILURE-0.0.156.txt",
        "docs/history/reports/STEP-36.0.2-PHYSICAL-STATIC-MAP-0.0.156.txt",
        ...
        "docs/history/reports/STEP-35.0.32-PHYSICAL-MODEL-BOOTSTRAP-GATE-A-0.0.157-CHECKPOINT.txt",
        "docs/history/reports/STEP-35.0.32-PHYSICAL-MODEL-BOOTSTRAP-GATE-A-0.0.157-REPORT.txt",
        "docs/history/steps/STEP-36.0.4-MODELDB-DICTIONARY-MEMBERREF-CORRECTION.md",
        "docs/history/reports/STEP-36.0.4-PHYSICAL-MISSING-GENERIC-MEMBERREF-0.0.158-CHECKPOINT.txt",
        "docs/history/reports/STEP-36.0.4-PHYSICAL-MISSING-GENERIC-MEMBERREF-0.0.158-STATIC-MAP.txt",
        "docs/history/reports/STEP-36.0.4-PHYSICAL-MISSING-GENERIC-MEMBERREF-0.0.158-LAST.txt",
        "docs/history/reports/STEP-36.0.4-PHYSICAL-MISSING-GENERIC-MEMBERREF-0.0.158-REPORT.txt",
        "docs/history/steps/STEP-36.0.5-ECMA-GENERIC-DICTIONARY-MEMBERREF-CORRECTION.md",
    ]
    require(all(f"  {relative}" in step36_manifest_text for relative in required_step36_manifest_paths), "Step 36 candidate manifest pins the physically proven resource-pack handoff, 0.0.155 nested-exception evidence, 0.0.156 failure-chain implementation/tests, and active release wiring")
    mismatches=[]
...
step37_manifest = ROOT / "tools/validation/candidate-step37-controlled-game-scene-admission.sha256"
require(step37_manifest.is_file(), "Step 37 closed-boundary hash manifest exists")
if step37_manifest.is_file():
    step37_manifest_text = step37_manifest.read_text()
    required_step37_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmission.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionGate.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionGateResult.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionGateSequence.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionProgress.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmissionSummary.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2EssentialInitialization.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameSceneAdmissionTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameSceneAdmission.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
        "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj",
        "src/StS2Launcher.iOS/Info.plist",
        ...
        "README.md", "docs/README.md", "docs/CURRENT-STATUS.md", "docs/MASTER-PLAN.md", "docs/TESTING.md",
        "docs/REGRESSION-CONTRACTS.md", "docs/REPORTS.md", "docs/RELEASE-CHECKLIST.md", "docs/history/INDEX.md",
        "docs/history/steps/STEP-37.0-CONTROLLED-GAME-SCENE-ADMISSION.md",
        "docs/history/reports/STEP-37.0-NATIVE-REFERENCE-ABI-AUDIT.md",
        "docs/history/reports/STEP-36.0.5-PHYSICAL-COMPLETE-0.0.159-CHECKPOINT.txt",
        "docs/history/reports/STEP-36.0.5-PHYSICAL-COMPLETE-0.0.159-STATIC-MAP.txt",
        "docs/history/reports/STEP-36.0.5-PHYSICAL-COMPLETE-0.0.159-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-36.0.5-PHYSICAL-COMPLETE-0.0.159-REPORT.txt",
        "docs/history/reports/STEP-37.0-CODEMAGIC-COMPILE-FAILURE-0.0.160.txt",
        "docs/history/reports/STEP-37.0-CODEMAGIC-CACHE-STATE-0.0.160.txt",
         "docs/history/steps/STEP-37.0.1-CSHARP-INTERPOLATION-COMPILE-CORRECTION.md",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-CHECKPOINT.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-STATIC-MAP.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-REPORT.txt",
    ...
    require(all(f"  {relative}" in step37_manifest_text for relative in required_step37_manifest_paths), "Step 37 closed-boundary manifest pins implementation/tests, physical Step-36/37 evidence, Step-38 handoff, and native-reference ABI provenance")
    mismatches=[]
    for line in step37_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
    require(not mismatches, "Step 37 closed implementation/release/provenance files are hash-pinned", ", ".join(mismatches))

step38_physical_164 = read("docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-CHECKPOINT.txt")
step38_static_164 = read("docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-STATIC-MAP.txt")
step38_report_164 = read("docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-REPORT.txt")
require(all(value in step38_physical_164 for value in ["App version: 0.0.164 (164)", "H_A_PASS", "H_B_PASS", "H_C_INVOKE_START", "System.NullReferenceException", "insideTreeAfterFailure=False", "initializerDelta=0", "rejectedDelta=0", "nativeDelta=0"]), "physical 0.0.164 preserves first real off-tree _EnterTree execution and managed NRE confinement evidence")
require(all(value in step38_static_164 for value in ["SentryService::Initialize", "Godot.Node::GetWindow()", "Window/SignalName::FilesDropped", "Godot.GodotObject::Connect", "GameStartupWrapper", "get_CompletedTask"]), "physical 0.0.164 static map preserves the exact Sentry/window/file-drop and inert-wrapper callsites")
require("STEP 38.1 INERT-STARTUP-WRAPPER NGAME _ENTERTREE ENTRY FAIL — 2/4" in step38_report_164, "physical 0.0.164 final report preserves Step-38.1 2/4 result")

step38_manifest = ROOT / "tools/validation/candidate-step38-controlled-ngame-entertree.sha256"
require(step38_manifest.is_file(), "Step 38 active candidate hash manifest exists")
if step38_manifest.is_file():
    step38_manifest_text = step38_manifest.read_text()
    required_step38_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntry.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryGate.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryGateResult.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryGateSequence.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntryProgress.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameLifecycleEntrySummary.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameSceneAdmission.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameLifecycleEntryTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameLifecycleEntry.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
        "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj",
        "src/StS2Launcher.iOS/Info.plist",
        ...
        "docs/TESTING.md", "docs/REGRESSION-CONTRACTS.md", "docs/REPORTS.md", "docs/RELEASE-CHECKLIST.md", "docs/history/INDEX.md",
        "docs/history/steps/STEP-38.0-CONTROLLED-NGAME-ENTERTREE.md",
        "docs/history/steps/STEP-38.0.1-DEFERRED-CECIL-METADATA-READ-CORRECTION.md",
        "docs/history/steps/STEP-38.1-INERT-GAMESTARTUPWRAPPER-COMPATIBILITY.md",
        "docs/history/steps/STEP-38.2-IOS-OFFTREE-ENTERTREE-COMPATIBILITY.md",
        "docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-CHECKPOINT.txt",
        "docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-STATIC-MAP.txt",
        "docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-REPORT.txt",
        "docs/history/reports/STEP-38.1-PHYSICAL-DIRECT-ENTERTREE-NRE-0.0.164-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-38.0.1-PHYSICAL-GATE-A-ENTERTREE-REACHES-GAMESTARTUPWRAPPER-0.0.163-CHECKPOINT.txt",
        "docs/history/reports/STEP-38.0.1-PHYSICAL-GATE-A-ENTERTREE-REACHES-GAMESTARTUPWRAPPER-0.0.163-REPORT.txt",
        "docs/history/reports/STEP-38.0.1-PHYSICAL-GATE-A-ENTERTREE-REACHES-GAMESTARTUPWRAPPER-0.0.163-LAST.txt",
        "docs/history/reports/STEP-38.0-PHYSICAL-GATE-A-CECIL-GODOTSHARP-RESOLUTION-FAILURE-0.0.162-CHECKPOINT.txt",
        "docs/history/reports/STEP-38.0-PHYSICAL-GATE-A-CECIL-GODOTSHARP-RESOLUTION-FAILURE-0.0.162-REPORT.txt",
        "docs/history/reports/STEP-38.0-PHYSICAL-GATE-A-CECIL-GODOTSHARP-RESOLUTION-FAILURE-0.0.162-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-CHECKPOINT.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-STATIC-MAP.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-37.0.1-PHYSICAL-COMPLETE-0.0.161-REPORT.txt",
    ...
    require(all(f"  {relative}" in step38_manifest_text for relative in required_step38_manifest_paths), "Step 38 candidate manifest pins lifecycle-bootstrap transform, lifecycle implementation/tests, active release wiring, and physical Step-36/37/38 authority")
    mismatches=[]
    for line in step38_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
...
step39_physical_169 = read("docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-CHECKPOINT.txt")
step39_static_169 = read("docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-STATIC-MAP.txt")
step39_report_169 = read("docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-REPORT.txt")
require(all(value in step39_physical_169 for value in ["App version: 0.0.169 (169)", "I_B_PASS", "I_C_ADDCHILD_START", "I_C_PASS", "StopRendering returned=True", "I_D_FAIL", "MoreThanOneMatch", "nativeDelta=0"]), "physical 0.0.169 preserves Gate-B PASS, first real AddChild PASS, render freeze, and Gate-D reflection-only failure")
require(all(value in step39_static_169 for value in ["Actual off-tree game hierarchy nodes: 69", "Selected sts2 managed node types: 23", "Immediate managed lifecycle callbacks mapped including in-module base chains (_EnterTree/_Ready/_Notification): 26", "Transitive same-sts2 lifecycle closure methods mapped: 557", "Forbidden startup/native/platform references from transitive lifecycle closure: 0", "Unresolved same-sts2 references from transitive lifecycle closure: 0"]), "physical 0.0.169 static map preserves the exact clean lifecycle admission authority")
require("STEP 39.0 REAL SCENETREE ADMISSION FAIL — 3/4" in step39_report_169 and "Stage: frozen inserted-tree confinement" in step39_report_169 and "MoreThanOneMatch" in step39_report_169, "physical 0.0.169 final report localizes the remaining failure to Gate-D reflection bookkeeping")
...
require('!method.IsGenericMethodDefinition' in step39_scene_source and '!method.ContainsGenericParameters' in step39_scene_source and 'method.ReturnType.IsAssignableFrom(sceneTreeRoot.GetType())' in step39_scene_source and 'GetParent() exact non-generic SceneTree-root-compatible overload' in step39_scene_source, "Step 39 Gate D selects the exact non-generic root-compatible GetParent overload after physical 0.0.169 MoreThanOneMatch")
...
if step39_manifest.is_file():
    ...
    required_step39_manifest_paths = [
        ...
        "docs/history/steps/STEP-39.2-GATE-D-GETPARENT-REFLECTION-CORRECTION.md",
        "docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-CHECKPOINT.txt",
        "docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-STATIC-MAP.txt",
        "docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-39.1-PHYSICAL-0.0.169-REPORT.txt",
...
# ---------------------------------------------------------------------------
# Step 40 controlled render-pulse boundary
# ---------------------------------------------------------------------------
step40_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulse.cs")
step40_gate = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulseGate.cs")
step40_result = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulseGateResult.cs")
step40_sequence = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulseGateSequence.cs")
step40_summary = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulseSummary.cs")
step40_tests = read("tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2RenderPulseTests.cs")
step40_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2RenderPulse.cs")
step40_root_ui = read("src/StS2Launcher.iOS/UI/RootViewController.cs")
step40_presentation = read("src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs")

require(all(text for text in [step40_core, step40_gate, step40_result, step40_sequence, step40_summary, step40_tests, step40_ui]), "Step 40 implementation, sequence, summary, tests, and iOS UI surfaces exist")
require(all(value in step40_gate for value in [
    "ClosedStep39FrozenAuthority = 1",
    "FrameDrivenManagedSurfaceAudit = 2",
    "BoundedRenderPulseAndRefreeze = 3",
    "FrozenPostPulseConfinement = 4",
]), "Step 40 gate ordinals pin closed-Step39 authority, frame audit, bounded pulse/refreeze, and frozen confinement")
require("STEP 40.1 CONTROLLED REAL-GAME RENDER PULSE COMPLETE — 4/4" in step40_summary and "Gates.Count == 4" in step40_summary, "Step 40.1 summary requires exact four-of-four closure")
require("Step40RenderPulseTargetMilliseconds = 100" in step40_core and "Step40RenderPulseMaximumMilliseconds = 2000" in step40_core, "Step 40.1 requested stop delay and post-stop evidence ceiling are pinned to 100 ms / 2000 ms")
require(all(name in step40_core for name in ["_Process", "_PhysicsProcess", "_Draw", "_Input", "_ShortcutInput", "_UnhandledInput", "_UnhandledKeyInput", "_GuiInput"]), "Step 40 audits the planned managed frame/draw/input callback family")
require(all(value in step40_core for value in ["_exactStep39ClosurePassed", "_step39InsertionStarted", "_step39InsertionPassed", "RequireStep40InsertedAuthority", "GameStartupWrapper", "RequireInertGameStartupWrapper"]), "Step 40 Gate A requires exact same-process Step-39 4/4 inserted authority and re-verifies inert GameStartupWrapper")
require("new FileInfo(step39.SelectedPath).Length" not in step40_core, "Step 40 does not use a tautological selected-image length check")
require(all(value in step40_core for value in ["ReadingMode = ReadingMode.Deferred", "RejectingAssemblyResolver", "AuditStep39ManagedLifecycleClosure"]), "Step 40 Gate B reuses deferred rejecting-resolver fail-closed lifecycle-closure auditing")
require("resolver.Requests.Count != 0" in step40_core and "Forbidden startup/native/platform references from transitive frame/input closure: 0" in step40_core and "Unresolved same-sts2 references from transitive frame/input closure: 0" in step40_core, "Step 40 frame/input preflight fails closed on external resolution, forbidden edges, and unresolved same-sts2 edges")
require("GodotStep15NativeBridge.StartRendering" not in step40_core and "GodotStep15NativeBridge.StopRendering" not in step40_core, "Step 40 Core never starts or stops rendering; native render control remains iOS-UI confined")
require(step40_ui.count("GodotStep15NativeBridge.StartRendering()") == 1, "Step 40 iOS UI has exactly one static StartRendering call site")
require(step40_ui.count("GodotStep15NativeBridge.StopRendering()") == 1, "Step 40 iOS UI has exactly one static StopRendering call site")
step40_static_map_pos = step40_ui.find("J_B_STATIC_MAP_WRITE_RETURNED")
step40_start_pos = step40_ui.find("GodotStep15NativeBridge.StartRendering()")
step40_delay_pos = step40_ui.find("Task.Delay(TransformedRealStS2VeryEarlyInitialization.Step40RenderPulseTargetMilliseconds)")
step40_stop_helper_pos = step40_ui.find("StopStep40RenderingOnMainThreadAsync()", step40_delay_pos)
step40_stop_returned_checkpoint_pos = step40_ui.find("J_C_STOP_RENDERING_RETURNED", step40_stop_helper_pos)
step40_gatec_evidence_pos = step40_ui.find("RunStep40BoundedRenderPulseEvidence")
require(-1 not in [step40_static_map_pos, step40_start_pos, step40_delay_pos, step40_stop_helper_pos, step40_stop_returned_checkpoint_pos, step40_gatec_evidence_pos] and step40_static_map_pos < step40_start_pos < step40_delay_pos < step40_stop_helper_pos < step40_stop_returned_checkpoint_pos < step40_gatec_evidence_pos, "Step 40.1 writes the verified map before StartRendering, delays only after start, calls the synchronous stop helper before any stop telemetry, and refreezes before evaluating Gate C")
require("J_C_STOP_RENDERING_CALL —" not in step40_ui and "first managed continuation called StopRendering before telemetry" in step40_ui, "Step 40.1 removes the pre-stop checkpoint/file-I/O defect and records stop evidence only after StopRendering returns")
require(all(value in step40_ui for value in ["_step40PulseUiStarted", "BeginStep40BoundedRenderPulse", "do not retry", "Step40-CrashCheckpoint-", "Step40-RenderPulse-StaticMap-", "Step40-LastCheckpoint.txt", "Step40-TransformedRealStS2RenderPulse.txt"]), "Step 40 UI enforces one-shot pulse semantics and durable run-correlated telemetry")
require("ResetStep40State();" in step39_scene_source and "AddTransformedRealStS2RenderPulseControls(content);" in step40_root_ui, "Step 39 reset clears Step 40 state and RootViewController exposes the Step 40 control")
require(all(value in step40_presentation for value in ["STEP 40 CLOSED POSITIVE 4/4", "Step40ImplementationMarker", "second physical 0.0.171 Step-40 run", "StopRendering inactive at 103.3 ms"]), "release presentation preserves physical Step-40 4/4 closure while Step 41 becomes active")
require(not any(call in step40_core for call in [".GameStartup(", ".InitializePlatform(", ".LaunchMainMenu(", ".ExecuteDeferred("]), "Step 40 Core does not directly invoke forbidden game startup/platform/main-menu/deferred methods")
require(not any(call in step40_ui for call in [".GameStartup(", ".InitializePlatform(", ".LaunchMainMenu(", ".ExecuteDeferred("]), "Step 40 UI does not directly invoke forbidden game startup/platform/main-menu/deferred methods")
require(all(value in step40_tests for value in ["Assert.ThrowsExactly<InvalidOperationException>", "Step40FirstOpportunityEvidenceWindowIsPinned", "523.5 <=", "Step40GateOrdinalsAreStable"]), "Step 40.1 host regressions pin gate sequencing, the 100/2000 evidence window, and physical 523.5 ms acceptance with MSTest-v4-compatible assertions")

step39_physical_170 = read("docs/history/reports/STEP-39.2-PHYSICAL-COMPLETE-0.0.170-CHECKPOINT.txt")
step39_static_170 = read("docs/history/reports/STEP-39.2-PHYSICAL-COMPLETE-0.0.170-STATIC-MAP.txt")
step39_report_170 = read("docs/history/reports/STEP-39.2-PHYSICAL-COMPLETE-0.0.170-REPORT.txt")
step39_last_170 = read("docs/history/reports/STEP-39.2-PHYSICAL-COMPLETE-0.0.170-LAST-CHECKPOINT.txt")
require("STEP 39.0 REAL SCENETREE ADMISSION COMPLETE — 4/4" in step39_report_170, "physical 0.0.170 final report seals Step 39 at 4/4")
require(all(value in step39_physical_170 for value in ["App version: 0.0.170 (170)", "I_B_PASS", "I_C_PASS", "StopRendering returned=True", "I_D_PASS", "RUN_STEP39_4OF4", "initializerDelta=0", "rejectedDelta=0", "nativeDelta=0"]), "physical 0.0.170 checkpoint seals clean Gate-B/C/D authority, immediate render freeze, and zero forbidden deltas")
require(all(value in step39_static_170 for value in ["Actual off-tree game hierarchy nodes: 69", "Selected sts2 managed node types: 23", "Immediate managed lifecycle callbacks mapped including in-module base chains (_EnterTree/_Ready/_Notification): 26", "Transitive same-sts2 lifecycle closure methods mapped: 557", "Forbidden startup/native/platform references from transitive lifecycle closure: 0", "Unresolved same-sts2 references from transitive lifecycle closure: 0"]), "physical 0.0.170 static map pins the 69/23/26/557 clean SceneTree lifecycle authority")
require("RUN_END" in step39_last_170 and "Step 39 never restarts Godot rendering" in step39_last_170, "physical 0.0.170 last checkpoint proves normal Step-39 return with rendering not restarted")

step40_physical_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-CHECKPOINT.txt")
step40_static_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-STATIC-MAP.txt")
step40_report_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-REPORT.txt")
step40_last_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-LAST-CHECKPOINT.txt")
require("STEP 40.0 CONTROLLED REAL-GAME RENDER PULSE FAIL — 2/4" in step40_report_171 and "observed=523.5ms" in step40_report_171, "physical 0.0.171 final report seals the old 500 ms classifier as the only reported Step-40 Gate-C failure")
require(all(value in step40_physical_171 for value in ["App version: 0.0.171 (171)", "J_A_PASS", "J_B_PASS", "J_C_START_RENDERING_RETURNED — returned=True; renderingActiveAfterStart=True", "elapsedBeforeStopMs=522.8", "J_C_STOP_RENDERING_RETURNED — returned=True; renderingActiveAfterStop=False; observedElapsedMs=523.5", "RUN_END"]), "physical 0.0.171 checkpoint proves Gate A/B clean, real render start, safe refreeze, 523.5 ms observation, and normal return")
require(all(value in step40_static_171 for value in ["Actual in-tree game hierarchy nodes: 68", "Selected sts2 managed node types: 23", "Immediate managed frame/input callbacks mapped including in-module base chains (_Process/_PhysicsProcess/_Draw/_Input/_ShortcutInput/_UnhandledInput/_UnhandledKeyInput/_GuiInput): 12", "Transitive same-sts2 frame/input closure methods mapped: 447", "Forbidden startup/native/platform references from transitive frame/input closure: 0", "Unresolved same-sts2 references from transitive frame/input closure: 0"]), "physical 0.0.171 static map pins the 68/23/12/447 clean in-tree frame/input authority")
require("RUN_END" in step40_last_171 and "Step 40 never restarts rendering after its single StopRendering call" in step40_last_171, "physical 0.0.171 last checkpoint proves normal return with rendering not restarted after stop")

step40_success_physical_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-CHECKPOINT.txt")
step40_success_static_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-STATIC-MAP.txt")
step40_success_report_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-REPORT.txt")
step40_success_last_171 = read("docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-LAST-CHECKPOINT.txt")
require("STEP 40.0 CONTROLLED REAL-GAME RENDER PULSE COMPLETE — 4/4" in step40_success_report_171, "second physical 0.0.171 report closes Step 40 at 4/4")
require(all(value in step40_success_physical_171 for value in ["App version: 0.0.171 (171)", "J_A_PASS", "J_B_PASS", "elapsedBeforeStopMs=101.8", "observedElapsedMs=103.3", "J_C_PASS", "J_D_PASS", "RUN_STEP40_4OF4", "nativeDelta=0"]), "second physical 0.0.171 checkpoint proves controlled 103.3 ms pulse and frozen zero-native confinement")
require(all(value in step40_success_static_171 for value in ["Actual in-tree game hierarchy nodes: 68", "Selected sts2 managed node types: 23", "Immediate managed frame/input callbacks mapped including in-module base chains (_Process/_PhysicsProcess/_Draw/_Input/_ShortcutInput/_UnhandledInput/_UnhandledKeyInput/_GuiInput): 12", "Transitive same-sts2 frame/input closure methods mapped: 447", "Forbidden startup/native/platform references from transitive frame/input closure: 0", "Unresolved same-sts2 references from transitive frame/input closure: 0"]), "successful physical 0.0.171 static map repeats the clean 68/23/12/447 frame/input authority")
require("RUN_END" in step40_success_last_171 and "Step 40 never restarts rendering after its single StopRendering call" in step40_success_last_171, "successful physical 0.0.171 run ended normally with rendering not restarted")
require((ROOT / "docs/history/steps/STEP-40.1-FIRST-MAIN-THREAD-OPPORTUNITY-TIMING-CORRECTION.md").is_file(), "Step 40.1 timing-correction design/provenance record is present")

step40_manifest = ROOT / "tools/validation/candidate-step40-controlled-real-game-render-pulse.sha256"
require(step40_manifest.is_file(), "Step 40 historical candidate hash manifest remains present")

active_step40_text = "\n".join([step40_core, step40_tests, step40_ui, step40_presentation, current_status, testing_doc, release_checklist_doc, read("docs/MASTER-PLAN.md"), read("docs/REGRESSION-CONTRACTS.md"), read("docs/REPORTS.md"), read("README.md"), read("docs/README.md")])
require("<=500" not in active_step40_text and "MaximumMilliseconds = 500" not in active_step40_text, "active Step-40.1 source/docs contain no stale 500 ms acceptance contract")
require("2000 ms" in active_step40_text and "before telemetry" in active_step40_text, "active Step-40.1 source/docs describe the 2000 ms post-stop evidence ceiling and Stop-before-telemetry invariant")

# ---------------------------------------------------------------------------
# Step 41 non-invoking GameStartup async frontier map
# ---------------------------------------------------------------------------
step41_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontier.cs")
step41_gate = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierGate.cs")
step41_result = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierGateResult.cs")
step41_sequence = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierGateSequence.cs")
step41_summary = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierSummary.cs")
step41_tests = read("tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameStartupFrontierTests.cs")
step41_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameStartupFrontier.cs")
step41_root_ui = read("src/StS2Launcher.iOS/UI/RootViewController.cs")
require(all(text for text in [step41_core, step41_gate, step41_result, step41_sequence, step41_summary, step41_tests, step41_ui]), "Step 41 implementation, gate sequence, tests, and iOS UI surfaces exist")
require(all(value in step41_gate for value in ["ClosedStep40FrozenAuthority = 1", "GameStartupAsyncStateMachineMap = 2", "TransitiveStartupBoundaryMap = 3", "FrozenNoInvocationConfinement = 4"]), "Step 41 gate ordinals pin frozen Step40 authority, async-state-machine map, closure map, and no-invocation confinement")
require("STEP 41.0 GAMESTARTUP ASYNC FRONTIER MAP COMPLETE — 4/4" in step41_summary and "Gates.Count == 4" in step41_summary, "Step 41 summary requires exact four-of-four closure")
require(all(value in step41_core for value in ["AsyncStateMachineAttribute", "MoveNext", "BuildStep41DirectStateMachineMap", "AuditStep41StartupClosure", "BuildStep41StaticMap", "ReadingMode = ReadingMode.Deferred", "RejectingAssemblyResolver"]), "Step 41 maps exact async state-machine IL and transitive same-sts2 closure with deferred rejecting-resolver Cecil")
require(all(value in step41_core for value in ["INITIALIZE_PLATFORM", "LAUNCH_MAIN_MENU", "LOAD_DEFERRED_STARTUP_ASSETS", "PLATFORM", "STEAM", "FMOD", "SPINE", "SENTRY_INERT_WRAPPER", "SENTRY_EXTERNAL", "NATIVE_EXTENSION"]), "Step 41 classifies the planned startup/platform/native boundary families")
require("GameStartup invoked: NO" in step41_core and "Rendering restarted: NO" in step41_core, "Step 41 static map explicitly records no GameStartup invocation and no render restart")
require("GodotStep15NativeBridge.StartRendering()" not in step41_ui and "GodotStep15NativeBridge.StopRendering()" not in step41_ui, "Step 41 UI contains no render restart/refreeze call sites")
require(not any(call in step41_core for call in [".GameStartup(", ".InitializePlatform(", ".LaunchMainMenu(", ".ExecuteDeferred("]), "Step 41 Core never directly invokes startup/platform/main-menu/deferred methods")
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "physical 0.0.175 STEP 42 CLOSED POSITIVE 4/4", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "legacy Steps 53–57", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "Step 57", "Steps 58", "Step 62", "Step 63"]), "active release presentation preserves Step-42/history authority while making Step 52 the current-architecture baseline for Steps 58-62")
...
step41_manifest = ROOT / "tools/validation/candidate-step41-gamestartup-async-frontier.sha256"
require(step41_manifest.is_file(), "Step 41 historical candidate hash manifest exists")
if step41_manifest.is_file():
    step41_manifest_text = step41_manifest.read_text()
    required_step41_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontier.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierGate.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierGateResult.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierGateSequence.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontierSummary.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameStartupFrontierTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameStartupFrontier.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulse.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
        "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj", "src/StS2Launcher.iOS/Info.plist",
        ...
        "docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-CHECKPOINT.txt",
        "docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-STATIC-MAP.txt",
        "docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-40.0-PHYSICAL-0.0.171-SUCCESS-REPORT.txt",
    ]
    require(all(f"  {relative}" in step41_manifest_text for relative in required_step41_manifest_paths), "Step 41 candidate manifest pins implementation/tests/UI, active release wiring/docs, and physical Step-40 closure")
    mismatches=[]
    for line in step41_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
    require(not mismatches, "Step 41 historical implementation/release/provenance files are hash-pinned against the current sealed tree", ", ".join(mismatches))

# ---------------------------------------------------------------------------
# Physical Step 41 closure and Step 42 controlled InitPools
# ---------------------------------------------------------------------------
step41_physical_report = read("docs/history/reports/STEP-41.0-PHYSICAL-0.0.173-REPORT.txt")
step41_physical_checkpoint = read("docs/history/reports/STEP-41.0-PHYSICAL-0.0.173-CHECKPOINT.txt")
step41_physical_static_map = read("docs/history/reports/STEP-41.0-PHYSICAL-0.0.173-STATIC-MAP.txt")
step41_physical_last = read("docs/history/reports/STEP-41.0-PHYSICAL-0.0.173-LAST-CHECKPOINT.txt")
require("STEP 41.0 GAMESTARTUP ASYNC FRONTIER MAP COMPLETE — 4/4" in step41_physical_report and "App version: 0.0.173 (173)" in step41_physical_report, "physical 0.0.173 final report closes Step 41 at 4/4")
require(all(value in step41_physical_checkpoint for value in ["K_B_PASS", "MoveNext token=0x06001C31", "transitiveSameSts2StartupClosureMethods=691", "classifiedBoundaryRefs=46", "unresolvedSameSts2Refs=0", "externalResolutionRequests=0", "GameStartup invoked=NO", "K_D_PASS", "nativeDelta=0", "RUN_STEP41_4OF4"]), "physical Step-41 checkpoint pins exact async map, 691/46 closure authority, no invocation, and frozen zero-native confinement")
require(all(value in step41_physical_static_map for value in ["GameStartup token: 0x06001BD1", "MoveNext token: 0x06001C31", "Transitive same-sts2 startup closure methods mapped: 691", "Classified startup/platform/native boundary references: 46", "Unresolved same-sts2 references: 0", "External Cecil resolution requests: 0", "GameStartup invoked: NO", "Rendering restarted: NO", "System.Void MegaCrit.Sts2.Core.Nodes.NGame::InitPools()"]), "physical Step-41 static map preserves exact GameStartup/MoveNext authority and identifies InitPools in the mapped startup closure")
require("RUN_END" in step41_physical_last and "GameStartup was never invoked" in step41_physical_last and "never restarted rendering" in step41_physical_last, "physical Step-41 run ended normally without GameStartup invocation or render restart")

step42_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPools.cs")
step42_gate = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsGate.cs")
step42_result = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsGateResult.cs")
step42_sequence = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsGateSequence.cs")
step42_summary = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsSummary.cs")
step42_tests = read("tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameStartupInitPoolsTests.cs")
step42_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameStartupInitPools.cs")
require(all(text for text in [step42_core, step42_gate, step42_result, step42_sequence, step42_summary, step42_tests, step42_ui]), "Step 42 implementation, gate sequence, tests, and iOS UI surfaces exist")
require(all(value in step42_gate for value in ["ClosedStep41FrozenAuthority = 1", "InitPoolsStaticClosureAudit = 2", "ControlledInitPoolsInvocation = 3", "FrozenPostInitPoolsConfinement = 4"]), "Step 42 gate ordinals pin Step41 authority, exact closure audit, one-shot invocation, and frozen confinement")
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.210\"", "ExpectedBuildVersion = \"210\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL", "Baseline", "Wrapper", "Full"]), "active release presentation pins Step-52 current architecture and the 0.0.210 scripted-property Steps-58–62 identity")
...
if step42_manifest.is_file():
    step42_manifest_text = step42_manifest.read_text()
    required_step42_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPools.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsGate.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsGateResult.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsGateSequence.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupInitPoolsSummary.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2GameStartupInitPoolsTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2GameStartupInitPools.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2GameStartupFrontier.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
        "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj", "src/StS2Launcher.iOS/Info.plist",
        ...
        "docs/history/reports/STEP-41.0-PHYSICAL-0.0.173-REPORT.txt",
    ]
    require(all(f"  {relative}" in step42_manifest_text for relative in required_step42_manifest_paths), "Step 42 candidate manifest pins implementation/tests/UI, release wiring/docs, design provenance, and physical Step-41 closure")
    mismatches=[]
    for line in step42_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
    require(not mismatches, "Step 42 active implementation/release/provenance files are hash-pinned", ", ".join(mismatches))


# ---------------------------------------------------------------------------
# Physical Step 42 closure, Step 45 physical closure via 0.0.178, and active
# Steps 43-57 guarded startup/direct-main-menu/single-player ladder.
# ---------------------------------------------------------------------------
step42_physical_checkpoint = read("docs/history/reports/STEP-42.0-PHYSICAL-0.0.175-CHECKPOINT.txt")
step42_physical_map = read("docs/history/reports/STEP-42.0-PHYSICAL-0.0.175-STATIC-MAP.txt")
step42_physical_report = read("docs/history/reports/STEP-42.0-PHYSICAL-0.0.175-REPORT.txt")
step42_physical_last = read("docs/history/reports/STEP-42.0-PHYSICAL-0.0.175-LAST-CHECKPOINT.txt")
require(all(value in step42_physical_map for value in ["InitPools token: 0x06001BFA", "InitPools IL instructions: 3", "Transitive same-sts2 InitPools closure methods mapped: 22", "Classified startup/platform/native boundary references: 0", "Unresolved same-sts2 references: 0", "External Cecil resolution requests: 0"]), "physical Step-42 static map seals exact InitPools token/IL/22-method zero-boundary closure")
require(all(value in step42_physical_checkpoint for value in ["App version: 0.0.175 (175)", "L_C_INVOKE_RETURNED", "L_C_PASS", "initializerDelta=0", "rejectedDelta=0", "nativeDelta=0", "RUN_STEP42_4OF4"]), "physical Step-42 checkpoint proves one exact InitPools return and zero runtime escape")
require("STEP 42.0 CONTROLLED GAMESTARTUP INITPOOLS COMPLETE — 4/4" in step42_physical_report, "physical Step-42 final report closes 4/4")
require("RUN_END" in step42_physical_last and "rendering was never restarted" in step42_physical_last, "physical Step-42 run ended normally with renderer still frozen")

startup_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadder.cs")
direct_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2DirectMainMenuLadder.cs")
continuation_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2DirectMainMenuContinuation.cs")
startup_gate = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderGate.cs")
startup_result = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderGateResult.cs")
startup_sequence = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderGateSequence.cs")
startup_summary = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderSummary.cs")
startup_tests = read("tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2StartupLadderTests.cs")
startup_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2StartupLadder.cs")
direct_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2DirectMainMenuLadder.cs")
continuation_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2DirectMainMenuContinuation.cs")
singleplayer_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2SingleplayerContinuation.cs")
singleplayer_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2SingleplayerContinuation.cs")
character_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2CharacterSelectContinuation.cs")
character_ui = read("src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2CharacterSelectContinuation.cs")
startup_ui_all = startup_ui + "\n" + direct_ui + "\n" + continuation_ui + "\n" + singleplayer_ui + "\n" + character_ui
require(all(text for text in [startup_core, direct_core, continuation_core, singleplayer_core, startup_gate, startup_result, startup_sequence, startup_summary, startup_tests, startup_ui, direct_ui, continuation_ui, singleplayer_ui]), "Steps 43-57 Core/test/UI surfaces exist")
require(all(value in startup_gate for value in ["PrerequisiteAuthority = 1", "StaticAuditOrBinding = 2", "ControlledAction = 3", "PostActionConfinement = 4"]), "startup-ladder gate ordinals retain exact four-gate diagnosability")
require(all(value in startup_sequence for value in ["result.Step != _step", "result.StepName, _stepName", "var expected =", "cannot advance after a failed gate"]), "startup-ladder sequence binds every gate to one exact rung and stops after failure")
require(all(value in startup_summary for value in ["Gates.Count == 4", "Gates.All(gate => gate.Passed)", "COMPLETE — 4/4"]), "startup-ladder summary closes only exact four-of-four rungs")
require(all(value in startup_tests for value in ["GateOrdinalsAreStable", "GateSequenceRejectsOutOfOrderAdvance", "GateSequenceRejectsWrongStepOrName", "GateSequenceRejectsAdvanceAfterFailure", "STEP 46.0 LAUNCHMAINMENU IMMEDIATE FRONTIER MAP COMPLETE — 4/4", "DirectMainMenuRungNamesProduceDistinctFourOfFourSummaries", "MAIN MENU CONTROLLED RENDER PULSE", "SINGLEPLAYER FRONTIER MAP", "MAIN MENU SUSTAINED RENDER RESIDENCY", "CHARACTER SELECT RENDER RESIDENCY", "Assert.AreEqual(18, summaries.Count)"]), "startup-ladder host regressions pin 43-64 sequencing, identity, fail-stop behavior, and distinct summaries")
require("AddTransformedRealStS2StartupLadderControls(content);" in root_ui_text, "RootViewController exposes the Steps 43-57 ladder after the closed Step-42 surface")
require("ResetStartupLadderState();" in step42_core, "rerunning/resetting Step 42 invalidates all later ladder authority")

# Compile-surface regression after the 0.0.174 identifier failure.
require(all(value in startup_core for value in ["Step35ExecutionLoadContext", "InitializerBearingRequests", "RejectedManagedRequests", "NativeLoadAttempts", "FormatExceptionDiagnostic(ex)"]), "startup ladder uses existing proven load-context counters and shared exception formatter")
require(not any(value in startup_core for value in ["context.InitializerLoads", "context.RejectedLoads", "context.NativeLoads", "BuildFailureDiagnostic(ex)"]), "startup ladder contains none of the non-existent shorthand identifiers that broke 0.0.174")

# Steps 43-45 remain the already physically exercised ladder implementation.
require(all(value in startup_core for value in ["RunStep43ClosedStep42Authority", "RunStep43NullPlatformStaticAudit", "RunStep43NullPlatformReadProbe", "RunStep43FrozenConfinement", "NullPlatformTypeFullName"]), "Step 43 retains exact concrete Null-platform four-gate authority")
require(all(value in startup_core for value in ["RunStep44LegacyGuardStaticAudit", "RunStep44LegacyGuardProbe", '"HasLegacyData"', "mutationPerformed=NO", "read-only HasLegacyData closure unexpectedly reaches migration/archive mutation"]), "Step 44 remains read-only and rejects migration/archive reachability")
require(all(value in startup_core for value in ["RunStep45SaveInitializationStaticAudit", "RunStep45ControlledSaveInitialization", "RunStep45FrozenSaveConfinement", 'SaveManagerMockInstanceFieldName = "_mockInstance"', 'SaveManagerInstanceFieldName = "_instance"', "RequireExistingStartupLadderSaveManagerInstance", '"InitProfileId"', '"InitProgressData"', '"InitPrefsData"', "RequireStep45PlannedHostOnlyDelta", "_step45PostActionBaseline"]), "Step 45 retains exact singleton-safe local-save initialization and planned-host accounting")
require('<Instance>k__BackingField' not in startup_core, "Step 45 rejects the disproven auto-property singleton-field assumption")

# Latest physical 0.0.178 authority: Step45 is 4/4 because Step46 Gate A accepted it.
step46_178_report = read("docs/history/reports/STEP-46.0-PHYSICAL-0.0.178-FAIL-REPORT.txt")
step46_178_checkpoint = read("docs/history/reports/STEP-46.0-PHYSICAL-0.0.178-FAIL-CHECKPOINT.txt")
step46_178_last = read("docs/history/reports/STEP-46.0-PHYSICAL-0.0.178-FAIL-LAST-CHECKPOINT.txt")
require("STEP 46.0 LAUNCHMAINMENU ASYNC MAP FAIL — 2/4" in step46_178_report and "App version: 0.0.178 (178)" in step46_178_report, "physical 0.0.178 report pins safe Step46 mapping failure")
require(all(value in step46_178_checkpoint for value in ["M46_A_PASS", "renderingStopped=True", "state=2", "resolver/host/private/initializer/rejected/native deltas=0", "M46_B_PASS", "LaunchMainMenu token=0x06001BDF", "MoveNext token=0x06001C3D", "MoveNextIL=336", "M46_C_FAIL"]), "physical 0.0.178 checkpoint proves Step45 4/4 authority and exact LaunchMainMenu metadata before the old over-broad audit failed")
require(all(value in step46_178_report for value in ["LOAD_DEFERRED_STARTUP_ASSETS", "ONE_TIME_INITIALIZATION", "PLATFORM", "SPINE", "STEAM"]), "physical 0.0.178 Step46 report preserves the boundary classes that polluted the old transitive audit")
require("RUN_END" in step46_178_last and "never invokes LaunchMainMenu or restarts rendering" in step46_178_last, "physical 0.0.178 Step46 ended normally without invocation or rendering")
require((ROOT / "docs/history/steps/STEP-46.1-50.0-DIRECT-MAIN-MENU-LADDER.md").is_file(), "Step 46.1-50 direct-main-menu redesign record is present")

# Step 46.1: execution-opcode-qualified evidence map only; it can never authorize original LaunchMainMenu.
require(all(value in startup_core for value in ["RunStep46LaunchMainMenuStateMachineMap", "RunStep46LaunchMainMenuClosureAudit", "RunStep46FrozenNoInvocationConfinement", "AuditStartupLadderInvocationFrontier", "MarkStep46StaticMapDurablyWritten", "_step46StaticMapDurablyWritten", "_step46LaunchAdmissible = false", "launchAuthorized=NO"]), "Step 46.1 maps exact LaunchMainMenu evidence but never authorizes it")
require(all(value in direct_core for value in ["Code.Call or Code.Callvirt or Code.Newobj or Code.Jmp", "Code.Ldftn or Code.Ldvirtftn or Code.Ldtoken", "StartupLadderDeferredMethodFrontier", "ExpandInvocationFrontierStateMachine", "ImmediateClosureMethods", "DeferredMethodFrontiers"]), "Step 46.1 distinguishes immediate invocation opcodes from deferred/function-pointer references and expands only reached state machines")
require("RequireStartupLadderAuditAdmissible(audit" not in startup_core[startup_core.find("RunStep46LaunchMainMenuClosureAudit"):startup_core.find("RunStep46FrozenNoInvocationConfinement")], "Step 46.1 map does not falsely require the entire menu graph to be admissible")
require("RunDeprecatedStep47" not in startup_core and "ControlledLaunchMainMenuInvocation" not in startup_core, "deprecated live original-LaunchMainMenu Step47 implementation is absent from active Core")
require("LaunchMainMenu invoked=NO" in startup_core and "Rendering restarted: NO" in startup_core, "Step 46 remains non-invoking and frozen")
step46_ui_start = startup_ui.find("private async Task RunStep46StartupLadderAsync()")
step46_ui_end = startup_ui.find("private async Task RunStep47StartupLadderAsync()", step46_ui_start)
step46_ui_method = startup_ui[step46_ui_start:step46_ui_end] if step46_ui_start >= 0 and step46_ui_end > step46_ui_start else ""
require(all(step46_ui_method.find(v) >= 0 for v in ["RunStep46LaunchMainMenuClosureAudit", "WriteStartupLadderStaticMap(step", "MarkStep46StaticMapDurablyWritten", "RunStep46FrozenNoInvocationConfinement"]) and step46_ui_method.find("RunStep46LaunchMainMenuClosureAudit") < step46_ui_method.find("WriteStartupLadderStaticMap(step") < step46_ui_method.find("MarkStep46StaticMapDurablyWritten") < step46_ui_method.find("RunStep46FrozenNoInvocationConfinement"), "Step 46 UI durably writes the complete evidence map before Gate D")

# Step 47: exact PCK main-menu resource authority and private Spine-neutral background; no instantiation/rendering.
require(all(value in direct_core for value in [
    'MainMenuResourcePath = "res://scenes/screens/main_menu.tscn"', "ClosedMainMenuBytes = 19_087", 'ClosedMainMenuSha256 = "402b03596092097ffd7742a482642d740a293aa68dad6645f8b0c5aeab3376c0"',
    'MainMenuBackgroundResourcePath = "res://scenes/backgrounds/main_menu_bg.tscn"', "ClosedMainMenuBackgroundBytes = 7_916", 'ClosedMainMenuBackgroundSha256 = "133a2ce2e05fb8d2e72a8e2087019cb5405b105636e16889525e53cc379aeab7"',
    "Step47SpineNeutralBackgroundBytes = 6_375", 'Step47SpineNeutralBackgroundSha256 = "0cf0c664f97e36325ef1645986af4ef5efaeff41f0ef0ee8180d115c381b8f3f"',
    "RunStep47ClosedStep46MapAuthority", "RunStep47MainMenuResourcePreparation", "RunStep47MainMenuPackedSceneLoad", "RunStep47FrozenResourceConfinement", "TakeOverPath", "CacheMode.Ignore", "CacheMode.Reuse", "sceneInstantiation=NO", "originalLaunchMainMenu=NO"
]), "Step 47 pins exact menu/background bytes, deterministic Spine-neutral derivative, cache takeover, and no-instantiation resource loading")
require("RunStep47StartupLadderAsync" in startup_ui and "RunStep47MainMenuPackedSceneLoad" in startup_ui and "StartRendering()" not in startup_ui[startup_ui.find("private async Task RunStep47StartupLadderAsync()"):], "Step 47 UI keeps resource preparation frozen and does not render")
require(all(value in startup_ui for value in ["_step47ResourceLoadUiStarted", "M47_C_UI_ARMED", "button.Enabled = false", "mutationArmed: _step47ResourceLoadUiStarted"]), "Step 47 UI makes the cache-takeover/resource-load boundary one-shot after its durable map")

# Latest physical 0.0.179 authority: Steps 46.1/47 passed; exact NMainMenu instantiated off-tree; old lifecycle audit stopped safely.
step48_179_report = read("docs/history/reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-REPORT.txt")
step48_179_checkpoint = read("docs/history/reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-CHECKPOINT.txt")
step48_179_last = read("docs/history/reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-LAST-CHECKPOINT.txt")
require("STEP 48.0 MAIN MENU OFF-TREE INSTANTIATION FAIL — 2/4" in step48_179_report and "App version: 0.0.179 (179)" in step48_179_report, "physical 0.0.179 report pins safe Step48 lifecycle-audit stop")
require(all(value in step48_179_checkpoint for value in ["M48_A_PASS", "Step-47 4/4", "M48_B_PASS", "M48_C_UI_ARMED", "M48_C_INSTANTIATE_START", "M48_C_FAIL"]), "physical 0.0.179 checkpoint proves Step47 4/4 authority and exact off-tree NMainMenu instantiation boundary")
require(all(value in step48_179_report for value in ["NMainMenu::_Ready()", "CheckCommandLineArgs", "SaveManager::get_Instance()", "SetRichPresence", "SteamRemoteSaveStore"]), "physical Step48 report preserves the conditional lifecycle branches that polluted the old static audit")
require("RUN_END" in step48_179_last, "physical Step48 ended normally after the safe off-tree audit stop")
step48_180_report = read("docs/history/reports/STEP-48.1-PHYSICAL-0.0.180-REPORT.txt")
step48_180_checkpoint = read("docs/history/reports/STEP-48.1-PHYSICAL-0.0.180-CHECKPOINT.txt")
step48_180_last = read("docs/history/reports/STEP-48.1-PHYSICAL-0.0.180-LAST-CHECKPOINT.txt")
require("STEP 48.0 MAIN MENU OFF-TREE INSTANTIATION FAIL — 2/4" in step48_180_report and "App version: 0.0.180 (180)" in step48_180_report and "RootSceneContainer to be non-null" in step48_180_report, "physical 0.0.180 report localizes Step48 to nullable NGame.RootSceneContainer assumption")
require(all(value in step48_180_checkpoint for value in ["M48_A_PASS", "M48_B_PASS", "M48_C_UI_ARMED", "M48_C_INSTANTIATE_START", "runtime lifecycle guard rehearsal", "RootSceneContainer to be non-null"]), "physical 0.0.180 checkpoint proves revised Gate B and exact off-tree instantiation before root-property stop")
require("RUN_END" in step48_180_last, "physical 0.0.180 Step48 ended normally with no retry after arm")
...
require(all(value in direct_core for value in ["Step 49 Gate A pre-admission guard recheck", "Step 49 Gate C immediately before AddChild", "Step 50 Gate A guard recheck"]), "Steps 49-50 recheck retained Step48 runtime-guard authority at their boundaries")
require(all(value in direct_core for value in ["RequireStartupLadderRootSceneContainerPropertyIdentity(rootScene, 50", "requires retained exact RootSceneContainer authority"]), "Step 50 prerequisite chain requires repaired/exact NGame.RootSceneContainer identity")
require("RequireStep50Prerequisite(boundary)" in continuation_core and "RequireStep51Prerequisite(boundary)" in continuation_core, "Steps 51-52 inherit exact RootSceneContainer identity through the Step50 prerequisite chain")

# Step 48: one-shot off-tree exact NMainMenu + actual hierarchy/lifecycle immediate frontier.
require(all(value in direct_core for value in ["RunStep48ClosedStep47Authority", "RunStep48ExactInstantiationBinding", "RunStep48OffTreeInstantiationAndLifecycleAudit", "RunStep48FrozenOffTreeConfinement", 'MainMenuManagedRootTypeFullName = "MegaCrit.Sts2.Core.Nodes.Screens.MainMenu.NMainMenu"', "PackedScene.Instantiate", "GenEditState.Disabled", "IsInsideTree=false", "CollectStartupLadderActualNodeCallbackRoots", "RequireImmediateFrontierAdmissible", "_step48InstantiationStarted"]), "Step 48 is one-shot exact off-tree NMainMenu instantiation with actual lifecycle immediate-frontier audit")
require(all(value in direct_ui for value in ["_step48InstantiationUiStarted", "RunStep48OffTreeInstantiationAndLifecycleAudit", "MarkStep48StaticMapDurablyWritten", "RUN_STEP48_4OF4"]), "Step 48 UI locks retry and durably records actual off-tree authority")
...
require(all(v in scene_tree_core for v in ["int maxNodes = 256", "observations.Count >= maxNodes", "node-graph traversal exceeded {maxNodes} nodes"]) and "maxNodes: 4096" in character_core, "historical Step39 traversal defaults to 256 while Step59 diagnostics may explicitly widen to 4096")
...
require(direct_ui.count("GodotStep15NativeBridge.StartRendering()") == 1, "only one Step50 StartRendering call site exists in the direct-menu UI partial")
require(direct_ui.count("GodotStep15NativeBridge.StartRendering()") == 1 and continuation_ui.count("GodotStep15NativeBridge.StartRendering()") == 1 and startup_ui.count("GodotStep15NativeBridge.StartRendering()") == 0 and startup_ui.count("GodotStep15NativeBridge.StopRendering()") == 0, "rendering calls are isolated to Steps 50 and 52; Steps 43-49 and 51 have none")
step50_ui_start = direct_ui.find("private async Task RunStep50StartupLadderAsync()")
step50_ui_method = direct_ui[step50_ui_start:] if step50_ui_start >= 0 else ""
require(all(value in step50_ui_method for value in ["_step50PulseUiStarted", "BeginStep50BoundedRenderPulse", "Task.Delay(TransformedRealStS2VeryEarlyInitialization.Step40RenderPulseTargetMilliseconds)", "elapsedBeforeStop", "M50_C_STOP_RENDERING_RETURNED", "RunStep50BoundedRenderPulseEvidence", "RunStep50FrozenPostPulseConfinement", "RUN_STEP50_4OF4"]), "Step 50 UI is one-shot, requests the proven 100 ms pulse, and evaluates frozen evidence")
require(step50_ui_method.find("var stopReturned = GodotStep15NativeBridge.StopRendering();") < step50_ui_method.find("WriteStartupLadderCheckpoint(step, $\"M50_C_STOP_RENDERING_RETURNED"), "Step 50 stops rendering before post-stop telemetry/file I/O")
require("Step50RenderPulseEvidenceCeilingMilliseconds = 4_000" in direct_core and "elapsedMilliseconds > Step50RenderPulseEvidenceCeilingMilliseconds" in direct_core, "Step 50 uses a dedicated 4000 ms evidence ceiling")
require("Step40RenderPulseMaximumMilliseconds = 2000" in scene_tree_core.replace("TransformedRealStS2SceneTreeAdmission", "") if False else read("src/StS2Launcher.Core/Runtime/TransformedRealStS2RenderPulse.cs"), "historical Step 40 2000 ms evidence ceiling remains unchanged")

# Step 51: exact single-player handler frontier map, evidence only and no handler invocation.
require(all(value in continuation_core for value in ["RunStep51ClosedStep50Authority", "RunStep51SingleplayerHandlerBinding", "RunStep51SingleplayerFrontierMap", "RunStep51FrozenNoInvocationConfinement", "SingleplayerButtonPressed", "OpenSingleplayerSubmenu", "Handler invocation: NO", "Rendering restarted: NO"]), "Step 51 binds/maps exact single-player handler frontiers without invocation or rendering")
require("RequireImmediateFrontierAdmissible" not in continuation_core[continuation_core.find("RunStep51SingleplayerFrontierMap"):continuation_core.find("RunStep51FrozenNoInvocationConfinement")], "Step 51 records classified handler boundaries as evidence instead of silently authorizing them")
require(all(value in continuation_ui for value in ["RunStep51StartupLadderAsync", "Step51-SingleplayerFrontier-StaticMap", "Step51-TransformedRealStS2SingleplayerFrontier.txt", "MarkStep51StaticMapDurablyWritten", "RUN_STEP51_4OF4"]), "Step 51 UI has distinct durable non-invoking evidence")

# Step 52: fresh frame/input map then longer one-shot render residency, stop-before-telemetry, always refrozen.
require(all(value in continuation_core for value in ["Step52SustainedRenderTargetMilliseconds", "Step52SustainedRenderEvidenceCeilingMilliseconds", "RunStep52ClosedStep51Authority", "RunStep52SustainedFrameInputAudit", "BeginStep52SustainedRenderPulse", "RunStep52SustainedRenderPulseEvidence", "RunStep52FrozenPostResidencyConfinement", "_step52PulseStarted", "_step52PulsePassed"]), "Step 52 implements fresh audit plus bounded sustained-render/refreeze authority")
require("Step52SustainedRenderTargetMilliseconds = 1_500" in direct_core and "Step52SustainedRenderEvidenceCeilingMilliseconds = 6_000" in direct_core, "Step 52 pins 1500 ms requested residency and 6000 ms evidence ceiling")
require(continuation_ui.count("GodotStep15NativeBridge.StartRendering()") == 1, "Step 52 has exactly one StartRendering call site")
step52_ui_start = continuation_ui.find("private async Task RunStep52StartupLadderAsync()")
step52_ui_method = continuation_ui[step52_ui_start:] if step52_ui_start >= 0 else ""
require(all(value in step52_ui_method for value in ["_step52PulseUiStarted", "Task.Delay(TransformedRealStS2VeryEarlyInitialization.Step52SustainedRenderTargetMilliseconds)", "var stopReturned = GodotStep15NativeBridge.StopRendering();", "RunStep52SustainedRenderPulseEvidence", "RunStep52FrozenPostResidencyConfinement", "RUN_STEP52_4OF4"]), "Step 52 UI is one-shot, requests sustained residency, and evaluates only after synchronous refreeze")
require(step52_ui_method.find("var stopReturned = GodotStep15NativeBridge.StopRendering();") < step52_ui_method.find("WriteStartupLadderCheckpoint(step, $\"M52_C_STOP_RENDERING_RETURNED"), "Step 52 stops rendering before post-stop telemetry/file I/O")
require(all(value in continuation_core for value in ["initializerDelta != 0 || rejectedDelta != 0 || nativeDelta != 0", "_step52PostPulseBaseline = CaptureStartupLadderBaseline", "RequireStartupLadderBaselineUnchanged(context, post, \"Step 52 Gate D\")"]), "Step 52 forbids initializer/rejected/native escape and freezes the post-pulse baseline")

# Steps 53-57: narrow single-player submenu continuation. Physical closure remains through Step 52.
require(all(value in startup_core for value in [">= 53 and <= 57 => GetVerifiedSingleplayerContinuationStaticMap(step)", ">= 58 and <= 62 => GetVerifiedCharacterSelectContinuationStaticMap(step)", "ResetSingleplayerContinuationState();"]) and "ResetCharacterSelectContinuationState();" in singleplayer_core, "startup ladder routes Steps 53-62 and reset-invalidates character-select continuation authority")
require(all(value in startup_tests for value in ["SINGLEPLAYER OPEN FRONTIER", "SINGLEPLAYER SUBMENU FROZEN OPEN", "SINGLEPLAYER SUBMENU RENDER RESIDENCY", "CHARACTER SELECT FRONTIER MAP", "CHARACTER SELECT RESOURCE PREFLIGHT", "CHARACTER SELECT FACTORY FRONTIER", "CHARACTER SELECT FROZEN OFF-TREE ACQUISITION", "CHARACTER SELECT INITIALIZE FRONTIER", "CHARACTER SELECT OFF-TREE INITIALIZATION", "CHARACTER SELECT PUSH FRONTIER", "CHARACTER SELECT FROZEN SCENETREE ADMISSION", "CHARACTER SELECT RENDER RESIDENCY", "Assert.AreEqual(18, summaries.Count)"]), "host regressions include distinct Steps 53-64 four-of-four summaries")
require("AddSingleplayerContinuationControls(content);" in startup_ui and ">= 53 and <= 57 => GetSingleplayerContinuationControls(step)" in startup_ui and "AddCharacterSelectContinuationControls(content);" in startup_ui and ">= 58 and <= 62 => GetCharacterSelectContinuationControls(step)" in startup_ui, "startup UI exposes Steps 53-62 after the physically closed Step-52 surface")

# Step 53: exact direct submenu-open frontier, admissible and non-invoking.
...
step53_ui = singleplayer_ui[step53_ui_start:step54_ui_start] if step53_ui_start >= 0 and step54_ui_start > step53_ui_start else ""
require(all(value in step53_ui for value in ["RunStep53SingleplayerOpenBinding", "RunStep53SingleplayerOpenFrontierAudit", "WriteStartupLadderStaticMap", "MarkStep53StaticMapDurablyWritten", "RunStep53FrozenNoInvocationConfinement", "RUN_STEP53_4OF4"]), "Step 53 UI durably writes admissible non-invoking evidence before closure")

# Step 54: one-shot exact OpenSingleplayerSubmenu only, frozen, same retained submenu object.
step54_core_start = singleplayer_core.find("RunStep54ClosedStep53Authority")
...
require(all(value in step54_core for value in ["MainMenuSubmenuStackManagedTypeFullName", "MainMenuSubmenuStackPropertyName", "MainMenuSingleplayerFieldName", "RequireRuntimeDeclaredZeroArgSingleplayerSubmenuMethod", "RequireRuntimeExactObjectProperty", "RequireRuntimeExactInstanceField", "unchecked((uint)open.MetadataToken) != _step53OpenMethodToken", "_step54SubmenuExistedBefore", "_step54OpenStarted", "ReferenceEquals(submenuStack, _step54SubmenuStack)", "ReferenceEquals(fieldBefore, _step54SingleplayerSubmenu)", "returnedSubmenu = open.Invoke(menu, null)", "fieldAfter = field.GetValue(submenuStack)", "ReferenceEquals(returnedSubmenu, fieldAfter)", "IsVisibleInTree", "RequireRuntimeBoolProperty(submenu, \"Visible\"", "CaptureStartupLadderBaseline", "ExceptionDispatchInfo.Capture"]), "Step 54 token-matches exact zero-arg/NSingleplayerSubmenu-return method, binds the real SubmenuStack lazy field without pre-spawning, invokes once, and requires return/stack-field identity for visible/in-tree submenu authority")
...
step54_ui = singleplayer_ui[step54_ui_start:step55_ui_start] if step54_ui_start >= 0 and step55_ui_start > step54_ui_start else ""
require(all(value in step54_ui for value in ["_step54OpenUiStarted", "ONE-SHOT ALREADY ARMED", "MarkStep54StaticMapDurablyWritten", "M54_C_UI_ARMED", "RunStep54ControlledSingleplayerSubmenuOpen", "RunStep54FrozenOpenConfinement", "RUN_STEP54_4OF4"]), "Step 54 UI is one-shot and makes the runtime binding map durable before mutation")
require("StartRendering()" not in step54_ui and "StopRendering()" not in step54_ui, "Step 54 UI keeps rendering frozen")

# Step 55: actual visible submenu callbacks, one bounded residency, stop-before-telemetry.
step55_core_start = singleplayer_core.find("RunStep55ClosedStep54Authority")
step56_core_start = singleplayer_core.find("RunStep56ClosedStep55Authority")
step55_core = singleplayer_core[step55_core_start:step56_core_start] if step55_core_start >= 0 and step56_core_start > step55_core_start else ""
require("Step55SingleplayerRenderTargetMilliseconds = 750" in singleplayer_core and "Step55SingleplayerRenderEvidenceCeilingMilliseconds = 5_000" in singleplayer_core and all(value in step55_core for value in ["EnumerateStartupLadderMenuNodeGraph(submenu", "CollectStartupLadderActualNodeCallbackRoots", "RequireImmediateFrontierAdmissible", "BeginStep55SingleplayerRenderPulse", "RunStep55SingleplayerRenderPulseEvidence", "RequireNoInitializerRejectedNativeEscape", "_step55PostPulseBaseline"]), "Step 55 audits the actual submenu callback surface and pins 750/5000 ms bounded render/refreeze evidence")
step55_ui_start = singleplayer_ui.find("private async Task RunStep55StartupLadderAsync()")
step56_ui_start = singleplayer_ui.find("private async Task RunStep56StartupLadderAsync()")
step55_ui = singleplayer_ui[step55_ui_start:step56_ui_start] if step55_ui_start >= 0 and step56_ui_start > step55_ui_start else ""
require(step55_ui.count("GodotStep15NativeBridge.StartRendering()") == 1 and all(value in step55_ui for value in ["_step55PulseUiStarted", "MarkStep55StaticMapDurablyWritten", "BeginStep55SingleplayerRenderPulse", "Task.Delay(TransformedRealStS2VeryEarlyInitialization.Step55SingleplayerRenderTargetMilliseconds)", "RunStep55SingleplayerRenderPulseEvidence", "RunStep55FrozenPostResidencyConfinement", "RUN_STEP55_4OF4"]), "Step 55 UI has one render start, one-shot arming, durable map, evidence, and frozen Gate D")
require(step55_ui.find("var stopReturned = GodotStep15NativeBridge.StopRendering();") < step55_ui.find("WriteStartupLadderCheckpoint(step, $\"M55_C_STOP_RENDERING_RETURNED"), "Step 55 stops rendering before post-stop telemetry/file I/O")

# Step 56: exact OpenCharacterSelect evidence only; classified boundaries are not silently authorized.
step56_core_start = singleplayer_core.find("RunStep56ClosedStep55Authority")
step57_core_start = singleplayer_core.find("RunStep57ClosedStep56Authority")
step56_core = singleplayer_core[step56_core_start:step57_core_start] if step56_core_start >= 0 and step57_core_start > step56_core_start else ""
require(all(value in singleplayer_core for value in ['SingleplayerOpenCharacterSelectParameterTypeFullName = "MegaCrit.Sts2.Core.Nodes.GodotExtensions.NButton"', 'CharacterSelectSceneShortHint = "screens/character_select_screen"', 'CharacterSelectSceneResourcePath = "res://scenes/screens/character_select_screen.tscn"']) and all(value in step56_core for value in ["RequireExactStep56OpenCharacterSelectSignature(open)", "open.Parameters.Count != 1", "open.Parameters[0].ParameterType.FullName", "System.Void", "AuditStartupLadderInvocationFrontier([open]", "UnresolvedSameAssemblyReferences", "resolver.Requests.Count", "Code.Ldstr", "normalized.StartsWith(\"res://\"", "sceneHints", "_step56CharacterSelectSceneCandidates = sceneHints.ToArray()", "_step56FrontierMapped = true", "invocation=NO"]), "Step 56 pins exact OpenCharacterSelect(NButton)->void signature, maps frontier evidence without invocation, retains immediate-closure resource literals/hints as diagnostic evidence only, and fails on unresolved/external resolution")
require("RequireImmediateFrontierAdmissible" not in step56_core and ".Invoke(" not in step56_core and step56_core.count("RequireExactStep56OpenCharacterSelectSignature(open)") >= 2, "Step 56 keeps classified boundaries as evidence, performs no handler invocation, and rechecks the exact NButton signature before frontier mapping")
step56_ui_start = singleplayer_ui.find("private async Task RunStep56StartupLadderAsync()")
step57_ui_start = singleplayer_ui.find("private async Task RunStep57StartupLadderAsync()")
step56_ui = singleplayer_ui[step56_ui_start:step57_ui_start] if step56_ui_start >= 0 and step57_ui_start > step56_ui_start else ""
require(all(value in step56_ui for value in ["RunStep56CharacterSelectBinding", "RunStep56CharacterSelectFrontierMap", "MarkStep56StaticMapDurablyWritten", "RunStep56FrozenNoInvocationConfinement", "RUN_STEP56_4OF4"]), "Step 56 UI durably records character-select frontier/resource evidence without invocation")
require("StartRendering()" not in step56_ui and "OpenCharacterSelect(" not in step56_ui, "Step 56 UI neither renders nor invokes OpenCharacterSelect")
step56_physical_report = read("docs/history/reports/STEP-56.0-PHYSICAL-0.0.186-FAIL-REPORT.txt")
step56_physical_checkpoint = read("docs/history/reports/STEP-56.0-PHYSICAL-0.0.186-FAIL-CHECKPOINT.txt")
require(all(value in step56_physical_report for value in ["App version: 0.0.186 (186)", "STEP 56.0 CHARACTER SELECT FRONTIER MAP FAIL — 1/4", "observed params=1; return=System.Void"]), "physical 0.0.186 Step-56 report seals the one-parameter void signature localization")
require(all(value in step56_physical_checkpoint for value in ["M56_A_PASS", "Step-55 4/4 rendered/refrozen NSingleplayerSubmenu authority retained", "M56_B_FAIL", "renderingActive=False", "Step 56 never invokes OpenCharacterSelect"]), "physical 0.0.186 checkpoint proves Step-55 closure and safe frozen pre-invocation Step-56 stop")
require(all(value in read("docs/history/steps/STEP-56.0-NBUTTON-SIGNATURE-CORRECTION-0.0.187.md") for value in ["MegaCrit.Sts2.Core.Nodes.GodotExtensions.NButton", "Gate B", "Gate C", "Step 55 4/4/refrozen"]), "0.0.187 correction note pins exact NButton signature and retained Step-55 physical authority")
step57_physical_report = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.187-FAIL-REPORT.txt")
step57_physical_checkpoint = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.187-FAIL-CHECKPOINT.txt")
step57_physical_last = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.187-FAIL-LAST-CHECKPOINT.txt")
require(all(value in step57_physical_report for value in ["App version: 0.0.187 (187)", "STEP 57.0 CHARACTER SELECT RESOURCE PREFLIGHT FAIL — 0/4", "requires exactly one character-select TSCN candidate. observed=none"]), "physical 0.0.187 Step-57 report seals the original full-path-only candidate discovery failure")
require(all(value in step57_physical_checkpoint for value in ["RUN_START — Step 57.0 read-only exact character-select PCK resource preflight started", "M57_A_FAIL", "observed=none", "renderingActive=False", "no OpenCharacterSelect, ResourceLoader, PackedScene instantiation, or rendering"]), "physical 0.0.187 checkpoint proves safe frozen Step-57 Gate-A stop after Step-56 closure")
require("RUN_END" in step57_physical_last and "Step 57 is read-only" in step57_physical_last, "physical 0.0.187 Step-57 run ended normally without opening character-select execution")
require(all(value in read("docs/history/steps/STEP-57.0-SCENE-HINT-PCK-RESOLUTION-CORRECTION-0.0.188.md") for value in ["screens/character_select_screen", "res://scenes/screens/character_select_screen.tscn", "receipt-backed PCK directory", "physical closure is now recorded through **Step 56**"]), "0.0.188 correction note preserves the first scene-hint/PCK-resolution design and retained Step-56 physical authority")

step57_188_physical_report = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.188-FAIL-REPORT.txt")
step57_188_physical_checkpoint = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.188-FAIL-CHECKPOINT.txt")
step57_188_physical_last = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.188-FAIL-LAST-CHECKPOINT.txt")
require(all(value in step57_188_physical_report for value in ["App version: 0.0.188 (188)", "STEP 57.0 CHARACTER SELECT RESOURCE PREFLIGHT FAIL — 0/4", "requires the exact character-select scene hint from Step 56. observed=none"]), "physical 0.0.188 Step-57 report seals that the amended Step-56 literal-hint handoff still supplies no scene identity")
require(all(value in step57_188_physical_checkpoint for value in ["RUN_START — Step 57.0 read-only exact character-select PCK resource preflight started", "M57_A_FAIL", "requires the exact character-select scene hint from Step 56. observed=none", "renderingActive=False", "no OpenCharacterSelect, ResourceLoader, PackedScene instantiation, or rendering"]), "physical 0.0.188 checkpoint proves another safe frozen Gate-A stop before any PCK entry extraction")
require("RUN_END" in step57_188_physical_last and "Step 57 is read-only" in step57_188_physical_last, "physical 0.0.188 Step-57 run ended normally without opening character-select execution")
require(all(value in read("docs/history/steps/STEP-57.0-RETAINED-PACKEDSCENE-IDENTITY-CORRECTION-0.0.189.md") for value in ["NMainMenuSubmenuStack._characterSelectScreenScene", "Godot.PackedScene", "ResourcePath", "res://scenes/screens/character_select_screen.tscn", "Physical authority remains closed through **Step 56**"]), "0.0.189 correction note pins retained PackedScene identity proof, exact PCK path, and unchanged Step-56 physical authority")

# Step 57: read-only exact retained-PackedScene/PCK preflight only.
step57_core_start = singleplayer_core.find("RunStep57ClosedStep56Authority")
step57_core_end = singleplayer_core.find("private Step35ExecutionLoadContext RequireStep53Prerequisite", step57_core_start)
step57_core = singleplayer_core[step57_core_start:step57_core_end] if step57_core_start >= 0 and step57_core_end > step57_core_start else ""
require(all(value in singleplayer_core for value in [
    'MainMenuCharacterSelectSceneFieldName = "_characterSelectScreenScene"',
    'PackedSceneManagedTypeFullName = "Godot.PackedScene"',
    'GodotResourcePathPropertyName = "ResourcePath"',
    'CharacterSelectSceneResourcePath = "res://scenes/screens/character_select_screen.tscn"',
    "RequireRuntimeStringProperty",
    "RequireExactPckDirectoryResource",
]) and all(value in step57_core for value in [
    "stackType.Fields.Where(field => field.Name == MainMenuCharacterSelectSceneFieldName)",
    "sceneFields.Length != 1",
    "sceneFieldDefinition.IsStatic",
    "sceneFieldDefinition.FieldType.FullName != PackedSceneManagedTypeFullName",
    "resolver.Requests.Count != 0",
    "_step54SubmenuStack",
    "RequireRuntimeExactInstanceField(submenuStack.GetType(), MainMenuCharacterSelectSceneFieldName, PackedSceneManagedTypeFullName, step)",
    "runtimeSceneField.GetValue(submenuStack)",
    "RequireRuntimeStringProperty(packedScene, GodotResourcePathPropertyName, step)",
    "string.Equals(resourcePath, CharacterSelectSceneResourcePath, StringComparison.Ordinal)",
    "RequireExactPckDirectoryResource(pck, resourcePath, step)",
    "ExtractStartupLadderPckEntryUnpinned",
    "SHA256.HashData(entry.Bytes)",
    "new UTF8Encoding(false, true).GetString",
    "BuildCharacterSelectTextRiskAppendix",
]) and all(value in singleplayer_core for value in ["matches != 1", "magic != 0x43504447", "ClosedPckDirectoryEntries", "pathLength == 0 || pathLength > 1_048_576", "directoryOffset >= (ulong)stream.Length", "(entry.Flags & 1u) != 0", "MD5.HashData(bytes)", "Spine token occurrences", "FMOD/Fmod token occurrences", ".gdextension occurrences", ".dylib occurrences", ".dll occurrences", "Risk tokens are evidence only and authorize nothing"]), "Step 57 proves the actual retained character-select PackedScene field in selected metadata and runtime, reads only its exact ResourcePath, requires one receipt-backed PCK directory match, then validates PCK MD5/SHA/UTF-8 and maps textual native-extension risk read-only")
require("SelectExactCharacterSelectSceneCandidate" not in step57_core and "ResourceLoader.Load(" not in step57_core and "packedScene.Invoke" not in step57_core and ".Invoke(" not in step57_core and "StartRendering" not in step57_core, "Step 57 Core no longer trusts Step-56 literal hints and performs no Godot resource load, game handler invocation, PackedScene instantiation, or rendering")
step57_ui = singleplayer_ui[step57_ui_start:] if step57_ui_start >= 0 else ""
require(all(value in singleplayer_ui for value in ["_characterSelectScreenScene", "Godot.PackedScene", "ResourcePath", "res://scenes/screens/character_select_screen.tscn"]) and all(value in step57_ui for value in ["retained character-select PackedScene", "PackedScene.ResourcePath", "RunStep57CharacterSelectPckExtraction", "RunStep57CharacterSelectResourceRiskMap", "MarkStep57StaticMapDurablyWritten", "RunStep57FrozenReadOnlyConfinement", "RUN_STEP57_4OF4"]), "Step 57 UI describes retained PackedScene identity and persists exact read-only PCK preflight evidence before closure")
require("StartRendering()" not in step57_ui and "ResourceLoader" in step57_ui and "PackedScene.Instantiate" in step57_ui and "no OpenCharacterSelect" in step57_ui, "Step 57 UI explicitly remains non-rendering/non-loading/non-instantiating/non-invoking")

# Final physical Step-56/57 closure evidence supplied after the 0.0.189 run.
step56_188_pass_report = read("docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-REPORT.txt")
step56_188_pass_checkpoint = read("docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-CHECKPOINT.txt")
step56_188_pass_map = read("docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-STATIC-MAP.txt")
step57_189_pass_report = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.189-PASS-REPORT.txt")
step57_189_pass_checkpoint = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.189-PASS-CHECKPOINT.txt")
step57_189_pass_map = read("docs/history/reports/STEP-57.0-PHYSICAL-0.0.189-PASS-STATIC-MAP.txt")
require(all(v in step56_188_pass_report for v in ["App version: 0.0.188 (188)", "STEP 56.0 CHARACTER SELECT FRONTIER MAP COMPLETE — 4/4"]), "physical 0.0.188 closes Step 56 4/4")
require(all(v in step56_188_pass_checkpoint for v in ["M56_A_PASS", "M56_B_PASS", "M56_C_PASS", "M56_D_PASS", "RUN_STEP56_4OF4", "renderingActive=False"]), "physical 0.0.188 Step-56 checkpoint proves all four gates and frozen completion")
require(all(v in step56_188_pass_map for v in ["token=0x06003F71", "NButton", "GetSubmenuType<MegaCrit.Sts2.Core.Nodes.Screens.CharacterSelect.NCharacterSelectScreen>()", "InitializeSingleplayer()", "NSubmenuStack::Push", "OpenCharacterSelect invoked: NO", "Rendering restarted: NO", "[CHARACTER-SELECT SCENE HINTS FROM IMMEDIATE CLOSURE]", "  - none"]), "physical Step-56 map pins the exact sub-operation sequence and absence of literal scene hints")
require(all(v in step57_189_pass_report for v in ["App version: 0.0.189 (189)", "STEP 57.0 CHARACTER SELECT RESOURCE PREFLIGHT COMPLETE — 4/4"]), "physical 0.0.189 closes Step 57 4/4")
require(all(v in step57_189_pass_checkpoint for v in ["M57_A_PASS", "M57_B_PASS", "M57_C_PASS", "M57_D_PASS", "RUN_STEP57_4OF4", "renderingActive=False"]), "physical 0.0.189 Step-57 checkpoint proves all four gates and frozen completion")
require(all(v in step57_189_pass_map for v in ["Resource source: MegaCrit.Sts2.Core.Nodes.Screens.MainMenu.NMainMenuSubmenuStack._characterSelectScreenScene.ResourcePath", "Resource path: res://scenes/screens/character_select_screen.tscn", "Bytes: 16015", "SHA-256: 3d2305fdddae6f432e52ac4b8ea5035c5cecfd26fbc30c319f09b1cd4a05eb1e", "PCK MD5: 164a8ee7ae95a8c407a4745361b1bf16", "Combined textual native-risk occurrences: 0", "Godot resource load: NO", "OpenCharacterSelect invoked: NO", "Rendering restarted: NO"]), "physical Step-57 map pins exact retained PackedScene/PCK identity and zero textual native-risk evidence")
require(all(v in read("docs/history/steps/STEP-56-57-PHYSICAL-CLOSURE-0.0.189.md") for v in ["closes physical authority through **Step 57**", "16,015 bytes", "OpenCharacterSelect"]), "Step-56/57 physical closure note records the new baseline")

step59_200_report = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-REPORT.txt")
step59_200_checkpoint = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-CHECKPOINT.txt")
step59_200_map = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-STATIC-MAP.txt")
require(all(v in step59_200_report for v in ["App version: 0.0.200 (200)", "STEP 59.0 GLOBAL PACKEDSCENE UNIQUE-NAME COMPAT + REAL TRANSITION FAIL — 2/4", "NConfirmButton.OnEnable()", "NCharacterSelectScreen.OnSubmenuOpened()", "NSubmenuStack.Push", "NSingleplayerSubmenu.OpenCharacterSelect"]), "physical 0.0.200 localizes the next one-shot transition failure to retained NConfirmButton.OnEnable")
require(all(v in step59_200_checkpoint for v in ["M59_C_HANDLER_ARMED", "M59_C_HANDLER_INNER_EXCEPTION", "targetSite=Void OnEnable()", "logicalStackNull=False", "logicalStackExactRetained=True", "StartRunLobby", "lobbyPlayers=1", "renderingActive=False", "RUN_END"]), "physical 0.0.200 checkpoint seals post-arm logical-stack/lobby state and normal frozen run end")
require(all(v in step59_200_map for v in ["AscensionPanel: type=MegaCrit.Sts2.Core.Nodes.Screens.CharacterSelect.NAscensionPanel", "uniqueNameInOwner=True", "ActDropdown: type=MegaCrit.Sts2.Core.Nodes.Screens.CharacterSelect.NActDropdown", "RootSceneContainer: type=MegaCrit.Sts2.Core.Nodes.NSceneContainer", "MultiplayerTimeoutOverlay", "[CHARACTER-SELECT RETAINED LIVE INSTANCE]"]), "physical 0.0.200 map proves the PackedScene unique-name correction on retained/live audited roots")
require(all(v in read("docs/history/steps/STEP-59.7-NCONFIRMBUTTON-ONENABLE-PREFLIGHT-0.0.201.md") for v in ["NConfirmButton OnEnable preflight", "_outline", "_buttonImage", "_viewport", "_hotkeys", "no new repair", "Step 63+ remain unopened"]), "0.0.201 history note records the fail-closed NConfirmButton preflight and unchanged boundary")

# Steps 58-62: adopt real runtime ownership, conditionally use the real handler, then visibly render.
...
require(all(v in character_core for v in ["RunStep58CurrentOwnershipAcquisition", "RequireStep52Prerequisite", "RunStep58RuntimeOwnershipBinding", "ReadsFirstExplicitParameter", "RunStep58OpenCharacterSelectFrontierAudit", "RunStep58FrozenOwnershipConfinement", "M58_A_PASS — current ownership acquisition complete from Step-52 baseline", "legacy Steps53-57 not required", "_step58CurrentOwnershipAcquisitionPassed"]), "Step 58 starts from Step-52 current-architecture authority, acquires/adopts real game-owned submenu state, then audits exact OpenCharacterSelect ownership without requiring legacy Steps 53-57")
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.210 UI keeps active Step 58 directly on same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.210 active ownership path contains no obsolete Step-57 prerequisite gate")
very_early_path = ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs"
very_early_text_for_slice = very_early_path.read_text()
packed_slice_start = very_early_text_for_slice.find("private static void ApplyPackedSceneInstancedRootUniqueNameCompatibility")
packed_slice_end = very_early_text_for_slice.find("internal sealed class Step35ExecutionLoadContext", packed_slice_start)
packed_slice_bytes = very_early_text_for_slice[packed_slice_start:packed_slice_end].encode() if packed_slice_start >= 0 and packed_slice_end > packed_slice_start else b""
require(hashlib.sha256(packed_slice_bytes).hexdigest() == "36bbdd3f8bee48c4a71b29bbb4ac34a3e7ac4b7718f8abe25006535cff0f9395", "0.0.210 preserves the exact physically proven PackedScene compatibility implementation bytes")
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(marker not in very_early_core for marker in ["STEP59TRACE_", "ConfirmButtonCheckpointBridge", "Step59RuntimeTraceBridge", "ArmStep59RuntimeTrace", "DisarmStep59RuntimeTrace"]), "0.0.210 keeps the rejected 0.0.202 runtime-wide Step-59 trace bridge absent")
require(all(v in very_early_core for v in ["GodotSharp.step59.baseline-packedscene.dll", "GodotSharp.step59.wrapper-only.dll", "GodotSharp.step59.full-repair.dll", "PropertyTweenerExperimentProfile.Baseline", "PropertyTweenerExperimentProfile.Wrapper", "PropertyTweenerExperimentProfile.Full", "GodotSharpPropertyTweenerRepairEnabledFieldName", "GodotSharpPropertyTweenerObservationCountFieldName", "PATCHED_DEDICATED_HELPER_OBSERVE_REPAIR", "NATURAL_TYPED_RETURN", "PATCHED_TYPED_RETURN_EPILOGUE_OBSERVE_REPAIR", "Godot-typed helper signatures were verified statically before load and were not reflected at bootstrap"]), "0.0.209 preserves the 0.0.208 Baseline/Wrapper/Full GodotSharp profiles and keeps Godot-typed experimental helpers out of bootstrap reflection")
require("could not find PropertyTweener cast after native return" not in very_early_core and "followingIsinst" not in very_early_core, "0.0.209 retains the 0.0.206 removal of the 0.0.205 speculative fluent post-native isinst-shape assumption")
require(all(v in character_core for v in ["RunStep59ConfirmButtonTailWrapperRepairProbe", "RunStep59ConfirmButtonTailRepairProbe", "SetStep59PropertyTweenerRepairMode", "M59E_PROPERTY_TWEENER_REPAIR_MODE", "M59_C_PROPERTY_TWEENER_REPAIR_MODE", "PROPERTYTWEENER MANAGED-WRAPPER OBSERVE/REPAIR", "wrapperRepairEnabled=", "fluentRepairEnabled=", "wrongTypes=", "lastManagedType=", "fluentRepairs="]), "0.0.209 retains three-mode control/wrapper/full-repair state across 59T/59W/59X/59E/real Step59")
require(all(v in character_core for v in ["RunStep59NativeTweenRejectionMatrix", "NODE_INTERVAL_PRE_OLD_KILL", "NODE_INTERVAL_POST_OLD_KILL", "SCENETREE_INTERVAL", "SCENETREE_BOUND_INTERVAL", "NODE_PROPERTY_POSITION_CURRENT", "NODE_PROPERTY_POSITION_SHOWPOS", "NODE_PROPERTY_SCALE_CURRENT", "NODE_PROPERTY_MODULATE_CURRENT", "NODE_PROPERTY_OUTLINE_MODULATE_CURRENT", "SCENETREE_PROPERTY_POSITION_CURRENT", "SCENETREE_PROPERTY_MODULATE_CURRENT", "SCENETREE_PROPERTY_OUTLINE_MODULATE_CURRENT", "SCENETREE_PROPERTY_POSITION_SHOWPOS_FULLTAIL", "SCENETREE_BOUND_PROPERTY_POSITION_SHOWPOS_FULLTAIL", "IsValid", "IsRunning", "sameNativeAsOld"]), "0.0.209 Core carries a row-isolated native Tween rejection matrix spanning state, identity, creation path, operation kind, and target/property/value families")
require(all(v in character_core for v in ["RunStep59SceneTreeAlternativeTailProbe", "SceneTree.CreateTween", "Tween.BindNode(embark)", "PropertyTweener.SetEase", "PropertyTweener.SetTrans", "PropertyTweener.FromCurrent"]), "0.0.209 Core carries focused 59Y/59Z SceneTree alternative full-tail probes with repair disabled")
require(all(v in character_ui for v in ["Step 59N — native Tween rejection matrix", "RunStep59NativeTweenMatrixAsync", "Step 59Y — focused SceneTree.CreateTween", "Step 59Z — focused SceneTree.CreateTween + BindNode"]), "0.0.209 UI exposes 59N/59Y/59Z without replacing the existing Step-59 deck")
require(all(v in step35_tests for v in ["godot_icall_4_1441", "GodotSharpPropertyTweenerRepairEnabledFieldName", "GodotSharpPropertyTweenerFluentRepairEnabledFieldName", "GodotSharpPropertyTweenerWrongTypeCountFieldName", "GodotSharpPropertyTweenerFluentFallbackMethodName", "PATCHED_DEDICATED_HELPER_OBSERVE_REPAIR", "PATCHED_TYPED_RETURN_EPILOGUE_OBSERVE_REPAIR", "AddFluentTypedReturn"]), "0.0.209 retains serialization/reopen coverage for the unchanged profile transform while deliberately varying fluent IL shapes")
require((ROOT / "docs/history/steps/STEP-59.10-PROPERTYTWEENER-COMPAT-0.0.204.md").is_file(), "0.0.204 compatibility history remains preserved")
require((ROOT / "docs/history/steps/STEP-59.12-PROPERTYTWEENER-RETURN-EPILOGUE-0.0.206.md").is_file(), "0.0.206 history records the 0.0.205 preflight rejection and shape-independent correction")
require((ROOT / "docs/history/steps/STEP-59.14-PROFILE-ISOLATION-0.0.208.md").is_file(), "0.0.208 history records the 0.0.207 bootstrap-reflection failure and profile-isolation refactor")
require((ROOT / "docs/history/steps/STEP-59.15-NATIVE-TWEEN-NULL-0.0.209.md").is_file(), "0.0.209 history records the physical 0.0.208 native-null localization and new native rejection matrix")
step59w_208 = read("docs/history/reports/STEP-59W-PHYSICAL-0.0.208-STATIC-MAP.txt")
step59x_208 = read("docs/history/reports/STEP-59X-PHYSICAL-0.0.208-STATIC-MAP.txt")
require(all(v in step59w_208 for v in ["probeCode=59W", "wrapperRepairEnabled=True", "Tween.TweenProperty=FAIL", "observations=10; nativeNulls=7", "observations=11; nativeNulls=8", "lastNativePtr=0x0", "managedNulls=0", "wrongTypes=0", "repairs=0"]), "physical 0.0.208 59W proves the actual embark TweenProperty call advances to a native-null observation with no managed-wrapper repair opportunity")
require(all(v in step59x_208 for v in ["probeCode=59X", "wrapperRepairEnabled=True", "fluentRepairEnabled=True", "Tween.TweenProperty=FAIL", "lastNativePtr=0x0", "managedNulls=0", "wrongTypes=0", "repairs=0", "fluentObservations=4", "lastFluentManagedType=Godot.PropertyTweener"]), "physical 0.0.208 59X independently reproduces native-null while prior wrapper/fluent observations are healthy")
step35_207_checkpoint = read("docs/history/reports/STEP-35-PHYSICAL-0.0.207-CHECKPOINT.txt")
require(all(v in step35_207_checkpoint for v in ["A_RESULT — passed=True", "B_PASS_RETURN", "RESOLVE_PRIVATE_LOADFROMSTREAM_PASS — GodotSharp", "GODOT_PACKEDSCENE_UNIQUE_NAME_COMPAT_ARMED", "CB_INITIALIZE_MANAGED_FAIL", "FileNotFoundException"]), "0.0.208 history preserves the exact 0.0.207 derivative-load/bootstrap-reflection failure sequence")
require("GODOT_PROPERTY_TWEENER_PROFILE_ARMED" not in step35_207_checkpoint, "physical 0.0.207 failed before the PropertyTweener experiment arm checkpoint")
require(all(v in step35_tests for v in ["PropertyTweenerExperimentProfile.Baseline", "PropertyTweenerExperimentProfile.Wrapper", "profile=Baseline", "NATURAL_TYPED_RETURN", "wrapperPropertyTweener"]), "0.0.208 host regression explicitly reopens Baseline and Wrapper derivatives in addition to Full")
require("public PropertyTweenerExperimentProfile PropertyTweenerExperimentProfile" not in very_early_core and "public PropertyTweenerExperimentProfile SelectedPropertyTweenerProfile" in very_early_core, "0.0.208 avoids enum-type/member name shadowing in the profile selector")
require(all(v in very_early_core for v in ["fileName={notFound.FileName", "fusionLog={notFound.FusionLog", "fileName={load.FileName", "fusionLog={load.FusionLog"]), "0.0.208 loader diagnostics preserve FileName/FusionLog for FileNotFound/FileLoad exceptions")
require((ROOT / "docs/history/reports/STEP-59.13-CODEMAGIC-0.0.206-HOST-TEST-COMPILE-FAILURE.txt").is_file(), "0.0.208 preserves the supplied 0.0.206 Codemagic host-test compile evidence")
require(all(v in read("docs/history/reports/STEP-59.13-CODEMAGIC-0.0.206-HOST-TEST-COMPILE-FAILURE.txt") for v in ["VALIDATION PASS: 1437 checks", "StS2Launcher.Core: compiled successfully", "Assert' does not contain a definition for 'Greater'", "No iOS build/device result exists for 0.0.206"]), "0.0.208 history accurately classifies 0.0.206 as a host-test construction failure after successful Core compilation")
require(all(v in character_core for v in ["RunStep59ControllerSingletonRehearsal", "RunStep59RegisterHotkeysProbe", "RunStep59UpdateControllerButtonProbe", "RunStep59ConfirmButtonTailProbe", "RunStep59ConfirmButtonTailWrapperRepairProbe", "RunStep59ConfirmButtonTailRepairProbe", "RunStep59IsolatedEmbarkEnableProbe", "M59H_INVOKE_ARMED", "M59U_INVOKE_ARMED", "M59E_ENABLE_ARMED"]), "0.0.208 retains controller, hotkey, controller-update, control-tail, wrapper-only, full-repair and full-Enable branches in one compilation")
require(all(v in character_ui for v in ["Run Step 59R — CONTROLLER REHEARSAL", "Run Step 59H — REGISTER HOTKEYS ONLY", "Run Step 59U — UPDATE CONTROLLER ONLY", "Run Step 59T — CONTROL / OBSERVE", "Run Step 59W — WRAPPER REPAIR ONLY", "Run Step 59X — FULL REPAIR / FULL TAIL", "Run Step 59E — ISOLATED EMBARK ENABLE"]), "0.0.208 exposes all reusable Step-59 isolation branches in the same IPA")
register_start = character_core.find("public (bool Passed, string Diagnostics) RunStep59RegisterHotkeysProbe")
register_end = character_core.find("public (bool Passed, string Diagnostics) RunStep59UpdateControllerButtonProbe", register_start)
register_slice = character_core[register_start:register_end] if register_start >= 0 and register_end > register_start else ""
require('RequireStep59InstanceRuntimeMethod(embark.GetType(), "RegisterHotkeys", 0, step)' in register_slice and register_slice.count("RequireStep59InstanceRuntimeMethod") == 1 and "method.Invoke(embark, null)" in register_slice and "RunStep59RealHandlerTransition" not in register_slice, "Step 59H resolves and invokes only exact RegisterHotkeys; controller update/full Enable/real handler are not invoked by the probe")
tail_start = character_core.find("public (bool Passed, string Diagnostics) RunStep59ConfirmButtonTailProbe")
tail_end = character_core.find("public TransformedRealStS2StartupLadderGateResult RunStep59RealHandlerTransition", tail_start)
tail_slice = character_core[tail_start:tail_end] if tail_start >= 0 and tail_end > tail_start else ""
require(all(v in tail_slice for v in ["outline.Modulate=Transparent", "buttonImage.Modulate=White", "existingTween.Kill", "Node.CreateTween", "Tween.TweenProperty", "PropertyTweener.SetEase", "PropertyTweener.SetTrans", "PropertyTweener.FromCurrent"]) and all(v not in tail_slice for v in ["OpenCharacterSelect", "RegisterHotkeys()", "UpdateControllerButton()"]), "Step 59T isolates every post-base NConfirmButton visual/tween operation with staged evidence and no base/real-handler call")
require((ROOT / "docs/history/steps/STEP-59.9-CURRENT-OWNERSHIP-PIVOT-0.0.203.md").is_file() and all(v in read("docs/history/steps/STEP-59.9-CURRENT-OWNERSHIP-PIVOT-0.0.203.md") for v in ["0.0.202", "RefreshButtons", "byte-for-byte", "Step 52", "59R", "59H", "59U", "59T", "59E", "Step 63"]), "0.0.203 history note records the rejected trace regression, restored runtime, ownership pivot, multi-probe toolbox, and unopened Step 63")
...
require(all(v in character_core for v in ["Step59HandlerInvocationRequired", "RunStep59RealHandlerBinding", "RunStep59RealHandlerTransition", "CaptureStep59ForensicSnapshot", "M59_B_FORENSIC_PREFLIGHT", "IsNodeReady", "_charButtonContainer", "_ascensionPanel", "_remotePlayerContainer", "_randomCharacterButton", "HotkeyManager", "RemoteCursorContainer", "ReactionContainer", "TimeoutOverlay", "RootSceneContainer", "Progress", "Epochs", "EncounterStats", "CurrentScene", "M59_C_HANDLER_INNER_EXCEPTION", "M59_C_FORENSIC_POSTFAIL", "ExceptionDispatchInfo.Capture(inner).Throw()", "_step58RuntimeOpenCharacterSelect!.Invoke(submenu, new object?[] { _step59StandardButton })"]), "Step 59 performs forensic prerequisite localization, preserves inner game exception evidence, and captures post-failure state around one real handler invocation")
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(x in very_early_core for x in ["GodotSharp.step59.baseline-packedscene.dll", "GodotSharp.step59.wrapper-only.dll", "GodotSharp.step59.full-repair.dll", "GodotSharpPackedSceneCompatibilityCallbackFieldName", "PackedSceneInstantiateCompatibilityCallback", "ApplyPackedSceneInstancedRootUniqueNameCompatibility", "GetNodeInstance", "unique_name_in_owner", "GODOT_PACKEDSCENE_UNIQUE_NAME_COMPAT_ARMED", "CreateDiagnosticActionObjectObjectInvokeReference"]), "0.0.208 preserves the central private-GodotSharp PackedScene instanced-root unique-name compatibility hook across all three profiles")
require("private bool UsesExactPreparedGodotSharp => IsExactAuthorityMode;" in very_early_core, "0.0.199 active model-bootstrap path selects the verified GodotSharp compatibility derivative while exact-authority mode remains exact")
compat_start = very_early_core.find("private static void ApplyPackedSceneInstancedRootUniqueNameCompatibility")
compat_end = very_early_core.find("internal sealed class Step35ExecutionLoadContext", compat_start)
compat_slice = very_early_core[compat_start:compat_end] if compat_start >= 0 and compat_end > compat_start else ""
require(all(x in compat_slice for x in ["GetNodeInstance", 'string.Equals(propertyName, "unique_name_in_owner"', "UniqueNameInOwner", "GetChildren", "4096", "requiredPaths"]), "0.0.199 compatibility callback is limited to exact SceneState instanced-root unique-name metadata and path-matched runtime nodes")
require(all(x not in compat_slice for x in ["_ascensionPanel", "_actDropdown", "TimeoutOverlay", "SetStack", "OpenCharacterSelect", "_Ready("]), "0.0.199 global compatibility callback contains no screen/game-field or lifecycle-specific repair")
require("uniqueProperty.SetValue(node, true)" in compat_slice and "uniqueProperty.GetValue(node)" in compat_slice, "0.0.199 re-applies only the serialized node metadata and verifies the correction stuck")
require(all(v in character_core for v in ["BuildStep59ReadyBindingDiagnostics", "AscensionPanelManagedTypeFullNameForForensics", "AscensionPanelResourcePathForForensics", "\"%AscensionPanel\"", "UniqueNameInOwner", "selectedContext=", "CHARACTER-SELECT TSCN ASCENSION CONTEXT", "ASCENSION-PANEL TSCN ROOT CONTEXT", "M59_B_READY_BINDING_DIAGNOSIS", "M59_C_BLOCKED_READY_BINDING", "Step59ReadyBindingPreflightPassed"]), "0.0.196 Step 59 localizes the exact %AscensionPanel Ready binding using selected IL, live node/load-context/owner evidence, and receipt-backed TSCN context before handler arm")
require(all(x in character_core for x in ["BuildStep59UniqueNameProvenanceDiagnostics", "DescribeStep59SceneState", "GetNodePropertyCount", "GetNodePropertyName", "GetNodePropertyValue", "InstantiateStep59DiagnosticPackedScene", "CHARACTER-SELECT TEMPORARY OFF-TREE INSTANCE", "GAME TEMPORARY OFF-TREE INSTANCE", "GetNodeOrNull", "percentLookup=", "M59_B_UNIQUE_NAME_PROVENANCE"]), "0.0.198 compares retained SceneState, temporary off-tree instances, live unique flags, and direct/%Name lookups before handler arm")
require(all(x in character_core for x in ["BuildStep59ConfirmButtonOnEnableDiagnostics", "M59_B_CONFIRM_BUTTON_PREFLIGHT", "ConfirmButtonManagedTypeFullNameForForensics", "ConfirmButtonResourcePathForForensics", "NConfirmButton._Ready", "NConfirmButton.OnEnable", "NButton.OnEnable", "RegisterHotkeys", "_outline", "_buttonImage", "_viewport", "_hotkeys", "CHARACTER-SELECT SCENESTATE EMBARK ROOT", "CONFIRM-BUTTON TSCN OUTLINE CONTEXT", "CONFIRM-BUTTON TSCN IMAGE CONTEXT"]), "0.0.201 adds the retained embark NConfirmButton Ready/OnEnable IL/field/node/SceneState/TSCN preflight before handler arm")
confirm_diag_slice = character_core[character_core.find("private string BuildStep59ConfirmButtonOnEnableDiagnostics"):character_core.find("private string BuildStep59UniqueNameProvenanceDiagnostics")]
require("_Ready().Invoke" not in confirm_diag_slice and "OnEnable().Invoke" not in confirm_diag_slice and "_ascensionPanel" not in confirm_diag_slice, "0.0.201 confirm-button preflight remains free of direct game-field/Ready/OnEnable repair; 0.0.206 may toggle only its GodotSharp bridge fields")
require(all(x in confirm_diag_slice for x in ['_embarkButton IsInsideTree()=false', '_embarkButton IsNodeReady()=false', 'foreach (var fieldName in new[] { "_outline", "_buttonImage", "_viewport", "_hotkeys" })', 'fieldValues[fieldName] is null', 'failures.Add($"_embarkButton {fieldName}=NULL")', 'AppendStep59ConfirmButtonBlocker(failures)']), "0.0.201 definite NConfirmButton prerequisites fail closed into the existing Step-59 handler blocker")
require(character_core.find("M59_B_CONFIRM_BUTTON_PREFLIGHT") < character_core.find("M59_C_BLOCKED_READY_BINDING") < character_core.find("_step59TransitionStarted = true"), "0.0.201 writes confirm-button evidence and checks the shared blocker before the one-shot handler can arm")
unique_diag_start = character_core.find("private string BuildStep59UniqueNameProvenanceDiagnostics")
unique_diag_end = character_core.find("private static string DescribeStep59SceneState", unique_diag_start)
unique_diag_slice = character_core[unique_diag_start:unique_diag_end] if unique_diag_start >= 0 and unique_diag_end > unique_diag_start else ""
require("SetValue(" not in unique_diag_slice, "0.0.198 provenance diagnostics do not write game fields")
require("SetUniqueNameInOwner" not in character_core and ".UniqueNameInOwner =" not in character_core, "0.0.198 never repairs unique-name flags in launcher code")
require("TryReleaseStep39OffTreeInstance(temporaryCharacter" in character_core and "TryReleaseStep39OffTreeInstance(temporaryGame" in character_core, "0.0.198 releases temporary off-tree provenance clones")
require("AddChild" not in character_core[character_core.find("BuildStep59UniqueNameProvenanceDiagnostics"):character_core.find("BuildStep59TscnContext")], "0.0.198 temporary provenance clones are never admitted to the SceneTree")
require(character_core.find("M59_C_BLOCKED_READY_BINDING") < character_core.find("_step59TransitionStarted = true"), "0.0.196 ready-binding blocker is checked before the one-shot transition flag can arm")
ready_diag_start = character_core.find("private string BuildStep59ReadyBindingDiagnostics")
ready_diag_end = character_core.find("private string BuildStep59ConfirmButtonOnEnableDiagnostics", ready_diag_start)
ready_diag_slice = character_core[ready_diag_start:ready_diag_end] if ready_diag_start >= 0 and ready_diag_end > ready_diag_start else ""
require("SetValue(" not in ready_diag_slice, "0.0.196 ready-binding diagnostics never patch _ascensionPanel or any game field")
require('method.Name == "_Ready"' in character_core and 'field.Name == "_charButtonContainer"' in character_core and 'value == "%AscensionPanel"' in character_core and 'field.Name == "_ascensionPanel"' in character_core, "0.0.196 selected Cecil audit pins _Ready ordering around the exact ascension lookup")
...
step59_binding_start = character_core.find("public TransformedRealStS2StartupLadderGateResult RunStep59RealHandlerBinding")
step59_binding_end = character_core.find("public (bool Passed, string Diagnostics) RunStep59RegisterHotkeysProbe", step59_binding_start)
step59_transition_start = character_core.find("public TransformedRealStS2StartupLadderGateResult RunStep59RealHandlerTransition")
step59_transition_end = character_core.find("public TransformedRealStS2StartupLadderGateResult RunStep60ClosedStep59Authority", step59_transition_start)
step59_core_slice = (character_core[step59_binding_start:step59_binding_end] if step59_binding_start >= 0 and step59_binding_end > step59_binding_start else "") + (character_core[step59_transition_start:step59_transition_end] if step59_transition_start >= 0 and step59_transition_end > step59_transition_start else "")
require("submenuStackField.SetValue" not in step59_core_slice and "_ascensionPanel" not in step59_core_slice, "real Step 59 binding/transition never repairs submenu/game ownership by directly writing game fields; 0.0.206 may toggle only its GodotSharp repair bridge fields")
require("throw tie.InnerException" not in step59_core_slice and "throw inner" not in step59_core_slice, "Step 59 does not reset the original inner exception stack at the reflection boundary")
...
require(all(v in character_ui for v in ["M59_B_STATIC_MAP_WRITE_RETURNED", "Step59ReadyBindingPreflightPassed", "_step59TransitionUiStarted = true", "RunStep59RealHandlerTransition"]), "Step 59 UI durably writes ready-binding forensics before Gate C and one-shot bookkeeping arms only after a clean preflight")
require(all(v in character_core for v in ["BuildStep59CompilationEfficientDiagnosticDeck", "COMPILATION-EFFICIENT STEP-59 DIAGNOSTIC DECK", "RETAINED EMBARK FULL INHERITED FIELD MATRIX", "LIVE NCONFIRMBUTTON PEER MATRIX — WHOLE RETAINED SCENETREE", "NULL-FIELD LIVE NODE CANDIDATE MATRIX", "BuildStep59FieldAccessAndCallOrder", "TRANSITIVE EXECUTION FRONTIER", "NClickableControl", "OnSubmenuOpened"]), "0.0.201 compilation-efficient deck batches full fields, peers, null candidates, IL order and transitive Ready/Enable frontiers into one Gate-B map")
deck_start = character_core.find("private string BuildStep59CompilationEfficientDiagnosticDeck")
deck_end = character_core.find("private void AppendStep59ConfirmButtonBlocker", deck_start)
deck_slice = character_core[deck_start:deck_end] if deck_start >= 0 and deck_end > deck_start else ""
require(all(v not in deck_slice for v in ['_ascensionPanel").SetValue', '_embarkButton").SetValue', 'GetMethod("_Ready")', 'GetMethod("OnEnable")']), "0.0.201 compilation-efficient diagnostic deck performs no direct game-field write or reflective Ready/OnEnable replay")
step59d_start = character_ui.find("private async Task RunStep59DiagnosticDeckAsync")
step59d_end = character_ui.find("private async Task RunStep59StartupLadderAsync", step59d_start)
step59d_slice = character_ui[step59d_start:step59d_end] if step59d_start >= 0 and step59d_end > step59d_start else ""
require(all(v in step59d_slice for v in ["RunStep59ClosedStep58Authority", "RunStep59RealHandlerBinding", "M59D_B_STATIC_MAP_WRITE_RETURNED", "RUN_STEP59D_COMPLETE", "handlerArmed=False"]), "0.0.201 Step 59D executes the complete observational Gate-A/B deck and durably reports completion")
require("RunStep59RealHandlerTransition" not in step59d_slice and "_step59TransitionUiStarted = true" not in step59d_slice, "0.0.201 Step 59D cannot arm or invoke the real handler")
require(all(v in character_ui for v in ["FRESH PROCESS REQUIRED", "A Step-59 observational diagnostic already ran in this process", "M59_C_POSTFAIL_STATIC_MAP_REFRESH_RETURNED"]), "0.0.203 separates observational/mutating/real-handler phone experiments by fresh process and persists post-failure map evidence")
require(all(v in character_core for v in ["BuildStep59PostFailureRuntimeDiagnosticDeck", "POST-FAILURE CONFIRM-BUTTON RUNTIME DECK", "M59_C_CONFIRM_BUTTON_POSTFAIL"]), "0.0.201 real-handler failure captures a post-failure embark runtime deck before preserved exception return")
require(all(v in read("docs/TESTING.md") for v in ["Step 59D", "RUN_START_DIAGNOSTIC_DECK", "M59D_B_STATIC_MAP_WRITE_RETURNED", "M59_C_CONFIRM_BUTTON_POSTFAIL", "M59_C_POSTFAIL_STATIC_MAP_REFRESH_RETURNED"]), "0.0.201 testing procedure prioritizes compilation-efficient Step59D and same-IPA fresh-process follow-up")
require('public const string Step59GateName = "GLOBAL PACKEDSCENE UNIQUE-NAME COMPAT + REAL TRANSITION"' in character_core and '_step59Gates = new(59, TransformedRealStS2VeryEarlyInitialization.Step59GateName)' in character_ui, "0.0.199 retains one shared Step-59 gate-name authority between Core and iOS UI")
require('_step59Gates = new(59, "FORENSIC REAL OPENCHARACTERSELECT FROZEN TRANSITION")' not in character_ui, "0.0.197 removes the stale Step-59 UI gate-name literal that caused the physical 0.0.196 harness failure")
...
require(all(v in character_core for v in ["RunStep62ClosedStep61Authority", "RunStep62SustainedRenderPreflight", "BeginStep62SustainedRenderPulse", "RunStep62SustainedRenderEvidence", "RunStep62FrozenPostResidencyConfinement", "_step62PulseStarted"]), "Step 62 is a distinct 10-second visible render/refreeze rung")
require("StartRendering" not in character_core and character_ui.count("GodotStep15NativeBridge.StartRendering()") == 2, "Core never starts rendering and Steps 61/62 UI own exactly two new render starts")
require(all(v in character_ui for v in ["_step59TransitionUiStarted", "_step61PulseUiStarted", "_step62PulseUiStarted", "ONE-SHOT ALREADY ARMED"]) and "Step59HandlerInvocationRequired" in character_ui, "Step 59 exposes conditional handler one-shot state while Steps 61/62 retain explicit render one-shot guards")
require(character_ui.find("MarkStep61StaticMapDurablyWritten") < character_ui.find("BeginStep61BoundedRenderPulse") < character_ui.find("GodotStep15NativeBridge.StartRendering()"), "Step 61 makes pre-render evidence durable before arming/rendering")
step61_ui_start = character_ui.find("private async Task RunStep61StartupLadderAsync()")
step62_ui_start = character_ui.find("private async Task RunStep62StartupLadderAsync()")
step61_ui = character_ui[step61_ui_start:step62_ui_start] if step61_ui_start >= 0 and step62_ui_start > step61_ui_start else ""
step62_ui = character_ui[step62_ui_start:] if step62_ui_start >= 0 else ""
require(step61_ui.find("GodotStep15NativeBridge.StopRendering()") >= 0 and step61_ui.find("GodotStep15NativeBridge.StopRendering()") < step61_ui.find("RunStep61RenderPulseEvidence"), "Step 61 synchronously stops rendering before post-stop evidence")
require(character_ui.find("MarkStep62StaticMapDurablyWritten") < character_ui.find("BeginStep62SustainedRenderPulse") < character_ui.rfind("GodotStep15NativeBridge.StartRendering()"), "Step 62 makes pre-render evidence durable before arming/rendering")
require(step62_ui.find("GodotStep15NativeBridge.StopRendering()") >= 0 and step62_ui.find("GodotStep15NativeBridge.StopRendering()") < step62_ui.find("RunStep62SustainedRenderEvidence"), "Step 62 synchronously stops rendering before post-stop evidence")
require("Step 63" in character_ui and "RunStep63" not in startup_ui_all and "ExactStep63" not in (startup_core + direct_core + continuation_core + singleplayer_core + character_core), "Step 63 remains explicitly unopened")
step58_pivot = read("docs/history/steps/STEP-58.0-RUNTIME-OWNERSHIP-PIVOT-0.0.192.md")
step58_stack_correction = read("docs/history/steps/STEP-58.0-LOGICAL-STACK-BINDING-CORRECTION-0.0.193.md")
step58_192_report = read("docs/history/reports/STEP-58.0-PHYSICAL-0.0.192-FAIL-REPORT.txt")
step58_192_checkpoint = read("docs/history/reports/STEP-58.0-PHYSICAL-0.0.192-FAIL-CHECKPOINT.txt")
...
require(all(v in step58_192_checkpoint for v in ["M58_A_PASS", "M58_B_FAIL", "renderingActive=False", "Step 58 never invokes OpenCharacterSelect or restarts rendering", "RUN_END"]), "raw physical 0.0.192 checkpoint proves retained Step-57 authority and frozen no-mutation stop")
...
step59_forensic_195 = read("docs/history/steps/STEP-59.1-FORENSIC-LOCALIZATION-0.0.195.md")
step59_194_fail_report = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.194-FAIL-REPORT.txt")
step59_194_fail_checkpoint = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.194-FAIL-CHECKPOINT.txt")
require(all(v in step59_194_fail_report for v in ["App version: 0.0.194 (194)", "STEP 59.0 REAL OPENCHARACTERSELECT FROZEN TRANSITION FAIL"]), "physical 0.0.194 Step-59 failure report is preserved")
require(all(v in step59_194_fail_checkpoint for v in ["M59_B_PASS", "navigationRepairRequired=False", "M59_C_NAVIGATION_REPAIR_SKIPPED", "M59_C_HANDLER_ARMED", "System.NullReferenceException"]), "physical 0.0.194 checkpoint disproves missing single-player stack/button theory")
require(all(v in step59_forensic_195 for v in ["Physical **0.0.194 (194)**", "normal game preload state", "IsNodeReady", "Progress.Epochs", "Progress.EncounterStats", "TargetSite", "original StackTrace", "ExceptionDispatchInfo", "No direct character-select field write"]), "0.0.195 history note records the forensic localization design and no-speculative-repair boundary")
step59_195_fail_report = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.195-FAIL-REPORT.txt")
step59_195_fail_checkpoint = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.195-FAIL-CHECKPOINT.txt")
step59_195_fail_last = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.195-FAIL-LAST-CHECKPOINT.txt")
step59_ready_196 = read("docs/history/steps/STEP-59.2-ASCENSION-READY-BINDING-FORENSICS-0.0.196.md")
require(all(v in step59_195_fail_report for v in ["App version: 0.0.195 (195)", "FAIL — 1/4", "_ascensionPanel is null"]), "physical 0.0.195 report localizes the pre-handler blocker to null _ascensionPanel")
require(all(v in step59_195_fail_checkpoint for v in ["M59_A_PASS", "M59_B_FAIL", "IsNodeReady()=True", "_ascensionPanel is null", "renderingActive=False", "RUN_END"]), "physical 0.0.195 checkpoint proves safe frozen Gate-B localization before handler arm")
require("Step-59 UI operation ended normally" in step59_195_fail_last, "physical 0.0.195 last checkpoint preserves normal frozen Step-59 end")
require(all(v in step59_ready_196 for v in ["Physical **0.0.195 (195)**", "%AscensionPanel", "not** a `[Godot.Export]` field", "UniqueNameInOwner", "No launcher code writes `_ascensionPanel`"]), "0.0.196 history note records the exact Ready-binding diagnosis and no-field-patch boundary")
step59_196_harness_report = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.196-HARNESS-FAIL-REPORT.txt")
step59_196_harness_checkpoint = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.196-HARNESS-FAIL-CHECKPOINT.txt")
step59_196_harness_last = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.196-HARNESS-FAIL-LAST-CHECKPOINT.txt")
step59_gate_197 = read("docs/history/steps/STEP-59.3-GATE-NAME-CONTRACT-CORRECTION-0.0.197.md")
require(all(v in step59_196_harness_report for v in ["App version: 0.0.196 (196)", "STEP 59.0: EXCEPTION", "gate sequence received a result"]), "physical 0.0.196 report preserves the pre-Gate-B harness-name mismatch")
require(all(v in step59_196_harness_checkpoint for v in ["M59_A_PASS", "RUN_MANAGED_EXCEPTION", "CHARACTER SELECT READY-BINDING FORENSICS + REAL TRANSITION", "renderingActive=False", "RUN_END"]), "physical 0.0.196 checkpoint proves Gate A passed, no game mutation armed, and the run stayed frozen")
require("Step-59 UI operation ended normally" in step59_196_harness_last, "physical 0.0.196 last checkpoint preserves normal frozen harness-failure end")
require(all(v in step59_gate_197 for v in ["Physical **0.0.196 (196)**", "bookkeeping regression", "single public compile-time authority", "experiment is otherwise unchanged"]), "0.0.197 history note records the gate-name contract correction without widening the game experiment")
step59_197_report = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-FAIL-REPORT.txt")
step59_197_checkpoint = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-FAIL-CHECKPOINT.txt")
step59_197_last = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-FAIL-LAST-CHECKPOINT.txt")
step59_197_map = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-READY-BINDING-STATIC-MAP.txt")
step59_unique_198 = read("docs/history/steps/STEP-59.4-UNIQUE-NAME-PROVENANCE-FORENSICS-0.0.198.md")
require(all(x in step59_197_report for x in ["App version: 0.0.197 (197)", "FAIL — 2/4", "handler intentionally blocked before one-shot arm"]), "physical 0.0.197 report preserves the safe ready-binding block")
require(all(x in step59_197_checkpoint for x in ["M59_B_READY_BINDING_DIAGNOSIS", "M59_B_STATIC_MAP_WRITE_RETURNED", "M59_C_BLOCKED_READY_BINDING", "renderingActive=False", "RUN_END"]), "physical 0.0.197 checkpoint proves the diagnostic map was durable before the safe Gate-C block")
require("Step-59 UI operation ended normally" in step59_197_last, "physical 0.0.197 last checkpoint preserves normal frozen end")
require(all(x in step59_197_map for x in ["uniqueNameInOwner=False; owner=character-select-root", 'L316: unique_name_in_owner = true', "selectedContext=True", "AscensionIcon", "uniqueNameInOwner=True"]), "physical 0.0.197 static map proves exact AscensionPanel unique-name override loss while a nested unique node survives")
require(all(x in step59_unique_198 for x in ["Physical **0.0.197 (197)**", "PackedScene.GetState()", "temporary **off-tree**", "TSCN true + SceneState", "No gameplay or Step 63"]), "0.0.198 history note records the unique-name provenance experiment and interpretation")
step59_198_report = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-REPORT.txt")
step59_198_checkpoint = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-CHECKPOINT.txt")
step59_198_map = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-STATIC-MAP.txt")
step59_198_last = read("docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-LAST-CHECKPOINT.txt")
step59_compat_199 = read("docs/history/steps/STEP-59.5-GLOBAL-PACKEDSCENE-UNIQUE-NAME-COMPAT-0.0.199.md")
require(all(x in step59_198_report for x in ["App version: 0.0.198 (198)", "FAIL — 2/4", "handler intentionally blocked before one-shot arm"]), "physical 0.0.198 report preserves safe pre-handler block")
require(all(x in step59_198_checkpoint for x in ["M59_B_UNIQUE_NAME_PROVENANCE", "M59_B_STATIC_MAP_WRITE_RETURNED", "M59_C_BLOCKED_READY_BINDING", "renderingActive=False", "RUN_END"]), "physical 0.0.198 checkpoint proves provenance evidence was durable before safe frozen block")
require(all(x in step59_198_map for x in ["[CHARACTER-SELECT PACKEDSCENE SCENESTATE]", "instance=YES:Godot.PackedScene; properties=[unique_name_in_owner=True]", "[CHARACTER-SELECT TEMPORARY OFF-TREE INSTANCE]", "AscensionPanel:", "uniqueNameInOwner=False", "[GAME TEMPORARY OFF-TREE INSTANCE]", "InputManager:", "uniqueNameInOwner=True", "MultiplayerTimeoutOverlay:", "[MAIN-MENU RETAINED LIVE INSTANCE]"]), "physical 0.0.198 map proves SceneState true -> instanced-root false while ordinary nodes preserve true")
require("Step-59 UI operation ended normally" in step59_198_last, "physical 0.0.198 last checkpoint preserves normal frozen end")
require(all(x in step59_compat_199 for x in ["Physical **0.0.198 (198)**", "instanced-subscene root", "Godot.PackedScene.Instantiate", "GetNodeInstance(index)", "no `_ascensionPanel`, `_actDropdown`, `TimeoutOverlay`", "intended long-term launcher architecture"]), "0.0.199 history note records the global compatibility correction and non-screen-specific boundary")
...
# Cross-rung restrictions and distinct evidence surfaces.
require("RunStep58" in character_core and "ExactStep62ClosurePassed" in character_core and "Step 63" in character_ui and "RunStep63" not in startup_ui_all and "ExactStep63" not in (startup_core + direct_core + continuation_core + singleplayer_core + character_core), "Steps 58-62 are opened as the active ownership/render continuation while Step 63 remains unopened")
require(".GameStartup(" not in startup_ui_all and ".LaunchMainMenu(" not in startup_ui_all and ".DoCloudSync(" not in startup_ui_all and ".InitializePlatform(" not in startup_ui_all and ".ExecuteDeferred(" not in startup_ui_all, "iOS ladder UI has no direct GameStartup/original LaunchMainMenu/cloud/platform/deferred call")
require("RunDeprecatedStep47" not in startup_ui_all and "ControlledLaunchMainMenu" not in startup_ui_all, "iOS ladder has no stale original live-launch control path")
for step, map_stem, report in [
    (43, "Step43-Platform-StaticMap", "Step43-TransformedRealStS2NullPlatformAuthority.txt"),
    (44, "Step44-LegacyGuard-StaticMap", "Step44-TransformedRealStS2LegacyMigrationGuard.txt"),
    (45, "Step45-LocalSave-StaticMap", "Step45-TransformedRealStS2LocalSaveInitialization.txt"),
    (46, "Step46-LaunchMainMenu-ImmediateFrontier-StaticMap", "Step46-TransformedRealStS2LaunchMainMenuImmediateFrontierMap.txt"),
    (47, "Step47-MainMenuResources-StaticMap", "Step47-TransformedRealStS2MainMenuResourcePreparation.txt"),
    (48, "Step48-MainMenuOffTree-StaticMap", "Step48-TransformedRealStS2MainMenuOffTreeInstantiation.txt"),
    (49, "Step49-MainMenuAdmission-StaticMap", "Step49-TransformedRealStS2MainMenuFrozenAdmission.txt"),
    (50, "Step50-MainMenuFrameInput-StaticMap", "Step50-TransformedRealStS2MainMenuRenderPulse.txt"),
    (51, "Step51-SingleplayerFrontier-StaticMap", "Step51-TransformedRealStS2SingleplayerFrontier.txt"),
    (52, "Step52-MainMenuSustainedFrameInput-StaticMap", "Step52-TransformedRealStS2MainMenuSustainedRender.txt"),
    (53, "Step53-SingleplayerOpen-StaticMap", "Step53-TransformedRealStS2SingleplayerOpenFrontier.txt"),
    (54, "Step54-SingleplayerSubmenuOpen-StaticMap", "Step54-TransformedRealStS2SingleplayerSubmenuFrozenOpen.txt"),
    (55, "Step55-SingleplayerSubmenuFrameInput-StaticMap", "Step55-TransformedRealStS2SingleplayerSubmenuRender.txt"),
    (56, "Step56-CharacterSelectFrontier-StaticMap", "Step56-TransformedRealStS2CharacterSelectFrontier.txt"),
    (57, "Step57-CharacterSelectResource-StaticMap", "Step57-TransformedRealStS2CharacterSelectResourcePreflight.txt"),
    (58, "Step58-CharacterSelectOwnership-StaticMap", "Step58-TransformedRealStS2CharacterSelectOwnership.txt"),
    (59, "Step59-RealOpenCharacterSelect-StaticMap", "Step59-TransformedRealStS2OpenCharacterSelectFrozen.txt"),
    (60, "Step60-CharacterSelectActiveSurface-StaticMap", "Step60-TransformedRealStS2CharacterSelectActiveSurface.txt"),
    (61, "Step61-CharacterSelectShortRender-StaticMap", "Step61-TransformedRealStS2CharacterSelectShortRender.txt"),
    (62, "Step62-CharacterSelectSustainedRender-StaticMap", "Step62-TransformedRealStS2CharacterSelectSustainedRender.txt"),
]:
    require(map_stem in startup_ui_all and report in startup_ui_all, f"Step {step} has distinct static-map/report evidence surface")
require('$"Step{step}-CrashCheckpoint-{runId}.txt"' in startup_ui and '$"Step{step}-LastCheckpoint.txt"' in startup_ui, "all ladder rungs share run-correlated but step-distinct checkpoint/last-checkpoint naming")

# New direct-main-menu active candidate manifest.
startup_manifest = ROOT / "tools/validation/candidate-steps43-52-guarded-direct-main-menu.sha256"
require(startup_manifest.is_file(), "Steps 43-52 guarded direct-main-menu candidate hash manifest exists")
if startup_manifest.is_file():
    startup_manifest_text = startup_manifest.read_text()
    required_startup_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadder.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2DirectMainMenuLadder.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2DirectMainMenuContinuation.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderGate.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderGateResult.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderGateSequence.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadderSummary.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2StartupLadderTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2StartupLadder.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2DirectMainMenuLadder.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2DirectMainMenuContinuation.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.PhysicallyClosedPath.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs", "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj", "src/StS2Launcher.iOS/Info.plist",
        ...
        "README.md", "docs/README.md", "docs/CURRENT-STATUS.md", "docs/MASTER-PLAN.md", "docs/TESTING.md", "docs/RELEASE-CHECKLIST.md", "docs/history/INDEX.md",
        ...
        "docs/history/reports/CODEMAGIC-0.0.182-BUILD-PERFORMANCE-ANALYSIS.txt",
        "docs/history/reports/STEP-50.0-PHYSICAL-0.0.182-FIRST-ATTEMPT-REPORT.txt",
        "docs/history/reports/STEP-50.0-PHYSICAL-0.0.182-FIRST-ATTEMPT-CHECKPOINT.txt",
        "docs/history/reports/STEP-50.0-PHYSICAL-0.0.182-FIRST-ATTEMPT-STATIC-MAP.txt",
        "docs/history/reports/STEP-50.0-PHYSICAL-0.0.182-FIRST-ATTEMPT-LAST-CHECKPOINT.txt",
        ...
        "docs/history/reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-REPORT.txt",
        "docs/history/reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-48.0-PHYSICAL-0.0.179-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-46.0-PHYSICAL-0.0.178-FAIL-REPORT.txt",
        "docs/history/reports/STEP-46.0-PHYSICAL-0.0.178-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-46.0-PHYSICAL-0.0.178-FAIL-LAST-CHECKPOINT.txt",
    ...
    require(all(f"  {relative}" in startup_manifest_text for relative in required_startup_manifest_paths), "Steps 43-52 manifest pins guarded direct-menu implementation/tests/UI/release wiring and latest physical authority")
    mismatches=[]
    for line in startup_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
    require(not mismatches, "Steps 43-52 guarded direct-main-menu active files are hash-pinned", ", ".join(mismatches))

# Focused 0.0.183 stabilization/ergonomics/CI manifest. This is intentionally
# separate from the guarded direct-menu provenance manifest so later releases can
# preserve the exact build-performance and convenience-runner contract.
stabilization_manifest = ROOT / "tools/validation/candidate-steps43-52-physically-closed-stabilization.sha256"
require(stabilization_manifest.is_file(), "Steps 43-52 physically closed stabilization candidate hash manifest exists")
if stabilization_manifest.is_file():
    stabilization_manifest_text = stabilization_manifest.read_text()
    required_stabilization_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2DirectMainMenuLadder.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.PhysicallyClosedPath.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
    ...
    require(all(f"  {relative}" in stabilization_manifest_text for relative in required_stabilization_manifest_paths), "Steps 43-52 stabilization manifest pins fast-path UI, Step-50 calibration, AOT-cache wiring, release docs, and supplied physical/CI evidence")
    mismatches=[]
    for line in stabilization_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
...
# Dedicated 0.0.189 Steps 53-57 single-player continuation manifest. This pins
...
require(singleplayer_manifest.is_file(), "Steps 53-57 single-player continuation candidate hash manifest exists")
if singleplayer_manifest.is_file():
    singleplayer_manifest_text = singleplayer_manifest.read_text()
    required_singleplayer_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2SingleplayerContinuation.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadder.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2StartupLadderTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2SingleplayerContinuation.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2StartupLadder.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.PhysicallyClosedPath.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
        "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj",
        ...
        "docs/history/steps/STEP-54.0-SUBMENU-STACK-LAZY-BINDING-CORRECTION-0.0.186.md",
        "docs/history/steps/STEP-56.0-NBUTTON-SIGNATURE-CORRECTION-0.0.187.md",
        "docs/history/steps/STEP-57.0-SCENE-HINT-PCK-RESOLUTION-CORRECTION-0.0.188.md",
        "docs/history/steps/STEP-57.0-RETAINED-PACKEDSCENE-IDENTITY-CORRECTION-0.0.189.md",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.188-FAIL-REPORT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.188-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.188-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.187-FAIL-REPORT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.187-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.187-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.186-FAIL-REPORT.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.186-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.186-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-54.0-PHYSICAL-0.0.185-FAIL-REPORT.txt",
        "docs/history/reports/STEP-54.0-PHYSICAL-0.0.185-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-54.0-PHYSICAL-0.0.185-FAIL-LAST-CHECKPOINT.txt",
    ...
    require(all(f"  {relative}" in singleplayer_manifest_text for relative in required_singleplayer_manifest_paths), "Steps 53-57 manifest pins new Core/UI/tests/release/docs design and closed-through-52 authority")
    mismatches=[]
    for line in singleplayer_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
...
# Dedicated 0.0.201 Steps 58-62 NConfirmButton OnEnable-preflight / PackedScene compatibility candidate manifest.
...
if character_manifest.is_file():
    character_manifest_text = character_manifest.read_text()
    required_character_manifest_paths = [
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2CharacterSelectContinuation.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2SingleplayerContinuation.cs",
        "src/StS2Launcher.Core/Runtime/TransformedRealStS2StartupLadder.cs",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2StartupLadderTests.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2CharacterSelectContinuation.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2SingleplayerContinuation.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.TransformedRealStS2StartupLadder.cs",
        "src/StS2Launcher.iOS/UI/RootViewController.PhysicallyClosedPath.cs",
        "src/StS2Launcher.iOS/UI/CurrentReleasePresentation.cs",
        "src/StS2Launcher.iOS/StS2Launcher.iOS.csproj",
        "src/StS2Launcher.iOS/Info.plist",
        "scripts/lib/current-release.sh", "scripts/build-ios.sh", "scripts/test.sh", "scripts/codemagic.sh", "scripts/verify-ipa.sh", "codemagic.yaml",
        "tools/validate_current.py", "README.md", "docs/README.md", "docs/CURRENT-STATUS.md", "docs/MASTER-PLAN.md", "docs/TESTING.md", "docs/REPORTS.md", "docs/RELEASE-CHECKLIST.md", "docs/history/INDEX.md",
        "docs/history/steps/STEP-56-57-PHYSICAL-CLOSURE-0.0.189.md",
        ...
        "docs/history/steps/STEP-59.1-FORENSIC-LOCALIZATION-ANALYSIS-0.0.195.md",
        "docs/history/steps/STEP-59.1-FORENSIC-LOCALIZATION-0.0.195.md",
        "docs/history/steps/STEP-59.2-ASCENSION-READY-BINDING-FORENSICS-0.0.196.md",
        "docs/history/steps/STEP-59.3-GATE-NAME-CONTRACT-CORRECTION-0.0.197.md",
        "docs/history/steps/STEP-59.4-UNIQUE-NAME-PROVENANCE-FORENSICS-0.0.198.md",
        "docs/history/steps/STEP-59.5-GLOBAL-PACKEDSCENE-UNIQUE-NAME-COMPAT-0.0.199.md",
        "docs/history/steps/STEP-59.6-HOST-REGRESSION-CONTRACT-0.0.200.md",
        "docs/history/steps/STEP-59.7-NCONFIRMBUTTON-ONENABLE-PREFLIGHT-0.0.201.md",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-STATIC-MAP.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-REPORT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.200-FAIL-LAST-CHECKPOINT.txt",
        "tests/StS2Launcher.Core.Tests/Runtime/TransformedRealStS2VeryEarlyInitializationTests.cs",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-STATIC-MAP.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-REPORT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.198-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-READY-BINDING-STATIC-MAP.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-FAIL-REPORT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.197-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.196-HARNESS-FAIL-REPORT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.196-HARNESS-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.196-HARNESS-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.195-FAIL-REPORT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.195-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.195-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.194-FAIL-REPORT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.194-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-59.0-PHYSICAL-0.0.194-FAIL-LAST-CHECKPOINT.txt",
        ...
        "docs/history/reports/STEP-58.0-PHYSICAL-0.0.191-FAIL-REPORT.txt",
        "docs/history/reports/STEP-58.0-PHYSICAL-0.0.191-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-58.0-PHYSICAL-0.0.191-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-58.0-PHYSICAL-0.0.192-FAIL-REPORT.txt",
        "docs/history/reports/STEP-58.0-PHYSICAL-0.0.192-FAIL-CHECKPOINT.txt",
        "docs/history/reports/STEP-58.0-PHYSICAL-0.0.192-FAIL-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-REPORT.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-CHECKPOINT.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-STATIC-MAP.txt",
        "docs/history/reports/STEP-56.0-PHYSICAL-0.0.188-PASS-LAST-CHECKPOINT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.189-PASS-REPORT.txt",
        "docs/history/reports/STEP-57.0-PHYSICAL-0.0.189-PASS-CHECKPOINT.txt",
    ...
    require(all(f"  {relative}" in character_manifest_text for relative in required_character_manifest_paths), "Steps 58-62 manifest pins ownership/render design, exact game-owned Push repair, physical 0.0.200 NConfirmButton localization, 0.0.201 preflight, and final Step-56/57 authority")
    mismatches=[]
    for line in character_manifest.read_text().splitlines():
        if not line.strip(): continue
        digest, relative = line.split("  ",1); path=ROOT/relative
        if not path.is_file() or sha256(path)!=digest: mismatches.append(relative)
    require(not mismatches, "Steps 58-62 character-select ownership active files are hash-pinned", ", ".join(mismatches))
