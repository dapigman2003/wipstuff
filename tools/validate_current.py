...
print("StS2 Launcher — Step 35.0.8 generic delegate MemberRef fix + in-method localization validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>131</ApplicationVersion>" in project_text, "build version is 131")
require("<ApplicationDisplayVersion>0.0.131</ApplicationDisplayVersion>" in project_text, "display version is 0.0.131")
require(plist.get("CFBundleVersion") == "131", "Info.plist build version is 131")
require(plist.get("CFBundleShortVersionString") == "0.0.131", "Info.plist display version is 0.0.131")
...
require("STEP 35.0.8 — SAVE/PLATFORM/GODOT NATIVE-BOUNDARY LOCALIZATION" in release_presentation, "top launcher banner identifies active Step 35.0.8 generic-delegate MemberRef localization candidate")
require("STEP 32 CLOSED POSITIVE 4/4" in release_presentation and "STEP 33 CLOSED POSITIVE 4/4" in release_presentation and "STEP 34 CLOSED POSITIVE 4/4" in release_presentation and "0.0.129" in release_presentation and "MissingMethodException" in release_presentation and "Action<string>.Invoke(string)" in release_presentation and "Action<string>::Invoke(!0)" in release_presentation and "0.0.130" in release_presentation and "SaveManager.get_Instance" in release_presentation and "Godot.DirAccess.DirExistsAbsolute" in release_presentation and "diagnostic clone" in release_presentation, "top launcher banner preserves closed Steps 32-34, records the 0.0.129 bridge failure and physical 0.0.130 SaveManager frontier, and identifies the 0.0.131 Godot-boundary localization")
...
require('ExpectedDisplayVersion = "0.0.131"' in release_presentation and 'ExpectedBuildVersion = "131"' in release_presentation, "Step 35.0.8 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.131",
    "STS2_BUILD_VERSION": "131",
...
require("Step 35.0.8 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.8 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.8 — Save/Platform/Godot native-boundary localization with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.8 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.8 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.8 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
...
require("BuildStaticInstructionMap" in step35_source and "CALLSITE#" in step35_source and "AWAIT-CANDIDATE" in step35_source and "DescribeMetadataScope" in step35_source and "GetVerifiedVeryEarlyStaticInstructionMap" in step35_source, "Step 35.0.8 retains the resolution-free static IL/callsite map from the already-verified exact transformed wrapper/MoveNext")
require(all(marker in step35_source for marker in ["CreateInstrumentedDiagnosticClone", "GetDiagnosticMarkerTargets", "GetDiagnosticCallsiteMarkerTargets", "HasInjectedEntryMarkerAtStart", "HasInjectedCallsiteMarkers", "InsertCallsiteMarkers", "DiagnosticBridgeTypeFullName", "INMETHOD_001", "INMETHOD_021", "INMETHOD_022", "INMETHOD_023", "INMETHOD_024", "INMETHOD_025", "INMETHOD_026", "INMETHOD_031", "INMETHOD_050", "INMETHOD_100", "INMETHOD_180", "INMETHOD_181", "INMETHOD_182", "INMETHOD_183", "System.Boolean Godot.DirAccess::DirExistsAbsolute(System.String)", "Godot.Error Godot.DirAccess::MakeDirRecursiveAbsolute(System.String)", "C_DIAGNOSTIC_BRIDGE_ARMED", "preflight.DiagnosticPath", "preflight.DiagnosticSha256"]), "Step 35.0.8 creates a separately verified diagnostic clone with Save/Platform/Godot entry markers and exact pre/post DirAccess callsite markers")
require(all(marker in step35_source for marker in ["module.Write(diagnosticPath", "verifyModule.Mvid != TransformedRealStS2AssemblyAdmission.ClosedStep32Mvid", "transformedSha256AfterDiagnosticEmission", "ComputeSha256Hex(preflight.TransformedPath)", "markerCountVerified != expectedMarkerCount"]), "Step 35.0.8 preserves/rechecks the exact transformed source and verifies diagnostic-clone MVID/hash/marker provenance")
require(all(marker in step35_source for marker in ["DiagnosticConstantMetadataWriteResolver", "DiagnosticCecilWriteSystemRuntimeIdentity", "DiagnosticCecilWriteSentryIdentity", "System.Reflection.BindingFlags", "Sentry.BreadcrumbLevel", "Sentry.SentryLevel", "CollectDiagnosticExternalConstantTypeRequirements", "resolver.Configure(module)", "resolver.ValidateWriteRequests()", "RealStS2PrepareMethodRewrite.ComputeConstantMetadataFingerprint", "verifiedConstantMetadataSha256.Equals(expectedConstantMetadataSha256", "External assembly bytes opened by the writer-only surrogate resolver: 0"]), "Step 35.0.8 reuses the Step-32 audited constant-metadata requirement set locally for Cecil serialization only and re-verifies constant metadata under rejecting reopen without changing protected Step-32 source")
require("new DiagnosticConstantMetadataWriteResolver()" in step35_source and "new RejectingAssemblyResolver()" in step35_source, "Step 35.0.8 separates bounded writer-only Cecil resolution from rejecting post-write verification")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.8 diagnostic clone mirrors Step-32 file-backed deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.8 encodes and post-write verifies the diagnostic Action<string> Invoke MemberRef with the declaring-type VAR(0) signature")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.8 rejects the physically disproven synthetic Action<string>::Invoke(string) MemberRef encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.8 host regressions serialize/reopen the exact Action<string>::Invoke(!0) declaring-type VAR(0) MemberRef shape")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.8 host regressions serialize/reopen the new Godot pre/post callsite marker shape")
...
require(all(marker in step35_source for marker in ["B_LOADFROMSTREAM_START", "B_LOADFROMSTREAM_PASS", "C_BIND_TYPE_START", "C_BIND_METHOD_START", "C_INVOKE_START", "C_INVOKE_RETURNED", "C_TASK_CONFIRMED", "C_WAIT_START", "C_WAIT_COMPLETED", "RESOLVE_MANAGED_START", "RESOLVE_PRIVATE_PASS", "RESOLVE_HOST_PASS", "RESOLVE_NATIVE_REJECT"]), "Step 35.0.8 core retains durable B→C/invocation/resolver crash-localization markers")
require(all(marker in step35_ui for marker in ["Step35-CurrentRun.txt", "Step35-LastCheckpoint.txt", "Step35-CrashCheckpoint-", "Step35-ExecuteVeryEarly-StaticMap-", "TryInitializeStep35RunTelemetry", "RUN_TELEMETRY_READY", "WriteStep35StaticMap", "Flush(flushToDisk: true)", "Environment.CurrentManagedThreadId", "CANCELLED / INCONCLUSIVE", "Output-only diagnostic"]), "Step 35.0.8 iOS surface retains fail-visible run-correlated durable telemetry and keeps cancellation inconclusive")
require("crashCheckpoints.Any" in step35_tests and "B_LOADFROMSTREAM_START" in step35_tests and "RESOLVE_INITIALIZER_BEARING_REJECT" in step35_tests and "StaticInstructionMapCapturesCallsitesAndAwaitCandidatesWithoutResolution" in step35_tests, "Step 35.0.8 host regressions retain crash-checkpoint and static-map callsite/await coverage")
require("OrderedDiagnosticLocalizationGatesReachFourOfFourWithoutClaimingClosure" in step35_tests and "VeryEarlyInitializationStopsAfterFirstFailure" in step35_tests and "VeryEarlyContextLoadsInitializerFreePrivateDependencyAndRejectsInitializerBearingDependency" in step35_tests and "Step35PinsTheExactVeryEarlyManagedInitializationTarget" in step35_tests, "Step 35.0.8 host regressions protect diagnostic gate order, exact source target and initializer-bearing refusal")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.8 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.8 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.8 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.8 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.8 active source/UI reject stale exact-closure wording")
...
require("Step35ImplementationMarker" in release_presentation and "0x06007D02" in release_presentation and "0x0600BC71" in release_presentation and "ExecuteVeryEarly" in release_presentation, "release presentation pins the Step-35.0.8 exact-source/instrumented-clone boundary")
...
require("Steps 01–26" in current_status and "Step 27" in current_status and "CLOSED NEGATIVE" in current_status and "CLOSED POSITIVE 5/5" in current_status and "Step 35 remains OPEN" in current_status and "Active candidate — Step 35.0.8 / 0.0.131 (131)" in current_status and "0.0.126" in current_status and "0.0.127" in current_status and "0.0.128" in current_status and "0.0.129" in current_status and "0.0.130" in current_status and "MissingMethodException" in current_status and "Action<string>::Invoke(!0)" in current_status and "SaveManager.get_Instance" in current_status and "Godot.DirAccess.DirExistsAbsolute" in current_status and "INMETHOD_180" in current_status and "0x06007D02" in current_status and "0x0600BC71" in current_status and "diagnostic clone" in current_status, "current status preserves the exact 0.0.126 authority, records the 0.0.129 bridge fix and physical 0.0.130 SaveManager frontier, and identifies the active 0.0.131 Godot-boundary localization")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.131 (131)" in testing_doc and "0.0.131 (131)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.131 (131) release identity")
require("0.0.131 (130)" not in testing_doc and "0.0.131 (129)" not in testing_doc and "0.0.131 (130)" not in release_checklist_doc and "0.0.131 (129)" not in release_checklist_doc, "release docs reject display/build-number drift like the prior 0.0.130 (129) documentation bug")
...
require("Step 35.0.8 In-Method Pre-First-Await Localization build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.8")
...
require("Expected device UI: STEP 35.0.8 — SAVE/PLATFORM/GODOT NATIVE-BOUNDARY LOCALIZATION" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.8 device boundary")
