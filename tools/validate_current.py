...
print("StS2 Launcher — Step 35.0.32 / Step 36.0.3 ModelDb Bootstrap Compatibility + Full ExecuteEssential validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>157</ApplicationVersion>" in project_text, "build version is 157")
require("<ApplicationDisplayVersion>0.0.157</ApplicationDisplayVersion>" in project_text, "display version is 0.0.157")
require(plist.get("CFBundleVersion") == "157", "Info.plist build version is 157")
require(plist.get("CFBundleShortVersionString") == "0.0.157", "Info.plist display version is 0.0.157")
...
require("STEP 35.0.32 / STEP 36.0.3 — MODELDB BOOTSTRAP COMPATIBILITY + FULL EXECUTEESSENTIAL" in release_presentation, "top launcher banner identifies active Step 35.0.32 / Step 36.0.3 ModelDb-bootstrap candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "STEP 35 EXACT CORE CLOSURE POSITIVE", "0.0.146", "37-pointer", "ManagedCallbacks", "GD_OnCoreApiAssemblyLoaded", "0.0.149", "RanToCompletion", "214/214", "0.0.152", "0.0.153", "D_WORKER_RETURN", "ConfigureAwait(false)", "InvokeOnMainThread", "0.0.154", "res://localization/eng", "0.0.155", "TargetInvocationException", "0.0.156", "BowlbugsNormal", "BowlbugEgg", "KeyNotFoundException", "0.0.157", "dependency-aware ModelDb bootstrap", "ExecuteEssential"]), "top launcher banner preserves bridge/UI/resource provenance, records the physical 0.0.156 ModelDb bootstrap failure, and identifies the 0.0.157 compatibility attempt")
...
require('ExpectedDisplayVersion = "0.0.157"' in release_presentation and 'ExpectedBuildVersion = "157"' in release_presentation, "Step 35.0.32 / Step 36.0.3 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.157",
    "STS2_BUILD_VERSION": "157",
...
require("Step 36.0.3 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-36.0.2 candidate rather than retired Step 27")
...
require(all(marker in step36_source for marker in ["RequireExactStep35CoreClosure", "IsExactAuthorityMode", "IsModelBootstrapCompatibilityMode", "RequireStep36BaselineUnchanged", "_managedPluginReverseBridgePrepared"]) and "EssentialCompatibilityAuthorityPassed" in step35_source, "Step 36.0.3 requires same-process verified Step-35 compatibility authority plus the proven bridge before any new boundary")
...
require(all(marker in step36_source for marker in ["FormatExceptionDiagnostic", "Exception depth", "GetBaseException()", "ReflectionTypeLoadException", "LoaderExceptions", "E_C_EXCEPTION_CAPTURED", "E_C_EXCEPTION_DEPTH", "E_C_BASE_EXCEPTION", "E_C_POST_FAILURE_CONTEXT", "stateAfterFailure", "resolverCountBefore", "hostLoadCountBefore", "privateLoadCountBefore", "sts2ContextSame", "godotSharpContextSame"]), "Step 36.0.3 captures complete nested reflection/base/loader exceptions plus post-failure state, resolver/load deltas, and authority-context continuity")
require("binding.Method.Invoke(null, null)" in step36_source and step36_source.count("binding.Method.Invoke(null, null)") == 1 and "state reset" not in step36_source.lower(), "Step 36.0.3 preserves exactly one launcher ExecuteEssential invocation and adds no state-reset retry path")
...
require("Step 35.0.32 / Step 36.0.3 ModelDb Bootstrap Compatibility + Full ExecuteEssential build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies the combined Step 35.0.32 / Step 36.0.3 candidate")
...
require("Expected device UI: STEP 35.0.32 / STEP 36.0.3 — MODELDB BOOTSTRAP COMPATIBILITY + FULL EXECUTEESSENTIAL" in verify_ipa and "step36-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active combined Step-36.0.3 candidate")
