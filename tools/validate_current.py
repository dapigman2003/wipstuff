...
print("StS2 Launcher — Step 35.0.23 Managed-Plugin Bridge Bootstrap validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>146</ApplicationVersion>" in project_text, "build version is 146")
require("<ApplicationDisplayVersion>0.0.146</ApplicationDisplayVersion>" in project_text, "display version is 0.0.146")
require(plist.get("CFBundleVersion") == "146", "Info.plist build version is 146")
require(plist.get("CFBundleShortVersionString") == "0.0.146", "Info.plist display version is 0.0.146")
...
require("STEP 35.0.23 — GODOT MANAGED-PLUGIN BRIDGE BOOTSTRAP" in release_presentation, "top launcher banner identifies active Step 35.0.23 managed-plugin bridge bootstrap candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.140", "0.0.141", "210/211", "GS031", "GS024", "GodotFileIo.CreateDirectory", "NATURAL", "OS-RECON", "FORWARD", "CORE-HANDOFF", "NativeFuncs.Initialize(IntPtr,int)", "0.0.143", "1,800-byte", "225-pointer", "initialized=true", "GetCmdlineArgs", "OS.get_Singleton", "EngineGetSingleton", "0.0.144", "GS035", "0.0.145", "godotApiCacheUpdated=false", "0.0.146", "37-pointer", "ManagedCallbacks", "GD_OnCoreApiAssemblyLoaded"]), "top launcher banner preserves prior CI/physical provenance, records the positive 0.0.145 reverse-binding verdict, and identifies the 0.0.146 managed-plugin bootstrap")
...
require('ExpectedDisplayVersion = "0.0.146"' in release_presentation and 'ExpectedBuildVersion = "146"' in release_presentation, "Step 35.0.23 source pins expected bundle release identity")
...
require(not unexpected_platform_changed, "platform/native behavior remains protected except the explicit Step-35.0.23 callback-handoff bridge delta", ", ".join(unexpected_platform_changed))
require(step35_callback_platform_delta.issubset(set(platform_changed)), "Step-35.0.23 callback-handoff bridge delta is explicit rather than silently matching the old no-callback baseline")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.146",
    "STS2_BUILD_VERSION": "146",
...
require("Step 35.0.23 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.23 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.23 — reverse-binding readiness preflight with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.23 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.23 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.23 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
require("STEP 35.0.23 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.23 host gate-summary assertion matches the active production summary identity")
require("STEP 35.0.15 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" not in step35_tests, "Step 35.0.23 static validation rejects the stale 0.0.139 Step-35.0.15 gate-summary assertion")
...
require(sweep_definition >= 0 and sweep_skip > sweep_definition and sweep_increment > sweep_skip, "Step 35.0.23 excludes injected diagnostic bridge calls before exact-source callsite ordinal accounting")
require("InsertCommandLineHelperCriticalBoundaryMarkers(commandLineCctor, emitReference)" in step35_source and "could not locate Godot.OS.GetCmdlineArgs" in step35_source, "Step 35.0.23 fails Gate A closed unless stack-neutral critical localization contains Godot.OS.GetCmdlineArgs")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.23 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.23 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.23 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.23 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.23 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.23 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require("string expectedBridgeTypeFullName = DiagnosticBridgeTypeFullName" in step35_source and "HasInjectedEntryMarkerAtStart(method, item.Marker, GodotSharpDiagnosticBridgeTypeFullName)" in godot_clone_block and "call.DeclaringType.FullName == expectedBridgeTypeFullName" in step35_source, "Step 35.0.23 verifies serialized entry markers against the derivative-specific bridge type and protects the 0.0.137 208/209 regression")
require(all(marker in step35_mode for marker in ["NaturalGodotDictionaryRecon = 0", "ManagedDictionaryCompatibility = 1", "ManagedCommandLineCompatibility = 2", "GodotCoreCallbackHandoff = 3"]), "Step 35.0.23 exposes the NATURAL, OS-RECON, FORWARD, and Step15-live CORE-HANDOFF mode identities")
require(all(marker in step35_source for marker in ["RunGodotCoreCallbackHandoffInitialization", "CB_INIT_ENTRY", "CB_GODOTSHARP_LOAD_START", "CB_NATIVEFUNCS_BIND_PASS", "CB_INITIALIZE_INVOKE_START", "CB_INITIALIZE_INVOKE_RETURNED", "CB_INITIALIZE_PASS", "Godot.NativeInterop.NativeFuncs", "Initialize(IntPtr,int)", "initializedField", "CallbackHandoffSnapshot", "GodotCoreCallbackHandoff"]), "Step 35.0.23 CORE-HANDOFF loads only the verified private GodotSharp derivative, binds exact NativeFuncs.Initialize(IntPtr,int), requires initialized=false, invokes once, and preserves a resolver baseline before Gate C")
require(all(marker in step35_tests for marker in ["GodotCoreCallbackHandoffRejectsMissingTableBeforeAnyPreflightOrClrWork", "IntPtr.Zero", "Assert.AreEqual(1, checkpoints.Count", "stage=initialization", "System.ArgumentException", "CB_INIT_ENTRY"]), "Step 35.0.23 host regression rejects invalid native callback metadata before preflight/CLR work while preserving exactly one durable managed-failure checkpoint and proving CB_INIT_ENTRY was not reached")
require(all(marker in step35_tests for marker in ["GodotManagedPluginReverseBridgeRejectsInvalidNativeSizeBeforePreflightOrClrWork", "PrepareGodotManagedPluginReverseBridge(0", "CB_REVERSE_PREP_MANAGED_FAIL", "CB_REVERSE_PREP_ENTRY"]), "Step 35.0.23 host regression rejects invalid native ManagedCallbacks size before preflight/CLR work")
require(all(marker in step15_objc_bridge for marker in ["modules/mono/glue/runtime_interop.h", "CSharpLanguage::get_singleton()", "GDMono::get_singleton()", "godotsharp::get_runtime_interop_funcs", "sts2_step15_is_runtime_interop_ready", "sts2_step15_has_dotnet_feature", "sts2_step15_is_dotnet_runtime_initialized", "sts2_step15_get_runtime_interop_funcs", 'has_feature("dotnet")']), "Step 35.0.23 native Step-15 bridge exposes only the source-built Godot runtime interop table after C# native scaffolding readiness and reports competing dotnet state")
require(all(marker in step15_native_bridge for marker in ["IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "sts2_step15_get_runtime_interop_funcs"]), "Step 35.0.23 managed native bridge exposes callback-table readiness/state/pointer without loading a game native image")
require(all(marker in step15_objc_bridge for marker in ["modules/mono/mono_gd/gd_mono_cache.h", "sts2_step15_has_csharp_language_singleton", "sts2_step15_is_godot_api_cache_updated", "sts2_step15_has_managed_create_binding_callback", "sts2_step15_is_reverse_binding_ready", "sts2_step15_get_managed_callbacks_size", "sts2_step15_install_external_managed_callbacks", "sts2_step15_signal_external_core_api_loaded", "GDMonoCache::update_godot_api_cache", "ScriptManagerBridge_CreateManagedForGodotObjectBinding", "GD_OnCoreApiAssemblyLoaded"]), "Step 35.0.23 native bridge preserves readiness telemetry and adds complete-struct cache adoption plus the standard isolated core-API reverse callback")
require("GDMono::initialize()" not in step15_objc_bridge and "runtime_initialized =" not in step15_objc_bridge, "Step 35.0.23 external bridge does not start or fake Godot runtime ownership")
require(all(marker in step15_native_bridge for marker in ["HasCSharpLanguageSingleton", "IsGodotApiCacheUpdated", "HasManagedCreateBindingCallback", "IsReverseBindingReady", "ManagedCallbacksSizeBytes", "IsExternalManagedBridgeInstalled", "InstallExternalManagedCallbacks", "SignalExternalCoreApiLoaded", "DidExternalCoreApiSignalReturn"]), "Step 35.0.23 managed native bridge exposes complete reverse-bootstrap state and operations")
...
require(all(marker in project_text for marker in new_native_exports), "iOS ReferenceNativeSymbol roots preserve every Step-35.0.23 managed-plugin bootstrap export")
...
require(all(marker in step35_ui for marker in ["CORE-HANDOFF", "GodotCoreCallbackHandoff", "Run Step 15 Gates A-C first", "IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "CB_NATIVE_TABLE_REQUEST_RETURNED", "RunGodotCoreCallbackHandoffInitialization"]), "Step 35.0.23 iOS surface gates CORE-HANDOFF on an already-live Step-15 engine, rejects competing dotnet state, obtains the exact native table, and records the managed handoff")
require("using StS2Launcher.iOS.Platform;" in step35_ui and "GodotStep15NativeBridge" in step35_ui, "Step 35.0.23 Step-35 iOS partial explicitly imports the Platform namespace required to resolve GodotStep15NativeBridge and protects the 0.0.142 CS0103 compile regression")
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.23 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.23 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.23 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.23 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.23 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.23 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.23 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.23 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.23 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.23 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.23 active source/UI reject stale exact-closure wording")
...
require("Step 35.0.23 Managed-Plugin Bridge Bootstrap build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.23 managed-plugin bridge bootstrap candidate")
...
require("Expected device UI: STEP 35.0.23 — GODOT MANAGED-PLUGIN BRIDGE BOOTSTRAP" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.23 managed-plugin bridge bootstrap candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.23 candidate manifest includes implementation/tests, native/build bridge, prior CI/physical provenance, the physical 0.0.145 reverse-binding verdict, and the active 0.0.146 managed-plugin bootstrap design",
    )
    mismatches=[]
