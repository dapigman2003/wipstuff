...
require("<ApplicationVersion>200</ApplicationVersion>" in project_text, "build version is 200")
require("<ApplicationDisplayVersion>0.0.200</ApplicationDisplayVersion>" in project_text, "display version is 0.0.200")
require(plist.get("CFBundleVersion") == "200", "Info.plist build version is 200")
require(plist.get("CFBundleShortVersionString") == "0.0.200", "Info.plist display version is 0.0.200")
...
require("STEPS 35–57 PHYSICALLY CLOSED" in release_presentation and "0.0.200 STEPS 58–62 ACTIVE" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records physical closure through Step 57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.200 (200)**" in read("README.md") and "Active candidate: **0.0.200 (200)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.200 (200) release identity")
require("Active candidate — 0.0.200 host-regression-corrected PackedScene instanced-root unique-name compatibility" in read("docs/MASTER-PLAN.md") and "PackedScene.Instantiate(GenEditState)" in read("docs/MASTER-PLAN.md") and "private Godot managed compatibility boundary" in read("docs/MASTER-PLAN.md"), "master plan points to the active 0.0.200 host-regression-corrected PackedScene compatibility frontier")
...
require('ExpectedDisplayVersion = "0.0.200"' in release_presentation and 'ExpectedBuildVersion = "200"' in release_presentation, "startup-ladder source pins expected 0.0.200 bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.200",
    "STS2_BUILD_VERSION": "200",
...
require(all(marker in step35_tests for marker in ["var packedScene = new TypeDefinition(\"Godot\", \"PackedScene\"", "var instantiate = new MethodDefinition(\"Instantiate\"", "GodotSharpPackedSceneCompatibilityCallbackFieldName", "System.Action`2<System.Object,System.Object>"]), "0.0.200 host fixture models PackedScene.Instantiate and asserts the serialized Step-59.5 compatibility hook")
require(all(marker in step35_ui_main for marker in ["verified GodotSharp PackedScene compatibility derivative", "verified GodotSharp compatibility derivative is the selected bridge dependency", "exact prepared GodotSharp remains immutable source authority"]), "0.0.200 live MODEL-BOOTSTRAP evidence names the compatibility derivative as selected bridge authority without rewriting historical exact-authority claims")
require("MODELDB-BOOTSTRAP COMPATIBILITY + VERIFIED PACKEDSCENE-COMPAT GODOTSHARP" in step35_telemetry_ui and "MODELDB-BOOTSTRAP COMPATIBILITY + EXACT GODOTSHARP" not in step35_telemetry_ui, "0.0.200 Step-35 run telemetry identifies the selected PackedScene-compatible GodotSharp derivative instead of the exact source bytes")
require("CLR-LOADED — VERIFIED GODOTSHARP PACKEDSCENE COMPATIBILITY DERIVATIVE" in step35_source and "NOT CLR-LOADED — EXACT PREPARED GODOTSHARP USED" not in step35_source, "0.0.200 Gate-D evidence records the GodotSharp compatibility derivative as the MODEL-BOOTSTRAP CLR authority")
...
require("selected same-process GodotSharp bridge authority" in step36_ui and "through exact GodotSharp" not in step36_ui, "0.0.200 Step-36 live status follows the selected MODEL-BOOTSTRAP GodotSharp bridge authority")
...
require("selected same-process GodotSharp bridge authority" in step37_ui and "through exact GodotSharp" not in step37_ui and "exact GodotSharp loaded it as PackedScene" not in step37_ui, "0.0.200 Step-37 live evidence follows the selected same-process GodotSharp bridge authority")
...
require(all(marker in current_status for marker in ["Active candidate — Steps 58–62 global PackedScene unique-name compatibility / visible-render trial / 0.0.200 (200)", "capped at Step 52", "Step 57 4/4/frozen", "What physical 0.0.198 changed", "SceneState", "temporary **off-tree**", "PackedScene.Instantiate", "unique_name_in_owner", "GodotSharp compatibility derivative", "Step 59.0 — forensic prerequisites + one-shot real OpenCharacterSelect transition", "Step 60.0 — actual active character-select surface audit", "Step 61.0 — short visible character-select render residency", "Step 62.0 — sustained visible character-select render residency", "Step 63", "skip Step 38", "Step 15 Gate D"]), "current status records stable Step-57 baseline, physical 0.0.198 instantiation localization, and active 0.0.200 host-regression-corrected compatibility design")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.200" in testing_doc and "0.0.200" in release_checklist_doc and "0.0.200 PackedScene compatibility expectations" in testing_doc and "0.0.200 central PackedScene compatibility requirements" in release_checklist_doc, "testing and release-checklist docs pin the 0.0.200 host-regression-corrected PackedScene compatibility contract")
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.200\"", "ExpectedBuildVersion = \"200\"", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL"]), "active release presentation pins physical closure and Steps-58–62 / 0.0.200 identity")
...
# Dedicated 0.0.200 Steps 58-62 host-regression-corrected PackedScene compatibility / visible-render candidate manifest.
