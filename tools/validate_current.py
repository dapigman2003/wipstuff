...
print("StS2 Launcher — Step 35.0.27 Exact Authority Closure + Gate D Finalization validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>150</ApplicationVersion>" in project_text, "build version is 150")
require("<ApplicationDisplayVersion>0.0.150</ApplicationDisplayVersion>" in project_text, "display version is 0.0.150")
require(plist.get("CFBundleVersion") == "150", "Info.plist build version is 150")
require(plist.get("CFBundleShortVersionString") == "0.0.150", "Info.plist display version is 0.0.150")
...
require('ExpectedDisplayVersion = "0.0.150"' in release_presentation and 'ExpectedBuildVersion = "150"' in release_presentation, "Step 35.0.27 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.150",
    "STS2_BUILD_VERSION": "150",
...
require("Step 35.0.27 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.27 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.27 — exact-authority closure candidate + Gate-D finalization with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.27 retains exactly four ordered A-D gates shared by diagnostic and exact-authority modes")
require("STEP 35.0.27 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.27 diagnostic summary reports four-of-four completion without claiming exact Step-35 closure")
require("STEP 35.0.27 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.27 host gate-summary assertion matches the active production diagnostic summary identity")
...
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.27 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.27 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.27 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require(all(marker in step35_source for marker in ["PrepareGodotManagedPluginReverseBridge", "GodotPlugins.Game.Main", "InitializeFromGameProject", "godotsharp_game_main_init", "Godot.Bridge.ManagedCallbacks", "callbackFields.Length != 37", "ManagedCallbacks.Create", "ScriptManagerBridge", "LookupScriptsInAssembly", "CB_REVERSE_PREP_PASS"]), "Step 35.0.27 core reproduces and verifies the generated game-plugin managed bootstrap contract instead of invoking the UnmanagedCallersOnly entry point directly")
...
require(all(marker in step35_ui for marker in ["CORE-HANDOFF", "GodotCoreCallbackHandoff", "Run Step 15 Gates A-C first", "IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "CB_NATIVE_TABLE_REQUEST_RETURNED", "RunGodotCoreCallbackHandoffInitialization"]), "Step 35.0.27 iOS surface gates CORE-HANDOFF on an already-live Step-15 engine, rejects competing dotnet state, obtains the exact native table, and records the managed handoff")
...
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.27 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.27 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.27 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.27 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.27 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.27 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require(all(marker in step35_ui for marker in ["DIAGNOSTIC COMPLETE: STEP 35.0.27 — 4/4", "EXACT STEP 35 CLOSURE CANDIDATE: COMPLETE — 4/4", "RUN_EXACT_STEP35_4OF4", "GodotCoreExactClosure"]), "Step 35.0.27 iOS surface separates diagnostic 4/4 from the explicit exact-authority closure candidate")
require(all(marker in step35_source for marker in ["Gate D OfflineReady", "CallbackProgress<SteamOfflineInstallProgress>", "value.CompletedBytes", "value.TotalBytes"]), "Step 35.0.27 Gate D forwards protected OfflineReady file/byte checkpoints into the Step-35 progress contract")
step35_gate_d_ui = read("src/StS2Launcher.iOS/UI/RootViewController.Step35GateDProgress.cs")
require(all(marker in step35_gate_d_ui for marker in ["UIProgressView", "Gate D receipt hash", "ProcessedBytes", "TotalBytes", "Latest verifier file:", "GiB", "/s", "NSTimer.CreateRepeatingScheduledTimer", "last verifier progress"]), "Step 35.0.27 iOS surface exposes dedicated Gate-D file/byte progress plus a live heartbeat during long single-file hashes")
require("SteamOfflineInstallInspection.cs" not in ["changed-by-step35-gated-progress"], "Step 35.0.27 leaves the physically protected Step-13 OfflineReady verifier implementation unchanged")
require("STEP-35 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source and "EXACT CLOSED STEP-32 TRANSFORMED EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY" in step35_source, "Step 35.0.27 Gate-C PASS wording distinguishes diagnostic from exact authority")
require(all(marker in step35_mode for marker in ["GodotCoreExactClosure = 4", "exact-authority closure candidate"]) and all(marker in step35_source for marker in ["IsExactAuthorityMode", "preflight.TransformedPath", "exact closed Step-32 transformed sts2 image", "C_EXACT_AUTHORITY_PASS", "D_EXACT_PRIMARY_REPROOF_PASS"]) and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.27 exact-authority mode uses the existing four-gate contract and exact transformed CLR authority without inventing a fifth gate")
require("GodotCoreExactClosureRejectsMissingTableBeforeAnyPreflightOrClrWork" in step35_tests and "Assert.AreEqual(4, (int)Step35DiagnosticMode.GodotCoreExactClosure)" in step35_tests, "Step 35.0.27 host regressions pin exact-closure mode value and fail-closed callback metadata")
require(all(marker in step35_source for marker in ["D_AUDIT_ENTRY", "D_OFFLINE_READY_RETURNED", "D_FINAL_CHECKS_PASS", "D_RESULT_CONSTRUCT_START", "D_RESULT_CONSTRUCT_RETURNED", "D_FINAL_PROGRESS_EMITTED", "D_TASK_RETURN_START"]), "Step 35.0.27 core durably localizes Gate-D finalization around result construction/progress/return")
require(all(marker in step35_ui for marker in ["D_TASK_AWAIT_RESUMED", "D_RESULT_RECORD_PASS", "CompleteStep35GateDProgress"]), "Step 35.0.27 UI durably records Gate-D Task resumption and result recording")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.27 / 0.0.150 (150)", "Physical 0.0.149", "C_INVOKE_RETURNED", "RanToCompletion", "C_RESULT_RECORD_PASS", "D_START", "GodotCoreExactClosure", "D_RESULT_CONSTRUCT_RETURNED", "0x06007D02", "0x0600BC71"]), "current status records physical 0.0.149 Gate-C completion/Gate-D boundary and active 0.0.150 exact-authority/finalization candidate")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.150 (150)" in testing_doc and "0.0.150 (150)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.150 (150) release identity")
...
require("Step 35.0.27 Exact Authority Closure + Gate D Finalization build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.27 exact-authority/Gate-D-finalization candidate")
...
require("Expected device UI: STEP 35.0.27 — EXACT AUTHORITY CLOSURE + GATE D FINALIZATION" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.27 exact-authority/Gate-D-finalization candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.27 candidate manifest includes implementation/tests, native/build bridge, prior provenance, physical 0.0.149 Gate-C/Gate-D-boundary evidence, and active exact-authority/finalization design",
    )
    mismatches=[]
