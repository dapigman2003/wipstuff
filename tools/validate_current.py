...
require("<ApplicationVersion>208</ApplicationVersion>" in project_text, "build version is 208")
require("<ApplicationDisplayVersion>0.0.208</ApplicationDisplayVersion>" in project_text, "display version is 0.0.208")
require(plist.get("CFBundleVersion") == "208", "Info.plist build version is 208")
require(plist.get("CFBundleShortVersionString") == "0.0.208", "Info.plist display version is 0.0.208")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.208 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.208 (208)**" in read("README.md") and "Active candidate: **0.0.208 (208)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.208 (208) release identity")
require(all(v in read("docs/MASTER-PLAN.md") for v in ["0.0.208", "BASELINE", "WRAPPER", "FULL", "59T", "59W", "59X", "Step 63"]), "master plan points to the active 0.0.208 profile-isolated compilation-efficient PropertyTweener frontier")
...
require('ExpectedDisplayVersion = "0.0.208"' in release_presentation and 'ExpectedBuildVersion = "208"' in release_presentation, "startup-ladder source pins expected 0.0.208 bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.208",
    "STS2_BUILD_VERSION": "208",
...
require(used_assert_methods <= known_mstest_v4_assert_methods, "0.0.208 test sources use only the repository's known compiling MSTest-v4 Assert API surface", "unexpected=" + ",".join(sorted(used_assert_methods - known_mstest_v4_assert_methods)))
require("Assert.Greater(" not in all_test_text and "Assert.IsTrue(fluentRepairCalls > 0, fluentName);" in step35_tests, "0.0.208 replaces the 0.0.206 unsupported Assert.Greater host-test compile stop with an established Assert.IsTrue form")
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.208\"", "ExpectedBuildVersion = \"208\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL", "BASELINE", "WRAPPER", "FULL"]), "active release presentation pins Step-52 current architecture and the 0.0.208 profile-isolated Steps-58–62 identity")
...
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.208 UI unlocks active Step 58 directly from same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.208 active ownership path contains no obsolete Step-57 prerequisite gate")
