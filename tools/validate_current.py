...
print("StS2 Launcher — Step 35.0.14 Managed Command-Line Dictionary compatibility validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>137</ApplicationVersion>" in project_text, "build version is 137")
require("<ApplicationDisplayVersion>0.0.137</ApplicationDisplayVersion>" in project_text, "display version is 0.0.137")
require(plist.get("CFBundleVersion") == "137", "Info.plist build version is 137")
require(plist.get("CFBundleShortVersionString") == "0.0.137", "Info.plist display version is 0.0.137")
...
require("STEP 35.0.14 — MANAGED COMMAND-LINE DICTIONARY COMPATIBILITY PROBE" in release_presentation, "top launcher banner identifies active Step 35.0.14 managed Command-Line dictionary compatibility candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.129", "Action<string>::Invoke(!0)", "0.0.132", "0.0.133", "InvalidProgramException", "0.0.135", "0.0.136", "CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "System.Collections.Generic.Dictionary<string,string>", "Godot.OS.GetCmdlineArgs", "0.0.137", "diagnostic"]), "top launcher banner preserves the physical Step-35 history and identifies active 0.0.137 managed-dictionary diagnostic compatibility derivative")
...
require('ExpectedDisplayVersion = "0.0.137"' in release_presentation and 'ExpectedBuildVersion = "137"' in release_presentation, "Step 35.0.14 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.137",
    "STS2_BUILD_VERSION": "137",
...
require("Step 35.0.14 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.14 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.14 — managed Command-Line dictionary compatibility probe with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.14 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.14 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.14 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
...
require(all(marker in step35_source for marker in ["BuildStaticInstructionMap", "CALLSITE#", "AWAIT-CANDIDATE", "DescribeMetadataScope", "GetVerifiedVeryEarlyStaticInstructionMap", "[NULL PLATFORM CTOR IL]", "[COMMAND LINE HELPER CCTOR IL]", "[COMMAND LINE HELPER TRYGETVALUE IL]", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName"]), "Step 35.0.14 static map covers the exact NullPlatform constructor plus CommandLineHelper cctor/TryGetValue callsite ordinals without resolving dependencies")
require(all(marker in step35_source for marker in ["CreateInstrumentedDiagnosticClone", "GetDiagnosticMarkerTargets", "HasInjectedEntryMarkerAtStart", "InsertNullPlatformConstructorCallsiteMarkers", "InsertCommandLineHelperCriticalBoundaryMarkers", "HasCommandLineHelperCriticalBoundaryMarkers", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "CommandLineManagedDictionarySubstitutionCount", "CommandLineCctorDictionaryBeforeMarker", "CommandLineCctorDictionaryAfterMarker", "CommandLineCctorGetCmdlineArgsBeforeMarker", "CommandLineCctorGetCmdlineArgsAfterMarker", "CommandLineHelper cctor MaxStack exact-source/diagnostic", "CommandLineHelper managed dictionary compatibility substitutions", "NullPlatformTypeFullName", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName", "INMETHOD_NP", "INMETHOD_CL_CRITICAL", "INMETHOD_027", "DiagnosticBridgeTypeFullName", "INMETHOD_001", "INMETHOD_021", "INMETHOD_022", "INMETHOD_023", "INMETHOD_024", "INMETHOD_025", "INMETHOD_026", "INMETHOD_031", "INMETHOD_050", "INMETHOD_100", "INMETHOD_180", "INMETHOD_181", "INMETHOD_182", "INMETHOD_183", "C_DIAGNOSTIC_BRIDGE_ARMED", "preflight.DiagnosticPath", "preflight.DiagnosticSha256"]), "Step 35.0.14 preserves prior diagnostics and adds the narrowly verified managed CommandLine dictionary compatibility rewrite")
...
require(sweep_definition >= 0 and sweep_skip > sweep_definition and sweep_increment > sweep_skip, "Step 35.0.14 excludes injected diagnostic bridge calls before exact-source callsite ordinal accounting")
require("InsertCommandLineHelperCriticalBoundaryMarkers(commandLineCctor, emitReference)" in step35_source and "could not locate Godot.OS.GetCmdlineArgs" in step35_source, "Step 35.0.14 fails Gate A closed unless stack-neutral critical localization contains Godot.OS.GetCmdlineArgs")
...
require(all(marker in step35_source for marker in ["B_LOADFROMSTREAM_START", "B_LOADFROMSTREAM_PASS", "C_BIND_TYPE_START", "C_BIND_METHOD_START", "C_INVOKE_START", "C_INVOKE_RETURNED", "C_TASK_CONFIRMED", "C_WAIT_START", "C_WAIT_COMPLETED", "RESOLVE_MANAGED_START", "RESOLVE_PRIVATE_PASS", "RESOLVE_HOST_PASS", "RESOLVE_NATIVE_REJECT"]), "Step 35.0.14 core retains durable B→C/invocation/resolver crash-localization markers")
require(all(marker in step35_ui for marker in ["Step35-CurrentRun.txt", "Step35-LastCheckpoint.txt", "Step35-CrashCheckpoint-", "Step35-ExecuteVeryEarly-StaticMap-", "TryInitializeStep35RunTelemetry", "RUN_TELEMETRY_READY", "WriteStep35StaticMap", "Flush(flushToDisk: true)", "Environment.CurrentManagedThreadId", "CANCELLED / INCONCLUSIVE", "Output-only diagnostic", "NP003_PRE", "NP002_PRE", "InvalidProgramException", "RUN_END", "CALLSITE#002", "0.0.136", "CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "System.Collections.Generic.Dictionary<string,string>", "INMETHOD_CL_CRITICAL_001_PRE", "INMETHOD_CL_CRITICAL_002_PRE"]), "Step 35.0.14 iOS surface records the 0.0.132/0.0.133 history, physical 0.0.136 dictionary-construction frontier, and active managed dictionary compatibility derivative")
require("crashCheckpoints.Any" in step35_tests and "B_LOADFROMSTREAM_START" in step35_tests and "RESOLVE_INITIALIZER_BEARING_REJECT" in step35_tests and "StaticInstructionMapCapturesCallsitesAndAwaitCandidatesWithoutResolution" in step35_tests, "Step 35.0.14 host regressions retain crash-checkpoint and static-map coverage")
require("OrderedDiagnosticLocalizationGatesReachFourOfFourWithoutClaimingClosure" in step35_tests and "VeryEarlyInitializationStopsAfterFirstFailure" in step35_tests and "VeryEarlyContextLoadsInitializerFreePrivateDependencyAndRejectsInitializerBearingDependency" in step35_tests and "Step35PinsTheExactVeryEarlyManagedInitializationTarget" in step35_tests, "Step 35.0.14 host regressions protect diagnostic gate order, exact source target and initializer-bearing refusal")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.14 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.14 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.14 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.14 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.14 active source/UI reject stale exact-closure wording")
...
require("Step35ImplementationMarker" in release_presentation and "0x06007D02" in release_presentation and "0x0600BC71" in release_presentation and "ExecuteVeryEarly" in release_presentation and "0.0.137" in release_presentation, "release presentation pins the Step-35.0.14 exact-source/instrumented-clone boundary")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.14 / 0.0.137 (137)", "0.0.126", "0.0.129", "0.0.130", "0.0.131", "0.0.132", "0.0.133", "0.0.135", "0.0.136", "Action<string>::Invoke(!0)", "NullPlatformUtilStrategy..ctor", "INMETHOD_NP003_PRE", "INMETHOD_CL_CRITICAL_001_PRE", "Godot.Collections.Dictionary<string,string>", "System.Collections.Generic.Dictionary<string,string>", "Godot.OS.GetCmdlineArgs", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact authority, prior instrumentation defects, physical 0.0.136 dictionary frontier, and active 0.0.137 managed dictionary compatibility probe")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.137 (137)" in testing_doc and "0.0.137 (137)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.137 (137) release identity")
require("0.0.137 (132)" not in testing_doc and "0.0.137 (131)" not in testing_doc and "0.0.137 (132)" not in release_checklist_doc and "0.0.137 (131)" not in release_checklist_doc, "release docs reject display/build-number drift like the prior 0.0.130 (129) documentation bug")
...
require((ROOT / "docs/history/steps/STEP-35.0.14-MANAGED-COMMANDLINE-DICTIONARY-COMPATIBILITY.md").is_file(), "Step 35.0.14 managed CommandLine dictionary compatibility design record is present")
...
require("Step 35.0.14 Managed Command-Line Dictionary Compatibility Probe build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.14 managed dictionary candidate")
...
require("Expected device UI: STEP 35.0.14 — MANAGED COMMAND-LINE DICTIONARY COMPATIBILITY PROBE" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.14 managed dictionary compatibility probe")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.14 candidate manifest includes active implementation/tests, physical 0.0.136 evidence, and the 0.0.137 design record",
    )
    mismatches=[]
