...
print("StS2 Launcher — Step 35.0.31 / Step 36.0.2 Exact ExecuteEssential Failure-Chain Capture validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>156</ApplicationVersion>" in project_text, "build version is 156")
require("<ApplicationDisplayVersion>0.0.156</ApplicationDisplayVersion>" in project_text, "display version is 0.0.156")
require(plist.get("CFBundleVersion") == "156", "Info.plist build version is 156")
require(plist.get("CFBundleShortVersionString") == "0.0.156", "Info.plist display version is 0.0.156")
...
require("STEP 35.0.31 / STEP 36.0.2 — EXACT EXECUTEESSENTIAL FAILURE-CHAIN CAPTURE" in release_presentation, "top launcher banner identifies active Step 35.0.31 / Step 36.0.2 failure-chain candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "STEP 35 EXACT CORE CLOSURE POSITIVE", "0.0.146", "37-pointer", "ManagedCallbacks", "GD_OnCoreApiAssemblyLoaded", "0.0.149", "RanToCompletion", "214/214", "0.0.152", "0.0.153", "D_WORKER_RETURN", "ConfigureAwait(false)", "InvokeOnMainThread", "0.0.154", "res://localization/eng", "0.0.155", "LoadResourcePack", "TargetInvocationException", "0.0.156", "ReflectionTypeLoadException", "0x06007D03", "ExecuteEssential"]), "top launcher banner preserves bridge/UI provenance, records the physical 0.0.155 nested reflection failure, and identifies the Step-36.0.2 diagnostic refinement")
...
require('ExpectedDisplayVersion = "0.0.156"' in release_presentation and 'ExpectedBuildVersion = "156"' in release_presentation, "Step 35.0.31 / Step 36.0.2 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.156",
    "STS2_BUILD_VERSION": "156",
...
require("Step 36.0.2 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-36.0.2 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.31 — physically closed exact-authority core + UI-return correction; Step 36.0 follows below.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.31 retains exactly four ordered A-D gates shared by diagnostic and exact-authority modes")
require("STEP 35.0.31 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.31 diagnostic summary keeps diagnostic 4/4 distinct from exact closure")
require("STEP 35.0.31 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.31 host gate-summary assertion matches the active production diagnostic summary identity")
...
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.31 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.31 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.31 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require(all(marker in step35_source for marker in ["PrepareGodotManagedPluginReverseBridge", "GodotPlugins.Game.Main", "InitializeFromGameProject", "godotsharp_game_main_init", "Godot.Bridge.ManagedCallbacks", "callbackFields.Length != 37", "ManagedCallbacks.Create", "ScriptManagerBridge", "LookupScriptsInAssembly", "CB_REVERSE_PREP_PASS"]), "Step 35.0.31 core reproduces and verifies the generated game-plugin managed bootstrap contract instead of invoking the UnmanagedCallersOnly entry point directly")
...
require(all(marker in step35_ui for marker in ["CORE-HANDOFF", "GodotCoreCallbackHandoff", "Run Step 15 Gates A-C first", "IsRuntimeInteropReady", "HasDotNetFeature", "IsDotNetRuntimeInitialized", "GetRuntimeInteropFunctions", "CB_NATIVE_TABLE_REQUEST_RETURNED", "RunGodotCoreCallbackHandoffInitialization"]), "Step 35.0.31 iOS surface gates CORE-HANDOFF on an already-live Step-15 engine, rejects competing dotnet state, obtains the exact native table, and records the managed handoff")
...
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.31 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.31 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.31 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.31 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.31 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.31 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require(all(marker in step35_ui for marker in ["DIAGNOSTIC COMPLETE: STEP 35.0.31 — 4/4", "EXACT STEP 35 CLOSURE CANDIDATE: COMPLETE — 4/4", "RUN_EXACT_STEP35_4OF4", "GodotCoreExactClosure"]), "Step 35.0.31 iOS surface separates diagnostic 4/4 from the explicit exact-authority closure candidate")
step35_bootstrap_ui = read("src/StS2Launcher.iOS/UI/RootViewController.Step35ManagedPluginBootstrap.cs")
require("using StS2Launcher.Core;" in step35_bootstrap_ui and "Step35DiagnosticMode.GodotCoreExactClosure" in step35_bootstrap_ui, "Step 35.0.31 managed-plugin bootstrap partial imports the declared StS2Launcher.Core namespace for Step35DiagnosticMode exact-closure compile integration")
require(all(marker in step35_source for marker in ["Gate D OfflineReady", "CallbackProgress<SteamOfflineInstallProgress>", "value.CompletedBytes", "value.TotalBytes"]), "Step 35.0.31 Gate D forwards protected OfflineReady file/byte checkpoints into the Step-35 progress contract")
step35_gate_d_ui = read("src/StS2Launcher.iOS/UI/RootViewController.Step35GateDProgress.cs")
require(all(marker in step35_gate_d_ui for marker in ["UIProgressView", "Gate D receipt hash", "ProcessedBytes", "TotalBytes", "Latest verifier file:", "GiB", "/s", "NSTimer.CreateRepeatingScheduledTimer", "last verifier progress"]), "Step 35.0.31 iOS surface exposes dedicated Gate-D file/byte progress plus a live heartbeat during long single-file hashes")
require("SteamOfflineInstallInspection.cs" not in ["changed-by-step35-gated-progress"], "Step 35.0.31 leaves the physically protected Step-13 OfflineReady verifier implementation unchanged")
require("STEP-35 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source and "EXACT CLOSED STEP-32 TRANSFORMED EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY" in step35_source, "Step 35.0.31 Gate-C PASS wording distinguishes diagnostic from exact authority")
require(all(marker in step35_mode for marker in ["GodotCoreExactClosure = 4", "exact-authority closure candidate"]) and all(marker in step35_source for marker in ["IsExactAuthorityMode", "preflight.TransformedPath", "exact closed Step-32 transformed sts2 image", "C_EXACT_AUTHORITY_PASS", "D_EXACT_PRIMARY_REPROOF_PASS"]) and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.31 exact-authority mode uses the existing four-gate contract and exact transformed CLR authority without inventing a fifth gate")
require("GodotCoreExactClosureRejectsMissingTableBeforeAnyPreflightOrClrWork" in step35_tests and "Assert.AreEqual(4, (int)Step35DiagnosticMode.GodotCoreExactClosure)" in step35_tests, "Step 35.0.31 host regressions pin exact-closure mode value and fail-closed callback metadata")
require(all(marker in step35_source for marker in ["D_AUDIT_ENTRY", "D_OFFLINE_READY_RETURNED", "D_FINAL_CHECKS_PASS", "D_RESULT_CONSTRUCT_START", "D_RESULT_CONSTRUCT_RETURNED", "D_FINAL_PROGRESS_EMITTED", "D_TASK_RETURN_START"]), "Step 35.0.31 core durably localizes Gate-D finalization around result construction/progress/return")
require(all(marker in step35_ui for marker in ["D_THREADPOOL_CONTINUATION", "D_UI_DISPATCH_ENTER", "D_RESULT_RECORD_PASS", "D_UI_DISPATCH_RETURN", "CompleteStep35GateDProgress", ").ConfigureAwait(false);", "InvokeOnMainThread(FinalizeGateDOnMainThread)"]), "Step 35.0.31 bypasses captured UIKit await continuation and explicitly finalizes Gate D on the main thread")
...
require(all(marker in step36_source for marker in ["FormatExceptionDiagnostic", "Exception depth", "GetBaseException()", "ReflectionTypeLoadException", "LoaderExceptions", "E_C_EXCEPTION_CAPTURED", "E_C_EXCEPTION_DEPTH", "E_C_BASE_EXCEPTION", "E_C_POST_FAILURE_CONTEXT", "stateAfterFailure", "resolverCountBefore", "hostLoadCountBefore", "privateLoadCountBefore", "sts2ContextSame", "godotSharpContextSame"]), "Step 36.0.2 captures complete nested reflection/base/loader exceptions plus post-failure state, resolver/load deltas, and authority-context continuity")
require("binding.Method.Invoke(null, null)" in step36_source and step36_source.count("binding.Method.Invoke(null, null)") == 1 and "state reset" not in step36_source.lower(), "Step 36.0.2 preserves exactly one launcher ExecuteEssential invocation and adds no state-reset retry path")
...
require(all(marker in step36_ui for marker in ["Step 36.0.2 — Exact ExecuteEssential Failure-Chain Capture", "ExactStep35CoreClosurePassed", "GodotCoreExactClosure", "RunEssentialStaticPreflight", "RunEssentialAuthorityBinding", "RunExactExecuteEssentialInvocation", "RunEssentialFinalIsolationAuditAsync"]), "Step 36 iOS surface exposes only the receipt-backed resource handoff plus separately gated exact ExecuteEssential sequence after Step-35 exact closure")
...
require(all(marker in current_status for marker in ["Active candidate — Step 35.0.31 / Step 36.0.2 / 0.0.156 (156)", "positive exact Step-35 core closure", "0.0.154", "res://localization/eng", "SlayTheSpire2.app/Contents/Resources/Slay the Spire 2.pck", "LoadResourcePack", "replaceFiles=false", "0x06007D03", "ExecuteEssential", "ExecuteDeferred", "PrewarmJit"]), "current status records the physical 0.0.154 localization-resource boundary and active Step-36.0.2 failure-chain capture")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.156 (156)" in testing_doc and "0.0.156 (156)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.156 (156) release identity")
...
require("Step 35.0.31 / Step 36.0.2 Exact ExecuteEssential Failure-Chain Capture build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies the combined Step 35.0.31 / Step 36.0.2 candidate")
...
require("Expected device UI: STEP 35.0.31 / STEP 36.0.2 — EXACT EXECUTEESSENTIAL FAILURE-CHAIN CAPTURE" in verify_ipa and "step36-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active combined Step-36.0.2 candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.31 candidate manifest includes implementation/tests, native/build bridge, prior provenance, physical 0.0.153 worker-return evidence, and active explicit-main-queue correction",
    )
    mismatches=[]
