...
print("StS2 Launcher — Step 35.0.17 Comprehensive GodotSharp / Native Recon + Dual Mode validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>140</ApplicationVersion>" in project_text, "build version is 140")
require("<ApplicationDisplayVersion>0.0.140</ApplicationDisplayVersion>" in project_text, "display version is 0.0.140")
require(plist.get("CFBundleVersion") == "140", "Info.plist build version is 140")
require(plist.get("CFBundleShortVersionString") == "0.0.140", "Info.plist display version is 0.0.140")
...
require("STEP 35.0.17 — GODOT CALLBACK BOUNDARY + MANAGED COMMAND-LINE FORWARD PROBE" in release_presentation, "top launcher banner identifies active Step 35.0.17 Godot callback-boundary/managed-command-line forward-probe candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.136", "0.0.138", "GS014", "CL_CRITICAL_001_POST", "CL_CRITICAL_002_PRE", "Godot.OS..cctor", "GS032", "NativeFuncs._unmanagedCallbacks", "NATURAL", "COMPAT", "OS-RECON", "FORWARD", "0.0.139", "0.0.140"]), "top launcher banner preserves the physical 0.0.138 split and identifies active 0.0.140 callback-boundary/forward-probe derivative")
...
require('ExpectedDisplayVersion = "0.0.140"' in release_presentation and 'ExpectedBuildVersion = "140"' in release_presentation, "Step 35.0.17 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.140",
    "STS2_BUILD_VERSION": "140",
...
require("Step 35.0.17 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.17 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.17 — comprehensive GodotSharp/native reconnaissance + dual mode with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.17 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.17 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.17 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
require("STEP 35.0.17 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_tests, "Step 35.0.17 host gate-summary assertion matches the active production summary identity")
require("STEP 35.0.15 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" not in step35_tests, "Step 35.0.17 static validation rejects the stale 0.0.139 Step-35.0.15 gate-summary assertion")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.17 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.17 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.17 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.17 host regressions protect the Action<string>::Invoke(!0) encoding")
require(all(marker in step35_tests for marker in ["ComprehensiveGodotSharpDiagnosticCloneUsesEntryOnlyMarkersAndPreservesIdentity", "ComprehensiveGodotNativeReconnaissanceParsesReadOnlyMachOAndManagedMap", "CreateInstrumentedGodotSharpDiagnosticClone", "Step35GodotReconnaissance.BuildReport"]), "Step 35.0.17 host regressions cover the entry-only GodotSharp derivative and read-only Mach-O reconnaissance")
require(all(marker in step35_tests for marker in ["serializedGodotEntryMarkers", "GodotSharpDiagnosticBridgeTypeFullName", "emitCall.DeclaringType.FullName"]), "Step 35.0.17 host regression directly verifies every serialized GS entry marker calls the GodotSharp bridge")
...
require("string expectedBridgeTypeFullName = DiagnosticBridgeTypeFullName" in step35_source and "HasInjectedEntryMarkerAtStart(method, item.Marker, GodotSharpDiagnosticBridgeTypeFullName)" in godot_clone_block and "call.DeclaringType.FullName == expectedBridgeTypeFullName" in step35_source, "Step 35.0.17 verifies serialized entry markers against the derivative-specific bridge type and protects the 0.0.137 208/209 regression")
require(all(marker in step35_mode for marker in ["NaturalGodotDictionaryRecon = 0", "ManagedDictionaryCompatibility = 1", "ManagedCommandLineCompatibility = 2"]), "Step 35.0.17 exposes exactly the NATURAL, OS-RECON dictionary-compatibility, and managed-command-line FORWARD mode identities")
require(all(marker in step35_source for marker in ["ManagedCommandLineArgsBridgeMethodName", "ApplyCommandLineHelperManagedCommandLineCompatibilityRewrite", "VerifyCommandLineHelperManagedCommandLineCompatibilityRewrite", "CommandLineManagedCommandLineSubstitutionCount", "requireNaturalGetCmdlineArgs: false", "providerInstructions.Length != 3", "Code.Ldc_I4_0", "Code.Newarr"]), "Step 35.0.17 FORWARD mode is a bounded one-call-site empty-string-array substitution with post-write verification")
require("ManagedCommandLineCompatibilityReplacesExactlyOneGodotArgsCallWithLocalEmptyArrayProvider" in step35_tests and "ManagedCommandLineArgsBridgeMethodName" in step35_tests, "Step 35.0.17 host regression protects the exact one-site managed command-line provider rewrite")
require(all(marker in step35_source for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "selected.Count < 128", "depth >= 4"]), "Step 35.0.17 GodotSharp derivative expands entry-only closure around OS type initialization and callback thunks")
require(all(marker in step35_recon for marker in ["Godot.OS/MethodName", "Godot.StringName", "ClassDB_get_method_with_compatibility", "godotsharp_string_name_new_from_string", "godotsharp_method_bind_get_method_with_compatibility", "visited.Count < 128"]), "Step 35.0.17 read-only reconnaissance mirrors the expanded OS/callback closure")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.17 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "DiagnosticCommandLineCriticalMarkersAreStackNeutralAndSerializedWithMaxStackHeadroom", "DiagnosticCommandLineManagedDictionaryRewriteRoundTripsGenericVarMemberRefsAndRetainsNaturalGodotArgsCall", "ApplyCommandLineHelperManagedDictionaryCompatibilityRewrite", "VerifyCommandLineHelperManagedDictionaryCompatibilityRewrite", "ManagedStringDictionaryFullName", "DiagnosticCallsiteSweepRaisesMaxStackAndClrExecutesTightRewrittenCctor", "MaxStackSize = 3", "cctorAfter.Body.MaxStackSize >= 4", "LoadFromStream", 'GetMethod("Touch"', "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.17 regressions protect exact ordinals, managed dictionary generic MemberRefs, natural GetCmdlineArgs retention, critical boundaries and executable MaxStack safety")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.17 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.17 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.17 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.17 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.17 active source/UI reject stale exact-closure wording")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.17 / 0.0.140 (140)", "0.0.137", "208/209", "0.0.138", "0.0.139", "209", "210", "NATURAL", "COMPAT", "GS014", "CL_CRITICAL_001_POST", "CL_CRITICAL_002_PRE", "INMETHOD_GS033", "Godot.OS.GetCmdlineArgs", "NativeFuncs._unmanagedCallbacks", "ManagedCommandLineCompatibility", "FORWARD", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact authority, 0.0.137 CI provenance, physical 0.0.138 callback-boundary evidence, and active 0.0.140 forward-probe design")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.140 (140)" in testing_doc and "0.0.140 (140)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.140 (140) release identity")
...
require("Step 35.0.17 Godot Callback Boundary + Managed Command-Line Forward Probe build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.17 callback-boundary/forward-probe candidate")
...
require("Expected device UI: STEP 35.0.17 — GODOT CALLBACK BOUNDARY + MANAGED COMMAND-LINE FORWARD PROBE" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.17 callback-boundary/forward-probe candidate")
...
if step35_manifest.is_file():
    ...
    require(
        ...
        "Step 35.0.17 candidate manifest includes implementation/tests, 0.0.137 CI provenance, physical 0.0.138 NATURAL/COMPAT evidence, and the 0.0.140 release/test-consistency correction over the unchanged forward-probe design",
    )
    mismatches=[]
