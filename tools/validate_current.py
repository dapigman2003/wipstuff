...
changed_core = sorted(name for name, digest in expected_core.items() if name in current_core_by_name and sha256(current_core_by_name[name]) != digest)
...
require(not changed_core, "physically proven Step 22.2 Core behavior files remain byte-for-byte unchanged", ", ".join(changed_core))
...
required_docs = [
    ...
    "docs/TESTING.md", "docs/REPORTS.md", "docs/RELEASE-CHECKLIST.md", "docs/history/INDEX.md",
...
require("docs/CURRENT-STATUS.md" in master and "docs/history/INDEX.md" in master, "master plan defines self-contained resumption path")
