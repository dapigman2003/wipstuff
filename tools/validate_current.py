...
require("<ApplicationVersion>206</ApplicationVersion>" in project_text, "build version is 206")
require("<ApplicationDisplayVersion>0.0.206</ApplicationDisplayVersion>" in project_text, "display version is 0.0.206")
require(plist.get("CFBundleVersion") == "206", "Info.plist build version is 206")
require(plist.get("CFBundleShortVersionString") == "0.0.206", "Info.plist display version is 0.0.206")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.206 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.206 (206)**" in read("README.md") and "Active candidate: **0.0.206 (206)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.206 (206) release identity")
require(all(v in read("docs/MASTER-PLAN.md") for v in ["0.0.206", "maximizes information/optionality per compilation", "59T CONTROL", "59W WRAPPER-ONLY", "59X FULL REPAIR", "Step 63"]), "master plan points to the active 0.0.206 compilation-efficient PropertyTweener three-mode frontier")
...
require('ExpectedDisplayVersion = "0.0.206"' in release_presentation and 'ExpectedBuildVersion = "206"' in release_presentation, "startup-ladder source pins expected 0.0.206 bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.206",
    "STS2_BUILD_VERSION": "206",
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.206\"", "ExpectedBuildVersion = \"206\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL"]), "active release presentation pins Step-52 current architecture and Steps-58–62 / 0.0.206 identity")
...
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.206 UI unlocks active Step 58 directly from same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.206 active ownership path contains no obsolete Step-57 prerequisite gate")
...
require(all(v in character_core for v in ["RunStep59ControllerSingletonRehearsal", "RunStep59RegisterHotkeysProbe", "RunStep59UpdateControllerButtonProbe", "RunStep59ConfirmButtonTailProbe", "RunStep59ConfirmButtonTailWrapperRepairProbe", "RunStep59ConfirmButtonTailRepairProbe", "RunStep59IsolatedEmbarkEnableProbe", "M59H_INVOKE_ARMED", "M59U_INVOKE_ARMED", "M59E_ENABLE_ARMED"]), "0.0.206 retains controller, hotkey, controller-update, control-tail, wrapper-only, full-repair and full-Enable branches in one compilation")
require(all(v in character_ui for v in ["Run Step 59R — CONTROLLER REHEARSAL", "Run Step 59H — REGISTER HOTKEYS ONLY", "Run Step 59U — UPDATE CONTROLLER ONLY", "Run Step 59T — CONTROL / OBSERVE", "Run Step 59W — WRAPPER REPAIR ONLY", "Run Step 59X — FULL REPAIR / FULL TAIL", "Run Step 59E — ISOLATED EMBARK ENABLE"]), "0.0.206 exposes all reusable Step-59 isolation branches in the same IPA")
