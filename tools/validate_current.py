...
require("<ApplicationVersion>209</ApplicationVersion>" in project_text, "build version is 209")
require("<ApplicationDisplayVersion>0.0.209</ApplicationDisplayVersion>" in project_text, "display version is 0.0.209")
require(plist.get("CFBundleVersion") == "209", "Info.plist build version is 209")
require(plist.get("CFBundleShortVersionString") == "0.0.209", "Info.plist display version is 0.0.209")
...
require("STEP 52 CURRENT-ARCHITECTURE BASELINE" in release_presentation and "0.0.209 STEPS 58–62 ACTIVE" in release_presentation and "LEGACY 53–57 OPTIONAL" in release_presentation and "STEP 63 UNOPENED" in release_presentation, "top launcher presentation records Step-52 current-architecture baseline, optional legacy 53–57, active Steps 58–62, and Step 63 unopened")
require("Active candidate: **0.0.209 (209)**" in read("README.md") and "Active candidate: **0.0.209 (209)**" in read("docs/README.md"), "root and docs READMEs pin the exact 0.0.209 (209) release identity")
require(all(v in read("docs/MASTER-PLAN.md") for v in ["0.0.209", "native Tween rejection", "59N", "59Y", "59Z", "Step 63"]), "master plan points to the active 0.0.209 native Tween rejection frontier")
...
require('ExpectedDisplayVersion = "0.0.209"' in release_presentation and 'ExpectedBuildVersion = "209"' in release_presentation, "startup-ladder source pins expected 0.0.209 bundle release identity")
...
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs") == "c86c10e2de4097ef2232ab1b7a93fe4eae9be74cce4b8a6e5e59f8edf47d117e", "0.0.209 leaves the physically viable 0.0.208 GodotSharp derivative builder byte-for-byte unchanged")
require(sha256(ROOT / "src/StS2Launcher.Core/Runtime/PropertyTweenerExperimentProfile.cs") == "48d62538fcd495eeba0f3028e6e10b7fea75dfa577fca80598d9f6ec45f181d9", "0.0.209 leaves the 0.0.208 Baseline/Wrapper/Full profile enum byte-for-byte unchanged")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.209",
    "STS2_BUILD_VERSION": "209",
...
require(used_assert_methods <= known_mstest_v4_assert_methods, "0.0.209 test sources use only the repository's known compiling MSTest-v4 Assert API surface", "unexpected=" + ",".join(sorted(used_assert_methods - known_mstest_v4_assert_methods)))
require("Assert.Greater(" not in all_test_text and "Assert.IsTrue(fluentRepairCalls > 0, fluentName);" in step35_tests, "0.0.209 retains the 0.0.206 unsupported Assert.Greater correction with an established Assert.IsTrue form")
...
require(all(value in release_presentation for value in ["STEP 42 CLOSED POSITIVE 4/4", "Step42ImplementationMarker", "StartupLadderImplementationMarker", "ExpectedDisplayVersion = \"0.0.209\"", "ExpectedBuildVersion = \"209\"", "STEP 52 CURRENT-ARCHITECTURE BASELINE", "STEPS 58–62 — CHARACTER-SELECT REAL OWNERSHIP / VISIBLE RENDER TRIAL", "BASELINE", "WRAPPER", "FULL"]), "active release presentation pins Step-52 current architecture and the 0.0.209 native-Tween Steps-58–62 identity")
...
require("_step52Gates.Snapshot().Passed && _transformedRealStS2VeryEarlyInitialization.ExactStep52ClosurePassed" in character_ui and "RunStep58CurrentOwnershipAcquisition" in character_ui and "legacy Steps 53–57 are optional diagnostics" in character_ui, "0.0.209 UI keeps active Step 58 directly on same-process Step-52 authority and explicitly demotes legacy 53-57 to optional diagnostics")
require("RunStep58ClosedStep57Authority" not in character_ui and "RunStep58ClosedStep57Authority" not in character_core, "0.0.209 active ownership path contains no obsolete Step-57 prerequisite gate")
...
require(hashlib.sha256(packed_slice_bytes).hexdigest() == "36bbdd3f8bee48c4a71b29bbb4ac34a3e7ac4b7718f8abe25006535cff0f9395", "0.0.209 preserves the exact physically proven PackedScene compatibility implementation bytes")
very_early_core = read("src/StS2Launcher.Core/Runtime/TransformedRealStS2VeryEarlyInitialization.cs")
require(all(marker not in very_early_core for marker in ["STEP59TRACE_", "ConfirmButtonCheckpointBridge", "Step59RuntimeTraceBridge", "ArmStep59RuntimeTrace", "DisarmStep59RuntimeTrace"]), "0.0.209 keeps the rejected 0.0.202 runtime-wide Step-59 trace bridge absent")
