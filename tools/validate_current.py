...
print("StS2 Launcher — Step 35.0.10 Command-Line / Godot boundary localization validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>133</ApplicationVersion>" in project_text, "build version is 133")
require("<ApplicationDisplayVersion>0.0.133</ApplicationDisplayVersion>" in project_text, "display version is 0.0.133")
require(plist.get("CFBundleVersion") == "133", "Info.plist build version is 133")
require(plist.get("CFBundleShortVersionString") == "0.0.133", "Info.plist display version is 0.0.133")
...
require("STEP 35.0.10 — COMMAND-LINE/GODOT BOUNDARY LOCALIZATION" in release_presentation, "top launcher banner identifies active Step 35.0.10 Command-Line/Godot boundary-localization candidate")
require(all(marker in release_presentation for marker in ["STEP 32 CLOSED POSITIVE 4/4", "STEP 33 CLOSED POSITIVE 4/4", "STEP 34 CLOSED POSITIVE 4/4", "0.0.129", "MissingMethodException", "Action<string>.Invoke(string)", "Action<string>::Invoke(!0)", "0.0.130", "0.0.131", "0.0.132", "NP003", "CALLSITE#002", "CommandLineHelper.TryGetValue", "Godot.OS.GetCmdlineArgs", "0.0.133", "diagnostic"]), "top launcher banner preserves closed Steps 32-34 and physical 0.0.129-0.0.132 history while identifying the active 0.0.133 Command-Line/Godot diagnostic")
...
require('ExpectedDisplayVersion = "0.0.133"' in release_presentation and 'ExpectedBuildVersion = "133"' in release_presentation, "Step 35.0.10 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.133",
    "STS2_BUILD_VERSION": "133",
...
require("Step 35.0.10 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.10 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.10 — Command-Line / Godot boundary localization with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.10 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.10 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.10 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
...
require(all(marker in step35_source for marker in ["BuildStaticInstructionMap", "CALLSITE#", "AWAIT-CANDIDATE", "DescribeMetadataScope", "GetVerifiedVeryEarlyStaticInstructionMap", "[NULL PLATFORM CTOR IL]", "[COMMAND LINE HELPER CCTOR IL]", "[COMMAND LINE HELPER TRYGETVALUE IL]", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName"]), "Step 35.0.10 static map covers the exact NullPlatform constructor plus CommandLineHelper cctor/TryGetValue callsite ordinals without resolving dependencies")
require(all(marker in step35_source for marker in ["CreateInstrumentedDiagnosticClone", "GetDiagnosticMarkerTargets", "GetDiagnosticCallsiteMarkerTargets", "HasInjectedEntryMarkerAtStart", "HasInjectedCallsiteMarkers", "InsertCallsiteMarkers", "InsertDiagnosticCallsiteSweepMarkers", "InsertNullPlatformConstructorCallsiteMarkers", "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "HasInjectedDiagnosticCallsiteMarkers", "DiagnosticCallsiteSweepEntry", "NullPlatformTypeFullName", "NullPlatformConstructorFullName", "CommandLineHelperTypeFullName", "CommandLineHelperTryGetValueFullName", "INMETHOD_NP", "INMETHOD_CL", "INMETHOD_CLTV", "INMETHOD_027", "direct base-constructor call", "Code.Call or Code.Callvirt or Code.Newobj", "Godot.OS::GetCmdlineArgs(", "failOnBranchTarget", "DiagnosticBridgeTypeFullName", "INMETHOD_001", "INMETHOD_021", "INMETHOD_022", "INMETHOD_023", "INMETHOD_024", "INMETHOD_025", "INMETHOD_026", "INMETHOD_031", "INMETHOD_050", "INMETHOD_100", "INMETHOD_180", "INMETHOD_181", "INMETHOD_182", "INMETHOD_183", "System.Boolean Godot.DirAccess::DirExistsAbsolute(System.String)", "Godot.Error Godot.DirAccess::MakeDirRecursiveAbsolute(System.String)", "C_DIAGNOSTIC_BRIDGE_ARMED", "preflight.DiagnosticPath", "preflight.DiagnosticSha256"]), "Step 35.0.10 preserves prior diagnostic markers, corrects NP ordinal accounting and adds bounded CommandLineHelper cctor/TryGetValue sweeps")
...
require(sweep_definition >= 0 and sweep_skip > sweep_definition and sweep_increment > sweep_skip, "Step 35.0.10 excludes injected diagnostic bridge calls before exact-source callsite ordinal accounting")
require("commandLineCctorCallsitePlan.Any" in step35_source and "Godot.OS::GetCmdlineArgs(" in step35_source, "Step 35.0.10 fails Gate A closed unless the CommandLineHelper cctor diagnostic plan contains Godot.OS.GetCmdlineArgs")
require(all(marker in step35_source for marker in ["module.Write(diagnosticPath", "verifyModule.Mvid != TransformedRealStS2AssemblyAdmission.ClosedStep32Mvid", "transformedSha256AfterDiagnosticEmission", "ComputeSha256Hex(preflight.TransformedPath)", "markerCountVerified != expectedMarkerCount"]), "Step 35.0.10 preserves/rechecks the exact transformed source and verifies diagnostic-clone MVID/hash/marker provenance")
require(all(marker in step35_source for marker in ["DiagnosticConstantMetadataWriteResolver", "DiagnosticCecilWriteSystemRuntimeIdentity", "DiagnosticCecilWriteSentryIdentity", "System.Reflection.BindingFlags", "Sentry.BreadcrumbLevel", "Sentry.SentryLevel", "CollectDiagnosticExternalConstantTypeRequirements", "resolver.Configure(module)", "resolver.ValidateWriteRequests()", "RealStS2PrepareMethodRewrite.ComputeConstantMetadataFingerprint", "verifiedConstantMetadataSha256.Equals(expectedConstantMetadataSha256", "External assembly bytes opened by the writer-only surrogate resolver: 0"]), "Step 35.0.10 reuses the Step-32 audited constant-metadata requirement set locally for Cecil serialization only")
require("new DiagnosticConstantMetadataWriteResolver()" in step35_source and "new RejectingAssemblyResolver()" in step35_source, "Step 35.0.10 separates bounded writer-only Cecil resolution from rejecting post-write verification")
...
require("ReadingMode = ReadingMode.Deferred" in clone_block and "ReadingMode = ReadingMode.Immediate" not in clone_block and "InMemory = true" not in clone_block and clone_block.find("resolver.Configure(module)") < clone_block.find("module.Write(diagnosticPath") and "resolver.Requests.Count != 0" in clone_block, "Step 35.0.10 diagnostic clone preserves deferred-open -> zero-request -> configure -> write ordering")
require(all(marker in clone_block for marker in ["new GenericParameter(\"T\", actionOpen)", "actionOpen.GenericParameters.Add(actionTypeParameter)", "new ParameterDefinition(actionTypeParameter)", "Action<string>::Invoke(!0)", "bridgeInvokeParameter.Type != GenericParameterType.Type", "bridgeInvokeParameter.Position != 0"]), "Step 35.0.10 preserves the physically corrected Action<string>::Invoke(!0) MemberRef")
require("invoke.Parameters.Add(new ParameterDefinition(module.TypeSystem.String))" not in clone_block, "Step 35.0.10 rejects the physically disproven synthetic Action<string>::Invoke(string) encoding")
require("CreateDiagnosticActionStringInvokeReference" in step35_source and "DiagnosticActionStringInvokeMemberRefRoundTripsAsDeclaringTypeVarZero" in step35_tests, "Step 35.0.10 host regressions protect the Action<string>::Invoke(!0) encoding")
require("DiagnosticGodotCallsiteMarkersRoundTripImmediatelyBeforeAndAfterTargetCall" in step35_tests and "InsertCallsiteMarkers" in step35_tests and "DirExistsAbsolute" in step35_tests, "Step 35.0.10 retains prior Godot callsite marker round-trip regression")
require(all(marker in step35_tests for marker in ["DiagnosticNullPlatformConstructorCallsiteSweepRoundTripsEveryNonBaseCallLikeInstruction", "InsertSyntheticEntryMarker", "InsertNullPlatformConstructorCallsiteMarkers", "CallsiteOrdinal", "Physical 0.0.132", "DiagnosticCommandLineHelperSweepsIgnoreInjectedEntryBridgeAndRoundTripExactOrdinals", "DiagnosticCommandLineHelperSweepSkipsUnrelatedBranchTargetButPreservesExactOrdinals", "InsertCommandLineHelperCctorCallsiteMarkers", "InsertCommandLineHelperTryGetValueCallsiteMarkers", "Godot.OS::GetCmdlineArgs()"]), "Step 35.0.10 regressions reproduce the physical 0.0.132 entry-marker ordering bug and serialize/reopen corrected NP plus CommandLineHelper sweep ordinals")
...
require(all(marker in step35_source for marker in ["B_LOADFROMSTREAM_START", "B_LOADFROMSTREAM_PASS", "C_BIND_TYPE_START", "C_BIND_METHOD_START", "C_INVOKE_START", "C_INVOKE_RETURNED", "C_TASK_CONFIRMED", "C_WAIT_START", "C_WAIT_COMPLETED", "RESOLVE_MANAGED_START", "RESOLVE_PRIVATE_PASS", "RESOLVE_HOST_PASS", "RESOLVE_NATIVE_REJECT"]), "Step 35.0.10 core retains durable B→C/invocation/resolver crash-localization markers")
require(all(marker in step35_ui for marker in ["Step35-CurrentRun.txt", "Step35-LastCheckpoint.txt", "Step35-CrashCheckpoint-", "Step35-ExecuteVeryEarly-StaticMap-", "TryInitializeStep35RunTelemetry", "RUN_TELEMETRY_READY", "WriteStep35StaticMap", "Flush(flushToDisk: true)", "Environment.CurrentManagedThreadId", "CANCELLED / INCONCLUSIVE", "Output-only diagnostic", "NP003_PRE", "CALLSITE#002", "INMETHOD_CLxxx_PRE/POST", "INMETHOD_CLTVxxx_PRE/POST"]), "Step 35.0.10 iOS surface retains durable telemetry, records the 0.0.132 finding and advertises the corrected deeper localization")
require("crashCheckpoints.Any" in step35_tests and "B_LOADFROMSTREAM_START" in step35_tests and "RESOLVE_INITIALIZER_BEARING_REJECT" in step35_tests and "StaticInstructionMapCapturesCallsitesAndAwaitCandidatesWithoutResolution" in step35_tests, "Step 35.0.10 host regressions retain crash-checkpoint and static-map coverage")
require("OrderedDiagnosticLocalizationGatesReachFourOfFourWithoutClaimingClosure" in step35_tests and "VeryEarlyInitializationStopsAfterFirstFailure" in step35_tests and "VeryEarlyContextLoadsInitializerFreePrivateDependencyAndRejectsInitializerBearingDependency" in step35_tests and "Step35PinsTheExactVeryEarlyManagedInitializationTarget" in step35_tests, "Step 35.0.10 host regressions protect diagnostic gate order, exact source target and initializer-bearing refusal")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.10 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.10 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.10 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.10 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.10 active source/UI reject stale exact-closure wording")
...
require("Step35ImplementationMarker" in release_presentation and "0x06007D02" in release_presentation and "0x0600BC71" in release_presentation and "ExecuteVeryEarly" in release_presentation and "0.0.133" in release_presentation, "release presentation pins the Step-35.0.10 exact-source/instrumented-clone boundary")
...
require(all(marker in current_status for marker in ["Steps 01–26", "Step 27", "CLOSED NEGATIVE", "CLOSED POSITIVE 5/5", "Step 35 remains OPEN", "Active candidate — Step 35.0.10 / 0.0.133 (133)", "0.0.126", "0.0.127", "0.0.128", "0.0.129", "0.0.130", "0.0.131", "0.0.132", "MissingMethodException", "Action<string>::Invoke(!0)", "NullPlatformUtilStrategy..ctor", "GodotFileIo..ctor", "INMETHOD_NP003_PRE", "CALLSITE#002", "Godot.OS.GetCmdlineArgs", "INMETHOD_CL", "INMETHOD_CLTV", "[NULL PLATFORM CTOR IL]", "[COMMAND LINE HELPER CCTOR IL]", "[COMMAND LINE HELPER TRYGETVALUE IL]", "0x06007D02", "0x0600BC71", "diagnostic"]), "current status preserves exact 0.0.126 authority, records physical 0.0.132 TryGetValue/ordinal evidence, and identifies active 0.0.133 Command-Line/Godot localization")
testing_doc = read("docs/TESTING.md")
release_checklist_doc = read("docs/RELEASE-CHECKLIST.md")
require("0.0.133 (133)" in testing_doc and "0.0.133 (133)" in release_checklist_doc, "testing and release-checklist docs pin the exact 0.0.133 (133) release identity")
require("0.0.133 (132)" not in testing_doc and "0.0.133 (131)" not in testing_doc and "0.0.133 (132)" not in release_checklist_doc and "0.0.133 (131)" not in release_checklist_doc, "release docs reject display/build-number drift like the prior 0.0.130 (129) documentation bug")
...
require("Step 35.0.10 Command-Line / Godot Boundary Localization build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.10")
...
require("Expected device UI: STEP 35.0.10 — COMMAND-LINE/GODOT BOUNDARY LOCALIZATION" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.10 device boundary")
