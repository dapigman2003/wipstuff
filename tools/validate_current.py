...
print("StS2 Launcher — Step 35.0.5 in-method pre-first-await localization validation")
print(f"Root: {ROOT}")
...
require("<ApplicationVersion>128</ApplicationVersion>" in project_text, "build version is 128")
require("<ApplicationDisplayVersion>0.0.128</ApplicationDisplayVersion>" in project_text, "display version is 0.0.128")
require(plist.get("CFBundleVersion") == "128", "Info.plist build version is 128")
require(plist.get("CFBundleShortVersionString") == "0.0.128", "Info.plist display version is 0.0.128")
...
require("STEP 35.0.5 — IN-METHOD PRE-FIRST-AWAIT LOCALIZATION" in release_presentation, "top launcher banner identifies active Step 35.0.5 in-method localization candidate")
...
require('ExpectedDisplayVersion = "0.0.128"' in release_presentation and 'ExpectedBuildVersion = "128"' in release_presentation, "Step 35.0.5 source pins expected bundle release identity")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.128",
    "STS2_BUILD_VERSION": "128",
...
require("Step 35.0.5 IPA verification passed." in verify_ipa_text and "Step 27 IPA verification passed." not in verify_ipa_text, "IPA verification summary identifies the active Step-35.0.3 candidate rather than retired Step 27")
...
# ---------------------------------------------------------------------------
# Step 35.0.5 — diagnostic-clone in-method localization with exact-source authority preserved.
# ---------------------------------------------------------------------------
...
require(all(marker in step35_gate for marker in ["VerifiedExecutionPreflight = 1", "ExecutionCapableClrAdmission = 2", "DiagnosticExecuteVeryEarlyInvocation = 3", "FinalIsolationAudit = 4"]), "Step 35.0.5 exposes exactly four ordered diagnostic-localization gates")
require("STEP 35.0.5 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE" in step35_summary and "Gates.Count(g => g.Passed)}/4" in step35_summary, "Step 35.0.5 summary reports four-of-four diagnostic completion without claiming exact Step-35 closure")
...
require(all(marker in step35_source for marker in ["B_LOADFROMSTREAM_START", "B_LOADFROMSTREAM_PASS", "C_BIND_TYPE_START", "C_BIND_METHOD_START", "C_INVOKE_START", "C_INVOKE_RETURNED", "C_TASK_CONFIRMED", "C_WAIT_START", "C_WAIT_COMPLETED", "RESOLVE_MANAGED_START", "RESOLVE_PRIVATE_PASS", "RESOLVE_HOST_PASS", "RESOLVE_NATIVE_REJECT"]), "Step 35.0.5 core retains durable B→C/invocation/resolver crash-localization markers")
require(all(marker in step35_ui for marker in ["Step35-CurrentRun.txt", "Step35-LastCheckpoint.txt", "Step35-CrashCheckpoint-", "Step35-ExecuteVeryEarly-StaticMap-", "TryInitializeStep35RunTelemetry", "RUN_TELEMETRY_READY", "WriteStep35StaticMap", "Flush(flushToDisk: true)", "Environment.CurrentManagedThreadId", "CANCELLED / INCONCLUSIVE", "Output-only diagnostic"]), "Step 35.0.5 iOS surface retains fail-visible run-correlated durable telemetry and keeps cancellation inconclusive")
require("crashCheckpoints.Any" in step35_tests and "B_LOADFROMSTREAM_START" in step35_tests and "RESOLVE_INITIALIZER_BEARING_REJECT" in step35_tests and "StaticInstructionMapCapturesCallsitesAndAwaitCandidatesWithoutResolution" in step35_tests, "Step 35.0.5 host regressions retain crash-checkpoint and static-map callsite/await coverage")
require("OrderedDiagnosticLocalizationGatesReachFourOfFourWithoutClaimingClosure" in step35_tests and "VeryEarlyInitializationStopsAfterFirstFailure" in step35_tests and "VeryEarlyContextLoadsInitializerFreePrivateDependencyAndRejectsInitializerBearingDependency" in step35_tests and "Step35PinsTheExactVeryEarlyManagedInitializationTarget" in step35_tests, "Step 35.0.5 host regressions protect diagnostic gate order, exact source target and initializer-bearing refusal")
...
require("DIAGNOSTIC COMPLETE: STEP 35.0.5 — 4/4. NOT STEP 35 CLOSURE." in step35_ui and "exact Step 35 remains OPEN" in step35_ui, "Step 35.0.5 iOS success surface cannot masquerade as exact Step-35 closure")
require("STEP-35.0.5 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE." in step35_source, "Step 35.0.5 Gate-C PASS wording explicitly remains diagnostic-only")
require("PASS: STEP 35 CONTROLLED VERY-EARLY INITIALIZATION" not in step35_ui and "FIRST CONTROLLED INVOCATION/AWAIT OF THE EXACT TRANSFORMED REAL-STS2" not in step35_source and "ExactExecuteVeryEarlyInvocation = 3" not in step35_gate, "Step 35.0.5 active source/UI reject stale exact-closure wording")
...
require("Steps 01–26" in current_status and "Step 27" in current_status and "CLOSED NEGATIVE" in current_status and "CLOSED POSITIVE 5/5" in current_status and "Step 32 CLOSED POSITIVE 4/4" in current_status and "Step 33 CLOSED POSITIVE 4/4" in current_status and "Step 34 CLOSED POSITIVE 4/4" in current_status and "Active candidate — Step 35.0.5 / 0.0.128 (128)" in current_status and "0.0.124" in current_status and "0.0.125" in current_status and "0.0.126" in current_status and "0.0.127" in current_status and "AssemblyResolutionException" in current_status and "System.Runtime, Version=9.0.0.0" in current_status and "program counter **0x0**" in current_status and "C_INVOKE_RETURNED" in current_status and "0x06007D02" in current_status and "0x0600BC71" in current_status and "Step35-CurrentRun.txt" in current_status and "Step35-LastCheckpoint.txt" in current_status and "Step35-CrashCheckpoint-<RunId>.txt" in current_status and "Step35-ExecuteVeryEarly-StaticMap-<RunId>.txt" in current_status and "INMETHOD_*" in current_status and "diagnostic clone" in current_status, "current status preserves Step-32/33/34 closures, the physical invoke frontier, 0.0.126 same-run evidence, and active Step-35.0.5 localization boundary")
...
require("Step 35.0.5 In-Method Pre-First-Await Localization build environment" in read("scripts/codemagic.sh"), "Codemagic build-environment report heading identifies Step 35.0.5")
...
require("Expected device UI: STEP 35.0.5 — IN-METHOD PRE-FIRST-AWAIT LOCALIZATION" in verify_ipa and "step35-ipa-verification-summary.log" in verify_ipa, "IPA verifier advertises the active Step 35.0.5 device boundary")
