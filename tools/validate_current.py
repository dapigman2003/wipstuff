...
require("<ApplicationVersion>97</ApplicationVersion>" in project_text, "build version is 97")
require("<ApplicationDisplayVersion>0.0.97</ApplicationDisplayVersion>" in project_text, "display version is 0.0.97")
require(plist.get("CFBundleVersion") == "97", "Info.plist build version is 97")
require(plist.get("CFBundleShortVersionString") == "0.0.97", "Info.plist display version is 0.0.97")
...
require("STEP 27.0.13 — SYNTHETIC PREFLIGHT SCOPE HARDENING" in release_presentation, "top launcher banner identifies active Step 27.0.13 candidate")
require("Codemagic" in release_presentation and "0.0.96" in release_presentation and "209/211" in release_presentation and "synthetic" in release_presentation.lower() and "canonical production target identity" in release_presentation and "byte-distinct normalized runtime image" in release_presentation and "No StS2 member" in release_presentation, "top launcher banner short description matches current synthetic-preflight scope hardening candidate")
...
require('ExpectedDisplayVersion = "0.0.97"' in release_presentation and 'ExpectedBuildVersion = "97"' in release_presentation, "Step 27.0.13 source pins expected bundle release identity")
require("GateSImplementationMarker" in release_presentation and "AddPrefix(MethodInfo) runtime invocation forbidden" in release_presentation, "Step 27.0.13 source pins the bounded Gate-S implementation marker")
require("GateTImplementationMarker" in release_presentation and "11-instruction iOS-normalized HarmonySharedState cctor loaded from memory" in release_presentation and "direct state only" in release_presentation and "PatchProcessor.Patch() remains after T6" in release_presentation, "Step 27.0.13 source pins the Gate-T normalized-cctor implementation marker")
...
for key, value in {
    ...
    "STS2_DISPLAY_VERSION": "0.0.97",
    "STS2_BUILD_VERSION": "97",
...
require(step27_source.count("CreateIosNormalizedHarmonyRuntimeImage(target.PreparedPath)") == 1, "Step 27.0.13 creates exactly one bounded normalized Harmony runtime image during Gate A")
require(step27_source.count("CreateSyntheticPassthroughRuntimeImage(target.PreparedPath)") == 1, "Step 27.0.13 has exactly one internal synthetic passthrough path alongside canonical normalization")
require("_targetSimpleName.Equals(TargetSimpleName, StringComparison.OrdinalIgnoreCase)" in step27_source and "_targetVersion == TargetVersion" in step27_source and "requiresIosHarmonyNormalization" in step27_source, "Step 27.0.13 scopes production normalization to exact canonical target identity/version")
require("requiresIosHarmonyNormalization &&" in step27_source and "produce a byte-distinct runtime image" in step27_source and "!requiresIosHarmonyNormalization &&" in step27_source and "internal synthetic-target replay must remain byte-identical" in step27_source, "Step 27.0.13 requires canonical normalization to be byte-distinct and synthetic replay to be byte-identical")
...
require("ReadHarmonyPatchEngineMetadata(path)" in step27_source and "refuses to normalize HarmonySharedState because the source 0Harmony patch-engine fingerprint" in step27_source, "Step 27.0.13 admits normalization only after the exact original patch-engine fingerprint passes")
require("module.Write(output)" in step27_source and "new MemoryStream(runtimeBytes, writable: false)" in step27_source and "instructions.Count != 11" in step27_source, "Step 27.0.13 writes/reopens the compatibility image in memory and pins the exact 11-instruction cctor")
require("using CecilOpCodes = Mono.Cecil.Cil.OpCodes;" in step27_source, "Step 27.0.13 explicitly aliases Cecil OpCodes alongside System.Reflection.Emit")
require(step27_source.count("Instruction.Create(CecilOpCodes.") == 11 and "Instruction.Create(OpCodes." not in step27_source, "Step 27.0.13 uses CecilOpCodes for all eleven generated cctor instructions and forbids bare ambiguous OpCodes")
require("GetOrCreateSharedStateType" in step27_source and "FieldRefAccess" in step27_source and "ReflectionHelper::Load" in step27_source and "Normalized HarmonySharedState::.cctor failed its exact 11-instruction iOS-safe fingerprint audit." in step27_source, "Step 27.0.13 normalized cctor audit forbids the original dynamic-singleton/FieldRefAccess load calls")
require("HarmonySharedStateInternalVersion = 102" in step27_source and 'RequireField("state"' in step27_source and 'RequireField("originals"' in step27_source and 'RequireField("originalsMono"' in step27_source and 'GetField("methodAddressRef"' in step27_source, "Step 27.0.13 normalization is pinned to the exact HarmonySharedState direct-state field surface")
require("SourcePreparedSha1" in step27_source and "RuntimeImageSha1" in step27_source and "RuntimeImageBytes" in step27_source and "produce a byte-distinct runtime image" in step27_source, "Step 27.0.13 records source/runtime hashes and requires a byte-distinct compatibility image")
require("new MemoryStream(_harmonyRuntimeImage.RuntimeImageBytes, writable: false)" in step27_source and "prepared.Plan.AssemblyFullName.Equals(_normalizedHarmonyAssemblyFullName" in step27_source, "Step 27.0.13 private load context uses retained memory bytes only for the exact admitted target identity")
...
require("generatedBeforeSharedInitialization.Length != 0" in step27_source and "unexpected pre-existing generated patch-engine assembly" in step27_source and "sharedGeneratedAssemblies.Length != 0" in step27_source, "Step 27.0.13 rejects generated HarmonySharedState/proxy state before and after normalized T5/T6")
require("harmonySharedStateStateField.GetValue(null) is null" in step27_source and "harmonySharedStateOriginalsField.GetValue(null) is null" in step27_source and "harmonySharedStateOriginalsMonoField.GetValue(null) is null" in step27_source and "harmonySharedStateMethodAddressRefField.GetValue(null) is not null" in step27_source, "Step 27.0.13 T6 validates three initialized dictionaries and null methodAddressRef")
...
require("BundleIdentityMatchesExpected" in step27_ui and "IDENTITY_FAIL" in step27_ui and "No Step-27 gate was run" in step27_ui, "Step 27.0.13 fails closed before Gate A on bundle/source identity mismatch")
require("App version:" in step27_ui and "Expected source version:" in step27_ui and "Candidate:" in step27_ui and "Gate S implementation:" in step27_ui and "Gate T implementation:" in step27_ui, "Step 27.0.13 crash checkpoint self-identifies release/candidate/Gate-S/Gate-T provenance")
...
require("Steps 01–26" in current_status and "0.0.89 (89)" in current_status and "0.0.90 (90)" in current_status and "Gate-T hard crash localized" in current_status and "0.0.91 (91)" in current_status and "clean Gate-O regression" in current_status and "0.0.93 (93)" in current_status and "provenance confirmed" in current_status and "0.0.94 (94)" in current_status and "netstandard" in current_status and "0.0.95 (95)" in current_status and "CS0104" in current_status and "0.0.96 (96)" in current_status and "209 passed" in current_status and "0.0.97 (97)" in current_status and "byte-identical" in current_status and "CecilOpCodes" in current_status and "11-instruction" in current_status and "T5a" in current_status and "T5b" in current_status and "HarmonySharedState" in current_status and "Step27-CrashCheckpoint.txt" in current_status and "Force-quit/relaunch before every Step-27 retry" in current_status and "26/26 PASS" in current_status and "Step 28" in current_status, "current status preserves Step 27 physical/build evidence and advances to Step 27.0.13 / 0.0.97 synthetic-preflight scope hardening candidate")
