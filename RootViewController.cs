...
public sealed class RootViewController : UIViewController
{
    ...
    public override void ViewDidLoad()
    {
        ...
        content.AddArrangedSubview(Label(
            "STEP 12.3 — VERIFIED CACHE UPDATE TEST",
        ...
        content.AddArrangedSubview(Label(
            "Step 12.3 fixes the update-test/cache-trust weakness found during physical testing. A completed Step 11 manifest-specific cache is no longer discarded merely because the Step 12 install receipt was intentionally made stale. Step 11 first downloads the real current Steam manifest and then directly re-hashes the existing final cache against that manifest; only a cache that fails exact path/size/SHA-1 verification is discarded and reacquired. Step 12 forwards that verification progress and preserves planned byte/file telemetry on cancellation. The synthetic update test is also stronger: it stales the local manifest ID and changes one file identity in the receipt only, so the normal update path must classify Update Available, replace at least one file from the verified current source, verify the full staging tree, write the real current receipt, and atomically commit. Step 12.1 AOT receipt JSON and Step 12.2/12.2.1 CDN timeout failover/catch ordering remain unchanged. Multi-depot composition, compatibility inspection, Godot, Cloud, and Workshop remain out of scope.",
        ...
        content.AddArrangedSubview(Label(
            "Step 12.3 — verified cache + stronger update test",
            UIFont.BoldSystemFontOfSize(25),
            UIColor.Label));
        ...
        _statusLabel = Label(
            "Status: Steps 01–11 passed on the physical iPhone. Step 12.0 exposed the AOT receipt-JSON bug; Step 12.1 fixed it. Step 12.2 added iOS CDN TimeoutException failover and Step 12.2.1 fixed the resulting Codemagic catch-order compile error. Physical update-state testing then showed that a deliberately stale Step 12 receipt caused a valid completed Step 11 cache to be thrown away and reacquired, with poor 0-byte cancellation telemetry. Step 12.3 independently revalidates that completed cache against Steam’s current manifest instead, forwards acquisition progress, and strengthens the synthetic update gate to require at least one source-file replacement plus atomic commit.",
            UIFont.SystemFontOfSize(14),
            UIColor.Label);
        ...
        Console.WriteLine("Step 12.3: RootViewController.ViewDidLoad complete");
    }
    ...
    private async Task RunManagedInstallAsync()
    {
        ...
        _statusLabel.Text = "STEP 12.3 RUNNING — existing Step 11 cache is reverified against the current Steam manifest before reuse; progress/planned bytes stay visible; previous good managed install remains authoritative until atomic commit.";
        _statusLabel.TextColor = UIColor.Label;
        ...
        try
        {
            ...
            InvokeOnMainThread(() =>
            {
                ...
                _statusLabel.Text = result.Success
                    ? $"PASS: Step 12.3 state {result.StateBefore} -> {result.StateAfter}; action {result.ActionTaken}; stable managed install is verified and current."
                    : $"Step 12.3 did not complete: {result.Error ?? result.Outcome.ToString()}. The prior good install was preserved when one existed.";
        ...
        catch (Exception ex)
        {
            InvokeOnMainThread(() =>
            {
                ...
                _statusLabel.Text = "FAIL: unhandled exception during Step 12.3 install/update/repair manager.";
                _statusLabel.TextColor = UIColor.SystemRed;
            });
        }
        finally
        ...
        try
        {
            ...
            InvokeOnMainThread(() =>
            {
                ...
                _statusLabel.Text = final.Passed
                    ...
                    : "FAIL: a proven foundation regression failed; stop Step 10 work until understood.";
