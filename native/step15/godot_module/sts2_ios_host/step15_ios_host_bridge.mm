...
void remove_marker_if_present(const char *path) {
    const String absolute_path = marker_absolute_path(path);
...
STS2_STEP15_EXPORT int sts2_step15_start(void *parent_controller_handle, void *container_view_handle, const char *project_path_utf8) {
    ...
    remove_marker_if_present("user://sts2_step15_render_ready.txt");
    remove_marker_if_present("user://sts2_step15_touch_ready.txt");
...
STS2_STEP15_EXPORT int sts2_step15_is_metal_layer_ready() {
    ...
    NSString *name = NSStringFromClass([g_view.renderingLayer class]);
    return [name rangeOfString:@"Metal" options:NSCaseInsensitiveSearch].location != NSNotFound ? 1 : 0;
}
