...
{
  echo "=== StS2 Launcher Step 12.3 environment ==="
  date -u
  uname -a
  xcodebuild -version
...
bash scripts/verify-step12-ipa.sh artifacts/StS2-Launcher-Step-12.3.ipa \
  2>&1 | tee artifacts/logs/step12-ipa-verification.log

{
  echo "StS2 Launcher iOS — Step 12.3 verified cache update test"
  ...
  echo "Content access: Step 11 remains the source-acquisition engine; Step 12.3 revalidates an existing final cache directly against the freshly downloaded current Steam manifest before reuse"
  echo "Integrity: Step 12.1 source-generated receipt JSON and Step 12.2/12.2.1 CDN timeout failover/catch ordering are retained; Step 12.3 prevents a stale Step 12 receipt from invalidating a separately Steam-manifest-verified Step 11 cache and strengthens the synthetic update test to force at least one source-file replacement before rollback-safe commit"
  echo "Still absent: multi-depot app composition, compatibility inventory, Godot/runtime execution, Cloud, Workshop"
  echo "IPA: artifacts/StS2-Launcher-Step-12.3.ipa"
  if command -v shasum >/dev/null 2>&1; then
    echo "IPA SHA-256: $(shasum -a 256 artifacts/StS2-Launcher-Step-12.3.ipa | awk '{print $1}')"
  fi
} > artifacts/step12.3-build-summary.txt

cat artifacts/step12.3-build-summary.txt
