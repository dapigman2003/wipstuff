...
public sealed class TransformedRealStS2VeryEarlyInitializationTests
{
    ...
    public void OrderedDiagnosticLocalizationGatesReachFourOfFourWithoutClaimingClosure()
    {
        ...
        Assert.AreEqual("STEP 35.0.32 DIAGNOSTIC LOCALIZATION COMPLETE — 4/4 — NOT STEP 35 CLOSURE", summary.Summary);
    }
    ...
    public void GodotCoreCallbackHandoffRejectsMissingTableBeforeAnyPreflightOrClrWork()
    {
        ...
        Assert.AreEqual(5, (int)Step35DiagnosticMode.GodotCoreModelBootstrapCompatibility);
    }
    ...
    [TestMethod]
    public void GodotCoreModelBootstrapCompatibilityRejectsMissingTableBeforeAnyPreflightOrClrWork()
    {
        using var temp = new TempTestDirectory("sts2-step35-model-bootstrap-callback-validation");
        using var subject = new TransformedRealStS2VeryEarlyInitialization(temp.Path, collectibleLoadContext: true)
        {
            DiagnosticMode = Step35DiagnosticMode.GodotCoreModelBootstrapCompatibility,
        };
        var checkpoints = new List<string>();
    ...
    [TestMethod]
    public void Step59RuntimeTraceMarkersRoundTripAndRetargetIncomingBranchToPreCallMarker()
    {
        using var temp = new TempTestDirectory("sts2-step59-runtime-trace-markers");
        var assemblyPath = Path.Combine(temp.Path, "Step59RuntimeTraceFixture.dll");
        using (var assembly = AssemblyDefinition.CreateAssembly(
                   new AssemblyNameDefinition("Step59RuntimeTraceFixture", new Version(1, 0, 0, 0)),
                   "Step59RuntimeTraceFixture",
                   ModuleKind.Dll))
        {
            var module = assembly.MainModule;
            ...
            emit.Body.GetILProcessor().Append(Instruction.Create(OpCodes.Ret));
            ...
            ping.Body.GetILProcessor().Append(Instruction.Create(OpCodes.Ret));
            helper.Methods.Add(ping);

            var target = new TypeDefinition("Fixture", "Target", Mono.Cecil.TypeAttributes.Public | Mono.Cecil.TypeAttributes.Class, module.TypeSystem.Object);
            module.Types.Add(target);
            var run = new MethodDefinition("Run", Mono.Cecil.MethodAttributes.Public | Mono.Cecil.MethodAttributes.Static, module.TypeSystem.Void);
            target.Methods.Add(run);
            var il = run.Body.GetILProcessor();
            var call = Instruction.Create(OpCodes.Call, ping);
            il.Append(Instruction.Create(OpCodes.Br_S, call));
            il.Append(Instruction.Create(OpCodes.Nop));
            il.Append(call);
            il.Append(Instruction.Create(OpCodes.Ret));

            var inventory = new List<(string MethodFullName, string Marker)>();
            var markerCount = TransformedRealStS2VeryEarlyInitialization.InsertStep59RuntimeTraceMethodMarkers(
                run, emit, "Fixture.Target.Run", inventory);

            Assert.AreEqual(4, markerCount, "Entry + pre-call + post-call + return markers are expected for this fixture.");
            Assert.AreEqual(markerCount, inventory.Count);
            Assert.IsTrue(inventory.All(entry => entry.Marker.StartsWith(TransformedRealStS2VeryEarlyInitialization.Step59RuntimeTraceMarkerPrefix, StringComparison.Ordinal)));
            var branch = run.Body.Instructions.Single(instruction => instruction.OpCode.Code == Code.Br_S);
            Assert.IsInstanceOfType(branch.Operand, typeof(Instruction));
            var branchTarget = (Instruction)branch.Operand;
            Assert.AreEqual(Code.Ldstr, branchTarget.OpCode.Code);
            StringAssert.Contains((string)branchTarget.Operand, "CALL#01 PRE");
            Assert.AreSame(call, run.Body.Instructions[run.Body.Instructions.IndexOf(branchTarget) + 2]);
            Assert.AreEqual(Code.Ldstr, run.Body.Instructions[run.Body.Instructions.IndexOf(call) + 1].OpCode.Code);
            StringAssert.Contains((string)run.Body.Instructions[run.Body.Instructions.IndexOf(call) + 1].Operand, "CALL#01 POST");
        ...
        var serialized = reopened.MainModule.Types.Single(type => type.FullName == "Fixture.Target").Methods.Single(method => method.Name == "Run");
        var serializedMarkers = serialized.Body.Instructions
            .Where(instruction => instruction.OpCode.Code == Code.Ldstr && instruction.Operand is string marker && marker.StartsWith(TransformedRealStS2VeryEarlyInitialization.Step59RuntimeTraceMarkerPrefix, StringComparison.Ordinal))
            .Select(instruction => (string)instruction.Operand)
            .ToArray();
        Assert.AreEqual(4, serializedMarkers.Length);
        Assert.IsTrue(serializedMarkers.Any(marker => marker.Contains("ENTRY", StringComparison.Ordinal)));
        Assert.IsTrue(serializedMarkers.Any(marker => marker.Contains("CALL#01 PRE", StringComparison.Ordinal)));
        Assert.IsTrue(serializedMarkers.Any(marker => marker.Contains("CALL#01 POST", StringComparison.Ordinal)));
        Assert.IsTrue(serializedMarkers.Any(marker => marker.Contains("RET#01", StringComparison.Ordinal)));
    }
