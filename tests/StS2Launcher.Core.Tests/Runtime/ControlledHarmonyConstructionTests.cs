...
[TestClass]
[DoNotParallelize]
public sealed class ControlledHarmonyConstructionTests
{
    [TestMethod]
    public void OrderedHarmonyConstructionGatesReachNineOfNinePass()
    {
        var gates = new ControlledHarmonyConstructionGateSequence();
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.InitializationPreflight, true, "preflight"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.ProvenLoadStateReplay, true, "replay"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.DeferredModuleInitialization, true, "initialize"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.ProvenInitializationAudit, true, "audit"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.HarmonyApiResolution, true, "resolve"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.HarmonyTypeInitialization, true, "type initialize"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.HarmonyTypeInitializationAudit, true, "type-init audit"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.HarmonyInstanceConstruction, true, "construct"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.PostConstructionAudit, true, "final audit"));

        var summary = gates.Snapshot();
        Assert.IsTrue(summary.Passed);
        Assert.AreEqual(9, summary.PassedGates);
        Assert.AreEqual("CONTROLLED HARMONY CONSTRUCTION BOUNDARY PASS — 9/9", summary.Summary);
    }

    [TestMethod]
    public void HarmonyConstructionGatesStopAfterFirstFailure()
    {
        var gates = new ControlledHarmonyConstructionGateSequence();
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.InitializationPreflight, true, "preflight"));
        gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.ProvenLoadStateReplay, false, "replay failed"));
        Assert.ThrowsExactly<InvalidOperationException>(() =>
            gates.Record(new ControlledHarmonyConstructionGateResult(ControlledHarmonyConstructionGate.DeferredModuleInitialization, true, "must not advance")));
        Assert.AreEqual(ControlledHarmonyConstructionGate.ProvenLoadStateReplay, gates.Snapshot().FirstFailingGate);
    }

    [TestMethod]
    public async Task SyntheticStep24ReplayThenExactHarmonyConstructionPasses()
    {
        var names = CreateNames();
        using var temp = new TempTestDirectory("sts2-step25-tests");
        ...
        StringAssert.Contains(gateE.Detail, "Harmony type initializer executed by Step 25: NO");
        ...
        StringAssert.Contains(gateH.Detail, ControlledHarmonyConstruction.HarmonyId);
    ...
    [TestMethod]
    public async Task GateARejectsReachablePInvokeBeforeAnyStep25ClrLoad()
    {
        var names = CreateNames();
        using var temp = new TempTestDirectory("sts2-step25-tests");
    ...
    [TestMethod]
    public async Task GateARejectsFunctionPointerIndirectionBeforeAnyStep25ClrLoad()
    {
        var names = CreateNames();
        using var temp = new TempTestDirectory("sts2-step25-tests");
    ...
    [TestMethod]
    public async Task GateARejectsImplicitTypeInitializerPInvokeBeforeAnyStep25ClrLoad()
    {
        var names = CreateNames();
        using var temp = new TempTestDirectory("sts2-step25-tests");
    ...
    public void GateAMetadataAuditDoesNotResolveExternalBaseForNominallyLocalMemberRef()
    {
        ...
        var reader = typeof(ControlledHarmonyConstruction).GetMethod(
            "ReadPreparedMetadata",
            BindingFlags.NonPublic | BindingFlags.Static)
            ?? throw new AssertFailedException("Step 25 metadata reader was not found.");
    ...
    {
        var decision = ControlledHarmonyConstruction.EvaluateInitializerHazardPolicy(
            ControlledHarmonyConstruction.TargetSimpleName,
            ControlledHarmonyConstruction.TargetVersion,
            ControlledHarmonyConstruction.ObservedMonoModLoggingDispatchHazards,
    ...
    {
        var hazards = ControlledHarmonyConstruction.ObservedMonoModLoggingDispatchHazards
    ...
    public void GateAConditionalMonoModPolicyRejectsNonInertLoggingState()
    {
        ...
        var debugger = ControlledHarmonyConstruction.EvaluateInitializerHazardPolicy(
            ControlledHarmonyConstruction.TargetSimpleName,
            ControlledHarmonyConstruction.TargetVersion,
            ControlledHarmonyConstruction.ObservedMonoModLoggingDispatchHazards,
            audits,
        ...
        var environment = ControlledHarmonyConstruction.EvaluateInitializerHazardPolicy(
            ControlledHarmonyConstruction.TargetSimpleName,
            ControlledHarmonyConstruction.TargetVersion,
            ControlledHarmonyConstruction.ObservedMonoModLoggingDispatchHazards,
            audits,
        ...
        var appContext = ControlledHarmonyConstruction.EvaluateInitializerHazardPolicy(
            ControlledHarmonyConstruction.TargetSimpleName,
            ControlledHarmonyConstruction.TargetVersion,
            ControlledHarmonyConstruction.ObservedMonoModLoggingDispatchHazards,
            audits,
    ...
    [TestMethod]
    public async Task GateARejectsHarmonyTypeInitializerShapeDriftBeforeAnyStep25ClrLoad()
    {
        var names = CreateNames();
        using var temp = new TempTestDirectory("sts2-step25-tests");
    ...
    private static ControlledHarmonyConstruction CreateSyntheticBoundary(string launcherRoot, SyntheticNames names)
        => new(
            launcherRoot,
            collectibleLoadContext: true,
            expectedPrimarySimpleName: names.PrimarySimpleName,
            targetSimpleName: names.TargetSimpleName,
            targetVersion: ControlledHarmonyConstruction.TargetVersion,
    ...
    private static SyntheticNames CreateNames()
    {
        ...
        return new SyntheticNames(
            "StS2Launcher.Step25.SyntheticPrimary." + suffix,
            "StS2Launcher.Step25.SyntheticHarmony." + suffix);
    }
    ...
    private static void AssertNotLoaded(string simpleName)
        => Assert.IsFalse(
            ...
            $"Synthetic Step 25 assembly '{simpleName}' was loaded before the intended gate.");
        ...
        bool driftHarmonyTypeInitializer = false)
    {
        ...
        WriteAssembly(
            ...
            new Version(0, 1, 0, 0),
            [systemLinqReference, new(names.TargetSimpleName, ControlledHarmonyConstruction.TargetVersion, string.Empty)],
        ...
        ICollection<RuntimeBindingEdge> edges)
    {
        ...
        foreach (var reference in module.AssemblyReferences)
        {
            ...
            throw new AssertFailedException("Synthetic Step 25 fixture emitted an unexpected AssemblyRef: " + reference.FullName);
        }
    }
        ...
        bool driftHarmonyTypeInitializer = false)
    {
        ...
        if (initializerKind != InitializerKind.None)
        {
            if (!declaredReferences.TryGetValue("System.Runtime", out var systemRuntimeReference))
                throw new AssertFailedException("Synthetic Step 25 initializer requires predeclared System.Runtime.");
            ...
            switch (initializerKind)
            {
                ...
                case InitializerKind.PInvoke:
                    var pinvokeModule = new ModuleReference("libstep25fixture.dylib");
                ...
                case InitializerKind.TypeInitializerPInvoke:
                    var typeInitModule = new ModuleReference("libstep25fixture-typeinit.dylib");
                    assembly.MainModule.ModuleReferences.Add(typeInitModule);
                    var autoInitType = new TypeDefinition(
                        "StS2Launcher.Step25.Tests",
                        "AutoInitType",
        ...
        {
            if (!declaredReferences.TryGetValue("System.Runtime", out var systemRuntimeReference))
                throw new AssertFailedException("Synthetic Step 25 Harmony type requires predeclared System.Runtime.");
    ...
    private static void WriteExternalBaseMemberRefFixture(string path)
    {
        Directory.CreateDirectory(Path.GetDirectoryName(path)!);
        using var assembly = AssemblyDefinition.CreateAssembly(
            new AssemblyNameDefinition("SyntheticHarmony", ControlledHarmonyConstruction.TargetVersion),
            "SyntheticHarmony",
            ModuleKind.Dll);
        ...
        // Step 25 must instead stop from local metadata with an unresolved-local hazard.
    ...
    private static MethodDefinition CreatePInvokeProbe(TypeReference voidType, ModuleReference nativeModule)
        => new(
            ...
            voidType)
        {
            PInvokeInfo = new PInvokeInfo(Mono.Cecil.PInvokeAttributes.CallConvCdecl, "step25_fixture_probe", nativeModule),
        };
