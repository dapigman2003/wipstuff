...
Active candidate: **0.0.191 (191)**. Physical runtime authority is closed through **Step 57 4/4/frozen**. Steps 58–64 are the active character-select continuation; Step 65 is unopened.
...
The corrected 0.0.191 block follows the exact Step-56 `OpenCharacterSelect(NButton)` body without invoking that handler. Physical 0.0.190 showed that the real stack may already hold an exact off-tree `_characterSelectSubmenu`; the launcher now adopts that game-owned cache instead of requiring null:

- **58** maps exact `NSubmenuStack.GetSubmenuType<NCharacterSelectScreen>()`, records whether the real cache is null or an exact off-tree retained screen, and never invokes the factory.
- **59** acquires the exact off-tree screen: reuse/audit the pre-existing exact cache without invocation when present; only a null cache may take the one-shot factory path.
...
Steps **61, 63, and 64 are always one-shot**. Step 59 is one-shot only when its null-cache factory path is armed. Never retry any armed rung in-process. Original `OpenCharacterSelect`, whole `GameStartup`, original `LaunchMainMenu`, cloud/migration/platform/Steam/deferred startup, character choice/confirm/embark/run-start, native game extensions, trusted-install mutation, and Step 65 remain unopened.
