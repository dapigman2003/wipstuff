# StS2 Launcher iOS — Step 32.0.3 Exact-Length Private IL Patch
...
**Step 32.0.3 / `0.0.118 (118)` — first real StS2 rewrite, now using exact-length private-copy IL patching**
...
0.0.118 therefore uses Cecil only to bind and verify the exact receipt-backed assembly/method/sites. Gate B maps the verified method RVA to PE bytes, confirms each selected instruction is the exact five-byte `call` plus expected metadata token, and changes only those ten five-byte windows on the launcher-private clone:
