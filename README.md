...
Active candidate: **Step 35.0.12 / 0.0.135 (135)** — MaxStack-safe Command-Line / Godot boundary localization.
...
0.0.135 reserves and post-write verifies MaxStack headroom, keeps corrected exact-source NP/CL/CLTV ordinals, and adds four redundant stack-neutral critical markers around `_args` dictionary construction/assignment and `Godot.OS.GetCmdlineArgs()` invocation/result storage. No Godot bootstrap, native game load, resolver broadening, later initialization phase, or runtime Harmony/MonoMod patching is authorized.
