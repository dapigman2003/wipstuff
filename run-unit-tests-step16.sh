...
echo "=== Step 16 host unit tests (Steps 01-15 regressions + managed-preparation ordered gates) ==="
dotnet test "$TEST_PROJECT" \
  -c Release \
  --nologo \
  --results-directory "$RESULTS_DIR" \
  --logger "trx;LogFileName=step16.trx" \
  2>&1 | tee artifacts/logs/step16-unit-tests.log
