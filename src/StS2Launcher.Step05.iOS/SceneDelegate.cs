...
public sealed class SceneDelegate : UIWindowSceneDelegate
{
    ...
    {
        Console.WriteLine("Step 05.10: SceneDelegate.WillConnect");

        if (scene is not UIWindowScene windowScene)
        {
            Console.Error.WriteLine(
                $"Step 05.10: expected UIWindowScene, received {scene.GetType().FullName}");
            return;
        }
        ...
        try
        {
            ...
            Console.WriteLine("Step 05.10: UIWindow is key and visible");
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"Step 05 startup exception: {ex}");
