...
public sealed class SceneDelegate : UIWindowSceneDelegate
{
    ...
    {
        Console.WriteLine("Step 06.3.1: SceneDelegate.WillConnect");

        if (scene is not UIWindowScene windowScene)
        {
            Console.Error.WriteLine(
                $"Step 06.3.1: expected UIWindowScene, received {scene.GetType().FullName}");
            return;
        }
        ...
        try
        {
            ...
            Console.WriteLine("Step 06.3.1: UIWindow is key and visible");
        }
        catch (Exception ex)
        {
            ...
            var label = new UILabel
            {
                ...
                Text = $"STEP 06.3.1 STARTUP ERROR\n\n{ex.GetType().Name}\n{ex.Message}"
            };
