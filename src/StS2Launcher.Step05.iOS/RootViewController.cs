...
public sealed class RootViewController : UIViewController
{
    ...
    public override void ViewDidLoad()
    {
        ...
        content.AddArrangedSubview(Label(
            "22/22 REQUIRED HOST ROOTS • TRANSITIVE DIAGNOSTICS ONLY • NO GAME CLR LOAD",
            UIFont.BoldSystemFontOfSize(14),
            UIColor.SecondaryLabel));

        content.AddArrangedSubview(Label(
            "Step 22.1 physically showed that all 22 measured direct host-binding roots load on iPhone, while 18 of the 22 transitive-only diagnostic expectations do not load by identity or simple name. Those 18 are implementation dependencies referenced only from framework sources such as netstandard/System.Runtime.Serialization/System.Xml, not direct game/private bindings. Step 22.2 therefore keeps the proven 22-root set and corrects Gate A: 22/22 direct roots are required; wider 44-name probes remain diagnostic only. Gate B then recomputes the real sts2.dll graph and is authoritative for residual blockers. Gates C–D still require zero blockers, no private System.*/netstandard assemblies, and Runtime closure ready=YES. StS2 is never CLR-loaded or executed in this step.",
        ...
        content.AddArrangedSubview(Label(
            "Step 22.2 — Host Binding Frontier Correction (ordered gates A–D)",
            UIFont.BoldSystemFontOfSize(25),
            UIColor.Label));

        _runtimeFrameworkBindingButton = SystemButton("Run Step 22.2 A–D — Qualify 22 Roots → Recompute Closure → Prepare Host-Bound Set → Audit", 17);
        ...
        _runtimeBindingDiagnosticsExportResultLabel = Label(
            "DIAGNOSTIC EXPORT: NOT RUN — after Step 22.2 Gate C/D this report reflects the recomputed closure plan",
        ...
        _statusLabel = Label(
            "Status: Step 22.1 Gate A measured 26/44 diagnostic framework names but, critically, 22/22 required direct roots passed and every one of the 18 failures was transitive-only. Step 22.2 corrects Gate A to require the actual 22-name host-binding frontier, then lets the unchanged Step 21 planner determine the real residual blocker set. No real StS2 CLR load should be attempted until A–D prove zero blockers and Runtime closure ready: YES.",
            UIFont.SystemFontOfSize(14),
            UIColor.Label);
        ...
        Console.WriteLine("Step 22.2: RootViewController.ViewDidLoad complete");
    }
