...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2EssentialInitializationControls(UIStackView content)
    {
        ...
        content.AddArrangedSubview(Label(
            "Step 36.0.4 — ModelDb Bootstrap Compatibility + Full ExecuteEssential Attempt",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _step36EssentialButton = SystemButton(
            "Run Step 36.0.4 A–D — Reprove Compatibility Plan → Mount PCK → Invoke Full ExecuteEssential Once → Audit",
            16);
    ...
    private async Task RunTransformedRealStS2EssentialInitializationAsync()
    {
            ...
            _transformedRealStS2VeryEarlyInitialization.DiagnosticMode != Step35DiagnosticMode.GodotCoreModelBootstrapCompatibility)
        {
            ...
            _step36DetailLabel.Text = "Step 36.0.4 requires the same-process MODEL-BOOTSTRAP Step-35 authority. From a fresh launch run Step 15 Gates A-C, then Step 35.0.32 MODEL-BOOTSTRAP once and require 4/4. That mode loads only the verified dependency-aware sts2 derivative with exact prepared GodotSharp; do not run historical EXACT-CLOSURE first because sts2 CLR identity is process-global for this experiment.";
            _statusLabel.Text = "STEP 36.0.4 REFUSED — same-process MODEL-BOOTSTRAP authority is not present.";
        ...
        _statusLabel.Text = $"STEP 36.0.4 RUN {_step36RunId} — MODEL-BOOTSTRAP authority is resident. Gate A re-proves the exact source ExecuteEssential semantics against the selected compatibility derivative and emits the dependency/order plan before any new game-member invocation.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep36Checkpoint("RUN_START — Step 36.0.4 full ExecuteEssential run started after same-process MODEL-BOOTSTRAP Step-35 authority closure.");
            ...
            if (!WriteStep36StaticMap(out var staticMapError))
                throw new IOException("Step 36.0.4 verified static map could not be durably written: " + staticMapError);
            ...
            _statusLabel.Text = "STEP 36.0.4 GATE B — prove MODEL-BOOTSTRAP CLR authority continuity, bind unchanged ExecuteEssential, mount the receipt-backed game PCK additively through exact GodotSharp, and prove res://localization/eng before invocation.";
            ...
            _statusLabel.Text = "STEP 36.0.4 GATE D — final OfflineReady + mounted-PCK presence + exact-source/compatibility-authority/plan/dependency/resolver/context/state reproof.";
            ...
            void FinalizeStep36GateDOnMainThread()
            {
                ...
                if (gateDRecorded)
                {
                    ...
                    _step36DetailLabel.Text =
                        "All four Step 36.0.4 gates passed. The selected dependency-aware ModelDb bootstrap derivative retained source-equivalent ExecuteEssential, the receipt-backed PCK/localization boundary remained positive, unchanged ExecuteEssential returned normally with OneTimeInitialization state 1→2, and the original post-ModelDb sequence (ModelIdSerializationCache.Init → ModelDb.InitIds → MessageTypes.Initialize → ActionTypes.Initialize) therefore also returned in the same call. Final OfflineReady/hash/resolver/context reproof passed. ExecuteDeferred, PrewarmJit, game entry, and native game loading remain separate future gates.";
                    _step36ProgressLabel.Text = "Step 36.0.4 Gate D COMPLETE — PASS";
                    _statusLabel.Text = "STEP 36.0.4 COMPLETE — 4/4. Essential initialization is closed under the bounded ModelDb compatibility authority; preserve artifacts before designing deferred initialization.";
                    _statusLabel.TextColor = UIColor.Label;
        ...
        catch (Exception ex)
        {
            ...
            void ApplyExceptionUi()
            {
                ...
                _step36ResultLabel.TextColor = UIColor.SystemRed;
                _step36DetailLabel.Text = $"Unhandled Step 36.0.4 exception: {ex.GetType().Name}: {ex.Message}";
        ...
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 36.0.4 ModelDb Bootstrap Compatibility + Full ExecuteEssential",
                _step36ResultLabel,
                _step36DetailLabel,
    ...
    private bool RecordStep36Gate(TransformedRealStS2EssentialInitializationGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 36.0.4 FAIL at Gate {letter} ({result.Gate}). Stop here; later gates were not run. Preserve Step36 artifacts and force-quit before retry if Gate C began.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        ...
        lock (_step36CheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 36.0.4 ModelDb-bootstrap ExecuteEssential checkpoint\n" +
                    ...
                    "Candidate: STEP 36.0.4 — DEPENDENCY-AWARE MODELDB BOOTSTRAP + FULL UNCHANGED EXECUTEESSENTIAL\n" +
