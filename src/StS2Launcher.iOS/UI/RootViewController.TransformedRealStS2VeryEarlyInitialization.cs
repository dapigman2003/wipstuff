...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2VeryEarlyInitializationControls(UIStackView content)
    {
        content.AddArrangedSubview(Label(
            "Step 35.0.18 — Godot core callback-handoff probe (four diagnostic modes; ordered gates A–D)",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _step35NaturalGodotReconButton = SystemButton(
            "Run Step 35.0.18 NATURAL — Reverify → Preserve Godot Dictionary/OS Path → Invoke Once",
            16);
        ...
        _transformedRealStS2VeryEarlyInitializationButton = SystemButton(
            "Run Step 35.0.18 OS-RECON — Reverify → Managed Dictionary → Natural Godot.OS → Invoke Once",
            17);
        ...
        _step35ManagedCommandLineCompatibilityButton = SystemButton(
            "Run Step 35.0.18 FORWARD — Reverify → Managed Dictionary + Managed Empty Args → Invoke Once",
            17);
        ...
        _step35GodotCoreCallbackHandoffButton = SystemButton(
            "Run Step 35.0.18 CORE-HANDOFF — Requires Step 15 Engine → Exact Godot Callback Table → NATURAL ExecuteVeryEarly",
            17);
        ...
        _transformedRealStS2VeryEarlyInitializationDetailLabel = Label(
            "Physical 0.0.140 completed the three controls: NATURAL reached GS031 inside the dictionary native thunk; OS-RECON reached Godot.OS..cctor -> StringName.op_Implicit -> GS024; FORWARD cleared both command-line critical boundaries and NP002_POST, then reached GodotFileIo.CreateDirectory -> Godot.DirAccess.DirExistsAbsolute -> StringName -> GS024. That independently reproduces one general GodotSharp/native callback boundary. 0.0.141 preserves those controls and adds CORE-HANDOFF: after the Step 15 smoke engine is live, use its exact source-built Godot 4.5.1 callback table to initialize only the verified private GodotSharp derivative, then run the natural diagnostic path.",
    ...
    private async Task RunTransformedRealStS2VeryEarlyInitializationAsync(Step35DiagnosticMode diagnosticMode)
    {
        ...
        if (!TryInitializeStep35RunTelemetry(out var telemetryError))
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text =
                $"Step 35.0.18 refused to begin Gate A because durable run-correlated telemetry could not be established. No CLR admission or ExecuteVeryEarly invocation was attempted. {telemetryError}";
        ...
        _statusLabel.Text = $"STEP 35.0.18 RUN {_step35RunId} — mode={diagnosticMode}. Gate A re-manufactures/reverifies the closed transform, emits the mode-specific sts2 derivative plus an entry-only GodotSharp derivative, and writes exact-source + Mach-O/native reconnaissance before any CLR admission.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep35CrashCheckpoint($"RUN_START — Step 35.0.18 diagnostic run started; mode={diagnosticMode}; callbackHandoffMode={callbackHandoffMode}; exact transformed source/resolver authority remains frozen. NATURAL/OS-RECON/FORWARD preserve their prior fresh-process contracts. CORE-HANDOFF intentionally requires the already-proven Step-15 smoke engine and retains natural sts2/GodotSharp callsites while supplying only the exact source-built native callback table.");
            ...
            _statusLabel.Text = "STEP 35.0.18 GATE B — re-hash the exact transformed source, then CLR-admit only the separately verified diagnostic clone into the strict execution context and re-check zero-resolution primary admission behavior.";
            ...
            _statusLabel.Text = $"STEP 35.0.18 GATE C — mode={diagnosticMode}; invoke the verified sts2 derivative once. The resolver substitutes only the separately verified GodotSharp diagnostic derivative and arms its entry-only callback before returning it; native game loads, bootstrap expansion beyond the explicit CORE-HANDOFF prerequisite, initializer-bearing and unplanned requests remain fail-closed.";
            ...
            _statusLabel.Text = "STEP 35.0.18 GATE D — re-prove OfflineReady, exact-source/diagnostic-clone/plan/dependency hashes, diagnostic-clone residency, and zero broader startup/native escape.";
        ...
        catch (Exception ex)
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail($"Unhandled Step 35.0.18 diagnostic exception: {ex.GetType().Name}: {ex.Message}");
            _statusLabel.Text = "STEP 35.0.18 DIAGNOSTIC FAIL — preserve the evidence; this derivative failure is not by itself an exact Step-35 compatibility verdict. Force-quit before retry if Gate B started.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 35.0.18 Comprehensive GodotSharp / Native Reconnaissance",
    ...
    private bool RecordTransformedRealStS2VeryEarlyInitializationGate(TransformedRealStS2VeryEarlyInitializationGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 35.0.18 DIAGNOSTIC FAIL at Gate {letter} ({result.Gate}). Stop here; later diagnostic gates were not run. Preserve the report; exact Step 35 remains OPEN. Force-quit before retry.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
    ...
    private string FormatTransformedRealStS2VeryEarlyInitializationDetail(string tail)
    {
        ...
        lines.Add("0.0.141 adds CORE-HANDOFF without changing the prior controls. CORE-HANDOFF requires the already-proven project-owned Step-15 smoke engine in the same process, requires the smoke project to lack the dotnet feature and Godot's own .NET runtime to remain uninitialized, obtains the exact source-built Godot 4.5.1 godotsharp::get_runtime_interop_funcs pointer/size, copies it once through the verified private GodotSharp NativeFuncs.Initialize(IntPtr,int), and only then invokes the NATURAL diagnostic derivative.");
        lines.Add("Exact Step-35 authority remains the natural static parameterless Task-returning MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::ExecuteVeryEarly(), source token 0x06007D02, on the exact closed transformed artifact. Candidate 0.0.141 executes only separately identified diagnostic derivatives for localization/bootstrap-readiness measurement; a diagnostic 4/4 cannot close Step 35.");
        ...
        lines.Add("Forbidden in every Step-35.0.18 mode: receipt-backed/prepared original sts2.dll CLR admission, intentional ExecuteEssential/ExecuteDeferred/PrewarmJit invocation by the launcher, game entry-point execution, Harmony/MonoMod API invocation or runtime patching, native game loading, arbitrary resolver fallback, or broad game startup sequencing. NATURAL/OS-RECON/FORWARD additionally forbid any Godot startup; CORE-HANDOFF permits only the already-proven Step-15 smoke engine plus the exact callback-table handoff described above.");
        ...
        lines.Add("After Gate B, the instrumented diagnostic sts2 clone remains CLR-resident until force-quit on the physical non-collectible context. After Gate C, diagnostic ExecuteVeryEarly has executed once; do not rerun Step 35.0.18 in the same process.");
        lines.Add(tail);
        return string.Join("\n", lines);
    }
        ...
        lock (_step35CrashCheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 35.0.18 Comprehensive GodotSharp / Native reconnaissance crash checkpoint\n" +
                    ...
                    "Candidate: STEP 35.0.18 — GODOT CORE CALLBACK-HANDOFF PROBE\n" +
    ...
    private bool WriteStep35GodotReconnaissance(out string error)
    {
        ...
        try
        {
            ...
            WriteStep35TextFileDurably(
                ...
                $"Expected source version: {CurrentReleasePresentation.ExpectedDisplayVersion} ({CurrentReleasePresentation.ExpectedBuildVersion})\n" +
                $"Diagnostic mode: {_transformedRealStS2VeryEarlyInitialization.DiagnosticMode}\n" +
                "Candidate: STEP 35.0.18 — COMPREHENSIVE GODOTSHARP / NATIVE RECONNAISSANCE + FOUR-MODE CALLBACK-HANDOFF DIAGNOSTIC\n" +
    ...
    private bool WriteStep35StaticMap(out string error)
    {
        ...
        try
        {
            ...
            WriteStep35TextFileDurably(
                ...
                $"App version: {CurrentReleasePresentation.DisplayVersion} ({CurrentReleasePresentation.DisplayBuild})\n" +
                $"Expected source version: {CurrentReleasePresentation.ExpectedDisplayVersion} ({CurrentReleasePresentation.ExpectedBuildVersion})\n" +
                "Candidate: STEP 35.0.18 — same-run exact-source static map + GodotSharp callback-boundary four-mode diagnostic\n" +
