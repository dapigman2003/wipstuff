...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2VeryEarlyInitializationControls(UIStackView content)
    {
        content.AddArrangedSubview(Label(
            "Step 35.0.11 — MaxStack-Safe Command-Line / Godot Boundary Localization (ordered gates A–D; diagnostic clone only)",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _transformedRealStS2VeryEarlyInitializationButton = SystemButton(
            "Run Step 35.0.11 A–D — Reverify → MaxStack-Harden + Critical CommandLine Boundaries → Admit → Invoke Once → Localize/Audit",
            17);
        ...
        _transformedRealStS2VeryEarlyInitializationDetailLabel = Label(
            "Physical 0.0.133 corrected the NP ordinal to NP002 but returned a managed InvalidProgramException before any CommandLineHelper cctor/CL marker executed, then reached normal RUN_END. That is a diagnostic-IL defect, not a new game frontier: the live-stack PRE/POST sweep added one transient evaluation-stack item without increasing the cctor MaxStack header. Step 35.0.11 / 0.0.134 raises and post-write verifies MaxStack, adds redundant stack-neutral critical markers around _args dictionary assignment and Godot.OS.GetCmdlineArgs result storage, and retains the exact-source CL/CLTV maps/sweeps.",
    ...
    private async Task RunTransformedRealStS2VeryEarlyInitializationAsync()
    {
        ...
        if (!TryInitializeStep35RunTelemetry(out var telemetryError))
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text =
                $"Step 35.0.11 refused to begin Gate A because durable run-correlated telemetry could not be established. No CLR admission or ExecuteVeryEarly invocation was attempted. {telemetryError}";
        ...
        _statusLabel.Text = $"STEP 35.0.11 RUN {_step35RunId} — durable telemetry established. Gate A re-manufactures/reverifies the closed transform, writes the same-run static map, then emits and verifies a MaxStack-safe CommandLine diagnostic clone with redundant stack-neutral critical markers; no CLR admission yet.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep35CrashCheckpoint("RUN_START — fresh-process Step 35.0.11 diagnostic run started; exact transformed source/resolver policy remain frozen, while Gate B/C use only a separately verified clone with corrected NP ordinals, explicit MaxStack headroom, stack-neutral critical CommandLine boundaries, and ordered cctor/TryGetValue sweeps.");
            ...
            _statusLabel.Text = "STEP 35.0.11 GATE B — re-hash the exact transformed source, then CLR-admit only the separately verified diagnostic clone into the strict execution context and re-check zero-resolution primary admission behavior.";
            ...
            _statusLabel.Text = "STEP 35.0.11 GATE D — re-prove OfflineReady, exact-source/diagnostic-clone/plan/dependency hashes, diagnostic-clone residency, and zero broader startup/native escape.";
        ...
        catch (Exception ex)
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail($"Unhandled Step 35.0.11 diagnostic exception: {ex.GetType().Name}: {ex.Message}");
            _statusLabel.Text = "STEP 35.0.11 DIAGNOSTIC FAIL — preserve the evidence; this derivative failure is not by itself an exact Step-35 compatibility verdict. Force-quit before retry if Gate B started.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 35.0.11 MaxStack-Safe Command-Line / Godot Boundary Localization",
    ...
    private bool RecordTransformedRealStS2VeryEarlyInitializationGate(TransformedRealStS2VeryEarlyInitializationGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 35.0.11 DIAGNOSTIC FAIL at Gate {letter} ({result.Gate}). Stop here; later diagnostic gates were not run. Preserve the report; exact Step 35 remains OPEN. Force-quit before retry.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
    ...
    private string FormatTransformedRealStS2VeryEarlyInitializationDetail(string tail)
    {
        ...
        lines.Add("Step 35.0.11 diagnostic scope: preserve every prior marker and exact source/resolver/startup authority; reserve one MaxStack slot for every live-stack PRE/POST sweep; post-write verify the CommandLineHelper cctor MaxStack increase; add four stack-neutral critical markers bracketing _args dictionary construction/assignment and Godot.OS.GetCmdlineArgs invocation/result storage; retain corrected NP, CL and CLTV exact-source ordinals. No game/Godot bootstrap or resolver broadening is authorized.");
        lines.Add("0.0.134 critical marker names: INMETHOD_CL_CRITICAL_001_PRE/POST bracket _args dictionary construction/assignment; INMETHOD_CL_CRITICAL_002_PRE/POST bracket Godot.OS.GetCmdlineArgs invocation/result storage. Generic sweeps remain INMETHOD_CLxxx_PRE/POST and INMETHOD_CLTVxxx_PRE/POST.");
        lines.Add("Exact Step-35 authority remains the 0.0.126 contract: the natural managed startup target is static parameterless Task-returning MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::ExecuteVeryEarly(), source token 0x06007D02, on the exact closed transformed artifact. Candidate 0.0.134 executes only a separately identified diagnostic derivative for localization; a diagnostic 4/4 cannot close Step 35.");
        ...
        lines.Add("After Gate B, the instrumented diagnostic sts2 clone remains CLR-resident until force-quit on the physical non-collectible context. After Gate C, diagnostic ExecuteVeryEarly has executed once; do not rerun Step 35.0.11 in the same process.");
        lines.Add(tail);
        return string.Join("\n", lines);
    }
        ...
        lock (_step35CrashCheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 35.0.11 MaxStack-Safe Command-Line / Godot boundary localization crash checkpoint\n" +
                    ...
                    "Candidate: STEP 35.0.11 — MAXSTACK-SAFE COMMAND-LINE/GODOT BOUNDARY LOCALIZATION\n" +
    ...
    private bool WriteStep35StaticMap(out string error)
    {
        ...
        try
        {
            ...
            WriteStep35TextFileDurably(
                ...
                $"App version: {CurrentReleasePresentation.DisplayVersion} ({CurrentReleasePresentation.DisplayBuild})\n" +
                $"Expected source version: {CurrentReleasePresentation.ExpectedDisplayVersion} ({CurrentReleasePresentation.ExpectedBuildVersion})\n" +
                "Candidate: STEP 35.0.11 — same-run exact-source static map + MaxStack-safe NullPlatform/CommandLine diagnostic clone\n" +
