...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2VeryEarlyInitializationControls(UIStackView content)
    {
        content.AddArrangedSubview(Label(
            "Step 35.0.8 — Save/Platform/Godot Native-Boundary Localization (ordered gates A–D; diagnostic clone only)",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _transformedRealStS2VeryEarlyInitializationButton = SystemButton(
            "Run Step 35.0.8 A–D — Reverify → Instrument Save/Platform/Godot Path → Admit → Invoke Once → Localize/Audit",
            17);
        ...
        _transformedRealStS2VeryEarlyInitializationDetailLabel = Label(
            "Physical 0.0.130 proved the Action<string>::Invoke(!0) bridge correction and emitted durable game-body markers through SaveManager.get_Instance plus the nested second TestMode.get_IsOn call. The process then hard-terminated before either settings-init method. Step 35.0.8 / 0.0.131 adds only narrow entry markers through SaveManager.ConstructDefault → UserDataPathProvider → PlatformUtil → NullPlatformUtilStrategy → GodotFileIo and verified pre/post callsite markers around Godot.DirAccess.DirExistsAbsolute and MakeDirRecursiveAbsolute. Godot startup and resolver authority remain unchanged and forbidden beyond the existing contract.",
    ...
    private async Task RunTransformedRealStS2VeryEarlyInitializationAsync()
    {
        ...
        if (!TryInitializeStep35RunTelemetry(out var telemetryError))
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text =
                $"Step 35.0.8 refused to begin Gate A because durable run-correlated telemetry could not be established. No CLR admission or ExecuteVeryEarly invocation was attempted. {telemetryError}";
        ...
        _statusLabel.Text = $"STEP 35.0.8 RUN {_step35RunId} — durable telemetry established. Gate A re-manufactures/reverifies the closed transform, writes the same-run static map, then emits and verifies an instrumented diagnostic clone; no CLR admission yet.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep35CrashCheckpoint("RUN_START — fresh-process Step 35.0.8 diagnostic run started; exact transformed source/resolver policy remain frozen, while Gate B/C use only a separately verified clone with Save/Platform/Godot entry and callsite markers.");
            ...
            _statusLabel.Text = "STEP 35.0.8 GATE B — re-hash the exact transformed source, then CLR-admit only the separately verified diagnostic clone into the strict execution context and re-check zero-resolution primary admission behavior.";
            ...
            _statusLabel.Text = "STEP 35.0.8 GATE C — bind the diagnostic clone's ExecuteVeryEarly(), arm durable Save/Platform/Godot entry + DirAccess pre/post checkpoints, invoke once, await up to 60s, and fail closed on initializer-bearing/unplanned/native requests.";
            ...
            _statusLabel.Text = "STEP 35.0.8 GATE D — re-prove OfflineReady, exact-source/diagnostic-clone/plan/dependency hashes, diagnostic-clone residency, and zero broader startup/native escape.";
        ...
        catch (Exception ex)
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail($"Unhandled Step 35.0.8 diagnostic exception: {ex.GetType().Name}: {ex.Message}");
            _statusLabel.Text = "STEP 35.0.8 DIAGNOSTIC FAIL — preserve the evidence; this derivative failure is not by itself an exact Step-35 compatibility verdict. Force-quit before retry if Gate B started.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 35.0.8 Diagnostic Clone In-Method Localization",
    ...
    private bool RecordTransformedRealStS2VeryEarlyInitializationGate(TransformedRealStS2VeryEarlyInitializationGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 35.0.8 DIAGNOSTIC FAIL at Gate {letter} ({result.Gate}). Stop here; later diagnostic gates were not run. Preserve the report; exact Step 35 remains OPEN. Force-quit before retry.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
    ...
    private string FormatTransformedRealStS2VeryEarlyInitializationDetail(string tail)
    {
        ...
        lines.Add("Exact sts2.dll analysis for 0.0.131 localizes the normal non-test branch further: SaveManager.ConstructDefault() → UserDataPathProvider.GetAccountScopedBasePath(...) → PlatformUtil.get_PrimaryPlatform() with PlatformUtil..cctor → NullPlatformUtilStrategy..ctor → GodotFileIo..ctor → GodotFileIo.CreateDirectory(string). CreateDirectory contains exactly one Godot.DirAccess.DirExistsAbsolute(string) call and, if the directory is absent, exactly one MakeDirRecursiveAbsolute(string) call.");
        lines.Add("Step 35.0.8 diagnostic scope: preserve the exact closed transformed source and resolver authority; add entry markers for that Save/Platform/Godot path plus serialized pre/post markers immediately around both Godot.DirAccess calls. The exact transformed source bytes are never overwritten or CLR-loaded, and Godot native/bootstrap startup is not added or authorized.");
        lines.Add("Exact Step-35 authority remains the 0.0.126 contract: the natural managed startup target is static parameterless Task-returning MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::ExecuteVeryEarly(), source token 0x06007D02, on the exact closed transformed artifact. Candidate 0.0.131 executes only a separately identified diagnostic derivative for localization; a diagnostic 4/4 cannot close Step 35.");
        ...
        lines.Add("After Gate B, the instrumented diagnostic sts2 clone remains CLR-resident until force-quit on the physical non-collectible context. After Gate C, diagnostic ExecuteVeryEarly has executed once; do not rerun Step 35.0.8 in the same process.");
        lines.Add(tail);
        return string.Join("\n", lines);
    }
        ...
        lock (_step35CrashCheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 35.0.8 in-method localization crash checkpoint\n" +
                    ...
                    "Candidate: STEP 35.0.8 — SAVE/PLATFORM/GODOT NATIVE-BOUNDARY LOCALIZATION\n" +
                    "Execution policy: exact source transform/resolver/later-boundary prohibitions unchanged; Gate B/C execute only a separately verified diagnostic clone with output-only entry and selected Godot callsite markers.\n" +
    ...
    private bool WriteStep35StaticMap(out string error)
    {
        ...
        try
        {
            ...
            WriteStep35TextFileDurably(
                ...
                $"App version: {CurrentReleasePresentation.DisplayVersion} ({CurrentReleasePresentation.DisplayBuild})\n" +
                $"Expected source version: {CurrentReleasePresentation.ExpectedDisplayVersion} ({CurrentReleasePresentation.ExpectedBuildVersion})\n" +
                "Candidate: STEP 35.0.8 — same-run exact-source static map + Save/Platform/Godot diagnostic-clone localization\n" +
