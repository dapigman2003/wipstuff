...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2VeryEarlyInitializationControls(UIStackView content)
    {
        content.AddArrangedSubview(Label(
            "Step 35.0.4 — In-Method Pre-First-Await Localization (ordered gates A–D; diagnostic clone only)",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _transformedRealStS2VeryEarlyInitializationButton = SystemButton(
            "Run Step 35.0.4 A–D — Correlate Run → Reverify + Instrument Clone → Admit → Invoke Once → Localize/Audit",
            17);
        ...
        _transformedRealStS2VeryEarlyInitializationDetailLabel = Label(
            "Physical 0.0.126 produced a fully same-run journal/map/manifest set and again stopped inside synchronous ExecuteVeryEarly execution after the final planned System.Collections.Concurrent 8→9 host resolution, with no C_INVOKE_RETURNED. Step 35.0.4 / 0.0.127 preserves the exact source transform, resolver plan, timeout and later-boundary prohibitions, but executes a separately generated diagnostic clone whose selected pre-first-await methods and relevant type initializers emit durable INMETHOD_* entry markers through a launcher callback. The exact Step-32 transformed source remains unchanged on disk and is re-hashed before the diagnostic clone is admitted.",
    ...
    private async Task RunTransformedRealStS2VeryEarlyInitializationAsync()
    {
        ...
        if (!TryInitializeStep35RunTelemetry(out var telemetryError))
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text =
                $"Step 35.0.4 refused to begin Gate A because durable run-correlated telemetry could not be established. No CLR admission or ExecuteVeryEarly invocation was attempted. {telemetryError}";
        ...
        _statusLabel.Text = $"STEP 35.0.4 RUN {_step35RunId} — durable telemetry established. Gate A re-manufactures/reverifies the closed transform, writes the same-run static map, then emits and verifies an instrumented diagnostic clone; no CLR admission yet.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep35CrashCheckpoint("RUN_START — fresh-process Step 35.0.4 diagnostic run started; exact transformed source/resolver policy remain frozen, while Gate B/C will use a separately verified in-method-instrumented diagnostic clone.");
            ...
            _statusLabel.Text = "STEP 35.0.4 GATE B — re-hash the exact transformed source, then CLR-admit only the separately verified diagnostic clone into the strict execution context and re-check zero-resolution primary admission behavior.";
            ...
            _statusLabel.Text = "STEP 35.0.4 GATE C — bind the diagnostic clone's ExecuteVeryEarly(), arm durable in-method checkpoints, invoke once, await its Task up to 60s, and fail closed on initializer-bearing/unplanned/native requests.";
            ...
            _statusLabel.Text = "STEP 35.0.4 GATE D — re-prove OfflineReady, exact-source/diagnostic-clone/plan/dependency hashes, diagnostic-clone residency, and zero broader startup/native escape.";
        ...
        catch (Exception ex)
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail($"Unhandled Step 35.0.4 diagnostic exception: {ex.GetType().Name}: {ex.Message}");
            _statusLabel.Text = "STEP 35.0.4 DIAGNOSTIC FAIL — preserve the evidence; this derivative failure is not by itself an exact Step-35 compatibility verdict. Force-quit before retry if Gate B started.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 35.0.4 Diagnostic Clone In-Method Localization",
    ...
    private bool RecordTransformedRealStS2VeryEarlyInitializationGate(TransformedRealStS2VeryEarlyInitializationGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 35.0.4 DIAGNOSTIC FAIL at Gate {letter} ({result.Gate}). Stop here; later diagnostic gates were not run. Preserve the report; exact Step 35 remains OPEN. Force-quit before retry.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
    ...
    private string FormatTransformedRealStS2VeryEarlyInitializationDetail(string tail)
    {
        ...
        lines.Add("Step 35.0.4 diagnostic scope: preserve the exact closed transformed source and resolver authority, create a separate identity/MVID-preserving diagnostic clone, inject durable entry markers into ExecuteVeryEarly.MoveNext plus the top-level pre-first-await game methods and relevant type initializers, arm one launcher-owned Action<string> callback immediately before Invoke, and use the last INMETHOD_* record to localize the hard kill. Exact transformed source bytes are never overwritten or CLR-loaded in this candidate.");
        lines.Add("Exact Step-35 authority remains the 0.0.126 contract: the natural managed startup target is static parameterless Task-returning MegaCrit.Sts2.Core.Helpers.OneTimeInitialization::ExecuteVeryEarly(), source token 0x06007D02, on the exact closed transformed artifact. Candidate 0.0.127 does not execute that exact artifact; it uses a separately identified diagnostic derivative only for localization.");
        ...
        lines.Add("After Gate B, the instrumented diagnostic sts2 clone remains CLR-resident until force-quit on the physical non-collectible context. After Gate C, diagnostic ExecuteVeryEarly has executed once; do not rerun Step 35.0.4 in the same process.");
        lines.Add(tail);
        return string.Join("\n", lines);
    }
        ...
        lock (_step35CrashCheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 35.0.4 in-method localization crash checkpoint\n" +
                    ...
                    "Candidate: STEP 35.0.4 — IN-METHOD PRE-FIRST-AWAIT LOCALIZATION\n" +
                    "Execution policy: exact source transform/resolver/later-boundary prohibitions unchanged; Gate B/C execute only a separately verified diagnostic clone with output-only in-method entry markers.\n" +
    ...
    private bool WriteStep35StaticMap(out string error)
    {
        ...
        try
        {
            ...
            WriteStep35TextFileDurably(
                ...
                $"App version: {CurrentReleasePresentation.DisplayVersion} ({CurrentReleasePresentation.DisplayBuild})\n" +
                $"Expected source version: {CurrentReleasePresentation.ExpectedDisplayVersion} ({CurrentReleasePresentation.ExpectedBuildVersion})\n" +
                "Candidate: STEP 35.0.4 — same-run exact-source static map + instrumented diagnostic-clone localization\n" +
