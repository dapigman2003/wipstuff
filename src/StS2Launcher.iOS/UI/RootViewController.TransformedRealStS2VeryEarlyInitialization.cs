...
public sealed partial class RootViewController
{
    ...
    private UIButton? _step35GodotModelBootstrapCompatibilityButton;
    ...
    private void AddTransformedRealStS2VeryEarlyInitializationControls(UIStackView content)
    {
        content.AddArrangedSubview(Label(
            "Step 35.0.32 — ModelDb bootstrap compatibility authority (existing diagnostics + exact closure + one Step-36 compatibility mode; ordered gates A–D)",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _step35NaturalGodotReconButton = SystemButton(
            "Run Step 35.0.32 NATURAL diagnostic — Reverify → Preserve Godot Dictionary/OS Path → Invoke Once",
            16);
        ...
        _transformedRealStS2VeryEarlyInitializationButton = SystemButton(
            "Run Step 35.0.32 OS-RECON diagnostic — Reverify → Managed Dictionary → Natural Godot.OS → Invoke Once",
            17);
        ...
        _step35ManagedCommandLineCompatibilityButton = SystemButton(
            "Run Step 35.0.32 FORWARD diagnostic — Reverify → Managed Dictionary + Managed Empty Args → Invoke Once",
            17);
        ...
        _step35GodotCoreCallbackHandoffButton = SystemButton(
            "Run Step 35.0.32 CORE-HANDOFF diagnostic — Step 15 → Proven Bridge → Diagnostic sts2/GodotSharp",
            17);
        ...
        _step35GodotExactClosureButton = SystemButton(
            "Run Step 35.0.32 EXACT-CLOSURE — Step 15 → Exact Transformed sts2 + Exact GodotSharp → Proven Bridge → Invoke Exact Authority",
            16);
    ...
    private async Task RunTransformedRealStS2VeryEarlyInitializationAsync(Step35DiagnosticMode diagnosticMode)
    {
        if (_transformedRealStS2VeryEarlyInitializationResultLabel is null ||
            ...
            _step35GodotModelBootstrapCompatibilityButton is null ||
        ...
        var callbackHandoffMode = diagnosticMode is Step35DiagnosticMode.GodotCoreCallbackHandoff or Step35DiagnosticMode.GodotCoreExactClosure or Step35DiagnosticMode.GodotCoreModelBootstrapCompatibility;
        var exactAuthorityMode = diagnosticMode == Step35DiagnosticMode.GodotCoreExactClosure;
        var modelBootstrapMode = diagnosticMode == Step35DiagnosticMode.GodotCoreModelBootstrapCompatibility;
        var step36AuthorityMode = exactAuthorityMode || modelBootstrapMode;
        var authorityLabel = modelBootstrapMode ? "MODEL-BOOTSTRAP" : exactAuthorityMode ? "EXACT-CLOSURE" : "CORE-HANDOFF";
        if (callbackHandoffMode)
        {
            ...
            {
                _statusLabel.Text = $"{authorityLabel} requires the Step 15 embedded smoke engine to be started and setup-complete in this same process. Run Step 15 Gates A-C first, then return directly to this button without force-quitting.";
                _statusLabel.TextColor = UIColor.SystemOrange;
                return;
            }
            if (!GodotStep15NativeBridge.IsRuntimeInteropReady)
            {
                _statusLabel.Text = $"{authorityLabel} refused: the source-built Godot C# native callback table is not ready. Native detail: {GodotStep15NativeBridge.LastError}";
                _statusLabel.TextColor = UIColor.SystemRed;
                return;
            }
            if (GodotStep15NativeBridge.HasDotNetFeature || GodotStep15NativeBridge.IsDotNetRuntimeInitialized)
            {
                _statusLabel.Text = $"{authorityLabel} refused: the embedded smoke project advertises the dotnet feature or Godot reports its own .NET runtime initialized. This path requires native Godot/C# scaffolding but no competing Godot-managed runtime.";
                _statusLabel.TextColor = UIColor.SystemRed;
                return;
            }
        }
        ...
        if (!TryInitializeStep35RunTelemetry(out var telemetryError))
        {
            ...
            _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
            _transformedRealStS2VeryEarlyInitializationDetailLabel.Text =
                $"Step 35.0.32 refused to begin Gate A because durable run-correlated telemetry could not be established. No CLR admission or ExecuteVeryEarly invocation was attempted. {telemetryError}";
        ...
        _statusLabel.Text = $"STEP 35.0.32 RUN {_step35RunId} — mode={diagnosticMode}; exactAuthority={exactAuthorityMode}; modelBootstrap={modelBootstrapMode}. Gate A re-manufactures/reverifies the closed transform and emits diagnostic derivatives for static/reconnaissance evidence. Gate B will admit {(modelBootstrapMode ? "the verified dependency-aware ModelDb bootstrap derivative" : exactAuthorityMode ? "the exact closed transformed sts2 bytes" : "the selected diagnostic sts2 derivative")}.";
        ...
        try
        {
            ...
            var progress = new Progress<TransformedRealStS2VeryEarlyInitializationProgress>(value =>
            {
                if (value.Gate == TransformedRealStS2VeryEarlyInitializationGate.FinalIsolationAudit)
                {
                    ...
                    // never resumed the Gate-D await; 0.0.152 removes that avoidable UI work and adds
            ...
            _statusLabel.Text = modelBootstrapMode
                ? "STEP 35.0.32 GATE D — compatibility-authority final reproof: OfflineReady, exact source plus ModelDb-bootstrap derivative hash, prepared dependency hashes, residency, and zero broader startup/native escape."
                : exactAuthorityMode
                    ? "STEP 35.0.32 GATE D — exact-authority final reproof: OfflineReady, exact transformed CLR input, plan/dependency hashes, exact residency, and zero broader startup/native escape."
                    : "STEP 35.0.32 GATE D — re-prove OfflineReady, exact-source/diagnostic-clone/plan/dependency hashes, diagnostic-clone residency, and zero broader startup/native escape.";
            WriteStep35CrashCheckpoint($"D_START — entering final isolation audit; exactAuthority={exactAuthorityMode}; modelBootstrap={modelBootstrapMode}.");
            WriteStep35CrashCheckpoint("D_WORKER_SCHEDULE — scheduling the Gate-D audit behind an outer Task.Run boundary so UIKit SynchronizationContext is not responsible for completing the core audit Task.");
            var gateD = await Task.Run(async () =>
            {
                var result = await _transformedRealStS2VeryEarlyInitialization
                    .RunFinalIsolationAuditAsync(progress, token, WriteStep35CrashCheckpoint)
                    .ConfigureAwait(false);
                WriteStep35CrashCheckpoint($"D_WORKER_RETURN — outer Gate-D worker observed core result; passed={result.Passed}; exactAuthority={exactAuthorityMode}; modelBootstrap={modelBootstrapMode}.");
                return result;
            }, token).ConfigureAwait(false);
            WriteStep35CrashCheckpoint($"D_THREADPOOL_CONTINUATION — outer Gate-D Task completed without recapturing UIKit SynchronizationContext; passed={gateD.Passed}; exactAuthority={exactAuthorityMode}; modelBootstrap={modelBootstrapMode}; managedThread={Environment.CurrentManagedThreadId}; isMain={NSThread.IsMain}.");

            var gateDRecorded = false;
            void FinalizeGateDOnMainThread()
            {
                WriteStep35CrashCheckpoint($"D_UI_DISPATCH_ENTER — explicit main-thread Gate-D finalization entered; managedThread={Environment.CurrentManagedThreadId}; isMain={NSThread.IsMain}.");
                CompleteStep35GateDProgress(gateD.Passed, modelBootstrapMode ? "ModelDb-bootstrap authority" : exactAuthorityMode ? "exact transformed authority" : "diagnostic derivative");
                WriteStep35CrashCheckpoint($"D_RESULT — passed={gateD.Passed}; gate={gateD.Gate}.");
                gateDRecorded = RecordTransformedRealStS2VeryEarlyInitializationGate(gateD);
                if (!gateDRecorded)
                {
                    WriteStep35CrashCheckpoint("D_RESULT_RECORD_FAIL_STOP — Gate D result was recorded as FAIL on the explicit main-thread dispatch.");
                    return;
                }
                WriteStep35CrashCheckpoint("D_RESULT_RECORD_PASS — Gate D PASS recorded in the ordered gate sequence on the explicit main-thread dispatch.");

                var snapshot = _transformedRealStS2VeryEarlyInitializationGates.Snapshot();
                _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.Label;
                if (step36AuthorityMode)
                {
                    if (modelBootstrapMode)
                    {
                        _transformedRealStS2VeryEarlyInitializationResultLabel.Text = "STEP 36 MODEL-BOOTSTRAP AUTHORITY: READY — 4/4";
                        _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail(
                            "All four compatibility-authority gates completed. Gate B admitted the statically verified dependency-aware ModelDb bootstrap derivative while preserving the exact Step-32 source; exact prepared GodotSharp received the physically proven source-built Godot 4.5.1 bridge; unchanged ExecuteVeryEarly returned successfully; Gate D re-proved OfflineReady, hashes, resolver/native confinement, and CLR ownership. Proceed directly to Step 36.0.3 in this same process.");
                        _statusLabel.Text = "MODEL-BOOTSTRAP AUTHORITY READY — 4/4. Run Step 36.0.3 once now; do not re-run Step 35 in this process.";
                    ...
                    else
                    {
                        ...
                        _statusLabel.Text = "EXACT STEP 35 CLOSURE CANDIDATE COMPLETE — 4/4. Preserve the run-correlated report/checkpoints before advancing to Step 36.";
                ...
                else
                {
                    ...
                    _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail(
                        "All four Step 35.0.32 diagnostic gates completed. Preserve this report, but do not use it as Step-35 closure evidence: Gate B/C executed an instrumented derivative rather than the exact closed Step-32 transformed bytes. The diagnostic 4/4 remains localization evidence; use a fresh process and the EXACT-CLOSURE button for the exact-authority attempt.");
                    _statusLabel.Text = "DIAGNOSTIC COMPLETE: STEP 35.0.32 — 4/4. Use a fresh process + Step 15 + MODEL-BOOTSTRAP for the Step-36 forward attempt.";
                    WriteStep35CrashCheckpoint("RUN_DIAGNOSTIC_4OF4 — all Step-35.0.32 diagnostic gates completed; derivative result remains diagnostic evidence only.");
                }
                _statusLabel.TextColor = UIColor.Label;
                WriteStep35CrashCheckpoint("D_UI_DISPATCH_RETURN — explicit main-thread Gate-D finalization completed.");
            }

            if (NSThread.IsMain)
                FinalizeGateDOnMainThread();
            else
                InvokeOnMainThread(FinalizeGateDOnMainThread);
        ...
        catch (OperationCanceledException)
        {
            ...
            void ApplyCancelledUi()
            {
                ...
                _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail(
                    $"Step 35.0.32 was cancelled and is INCONCLUSIVE. If Gate B had begun, force-quit before retrying because the selected {(modelBootstrapMode ? "ModelDb-bootstrap authority" : exactAuthorityMode ? "exact transformed authority" : "diagnostic derivative")} and dependencies may now be CLR-resident; if Gate C invocation began, ExecuteVeryEarly may also have executed despite cancellation.");
                _statusLabel.Text = step36AuthorityMode
                    ? $"STEP 35.0.32 {authorityLabel} CANCELLED / INCONCLUSIVE — no authority verdict; force-quit before retry if Gate B or C started."
                    : "STEP 35.0.32 DIAGNOSTIC CANCELLED / INCONCLUSIVE — exact Step 35 remains OPEN; force-quit before retry if Gate B or C started.";
        ...
        catch (Exception ex)
        {
            ...
            void ApplyExceptionUi()
            {
                ...
                _transformedRealStS2VeryEarlyInitializationResultLabel.TextColor = UIColor.SystemRed;
                _transformedRealStS2VeryEarlyInitializationDetailLabel.Text = FormatTransformedRealStS2VeryEarlyInitializationDetail($"Unhandled Step 35.0.32 {(modelBootstrapMode ? "model-bootstrap-authority" : exactAuthorityMode ? "exact-authority" : "diagnostic")} exception: {ex.GetType().Name}: {ex.Message}");
                _statusLabel.Text = step36AuthorityMode
                    ? $"STEP 35.0.32 {authorityLabel} FAIL — preserve the authority checkpoints; force-quit before retry if Gate B started."
                    : "STEP 35.0.32 DIAGNOSTIC FAIL — preserve the evidence; this derivative failure is not by itself an exact Step-35 compatibility verdict. Force-quit before retry if Gate B started.";
                _statusLabel.TextColor = UIColor.SystemRed;
            }
        ...
        {
            WriteStep35CrashCheckpoint($"RUN_FINALLY_ENTER — managed control reached the Step-35 finally block; managedThread={Environment.CurrentManagedThreadId}; isMain={NSThread.IsMain}; writing the normal deterministic report without requiring a captured UIKit continuation.");
            if (NSThread.IsMain)
                StopStep35GateDHeartbeat();
            else
                InvokeOnMainThread(StopStep35GateDHeartbeat);

            await WriteDeviceTestReportFromLabelsAsync(
                "Step35-TransformedRealStS2VeryEarlyInitialization.txt",
                "StS2 Launcher — Step 35.0.32 Exact-Authority Closure + Explicit Main-Queue Finalization",
            ...
            WriteStep35CrashCheckpoint("RUN_END — Step-35 UI operation ended normally after explicit main-thread teardown.");
        }
    }
    ...
    private bool RecordTransformedRealStS2VeryEarlyInitializationGate(TransformedRealStS2VeryEarlyInitializationGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 35.0.32 FAIL at Gate {letter} ({result.Gate}). Stop here; later gates were not run. Preserve the run-correlated evidence and force-quit before retry.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
    ...
    private string FormatTransformedRealStS2VeryEarlyInitializationDetail(string tail)
    {
        ...
        lines.Add("Physical 0.0.153 exact-authority evidence: exact ExecuteVeryEarly returned RanToCompletion; Gate D re-proved OfflineReady 428/428 and all authority/plan/dependency/resolver/context checks; the core returned passed=true/exactAuthority=true; and the outer worker durably recorded D_WORKER_RETURN. The remaining defect is only the captured UIKit continuation. 0.0.154 removes that capture with ConfigureAwait(false) and explicit main-thread finalization without changing core authority behavior.");
