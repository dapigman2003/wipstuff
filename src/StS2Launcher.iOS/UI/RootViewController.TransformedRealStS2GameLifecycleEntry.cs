...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2GameLifecycleEntryControls(UIStackView content)
    {
        ...
        content.AddArrangedSubview(Label(
            "Step 38.2 — iOS/off-tree-compatible NGame._EnterTree",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _step38LifecycleButton = SystemButton(
            "Run Step 38.2 A–D — Verify iOS/off-tree lifecycle derivative → Reinstantiate NGame → Invoke _EnterTree once → Confinement/release",
            16);
        ...
        _step38DetailLabel = Label(
            "Physical 0.0.164 passed Gate A/B and physically entered NGame._EnterTree, then failed with a managed NullReferenceException while the diagnostic NGame was intentionally off-tree. Step 38.2 keeps the proven inert GameStartupWrapper and preserves all singleton/property/GetNode setup, but NOP-substitutes exactly the early SentryService.Initialize call and the bounded GetWindow/FilesDropped/Connect block that is invalid without a containing SceneTree Window. Gate A verifies those exact serialized suppressions plus the inert wrapper and still rejects _Ready, GameStartup, InitializePlatform, LaunchMainMenu, deferred startup, or OneTimeInitialization re-entry. Gate B re-instantiates the proven FMOD-neutral scene off-tree. Gate C invokes the remaining compatibility NGame._EnterTree body once by MethodInfo. Gate D proves the node is still off-tree/state=2 with zero initializer-bearing/rejected/native activity, then frees it. No AddChild, _Ready, _ExitTree, GameStartup, InitializePlatform, main-menu, ExecuteDeferred, Steam initialization, native GDExtension loading, or gameplay is authorized.",
    ...
    private async Task RunTransformedRealStS2GameLifecycleEntryAsync()
    {
        ...
        if (!_step37Gates.Snapshot().Passed || !_transformedRealStS2VeryEarlyInitialization.ExactStep37ClosurePassed)
        {
            ...
            _step38DetailLabel.Text = "Step 38.2 requires the same-process physical Step-37.0.1 4/4 closure. From a fresh launch run Step 15 A-C, Step 35.0.32 MODEL-BOOTSTRAP 4/4, Step 36.0.5 4/4, then Step 37.0.1 4/4 before running Step 38 once.";
            _statusLabel.Text = "STEP 38.2 REFUSED — same-process Step-37.0.1 4/4 authority is not present.";
        ...
        _statusLabel.Text = $"STEP 38.2 RUN {_step38RunId} — Step 37 is closed. Gate A must verify the serialized inert GameStartupWrapper plus zero remaining Sentry/GetWindow/FilesDropped/Connect edges before any lifecycle method is invoked.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep38Checkpoint("RUN_START — Step 38.2 iOS/off-tree-compatible NGame._EnterTree experiment started after same-process Step-37.0.1 4/4 closure.");
            ...
            if (!WriteStep38StaticMap(out var staticMapError))
                throw new IOException("Step 38.2 verified lifecycle static map could not be durably written: " + staticMapError);
            ...
            _statusLabel.Text = "STEP 38.2 GATE B — reinstantiate the physically proven FMOD-neutral PackedScene off-tree and bind the verified Step-38.2 NGame._EnterTree. No lifecycle call yet.";
            ...
            _statusLabel.Text = "STEP 38.2 GATE C — invoke the verified Step-38.2 NGame._EnterTree exactly once on the off-tree instance. No AddChild/_Ready/GameStartup/platform/main-menu/deferred authorization.";
            ...
            _statusLabel.Text = "STEP 38.2 GATE D — prove post-_EnterTree confinement while still off-tree, then free the temporary node without invoking _ExitTree.";
            ...
            _step38DetailLabel.Text =
                "All four Step 38.2 gates passed. The selected compatibility authority contained the exact inert GameStartupWrapper, zero remaining SentryService.Initialize call, and zero remaining GetWindow/FilesDropped/Connect references in NGame._EnterTree; all singleton/property/GetNode setup remained admitted; the same-NGame closure contained no _Ready/GameStartup/platform/main-menu/deferred boundary; a fresh real NGame hierarchy was instantiated off-tree; the verified compatibility _EnterTree body returned once while IsInsideTree remained false; and final state/native/resolver confinement held before the node was released. SceneTree insertion, _Ready, _ExitTree, GameStartup, InitializePlatform, LaunchMainMenu, ExecuteDeferred, Steam/native extensions, and gameplay remain future boundaries.";
            _statusLabel.Text = "STEP 38.2 COMPLETE — 4/4. Controlled NGame._EnterTree is physically closed; preserve Step38 artifacts before authorizing any real SceneTree/_Ready boundary.";
            _statusLabel.TextColor = UIColor.Label;
            WriteStep38Checkpoint("RUN_STEP38_4OF4 — verified Step-38.2 compatibility NGame._EnterTree returned once through the inert GameStartupWrapper with Sentry/file-drop-window edges suppressed on an off-tree instance and confinement/release completed; no _Ready/GameStartup/platform/main-menu/deferred/native boundary crossed.");
        }
        catch (Exception ex)
        {
            ...
            _step38ResultLabel.TextColor = UIColor.SystemRed;
            _step38DetailLabel.Text = $"Unhandled Step 38.2 exception: {ex.GetType().Name}: {ex.Message}";
        ...
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 38.2 iOS/off-tree-compatible NGame _EnterTree Entry",
                _step38ResultLabel,
                _step38DetailLabel,
    ...
    private bool RecordStep38Gate(TransformedRealStS2GameLifecycleEntryGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 38.2 FAIL at Gate {letter} ({result.Gate}). Stop here; later gates were not run. Preserve Step38 artifacts and use a fresh process before retry after Gate C begins.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        ...
        lock (_step38CheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 38.2 controlled NGame._EnterTree checkpoint\n" +
                    ...
                    "Candidate: STEP 38.2 — VERIFIED IOS/OFFTREE LIFECYCLE COMPAT + STATIC MAP + OFF-TREE NGAME _ENTERTREE ONCE + CONFINEMENT/RELEASE\n" +
