...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2GameSceneAdmissionControls(UIStackView content)
    {
        ...
        content.AddArrangedSubview(Label(
            "Step 37.0.1 — Controlled game.tscn admission (FMOD-neutral, off-tree)",
            UIFont.BoldSystemFontOfSize(18),
            UIColor.Label));

        _step37GameSceneButton = SystemButton(
            "Run Step 37.0.1 A–D — Seal game.tscn → Prepare FMOD-neutral Copy → Load PackedScene → Instantiate Off-tree",
            16);
    ...
    private async Task RunTransformedRealStS2GameSceneAdmissionAsync()
    {
        ...
        if (!_transformedRealStS2VeryEarlyInitialization.ExactStep36ClosurePassed)
        {
            ...
            _step37DetailLabel.Text = "Step 37.0.1 requires the same-process physical Step-36.0.5 4/4 closure. From a fresh launch run Step 15 A-C, then Step 35.0.32 MODEL-BOOTSTRAP 4/4, then Step 36.0.5 A-D 4/4. Do not use a historical exact-closure authority or skip Step 36.";
            _statusLabel.Text = "STEP 37.0.1 REFUSED — same-process Step-36.0.5 4/4 authority is not present.";
        ...
        _statusLabel.Text = $"STEP 37.0.1 RUN {_step37RunId} — Step 36 is closed. Gate A reads only the PCK directory plus sealed game.tscn bytes; no scene load or native extension is allowed.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep37Checkpoint("RUN_START — Step 37.0.1 controlled game-scene admission started after same-process Step-36.0.5 4/4 closure.");
            ...
            if (!WriteStep37StaticMap(out var staticMapError))
                throw new IOException("Step 37.0.1 verified static map could not be durably written: " + staticMapError);
            ...
            _statusLabel.Text = "STEP 37.0.1 GATE B — create only the copied three-edit FMOD-neutral game.tscn derivative; trusted Step-12 install/PCK remains immutable.";
            ...
            _statusLabel.Text = "STEP 37.0.1 GATE C — ResourceLoader.Load the copied derivative as PackedScene with cache-ignore. Do not instantiate yet; native loads remain forbidden.";
            ...
            _statusLabel.Text = "STEP 37.0.1 GATE D — instantiate PackedScene off-tree only, verify managed root/expected hierarchy/inert FMOD nodes, release it immediately, and prove no native/initializer/rejected escape. No AddChild or startup.";
            ...
            _step37DetailLabel.Text =
                "All four Step 37.0.1 gates passed. The exact PCK game.tscn authority matched its sealed 10,414-byte SHA-256/MD5/layout; only a private copied derivative was modified; the selected same-process GodotSharp bridge authority loaded it as PackedScene without native escape; and an NGame hierarchy was instantiated off-tree with FmodBankLoader/FmodListener2D replaced by inert Godot.Node instances. The root never entered the SceneTree and was released immediately. ExecuteDeferred, NGame.GameStartup, main-menu launch, Steam initialization, FMOD/Spine/Sentry GDExtension loading, and gameplay remain separate future boundaries.";
            _statusLabel.Text = "STEP 37.0.1 COMPLETE — 4/4. Controlled off-tree NGame scene admission is closed; preserve Step37 artifacts before designing the first tree/startup boundary.";
            _statusLabel.TextColor = UIColor.Label;
        ...
        catch (Exception ex)
        {
            ...
            _step37ResultLabel.TextColor = UIColor.SystemRed;
            _step37DetailLabel.Text = $"Unhandled Step 37.0.1 exception: {ex.GetType().Name}: {ex.Message}";
        ...
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 37.0.1 Controlled FMOD-neutral Game-Scene Admission",
                _step37ResultLabel,
                _step37DetailLabel,
    ...
    private bool RecordStep37Gate(TransformedRealStS2GameSceneAdmissionGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 37.0.1 FAIL at Gate {letter} ({result.Gate}). Stop here; later gates were not run. Preserve Step37 artifacts and use a fresh process before retry if Gate C or D began.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        ...
        lock (_step37CheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 37.0.1 controlled game-scene admission checkpoint\n" +
                    ...
                    "Candidate: STEP 37.0.1 — SEALED GAME.TSCN + FMOD-NEUTRAL COPY + PACKEDSCENE LOAD + OFF-TREE INSTANTIATION\n" +
