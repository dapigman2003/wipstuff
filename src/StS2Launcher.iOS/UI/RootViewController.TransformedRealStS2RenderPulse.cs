...
public sealed partial class RootViewController
{
    ...
    private void AddTransformedRealStS2RenderPulseControls(UIStackView content)
    {
        ...
        _step40RenderPulseButton = SystemButton(
            $"Run Step 40.0 A–D — Audit frame callbacks → Render ~{TransformedRealStS2VeryEarlyInitialization.Step40RenderPulseTargetMilliseconds} ms → Refreeze → Confinement",
            16);
    ...
    private async Task RunTransformedRealStS2RenderPulseAsync()
    {
        ...
        if (!_step39Gates.Snapshot().Passed || !_transformedRealStS2VeryEarlyInitialization.ExactStep39ClosurePassed)
        {
            ...
            _step40DetailLabel.Text = "Step 40.0 requires Step 39.0 4/4 in this same process, because Step 39 intentionally retains the real NGame in-tree with rendering frozen. Fresh launch sequence: Step 15 A-C → Step 35.0.32 MODEL-BOOTSTRAP 4/4 → Step 36.0.5 4/4 → Step 37.0.1 4/4 → skip Step 38 → Step 39.0 4/4 → Step 40.0 once.";
            _statusLabel.Text = "STEP 40.0 REFUSED — complete Step 39.0 4/4 in this process first.";
        ...
        if (_step40PulseUiStarted)
        {
            ...
            _statusLabel.Text = "STEP 40.0 REFUSED — render pulse already attempted in this process.";
            _statusLabel.TextColor = UIColor.SystemOrange;
            return;
        }
        ...
        if (GodotStep15NativeBridge.IsRenderingActive)
        {
            ...
            _step40ResultLabel.TextColor = UIColor.SystemRed;
            _step40DetailLabel.Text = "Step 40.0 requires the renderer to still be stopped by Step 39. Do not run a pulse from an already-active renderer; preserve reports and relaunch.";
            _statusLabel.Text = "STEP 40.0 REFUSED — rendering is already active.";
            _statusLabel.TextColor = UIColor.SystemRed;
            return;
        }
        ...
        _statusLabel.Text = $"STEP 40.0 RUN {_step40RunId} — same-process Step 39 is closed 4/4 and renderer is frozen. Gate A re-verifies retained authority before any restart.";
        _statusLabel.TextColor = UIColor.Label;

        try
        {
            WriteStep40Checkpoint("RUN_START — Step 40.0 controlled real-game render pulse started from same-process Step-39 4/4 frozen authority. No render restart has occurred yet.");
            ...
            _statusLabel.Text = "STEP 40.0 GATE B — renderer remains frozen while the actual in-tree hierarchy's frame/input managed callbacks are Cecil-mapped fail-closed.";
            ...
            if (!WriteStep40StaticMap(out var staticMapError))
                throw new IOException("Step 40.0 verified frame-driven static map could not be durably written: " + staticMapError);
            ...
            _statusLabel.Text = $"STEP 40.0 GATE C — first/only controlled render pulse. StartRendering once, target {TransformedRealStS2VeryEarlyInitialization.Step40RenderPulseTargetMilliseconds} ms, then synchronously refreeze before evaluating the result.";
            if (!NSThread.IsMain)
                throw new InvalidOperationException("Step 40.0 Gate C must arm StartRendering on the UIKit main thread.");
            ...
            WriteStep40Checkpoint($"J_C_STOP_RENDERING_CALL — first continuation after pulse target; managedThread={Environment.CurrentManagedThreadId}; isMain={NSThread.IsMain}; elapsedBeforeStopMs={stopwatch.Elapsed.TotalMilliseconds:F1}. No other Step-40 work is authorized before StopRendering.");
            var stopObservation = await StopStep40RenderingOnMainThreadAsync();
            stopwatch.Stop();
            WriteStep40Checkpoint($"J_C_STOP_RENDERING_RETURNED — returned={stopObservation.StopReturned}; renderingActiveAfterStop={stopObservation.ActiveAfterStop}; observedElapsedMs={stopwatch.Elapsed.TotalMilliseconds:F1}; nativeError='{SanitizeStep40Checkpoint(GodotStep15NativeBridge.LastError)}'. Rendering is never restarted by Step 40 after this point.");
            ...
            if (!RecordStep40Gate(gateC))
            {
                _statusLabel.Text = !stopObservation.ActiveAfterStop
                    ? "STEP 40.0 FAIL at Gate C — renderer is frozen again. Preserve Step40 reports and relaunch; do not retry the pulse in-process."
                    : "STEP 40.0 FAIL at Gate C — renderer could not be confirmed frozen. Preserve reports and relaunch immediately; do not continue this process.";
                _statusLabel.TextColor = UIColor.SystemRed;
                return;
            }

            _step40ResultLabel.Text = "CONTROLLED REAL-GAME RENDER PULSE: GATE D RUNNING…";
            _statusLabel.Text = "STEP 40.0 GATE D — rendering must remain frozen; verify retained NGame singleton/parent/_window/state authority and zero initializer/rejected/native escape after the pulse.";
        ...
        catch (Exception ex)
        {
            ...
            _step40ResultLabel.TextColor = UIColor.SystemRed;
            _step40DetailLabel.Text = $"Unhandled Step 40.0 exception: {ex.GetType().Name}: {ex.Message}";
        ...
        finally
        {
            ...
            await WriteDeviceTestReportFromLabelsAsync(
                ...
                "StS2 Launcher — Step 40.0 Controlled Real-Game Render Pulse",
                _step40ResultLabel,
                _step40DetailLabel,
    ...
    private bool RecordStep40Gate(TransformedRealStS2RenderPulseGateResult result)
    {
        ...
        if (!result.Passed && _statusLabel is not null)
        {
            ...
            _statusLabel.Text = $"STEP 40.0 FAIL at Gate {letter} ({result.Gate}). Stop here; later gates were not run. Preserve Step40 artifacts and relaunch before another render-pulse attempt.";
            _statusLabel.TextColor = UIColor.SystemRed;
        }
        ...
        lock (_step40CheckpointSync)
        {
            ...
            try
            {
                ...
                WriteStep35TextFileDurably(
                    crashPath,
                    "StS2 Launcher — Step 40.0 controlled real-game render-pulse checkpoint\n" +
                    ...
                    $"Candidate: STEP 40.0 — CLOSED STEP-39 AUTHORITY + FRAME/INPUT AUDIT + ONE ~{TransformedRealStS2VeryEarlyInitialization.Step40RenderPulseTargetMilliseconds}MS RENDER PULSE + SYNCHRONOUS REFREEZE + FROZEN CONFINEMENT\n" +
