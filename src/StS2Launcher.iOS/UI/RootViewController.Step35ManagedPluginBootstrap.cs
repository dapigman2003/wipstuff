...
public sealed partial class RootViewController
{
    private void RunStep35ManagedPluginBootstrap()
    {
        ...
        if (!GodotStep15NativeBridge.IsEngineStarted || !GodotStep15NativeBridge.IsSetupFinished ||
            ...
            throw new InvalidOperationException($"{bridgeMode} native readiness changed after Gate B; refusing the controlled GodotSharp callback handoff.");
        ...
        if (callbackTable == IntPtr.Zero || callbackTableSizeBytes <= 0 || callbackTableSizeBytes % IntPtr.Size != 0)
            throw new InvalidOperationException($"{bridgeMode} native callback table was null/empty/misaligned: " + GodotStep15NativeBridge.LastError);
        ...
        if (!hasCSharpLanguage || apiCacheUpdated || hasCreateBindingCallback || reverseBindingReady || externalBridgeInstalled || GodotStep15NativeBridge.IsDotNetRuntimeInitialized)
            throw new InvalidOperationException($"{bridgeMode} Step-35.0.27 requires the physically proven 0.0.145 baseline: CSharpLanguage present, Godot API cache/reverse callbacks absent, no prior external bridge, and no Godot-owned .NET runtime.");
        ...
        if (managedCallbacksSizeBytes <= 0 || managedCallbacksSizeBytes % IntPtr.Size != 0)
            throw new InvalidOperationException($"{bridgeMode} native ManagedCallbacks size was invalid: " + managedCallbacksSizeBytes);
        ...
        if (!installReturned)
            throw new InvalidOperationException($"{bridgeMode} external ManagedCallbacks cache adoption failed: " + GodotStep15NativeBridge.LastError);
        ...
        if (!apiCacheUpdated || !hasCreateBindingCallback || !reverseBindingReady || !externalBridgeInstalled || GodotStep15NativeBridge.IsDotNetRuntimeInitialized)
            throw new InvalidOperationException($"{bridgeMode} cache adoption returned but native reverse-binding readiness did not become complete, or Godot unexpectedly claimed runtime ownership.");
