...
internal static class CurrentReleasePresentation
{
    public const string StepTitle =
        "STEP 35.0.12 — MAXSTACK-SAFE COMMAND-LINE/GODOT BOUNDARY LOCALIZATION";
    ...
    public const string Summary =
        "Physical 0.0.133 corrected the NullPlatform ordinal to NP002 but returned a faulted Task with nested InvalidProgramException before any CommandLineHelper cctor/CL marker executed, then reached normal RUN_END. This is a diagnostic IL defect, not a new exact-game frontier: the live-stack PRE/POST sweep added one transient stack item without increasing the cctor MaxStack header. Step 35.0.12 / 0.0.135 reserves and verifies MaxStack headroom and adds redundant stack-neutral markers around _args dictionary assignment and Godot.OS.GetCmdlineArgs while preserving every prior resolver/native/Godot-startup prohibition.";

    public const string InitialStatus =
        "Status: Steps 32–34 are CLOSED POSITIVE at 4/4. Step 35 remains OPEN. The authoritative physical game frontier remains the 0.0.132 interval inside CommandLineHelper type initialization triggered by NullPlatformUtilStrategy..ctor; 0.0.133 is CLOSED DIAGNOSTIC NEGATIVE because its new cctor sweep was CLR-invalid before instruction zero. Candidate 0.0.135 is diagnostic-only: it MaxStack-hardens every live-stack marker sweep, post-write verifies the CommandLineHelper cctor header, and adds stack-neutral critical boundaries so one physical run can distinguish dictionary construction from Godot.OS.GetCmdlineArgs without bootstrapping Godot or broadening runtime authority.";

    public const string ExpectedDisplayVersion = "0.0.135";
    public const string ExpectedBuildVersion = "135";
