...
internal static class CurrentReleasePresentation
{
    public const string StepTitle =
        "STEP 38.0.1 — CONTROLLED NGAME _ENTERTREE AFTER PHYSICAL STEP-37 CLOSURE";
    ...
    public const string Step38ImplementationMarker =
        "physical 0.0.161 Step-37.0.1 4/4 closure -> physical 0.0.162 Step-38 Gate-A safe stop before lifecycle execution because Cecil immediate/default metadata read attempted GodotSharp resolution -> 0.0.163 correction uses ReadingMode.Deferred + explicit RejectingAssemblyResolver + MetadataResolver and requires zero external resolution requests -> exact selected ModelDb-bootstrap sts2 compatibility image lifecycle map -> require NGame._EnterTree instance void IL body -> map _Ready/GameStartupWrapper/GameStartup/InitializePlatform/LaunchMainMenu/LoadDeferredStartupAssetsAsync -> reject same-NGame _EnterTree call closure reaching later startup/deferred or OneTimeInitialization re-entry -> reinstantiate already proven FMOD-neutral PackedScene off-tree -> bind exact _EnterTree token -> invoke _EnterTree once by MethodInfo while IsInsideTree=false -> require state=2 and zero initializer-bearing/rejected/native escape -> post-entry off-tree confinement -> release temporary node without _ExitTree; AddChild/_Ready/GameStartup/platform/main-menu/ExecuteDeferred/Steam/native GDExtensions remain forbidden";
