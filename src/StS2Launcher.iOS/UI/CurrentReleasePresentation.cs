...
internal static class CurrentReleasePresentation
{
    public const string StepTitle =
        "STEP 35.0.28 — EXACT AUTHORITY CLOSURE + GATE D FINALIZATION";
    ...
    public const string Summary =
        "Physical 0.0.149 completed diagnostic Gate C end-to-end and reached Gate D. 0.0.150 introduced durable Gate-D finalization boundaries plus EXACT-CLOSURE using exact transformed sts2 and exact prepared GodotSharp; Codemagic passed 895/895 static + 214/214 host + Step-15 native-link preflight, then iOS compile stopped on one missing Core.Runtime namespace import in the bootstrap partial. Step 35.0.28 / 0.0.151 corrects only that compile integration defect while preserving the exact-closure bridge/runtime contract.";

    public const string InitialStatus =
        "Status: Steps 32–34 are CLOSED POSITIVE at 4/4 and Step 35 remains OPEN pending an exact-authority device result. Physical 0.0.149 proves diagnostic Gate C completes normally and reaches Gate D. Codemagic 0.0.150 proved static/host/native-link readiness but exposed one iOS namespace compile defect before IPA creation. The current 0.0.151 candidate fixes that import only; exact-closure, Gate-D finalization, resolver policy, and the pinned .NET 9.0.314 + iOS workload 9.0.314.3 cache remain unchanged.";

    public const string ExpectedDisplayVersion = "0.0.151";
    public const string ExpectedBuildVersion = "151";
