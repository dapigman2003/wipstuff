...
internal static class CurrentReleasePresentation
{
    public const string StepTitle =
        "STEP 35.0.15 — COMPREHENSIVE GODOTSHARP / NATIVE RECON + DUAL MODE";
    ...
    public const string Summary =
        "Physical 0.0.136 entered CommandLineHelper..cctor and hard-terminated inside Godot.Collections.Dictionary<string,string> construction after CL_CRITICAL_001_PRE. Step 35.0.15 / 0.0.138 adds read-only bundle-wide Mach-O/native reconnaissance, a separately verified entry-only GodotSharp diagnostic derivative, and two fresh-process modes in one app: NATURAL preserves the original Godot dictionary for inner localization; COMPAT applies only the four-reference BCL Dictionary<string,string> substitution to advance toward the still-natural Godot.OS.GetCmdlineArgs/native-callback boundary.";

    public const string InitialStatus =
        "Status: Steps 32–34 are CLOSED POSITIVE at 4/4. Step 35 remains OPEN. Physical 0.0.136 localized the hard termination to Godot.Collections.Dictionary<string,string> construction before _args assignment; the final resolver event remains contextual rather than causal. The 0.0.137 Codemagic attempt stopped at host regression 208/209 because the GodotSharp entry-marker verifier checked the sts2 bridge type. Candidate 0.0.138 fixes only that verifier defect and carries both a natural deep-recon mode and the bounded managed-dictionary compatibility mode, plus a same-run GodotSharp IL/native-callback map and read-only Mach-O dependency/rpath/symbol/string inventory. Resolver authority, native-load refusal, Godot-startup prohibition, and exact Step-32 authority are unchanged.";

    public const string ExpectedDisplayVersion = "0.0.138";
    public const string ExpectedBuildVersion = "138";
