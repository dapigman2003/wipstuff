...
public sealed partial class RootViewController
{
    ...
    private void AddControlledHarmonyPatchExecutionControls(UIStackView content)
    {
        ...
        _controlledHarmonyPatchExecutionDetailLabel = Label(
            "Gates A–N replay the physically closed Step 26 boundary in one fresh dedicated context through the historical empty PatchProcessor probe. Gate O keeps the exact AddPrefix/Patch/Unpatch, HarmonyMethod, AccessTools, and HarmonySharedState → MethodCreator → MonoMod detour metadata audits. A synchronous crash checkpoint is flushed before/after every gate and at sensitive substages. Gate P now admits a dedicated launcher-owned DLL copied into the .app only after dotnet publish, resolves its interpreted Target(int), InvokeTarget(int), and Prefix(int, ref __result), then creates a fresh PatchProcessor for that exact interpreted Target through public Harmony.CreateProcessor(MethodBase). Gate Q proves baseline behavior through direct reflection plus an in-fixture IL call. Gate R explicitly completes the measured AccessTools initializer. Gate S constructs the bounded annotation-free interpreted prefix descriptor without invoking AddPrefix(MethodInfo). Gate T replays the normalized HarmonySharedState proof, then invokes exactly one public PatchProcessor.Patch(); Gate U audits before execution. Gate V requires both interpreted routes to return the prefix-controlled result while the original body is skipped. Gate W removes exactly that prefix by MethodInfo; Gate X audits; Gate Y requires original behavior to be restored through both interpreted routes; Gate Z performs the final full hash/OfflineReady/context audit. This is the single post-publish interpreted stop-rule experiment: if the representative fixture cannot patch, Step 27 stops iterating Harmony internals and Step 28 pivots to ahead-of-load managed IL transformation. StS2 reflection/patching/invocation, Harmony.Patch/PatchAll, patch classes/categories, transpilers/finalizers, Godot startup, and native game loading remain forbidden.",
    ...
    private async Task RunControlledHarmonyPatchExecutionAsync()
    {
        ...
        try
        {
            ...
            _statusLabel.Text = "STEP 27 GATE Q — invoking only the post-publish interpreted fixture before patching: Target via reflection plus InvokeTarget, whose IL directly calls Target.";
            ...
            _statusLabel.Text = "STEP 27 GATE T — PATCH-ENGINE RUNTIME BOUNDARY: replay physically crossed T1–T4, verify the Gate-A 11-instruction iOS-normalized HarmonySharedState runtime image, enter its direct-state cctor, then invoke PatchProcessor.Patch() exactly once only if T6 validates. Target is not invoked yet.";
            ...
            _statusLabel.Text = "STEP 27 GATE V — invoking the patched post-publish interpreted target through reflection plus the in-fixture direct-call route; exact prefix must replace result and skip original.";
            ...
            _statusLabel.Text = "STEP 27 GATE Y — invoking the post-publish interpreted target through both routes after unpatch; exact original value+1 behavior must be restored.";
            ...
            _controlledHarmonyPatchExecutionDetailLabel.Text = FormatControlledHarmonyPatchExecutionDetail(
                "All twenty-six Step 27 gates passed. The physically closed Step 26 state was reproduced; the dedicated post-publish interpreted fixture was admitted outside the iOS publish/AOT graph; a fresh processor targeted its exact Target MethodInfo; one real PatchProcessor.Patch() completed; both interpreted invocation routes returned the deterministic patched result while the original body was skipped; the exact prefix was then removed and original behavior was restored through both routes; final hashes, OfflineReady, context membership, and native/resolver isolation remained intact. Harmony is therefore viable for this representative dynamically loaded managed target. Run OfflineReady + Foundation 5/5 to close Step 27.");
    ...
    private string FormatControlledHarmonyPatchExecutionDetail(string tail)
    {
        ...
        lines.Add("Gate Q invokes the post-publish interpreted Target by reflection and InvokeTarget by reflection; InvokeTarget contains a direct IL call to Target. Both must return original value+1 behavior with prefix count zero.");
        ...
        lines.Add("Gate T decomposes the patch-engine runtime boundary: T1/T2 measure the bounded Reflection.Emit/RuntimeMethodHandle host preflight; T3/T4 resolve the normalized HarmonySharedState runtime surface; T5a/T5b/T6 re-prove the direct-state cctor; T6a/T6b retain the LINQ closure check; T7/T8 invoke exactly one public PatchProcessor.Patch() against the fresh post-publish interpreted target; T9 validates replacement/isolation. The interpreted target is not invoked after patch installation until Gate V.");
        ...
        lines.Add("Gate V invokes the patched interpreted Target directly through MethodInfo and then invokes InvokeTarget, whose own interpreted IL calls Target; both must return 1041, increment the prefix, and skip the original Target body.");
        lines.Add("Gate W removes exactly the interpreted fixture prefix via PatchProcessor.Unpatch(MethodInfo). Gate X audits before restored invocation. Gate Y invokes the interpreted Target through both routes and requires restored value+1 behavior with no additional prefix calls. Gate Z performs the final full byte/OfflineReady/context/native audit.");
