...
public sealed partial class RootViewController
{
    ...
    private async Task RunExpressionInterpreterCompatibilityAsync()
    {
        ...
        if (_godotProcessRequiresRestart)
        {
            _statusLabel.Text = "Step 15 Godot process-global state is still active. Force-quit/relaunch before Step 19 so the expression-runtime proof runs in a clean process.";
            _statusLabel.TextColor = UIColor.SystemOrange;
            return;
        }
        ...
        _expressionInterpreterCompatibilityDetailLabel.Text = "Gate A: proving Compile(), Compile(false), and Compile(true) in the current physical iOS AOT/interpreter process, requiring no dynamically compiled native code/JIT, then cloning a fresh receipt-backed ARM64/shared Step 19 workspace.";
        _statusLabel.Text = "STEP 19.2 GATE A — current host expression execution + receipt-backed workspace clone.";
        _statusLabel.TextColor = UIColor.Label;
        ...
        try
        {
            ...
            _expressionInterpreterCompatibilityResultLabel.TextColor = UIColor.Label;
            _expressionInterpreterCompatibilityDetailLabel.Text = FormatExpressionInterpreterCompatibilityDetail(
                "All four Step 19 gates passed under the current post-Step-20 runtime contract. The physical launcher proved Compile(), Compile(false), and Compile(true) in the iOS AOT/interpreter host with no dynamically compiled native code/JIT, classified real Compile sites across consumer/framework and IL-only/ReadyToRun boundaries, performed zero Cecil assembly writes, kept the complete prepared tree byte-identical, and proved trusted source/live-install isolation.");
            _statusLabel.Text = "PASS: STEP 19.2 EXPRESSION INTERPRETER COMPATIBILITY — 4/4. Current host expression support + framework boundary + zero-write prepared tree are proven; RuntimeFeature.IsDynamicCodeSupported is diagnostic after Step 20, while dynamically compiled native code/JIT remains disallowed by the regression contract.";
            _statusLabel.TextColor = UIColor.Label;
        }
        catch (OperationCanceledException)
        {
    ...
    private string FormatExpressionInterpreterCompatibilityDetail(string tail)
    {
        ...
        lines.Add("Behavioral rewrite scope: NONE in Step 19.2. Gate A proves host Compile()/Compile(false)/Compile(true) execution under the current non-JIT AOT/interpreter policy; Gate B read-only classifies real call sites; Gate C performs zero Cecil assembly writes and makes the prepared tree byte-identical. RuntimeFeature.IsDynamicCodeSupported is diagnostic because Step 20 intentionally enabled interpreter-backed dynamic-code support; RuntimeFeature.IsDynamicCodeCompiled must remain false on iOS. System.* framework and non-IL-only/ReadyToRun images are diagnostic-only.");
