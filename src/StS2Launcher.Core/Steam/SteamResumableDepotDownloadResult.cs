...
public sealed record SteamResumableDepotDownloadResult(
    ...
    bool ExistingFinalVerifiedAgainstManifest,
    ...
    uint? LoginId = null)
{
    ...
    public string Summary => Outcome switch
    {
        ...
        SteamResumableDepotDownloadOutcome.OutputAlreadyExists when ExistingFinalVerifiedAgainstManifest =>
            $"RESUME SOURCE PASS — existing manifest cache reverified ({VerifiedFileCount}/{PlannedFileCount} files)",
        SteamResumableDepotDownloadOutcome.OutputAlreadyExists => "RESUME BLOCKED — final manifest directory exists but did not verify",
