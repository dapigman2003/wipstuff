...
public sealed class SteamResumableDepotDownloadAttempt
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existingFinalVerifiedAgainstManifest = false;
        ...
        try
        {
            ...
            if (Directory.Exists(finalPath) || File.Exists(finalPath))
            {
                ...
                if (Directory.Exists(finalPath))
                {
                    ...
                    if (existingFinalVerifiedAgainstManifest)
                    {
                        completedFileCount = plannedFileCount;
                        verifiedFileCount = plannedFileCount;
                        reusedVerifiedFileCount = plannedFileCount;
                        reusedBytes = plannedBytes;
                        satisfiedBytes = plannedBytes;
                        error = null;
                    }
                    else
                    {
                        error = "The manifest-specific Step 11 final directory exists but failed exact current-Steam-manifest verification. Step 12 may discard and reacquire only this invalid cache.";
                    }
                }
                else
                {
                    error = "The manifest-specific Step 11 output path is occupied by a file instead of a verified depot directory.";
                }
            ...
            foreach (var file in plan.Files)
            {
                ...
                if (File.Exists(partPath))
                {
                    // Resume-scan accounting is committed only after the entire partial
                    // file has been scanned successfully. If an I/O error occurs midway,
                    // the partial file is discarded without leaving already-counted chunks
                    // that would later be counted a second time when redownloaded.
                    var scannedReusedChunkCount = 0;
                    ulong scannedReusedBytes = 0;
                    var scannedInvalidChunkCount = 0;
                        ...
                        else if (file.TotalSize > 0)
                        {
                            ...
                            foreach (var chunk in file.Chunks.OrderBy(chunk => chunk.Offset))
                            {
                                ...
                                if (await ChunkMatchesManifestAsync(existingPart, chunk, token).ConfigureAwait(false))
                                {
                                    ...
                                    scannedReusedChunkCount++;
                                    scannedReusedBytes += chunk.UncompressedLength;
                                }
                                else
                                {
                                    scannedInvalidChunkCount++;
                                }
                            }

                            reusedChunkCount += scannedReusedChunkCount;
                            reusedBytes += scannedReusedBytes;
                            satisfiedChunkCount += scannedReusedChunkCount;
                            satisfiedBytes += scannedReusedBytes;
                            invalidResumeChunkCount += scannedInvalidChunkCount;
                        }
                    }
                    catch (OperationCanceledException)
                    {
    ...
    private static async Task<bool> VerifyExistingFinalAgainstManifestAsync(
        string finalPath,
        SteamDepotDownloadPlan plan,
        IProgress<SteamDepotDownloadProgress>? progress,
        CancellationToken cancellationToken)
    {
        try
        {
        ...
        foreach (var file in plan.Files)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var normalized = SteamSingleFileTargetSelector.NormalizeRelativePath(file.FileName);
            var path = ResolveChildPath(finalPath, normalized);
            try
            {
                if (!await FileMatchesManifestAsync(path, file, cancellationToken).ConfigureAwait(false))
                    return false;
            }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or OverflowException)
            {
                // A final cache that cannot be read consistently is not trustworthy.
                // Report it as an invalid cache so Step 12 can discard/reacquire it
                // instead of aborting the whole manager as an unrelated exception.
                return false;
            }

            verifiedFiles++;
            verifiedBytes += file.TotalSize;
            progress?.Report(new SteamDepotDownloadProgress(
                SteamDepotDownloadPhase.Verifying,
                verifiedFiles,
                plan.TotalFileCount,
                CompletedChunks: 0,
                TotalChunks: plan.TotalChunkCount,
                CompletedBytes: verifiedBytes,
                TotalBytes: plan.TotalBytes,
                CurrentFile: file.FileName));
        }

        return verifiedFiles == plan.TotalFileCount && verifiedBytes == plan.TotalBytes;
    }
        ...
        SteamDepotDownloadPlan plan)
    {
        ...
        foreach (var expectedDirectory in expectedDirectories)
        {
            if (!Directory.Exists(expectedDirectory))
                throw new InvalidDataException($"Expected manifest directory is missing: {Path.GetRelativePath(root, expectedDirectory)}");
        }
    ...
    private static SteamResumableDepotDownloadResult EmptyResult(
        ...
        string? error) => new(
            ...
            ResumeDataPreserved: false,
            ExistingFinalVerifiedAgainstManifest: false,
