...
public sealed partial class TransformedRealStS2VeryEarlyInitialization
{
    ...
    private static void RequireInertGameStartupWrapper(MethodDefinition method)
    {
        if (method.IsStatic ||
            ...
            method.Body.Instructions[0].Operand is not MethodReference completedTask ||
            completedTask.DeclaringType.FullName != "System.Threading.Tasks.Task" ||
            completedTask.Name != "get_CompletedTask" ||
            completedTask.HasThis ||
            completedTask.Parameters.Count != 0 ||
