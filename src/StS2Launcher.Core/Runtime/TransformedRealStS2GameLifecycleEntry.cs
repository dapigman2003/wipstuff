...
/// <summary>
/// Step 38.0.1 boundary. Requires the same-process physically closed Step-37.0.1 4/4 authority.
...
public sealed partial class TransformedRealStS2VeryEarlyInitialization
{
    ...
    public string GetVerifiedNGameLifecycleStaticMap()
        => _step38LifecycleStaticAudit?.StaticMap
           ?? throw new InvalidOperationException("Step 38.0.1 Gate A has not produced a verified NGame lifecycle static map.");
        ...
        Action<string>? checkpoint = null)
    {
        ...
        try
        {
            ...
            var preflight = _preflight ?? throw new InvalidOperationException("Step 38.0.1 exact selected-authority preflight is absent.");
            if (!IsModelBootstrapCompatibilityMode)
                throw new InvalidOperationException("Step 38.0.1 requires the physically proven ModelDb-bootstrap selected authority.");
            ...
            if (!selectedSha256.Equals(preflight.DiagnosticSha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException($"Step 38.0.1 selected sts2 compatibility image hash drifted. expected={preflight.DiagnosticSha256}; actual={selectedSha256}.");
            ...
            var nGame = module.Types.SingleOrDefault(type => type.FullName == NGameTypeFullName)
                ?? throw new InvalidDataException($"Step 38.0.1 could not locate exact type {NGameTypeFullName}.");
            ...
            if (forbiddenRefs.Count != 0)
                throw new InvalidDataException("Step 38.0.1 _EnterTree same-NGame call closure reaches forbidden later-startup boundary/boundaries: " + string.Join(" | ", forbiddenRefs));
            ...
            if (oneTimeForbidden.Length != 0)
                throw new InvalidDataException("Step 38.0.1 _EnterTree same-NGame call closure reaches a forbidden OneTimeInitialization method: " + string.Join(" | ", oneTimeForbidden));

            if (resolver.Requests.Count != 0)
                throw new InvalidDataException("Step 38.0.1 lifecycle static audit unexpectedly attempted external assembly resolution: " + string.Join(" | ", resolver.Requests));
            ...
            Checkpoint(checkpoint, $"H_A_PASS — NGame lifecycle map sealed under deferred metadata-only Cecil read; externalResolutionRequests=0; enterTreeToken=0x{enterTree.MetadataToken.ToUInt32():X8}; sameNGameReachable={reachable.Count}; forbiddenLaterStartupReachable=0; no lifecycle execution/native escape.");
            return LifecyclePass(gate,
                "STEP 38.0.1 NGAME LIFECYCLE STATIC AUDIT PASSED.\n" +
        ...
        Action<string>? checkpoint = null)
    {
        ...
        try
        {
            ...
            var handoff = _callbackHandoff ?? throw new InvalidOperationException("Step 38.0.1 exact GodotSharp handoff disappeared.");
            ...
            catch (TargetInvocationException ex) when (ex.InnerException is not null)
            {
                throw new InvalidOperationException(
                    $"Step 38.0.1 PackedScene.Instantiate threw {ex.InnerException.GetType().FullName}: {ex.InnerException.Message}\n{FormatExceptionDiagnostic(ex.InnerException)}",
                    ex.InnerException);
            }

            if (instance is null || !nodeType.IsInstanceOfType(instance))
                throw new InvalidDataException($"Step 38.0.1 PackedScene.Instantiate did not return Godot.Node; observed {instance?.GetType().FullName ?? "<null>"}.");
            var rootType = instance.GetType();
            if (!string.Equals(rootType.FullName, NGameTypeFullName, StringComparison.Ordinal))
                throw new InvalidDataException($"Step 38.0.1 expected root {NGameTypeFullName}; observed {rootType.FullName ?? "<unknown>"}.");
            if (!ReferenceEquals(AssemblyLoadContext.GetLoadContext(rootType.Assembly), context))
                throw new InvalidDataException("Step 38.0.1 NGame instance is not owned by the exact Step-35/36/37 private load context.");

            var isInsideTree = RequireZeroArgBoolMethod(rootType, "IsInsideTree");
            if (isInsideTree.Invoke(instance, null) is not false)
                throw new InvalidDataException("Step 38.0.1 fresh NGame unexpectedly reports IsInsideTree=true before _EnterTree direct invocation.");
            ...
            if ((uint)enterTree.MetadataToken != RequireStep38StaticAudit().EnterTreeToken)
                throw new InvalidDataException($"Step 38.0.1 reflection _EnterTree token drifted: Cecil=0x{RequireStep38StaticAudit().EnterTreeToken:X8}; reflection=0x{enterTree.MetadataToken:X8}.");
            ...
            return LifecyclePass(gate,
                "STEP 38.0.1 OFF-TREE NGAME REINSTANTIATION PASSED.\n" +
        ...
        Action<string>? checkpoint = null)
    {
        ...
        try
        {
            ...
            _ = RequireStep38StaticAudit();
            var instance = _step38NGameInstance ?? throw new InvalidOperationException("Step 38.0.1 Gate B must retain a fresh off-tree NGame instance before Gate C.");
            var enterTree = _step38EnterTreeMethod ?? throw new InvalidOperationException("Step 38.0.1 Gate B must bind NGame._EnterTree before Gate C.");
            if (_step38EnterTreeInvocationStarted)
                throw new InvalidOperationException("Step 38.0.1 _EnterTree is one-shot in this process; retry is forbidden after invocation begins.");

            var isInsideTree = RequireZeroArgBoolMethod(instance.GetType(), "IsInsideTree");
            if (isInsideTree.Invoke(instance, null) is not false)
                throw new InvalidDataException("Step 38.0.1 NGame unexpectedly entered the SceneTree before the direct _EnterTree boundary.");
            ...
            catch (TargetInvocationException ex) when (ex.InnerException is not null)
            {
                ...
                throw new InvalidOperationException(
                    $"Step 38.0.1 NGame._EnterTree threw {ex.InnerException.GetType().FullName}: {ex.InnerException.Message}\n{FormatExceptionDiagnostic(ex.InnerException)}",
                    ex.InnerException);
            }

            if (isInsideTree.Invoke(instance, null) is not false)
                throw new InvalidDataException("Step 38.0.1 direct _EnterTree invocation unexpectedly changed Godot IsInsideTree=true without SceneTree insertion.");
            ...
            if (state != ExpectedStateAfterEssential)
                throw new InvalidDataException($"Step 38.0.1 OneTimeInitialization state drifted during _EnterTree: expected {ExpectedStateAfterEssential}; observed {state}.");
            ...
            return LifecyclePass(gate,
                "STEP 38.0.1 DIRECT NGAME._ENTERTREE INVOCATION PASSED.\n" +
        ...
        Action<string>? checkpoint = null)
    {
        ...
        try
        {
            ...
            if (!_step38EnterTreeInvocationStarted || !_step38EnterTreeInvocationPassed)
                throw new InvalidOperationException("Step 38.0.1 Gate C must return successfully before Gate D.");
            var instance = _step38NGameInstance ?? throw new InvalidOperationException("Step 38.0.1 retained NGame instance disappeared before Gate D.");
            ...
            if (TryReadIsInsideTree(instance) != "False")
                throw new InvalidDataException($"Step 38.0.1 post-_EnterTree instance is no longer off-tree; observed IsInsideTree={TryReadIsInsideTree(instance)}.");
            if (context.InitializerBearingRequests.Count != 0 || context.RejectedManagedRequests.Count != 0 || context.NativeLoadAttempts.Count != 0)
                throw new InvalidDataException("Step 38.0.1 post-_EnterTree confinement detected forbidden initializer-bearing/rejected/native activity. " + context.FormatResolverState());
            var state = ReadOneTimeInitializationState(RequireEssentialBinding().StateField);
            if (state != ExpectedStateAfterEssential)
                throw new InvalidDataException($"Step 38.0.1 post-_EnterTree state drifted: expected {ExpectedStateAfterEssential}; observed {state}.");
            ...
            return LifecyclePass(gate,
                "STEP 38.0.1 POST-_ENTERTREE CONFINEMENT PASSED.\n" +
    ...
    private Step38LifecycleStaticAuditSnapshot RequireStep38StaticAudit()
        => _step38LifecycleStaticAudit ?? throw new InvalidOperationException("Step 38.0.1 Gate A must pass before later gates.");
    ...
    private static MethodDefinition RequireLifecycleMethod(TypeDefinition type, string name, int parameterCount, string returnType)
    {
        ...
        if (matches.Length != 1 || !matches[0].HasBody || matches[0].IsStatic)
            throw new InvalidDataException($"Step 38.0.1 expected exactly one instance {type.FullName}::{name} with {parameterCount} parameter(s), return {returnType}, and managed IL body; found {matches.Length}.");
        return matches[0];
    }
    ...
    private static MethodDefinition RequireLifecycleMethodByName(TypeDefinition type, string name, int parameterCount)
    {
        ...
        if (matches.Length != 1 || !matches[0].HasBody || matches[0].IsStatic)
            throw new InvalidDataException($"Step 38.0.1 expected exactly one instance {type.FullName}::{name} with {parameterCount} parameter(s) and managed IL body; found {matches.Length}.");
        return matches[0];
    }
        ...
        IReadOnlyList<MethodDefinition> reachable)
    {
        ...
        {
            "StS2 Launcher — Step 38.0.1 NGame lifecycle static IL/callsite map",
