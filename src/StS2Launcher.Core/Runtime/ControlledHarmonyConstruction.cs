...
/// <summary>
/// Step 25 boundary. Replays the physically proven Step 24 controlled-initialization state in a fresh
/// private AssemblyLoadContext, then resolves exactly HarmonyLib.Harmony and constructs one inert
/// Harmony instance through the exact public string constructor. No patch API, game entry point,
/// game type/member, Godot startup, or native game library is intentionally invoked.
/// </summary>
public sealed class ControlledHarmonyConstruction : IDisposable
{
    public const string TargetSimpleName = "0Harmony";
    public static readonly Version TargetVersion = new(2, 4, 2, 0);
    public const string LoadContextName = "StS2Launcher-Step25-HarmonyConstruction";
    ...
    private HarmonyConstructionSnapshot? _harmonyConstruction;
    ...
    private Step25LoadContext? _loadContext;
    private bool _disposed;

    public ControlledHarmonyConstruction(string launcherDataRoot, bool collectibleLoadContext = false)
        : this(
            launcherDataRoot,
    ...
    internal ControlledHarmonyConstruction(
    ...
    public async Task<ControlledHarmonyConstructionGateResult> RunInitializationPreflightAsync(
        IProgress<ControlledHarmonyConstructionProgress>? progress = null,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            progress?.Report(new ControlledHarmonyConstructionProgress(
                ControlledHarmonyConstructionGate.InitializationPreflight,
                0,
                0,
                null,
            ...
            {
                throw new InvalidDataException(
                    "Step 25 Gate A requires HARMONY_DEBUG to be absent/empty so the exact Harmony constructor debug branch remains dormant. " +
            ...
            {
                throw new InvalidDataException(
                    "Step 25 Gate A refuses Harmony construction because the exact constructor metadata policy did not pass:\n" +
            ...
            return Pass(
                ...
                "Audited Harmony constructor IL:\n" + harmonyConstructorMetadata.ConstructorAudit + "\n" +
                "No real game/Harmony assembly was loaded by Step 25 Gate A: YES");
        }
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.InitializationPreflight, stage, ex);
        }
    }

    public ControlledHarmonyConstructionGateResult RunProvenLoadStateReplay()
    {
        var stage = "initialization";
        try
        {
            ...
            var context = new Step25LoadContext(
                LoadContextName,
                preflight.Plan,
            ...
            return Pass(
                ControlledHarmonyConstructionGate.ProvenLoadStateReplay,
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.ProvenLoadStateReplay, stage, ex);
        }
    }

    public ControlledHarmonyConstructionGateResult RunDeferredModuleInitialization()
    {
        var stage = "initialization";
        try
        {
            ...
            return Pass(
                ControlledHarmonyConstructionGate.DeferredModuleInitialization,
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.DeferredModuleInitialization, stage, ex);
        }
    }

    public async Task<ControlledHarmonyConstructionGateResult> RunProvenInitializationAuditAsync(
        IProgress<ControlledHarmonyConstructionProgress>? progress = null,
        CancellationToken cancellationToken = default)
    {
        var stage = "initialization";
        try
        {
            ...
            foreach (var item in preflight.PreparedAssemblies)
            {
                ...
                verified++;
                progress?.Report(new ControlledHarmonyConstructionProgress(
                    ControlledHarmonyConstructionGate.ProvenInitializationAudit,
                    verified,
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.ProvenInitializationAudit, stage, ex);
        }
    }

    [UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Step 25 intentionally resolves one exact receipt-verified post-publish Harmony type and API surface after metadata preflight.")]
    ...
    public ControlledHarmonyConstructionGateResult RunHarmonyApiResolution()
    {
        ...
        try
        {
            ...
            if (!_provenInitializationAuditPassed)
                throw new InvalidOperationException("Step 25 Gate D must pass before resolving the Harmony API surface.");

            stage = "target assembly ownership";
            var targetAssembly = initialization.TargetAssembly;
            if (!ReferenceEquals(AssemblyLoadContext.GetLoadContext(targetAssembly), context))
                throw new InvalidDataException("Step 25 Harmony target is not owned by the dedicated Step 25 load context.");
            var actualIdentity = targetAssembly.GetName().FullName ?? targetAssembly.GetName().Name ?? string.Empty;
            if (!actualIdentity.Equals(preflight.Target.Plan.AssemblyFullName, StringComparison.Ordinal))
                throw new InvalidDataException("Step 25 Harmony target identity drifted before API resolution.");
            ...
            var harmonyType = targetAssembly.GetType(HarmonyTypeFullName, throwOnError: true, ignoreCase: false)
                ?? throw new TypeLoadException("Step 25 could not resolve the exact HarmonyLib.Harmony type.");
            if (harmonyType.Assembly != targetAssembly || !harmonyType.IsClass || harmonyType.IsAbstract || !harmonyType.IsPublic)
                throw new InvalidDataException("Step 25 requires HarmonyLib.Harmony to be a public non-abstract class owned by exact 0Harmony.");
            var typeInitializer = harmonyType.TypeInitializer
                ?? throw new MissingMethodException("Step 25 requires the exact HarmonyLib.Harmony type initializer measured by Gate A; it is missing at runtime.");
            ...
            if (constructor is null || publicConstructors.Length != 1)
                throw new MissingMethodException("Step 25 requires exactly one public HarmonyLib.Harmony instance constructor and it must be .ctor(System.String).");
            ...
            if (idProperty is null || idProperty.PropertyType != typeof(string) || idProperty.GetMethod is null || !idProperty.GetMethod.IsPublic)
                throw new MissingMemberException("Step 25 requires the public instance Harmony.Id string getter for post-construction verification.");
            ...
            if (debugField is null || debugField.FieldType != typeof(bool))
                throw new MissingFieldException("Step 25 requires the public static Harmony.DEBUG boolean field for the inert-constructor precondition.");

            stage = "pre-type-initialization environment precondition";
            var harmonyDebugEnvironment = Environment.GetEnvironmentVariable("HARMONY_DEBUG");
            if (!string.IsNullOrEmpty(harmonyDebugEnvironment))
                throw new InvalidDataException("Step 25 refuses Harmony type initialization while HARMONY_DEBUG is non-empty.");

            _harmonyApi = new HarmonyApiSnapshot(harmonyType, typeInitializer, constructor, idProperty, debugField, actualIdentity);

            return Pass(
                ControlledHarmonyConstructionGate.HarmonyApiResolution,
                ...
                "Harmony type initializer executed by Step 25: NO\n" +
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.HarmonyApiResolution, stage, ex);
        }
    }

    [UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Step 25 explicitly executes only the exact Harmony type initializer whose IL shape was measured by Gate A.")]
    [UnconditionalSuppressMessage("Trimming", "IL2075", Justification = "Step 25 reads only exact reflection members resolved by Gate E after the explicit Harmony type-initialization barrier.")]
    public ControlledHarmonyConstructionGateResult RunHarmonyTypeInitialization()
    {
        ...
        try
        {
            ...
            var targetSha1 = ComputeSha1Hex(preflight.Target.PreparedPath);
            if (!targetSha1.Equals(preflight.Target.Plan.Sha1Hex, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step 25 target SHA-1 changed immediately before Harmony type initialization.");
            if (!string.IsNullOrEmpty(Environment.GetEnvironmentVariable("HARMONY_DEBUG")))
                throw new InvalidDataException("Step 25 refuses Harmony type initialization because HARMONY_DEBUG became non-empty after Gate E.");
            ...
            if (api.DebugField.GetValue(null) is not bool debugAfter || debugAfter)
                throw new InvalidDataException("Step 25 Harmony.DEBUG is true after the explicit Harmony type initializer.");
            if (context.NativeLoadAttempts.Count != nativeAttemptsBefore)
            ...
            if (!contextMembershipAfter.SequenceEqual(contextMembershipBefore, StringComparer.Ordinal))
                throw new InvalidDataException("Step 25 Harmony type initialization changed private-context assembly membership.");
            ...
            return Pass(
                ControlledHarmonyConstructionGate.HarmonyTypeInitialization,
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.HarmonyTypeInitialization, stage, ex);
        }
    }

    [UnconditionalSuppressMessage("Trimming", "IL2075", Justification = "Step 25 Gate G reads only exact Harmony reflection members resolved by Gate E after Gate F completed the type initializer.")]
    public ControlledHarmonyConstructionGateResult RunHarmonyTypeInitializationAudit()
    {
        ...
        try
        {
            ...
            if (!targetSha1.Equals(typeInitialization.PreparedSha1, StringComparison.OrdinalIgnoreCase) ||
                ...
                throw new InvalidDataException("Step 25 0Harmony hash drifted after the Harmony type initializer.");
            if (!string.IsNullOrEmpty(Environment.GetEnvironmentVariable("HARMONY_DEBUG")))
                throw new InvalidDataException("Step 25 HARMONY_DEBUG became non-empty after Harmony type initialization.");
            if (api.DebugField.GetValue(null) is not bool debugValue || debugValue)
                throw new InvalidDataException("Step 25 Harmony.DEBUG is true during the post-type-initialization audit.");
            if (_harmonyInstance is not null)
                throw new InvalidOperationException("Step 25 Harmony object exists before the instance-construction gate.");
            ...
            var actual = context.Assemblies
                ...
                .ToArray();
            if (!actual.SequenceEqual(expected, StringComparer.Ordinal))
                throw new InvalidDataException("Step 25 Harmony type initialization changed the physically proven Step 24 context membership.");
            if (context.NativeLoadAttempts.Count != 0)
                throw new DllNotFoundException("Step 25 observed native-library resolution after Harmony type initialization: " + string.Join(" | ", context.NativeLoadAttempts));
            if (context.RejectedManagedRequests.Count != 0)
                throw new FileLoadException("Step 25 observed rejected/unplanned managed requests after Harmony type initialization: " + string.Join(" | ", context.RejectedManagedRequests));

            _provenHarmonyTypeInitializationAuditPassed = true;
            return Pass(
                ControlledHarmonyConstructionGate.HarmonyTypeInitializationAudit,
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.HarmonyTypeInitializationAudit, stage, ex);
        }
    }

    [UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Step 25 invokes one exact constructor from receipt-verified post-publish 0Harmony after metadata, API-shape, and explicit type-initialization preflight.")]
    [UnconditionalSuppressMessage("Trimming", "IL2075", Justification = "Step 25 retains only exact reflection objects resolved and verified by Gate E after the explicit type-initialization boundary.")]
    public ControlledHarmonyConstructionGateResult RunHarmonyInstanceConstruction()
    {
        ...
        try
        {
            ...
            if (!_provenHarmonyTypeInitializationAuditPassed)
                throw new InvalidOperationException("Step 25 Gate G must pass before the Harmony instance constructor runs.");
            ...
            var targetSha1 = ComputeSha1Hex(preflight.Target.PreparedPath);
            if (!targetSha1.Equals(preflight.Target.Plan.Sha1Hex, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step 25 target SHA-1 changed immediately before Harmony construction.");
            if (!string.IsNullOrEmpty(Environment.GetEnvironmentVariable("HARMONY_DEBUG")))
                throw new InvalidDataException("Step 25 refuses construction because HARMONY_DEBUG became non-empty after Gate G.");
            if (api.DebugField.GetValue(null) is not bool debugBefore || debugBefore)
                throw new InvalidDataException("Step 25 refuses construction because Harmony.DEBUG became true after Gate G.");
            ...
            if (!ReferenceEquals(instance.GetType(), api.HarmonyType))
                throw new InvalidDataException("Step 25 constructor returned an unexpected runtime type.");
            if (!ReferenceEquals(AssemblyLoadContext.GetLoadContext(instance.GetType().Assembly), context))
                throw new InvalidDataException("Step 25 Harmony instance type escaped the dedicated load context.");
            var id = api.IdProperty.GetValue(instance) as string;
            if (!string.Equals(id, HarmonyId, StringComparison.Ordinal))
                throw new InvalidDataException($"Step 25 Harmony.Id mismatch. Expected '{HarmonyId}', observed '{id ?? "<null>"}'.");
            if (api.DebugField.GetValue(null) is not bool debugAfter || debugAfter)
                throw new InvalidDataException("Step 25 Harmony.DEBUG was true after inert constructor execution.");
            ...
            var contextMembershipAfter = context.Assemblies
                ...
                .ToArray();
            if (!contextMembershipAfter.SequenceEqual(contextMembershipBefore, StringComparer.Ordinal))
                throw new InvalidDataException("Step 25 Harmony construction changed private-context assembly membership.");
            ...
            _harmonyConstruction = new HarmonyConstructionSnapshot(
                id,
                postSha1,
                context.ManagedResolverRequests.Count - managedRequestsBefore,
                context.PrivateLoads.Count - privateLoadsBefore,
            ...
            return Pass(
                ControlledHarmonyConstructionGate.HarmonyInstanceConstruction,
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.HarmonyInstanceConstruction, stage, ex);
        }
    }

    [UnconditionalSuppressMessage("Trimming", "IL2026", Justification = "Step 25 final audit reads only exact reflection members already resolved and verified by Gate E.")]
    [UnconditionalSuppressMessage("Trimming", "IL2075", Justification = "Step 25 final audit retains only exact reflection objects already resolved and verified by Gate E.")]
    public async Task<ControlledHarmonyConstructionGateResult> RunPostConstructionAuditAsync(
        IProgress<ControlledHarmonyConstructionProgress>? progress = null,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var typeInitialization = RequireHarmonyTypeInitialization();
            var api = RequireHarmonyApi();
            var construction = RequireHarmonyConstruction();
            var context = RequireLoadContext();
            var instance = _harmonyInstance ?? throw new InvalidOperationException("Step 25 Harmony instance is missing after Gate H.");

            stage = "runtime plan rehash";
            var planSha256 = await ComputeSha256HexAsync(_planPath, cancellationToken).ConfigureAwait(false);
            if (!planSha256.Equals(preflight.PlanSha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step 25 runtime-binding plan changed during Harmony construction.");
            ...
            foreach (var item in preflight.PreparedAssemblies)
            {
                ...
                if (!preparedSha1.Equals(item.Plan.Sha1Hex, StringComparison.OrdinalIgnoreCase) ||
                    !liveSha1.Equals(item.Plan.Sha1Hex, StringComparison.OrdinalIgnoreCase))
                    throw new InvalidDataException("Step 25 prepared/live byte identity changed during Harmony construction: " + item.Plan.RelativePath);
                verified++;
                progress?.Report(new ControlledHarmonyConstructionProgress(
                    ControlledHarmonyConstructionGate.PostConstructionAudit,
                    verified,
                    preflight.PreparedAssemblies.Length,
                    item.Plan.RelativePath,
            ...
            var offline = await _offlineInspection.RunAsync(null, cancellationToken).ConfigureAwait(false);
            if (!offline.Success || !offline.ExactManagedTreeVerified || offline.InstalledManifestId != preflight.Plan.ManifestId)
                throw new InvalidDataException(offline.Error ?? "OfflineReady exact-tree verification failed after Step 25 Harmony construction.");
            ...
            var actual = context.Assemblies
                ...
                .ToArray();
            if (!actual.SequenceEqual(expected, StringComparer.Ordinal))
                throw new InvalidDataException("Step 25 post-construction private context differs from the physically proven Step 24 context membership.");
            if (context.NativeLoadAttempts.Count != 0)
                throw new DllNotFoundException("Step 25 observed native-library resolution: " + string.Join(" | ", context.NativeLoadAttempts));
            if (context.RejectedManagedRequests.Count != 0)
                throw new FileLoadException("Step 25 observed rejected/unplanned managed requests: " + string.Join(" | ", context.RejectedManagedRequests));

            stage = "Harmony object identity audit";
            if (!ReferenceEquals(instance.GetType(), api.HarmonyType))
                throw new InvalidDataException("Step 25 retained Harmony object changed runtime type.");
            var id = api.IdProperty.GetValue(instance) as string;
            if (!string.Equals(id, HarmonyId, StringComparison.Ordinal) || !string.Equals(id, construction.Id, StringComparison.Ordinal))
                throw new InvalidDataException("Step 25 retained Harmony object ID changed after construction.");
            if (api.DebugField.GetValue(null) is not bool debugValue || debugValue)
                throw new InvalidDataException("Step 25 Harmony.DEBUG is true during the final audit.");
            ...
            if (!targetSha1.Equals(initialization.PreparedSha1, StringComparison.OrdinalIgnoreCase) ||
                ...
                throw new InvalidDataException("Step 25 0Harmony prepared hash changed across module initialization/type initialization/construction.");

            return Pass(
                ControlledHarmonyConstructionGate.PostConstructionAudit,
                "Step 25 post-construction isolation audit passed.\n" +
                $"Prepared/live assemblies re-hashed: {verified:N0}/{preflight.PreparedAssemblies.Length:N0}\n" +
                $"Runtime plan SHA-256 unchanged: {planSha256}\n" +
                $"Private context: {actual.Length:N0}/{expected.Length:N0} expected assemblies\n" +
                ...
                "Godot/game initialization: NO\n" +
                "Native game library loaded by Step 25: NO\n" +
                "Process note: the Step 25 private managed context remains resident until process exit; force-quit before rerunning earlier fresh-process CLR-load regressions.");
        }
        catch (OperationCanceledException)
        {
            throw;
        ...
        {
            return Fail(ControlledHarmonyConstructionGate.PostConstructionAudit, stage, ex);
        }
    }
    ...
    private void ClearStep25ObjectState()
    {
    ...
    private void EnsureFreshProcess()
    {
        var matches = AppDomain.CurrentDomain.GetAssemblies()
            ...
            .ToArray();
        if (matches.Length != 0)
            throw new InvalidDataException("Step 25 requires a fresh process; a game/Harmony assembly is already loaded: " + string.Join(" | ", matches));
    }

    private static HarmonyConstructorMetadataSnapshot ReadHarmonyConstructorMetadata(string path)
    {
        using var resolver = new Step25MetadataOnlyResolver(path);
        ...
        if (typeInitializers.Length != 1 || !typeInitializers[0].HasBody)
        {
            return new HarmonyConstructorMetadataSnapshot(
                false,
                $"Step 25 requires exactly one managed HarmonyLib.Harmony type initializer; observed {typeInitializers.Length}.",
        ...
        if (constructor is null || publicConstructors.Length != 1 || !constructor.HasBody)
        {
            return new HarmonyConstructorMetadataSnapshot(
                false,
                "Step 25 requires exactly one public HarmonyLib.Harmony constructor, .ctor(System.String), with managed IL.",
    ...
    private DeferredInitializationSnapshot RequireInitialization()
        => _initialization ?? throw new InvalidOperationException("Step 25 Gate C must pass before later Step 25 gates run.");

    private HarmonyApiSnapshot RequireHarmonyApi()
        => _harmonyApi ?? throw new InvalidOperationException("Step 25 Gate E must pass before Harmony type initialization runs.");

    private HarmonyTypeInitializationSnapshot RequireHarmonyTypeInitialization()
        => _harmonyTypeInitialization ?? throw new InvalidOperationException("Step 25 Gate F must pass before later Harmony gates run.");

    private HarmonyConstructionSnapshot RequireHarmonyConstruction()
        => _harmonyConstruction ?? throw new InvalidOperationException("Step 25 Gate H must pass before the final Step 25 audit runs.");

    private Step25LoadContext RequireLoadContext()
        => _loadContext ?? throw new InvalidOperationException("Step 25 dedicated load context is unavailable.");

    private void ThrowIfDisposed()
    {
        if (_disposed)
            throw new ObjectDisposedException(nameof(ControlledHarmonyConstruction));
    }

    private static ControlledHarmonyConstructionGateResult Pass(ControlledHarmonyConstructionGate gate, string detail)
        => new(gate, true, detail);

    private static ControlledHarmonyConstructionGateResult Fail(ControlledHarmonyConstructionGate gate, string stage, Exception ex)
        => new(gate, false, $"Stage: {stage}\n{ex}");

    private sealed class Step25MetadataOnlyResolver(string auditedPath) : IAssemblyResolver, IMetadataResolver
    {
    ...
    private sealed record HarmonyConstructionSnapshot(
        string Id,
        string PreparedSha1,
    ...
    private sealed class Step25LoadContext : AssemblyLoadContext
    {
