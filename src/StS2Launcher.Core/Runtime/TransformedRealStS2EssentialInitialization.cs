...
/// <summary>
/// Step 36.0.5 boundary. The forward path is intentionally available only after the Step-35 MODEL-BOOTSTRAP
...
public sealed partial class TransformedRealStS2VeryEarlyInitialization
{
    ...
    public string GetVerifiedEssentialStaticInstructionMap()
        => _essentialPreflight?.StaticInstructionMap
           ?? throw new InvalidOperationException("Step 36.0.5 Gate A has not produced a verified ExecuteEssential static map.");
        ...
        Action<string>? checkpoint = null)
    {
        ...
        try
        {
            ...
            if (!IsExactAuthorityMode && !IsModelBootstrapCompatibilityMode)
                throw new InvalidOperationException("Step 36.0.5 requires either the exact Step-35 authority or the verified ModelDb bootstrap compatibility authority; legacy diagnostic derivatives are not accepted.");
    ...
    private void RequireExactStep35CoreClosure(string boundary)
    {
        if (!_exactStep35CoreClosurePassed || _step36Baseline is null)
            throw new InvalidOperationException($"{boundary} requires a successful Step-35 exact/model-bootstrap authority closure in this same process. Run Step 15 A-C, then Step 35 MODEL-BOOTSTRAP 4/4 (preferred for Step 36.0.5) or EXACT-CLOSURE 4/4 first.");
        if (!IsExactAuthorityMode && !IsModelBootstrapCompatibilityMode)
