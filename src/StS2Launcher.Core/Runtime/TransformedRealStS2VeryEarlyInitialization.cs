...
public sealed class TransformedRealStS2VeryEarlyInitialization : IDisposable
{
    ...
    private const string DiagnosticCloneFileName = "sts2.step35.0.24.instrumented.dll";
    private const string GodotSharpDiagnosticCloneFileName = "GodotSharp.step35.0.24.instrumented.dll";
    ...
    public TransformedRealStS2VeryEarlyInitializationGateResult RunExecutionCapableClrAdmission(Action<string>? crashCheckpoint)
    {
        ...
        try
        {
            ...
            return Pass(gate,
                "STEP-33 ZERO-RESOLUTION ADMISSION BEHAVIOR RE-ESTABLISHED FOR THE STEP-35.0.24 INSTRUMENTED DIAGNOSTIC CLONE; NO GAME MEMBER REFLECTION/INVOCATION YET.\n" +
    ...
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.24 callback-handoff mode deliberately loads the already hash-pinned GodotSharp diagnostic derivative into the dedicated context and reflects only its exact NativeFuncs.Initialize(IntPtr,int) callback-table receiver.")]
    ...
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.24 deliberately reflects and invokes one instrumented diagnostic clone of the exact async initialization method after re-verifying the exact transformed source. The dynamic payload is preserved by the physical copy/no-link runtime policy.")]
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.24 reflects and invokes only the separately verified diagnostic clone of ExecuteVeryEarly; this derivative is localization evidence and is never exact Step-35 closure evidence.")]
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.24 managed-plugin bootstrap preparation reflects only the exact GodotPlugins.Game.Main generated entry-point contract plus GodotSharp ManagedCallbacks.Create(IntPtr) and ScriptManagerBridge.LookupScriptsInAssembly(Assembly) in the already verified private assemblies.")]
        ...
        Action<string>? crashCheckpoint)
    {
        ...
        try
        {
            ...
            if (crashCheckpoint is null)
                throw new InvalidOperationException("Step-35.0.24 managed-plugin reverse-bridge preparation requires durable launcher-owned checkpoint telemetry.");
            ...
            if (_managedPluginReverseBridgePrepared)
                throw new InvalidOperationException("Step-35.0.24 managed-plugin reverse bridge has already been prepared in this execution context.");
            ...
            if (!ReferenceEquals(AssemblyLoadContext.GetLoadContext(godotAssembly), context) ||
                ...
                godotAssembly.ManifestModule.ModuleVersionId != preflight.GodotSharpDiagnostic.Mvid)
                throw new InvalidDataException("Step-35.0.24 reverse-bridge preparation lost the verified private GodotSharp identity/context.");
            if (context.InitializerBearingRequests.Count != 0 || context.RejectedManagedRequests.Count != 0 || context.NativeLoadAttempts.Count != 0)
                throw new InvalidDataException("Step-35.0.24 reverse-bridge preparation entered with dirty forbidden resolver state. " + context.FormatResolverState());
            ...
            if (context.InitializerBearingRequests.Count != 0 || context.RejectedManagedRequests.Count != 0 || context.NativeLoadAttempts.Count != 0)
                throw new InvalidDataException("Step-35.0.24 reverse-bridge preparation escaped the allowed resolver boundary. " + context.FormatResolverState());
    ...
    public string SealGodotManagedPluginBootstrapResolverBaseline(Action<string>? crashCheckpoint)
    {
        ...
        try
        {
            ...
            if (crashCheckpoint is null)
                throw new InvalidOperationException("Step-35.0.24 post-bootstrap resolver baseline requires durable launcher-owned checkpoint telemetry.");
            ...
            if (context.InitializerBearingRequests.Count != 0 || context.RejectedManagedRequests.Count != 0 || context.NativeLoadAttempts.Count != 0)
                throw new InvalidDataException("Step-35.0.24 bootstrap resolver sealing observed forbidden resolver/native activity. " + context.FormatResolverState());
            ...
            if (managedDelta.Length != expectedManagedDelta.Length ||
                ...
                throw new InvalidDataException("Step-35.0.24 generated-plugin bootstrap managed resolver delta drifted from the physically measured 0.0.146 eight-request host-framework closure. Observed: " + string.Join(" | ", managedDelta));
            ...
            if (hostRequestDelta.Length != expectedManagedDelta.Length ||
                ...
                throw new InvalidDataException("Step-35.0.24 generated-plugin bootstrap host-load delta drifted from the exact managed request closure. Observed: " + string.Join(" | ", hostDelta));
            if (privateDelta.Length != 0)
                throw new InvalidDataException("Step-35.0.24 generated-plugin bootstrap unexpectedly loaded an additional private dependency: " + string.Join(" | ", privateDelta));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (crashCheckpoint is null)
                throw new InvalidOperationException("Step-35.0.24 diagnostic Gate C requires a durable launcher-owned checkpoint callback; refusing to execute an instrumented clone without in-method telemetry.");
            ...
            if (DiagnosticMode == Step35DiagnosticMode.GodotCoreCallbackHandoff)
            {
                var handoff = _callbackHandoff ?? throw new InvalidOperationException("Step-35.0.24 GodotCoreCallbackHandoff mode requires the controlled NativeFuncs.Initialize handoff after Gate B and before Gate C.");
                var bootstrapBaseline = _managedPluginBootstrapResolverBaseline ?? throw new InvalidOperationException("Step-35.0.24 GodotCoreCallbackHandoff mode requires the generated-plugin resolver baseline to be sealed after bootstrap and before Gate C.");
            ...
            return Pass(gate,
                "STEP-35.0.24 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE.\n" +
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return Pass(gate,
                "STEP-35.0.24 DIAGNOSTIC-CLONE FINAL ISOLATION AUDIT PASSED; THIS DOES NOT CLOSE EXACT STEP 35.\n" +
        ...
        MethodDefinition? commandLineHelperTryGetValue = null)
    {
        ...
        if (nullPlatformConstructor is not null)
        {
            ...
            lines.Add("Step 35.0.24 dynamic constructor markers use the exact-source CALLSITE ordinals below; the direct base-constructor call is intentionally not wrapped.");
    ...
    private static DiagnosticCloneSnapshot CreateInstrumentedDiagnosticClone(string exactTransformedPath, string diagnosticPath, Step35DiagnosticMode diagnosticMode)
    {
               ...
               }))
        {
            ...
            // construction before _args assignment. Step 35.0.24 keeps the exact-source map and markers.
