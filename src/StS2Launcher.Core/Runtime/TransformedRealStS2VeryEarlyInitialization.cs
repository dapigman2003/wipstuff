...
public sealed class TransformedRealStS2VeryEarlyInitialization : IDisposable
{
    ...
    private const string DiagnosticCloneFileName = "sts2.step35.0.21.instrumented.dll";
    private const string GodotSharpDiagnosticCloneFileName = "GodotSharp.step35.0.21.instrumented.dll";
        ...
        try
        {
                   ...
                   }))
            {
                ...
                var transformedNullPlatformType = EnumerateTypes(transformedModule.Types).SingleOrDefault(type => type.FullName == NullPlatformTypeFullName)
                    ?? throw new MissingMemberException($"Step-35.0.21 static-map target type missing: {NullPlatformTypeFullName}.");
                var transformedNullPlatformConstructor = transformedNullPlatformType.Methods.SingleOrDefault(method => method.FullName == NullPlatformConstructorFullName && method.HasBody)
                    ?? throw new MissingMethodException($"Step-35.0.21 static-map target constructor missing: {NullPlatformConstructorFullName}.");
                var transformedCommandLineHelperType = EnumerateTypes(transformedModule.Types).SingleOrDefault(type => type.FullName == CommandLineHelperTypeFullName)
                    ?? throw new MissingMemberException($"Step-35.0.21 static-map target type missing: {CommandLineHelperTypeFullName}.");
                var transformedCommandLineHelperCctor = transformedCommandLineHelperType.Methods.SingleOrDefault(method => method.Name == ".cctor" && method.IsStatic && method.HasBody)
                    ?? throw new MissingMethodException($"Step-35.0.21 static-map target cctor missing: {CommandLineHelperTypeFullName}..cctor.");
                var transformedCommandLineHelperTryGetValue = transformedCommandLineHelperType.Methods.SingleOrDefault(method => method.FullName == CommandLineHelperTryGetValueFullName && method.HasBody)
                    ?? throw new MissingMethodException($"Step-35.0.21 static-map target method missing: {CommandLineHelperTryGetValueFullName}.");
            ...
            stage = "Step-35.0.21 diagnostic-clone instrumentation";
            ...
            var preparedGodotSharp = prepared.SingleOrDefault(item => !item.Plan.IsPrimary && string.Equals(item.AssemblyName.Name, "GodotSharp", StringComparison.OrdinalIgnoreCase))
                ?? throw new InvalidDataException("Step-35.0.21 comprehensive reconnaissance requires the exact prepared GodotSharp private dependency.");
            if (preparedGodotSharp.ModuleInitializerCount != 0)
                throw new InvalidDataException("Step-35.0.21 refuses to create a runtime diagnostic derivative from initializer-bearing GodotSharp metadata.");
            ...
            if (!godotSharpDiagnostic.AssemblyIdentity.Equals(preparedGodotSharp.Plan.AssemblyFullName, StringComparison.Ordinal))
                throw new InvalidDataException($"Step-35.0.21 GodotSharp diagnostic identity drifted from prepared plan: {godotSharpDiagnostic.AssemblyIdentity} != {preparedGodotSharp.Plan.AssemblyFullName}.");
            ...
            if (!godotSourceSha1AfterDiagnostic.Equals(preparedGodotSharp.Plan.Sha1Hex, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic-clone emission changed the exact prepared source; refusing to continue.");
            ...
            if (!offlineForRecon.Success || !offlineForRecon.ExactManagedTreeVerified || string.IsNullOrWhiteSpace(offlineForRecon.ManagedInstallRelativePath))
                throw new InvalidDataException("Step-35.0.21 native reconnaissance requires the exact Step-13 OfflineReady managed tree.");
            ...
            return Pass(gate,
                ...
                $"Step-35.0.21 diagnostic clone SHA-256: {diagnostic.Sha256}\n" +
                $"Step-35.0.21 diagnostic clone bytes: {diagnostic.Length:N0}\n" +
    ...
    public TransformedRealStS2VeryEarlyInitializationGateResult RunExecutionCapableClrAdmission(Action<string>? crashCheckpoint)
    {
        ...
        try
        {
            ...
            Checkpoint(crashCheckpoint, "B_HASH_START — rechecking the exact closed transformed primary and the Step-35.0.21 instrumented diagnostic clone before CLR admission.");
            ...
            VerifyFileLength(preflight.DiagnosticPath, preflight.DiagnosticLength, "Step-35.0.21 instrumented diagnostic clone");
            ...
            if (!immediateSha256.Equals(preflight.DiagnosticSha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step-35.0.21 diagnostic clone changed between Gate A instrumentation and Gate B CLR admission.");
            VerifyFileLength(preflight.GodotSharpDiagnostic.Path, preflight.GodotSharpDiagnostic.Length, "Step-35.0.21 GodotSharp diagnostic clone");
            ...
            if (!immediateGodotSha256.Equals(preflight.GodotSharpDiagnostic.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic clone changed between Gate A instrumentation and Gate B admission preparation.");
            ...
            Checkpoint(crashCheckpoint, "B_LOADPRIMARY_START — entering Step-35.0.21 instrumented diagnostic-clone LoadPrimary/LoadFromStream path; exact closed transformed source remains untouched on disk.");
            ...
            if (!ReferenceEquals(AssemblyLoadContext.GetLoadContext(assembly), context))
                throw new InvalidDataException("The Step-35.0.21 diagnostic sts2.dll clone did not load into the dedicated Step-35 AssemblyLoadContext.");
            ...
            {
                throw new InvalidDataException(
                    "Step-35.0.21 diagnostic-clone admission no longer matches the physically closed Step-33 zero-resolution admission behavior. " +
            ...
            return Pass(gate,
                "STEP-33 ZERO-RESOLUTION ADMISSION BEHAVIOR RE-ESTABLISHED FOR THE STEP-35.0.21 INSTRUMENTED DIAGNOSTIC CLONE; NO GAME MEMBER REFLECTION/INVOCATION YET.\n" +
    ...
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.21 callback-handoff mode deliberately loads the already hash-pinned GodotSharp diagnostic derivative into the dedicated context and reflects only its exact NativeFuncs.Initialize(IntPtr,int) callback-table receiver.")]
        ...
        Action<string>? crashCheckpoint)
    {
        ...
        try
        {
            ...
            if (crashCheckpoint is null)
                throw new InvalidOperationException("Step-35.0.21 callback handoff requires durable launcher-owned checkpoint telemetry.");
            ...
            if (_callbackHandoff is not null)
                throw new InvalidOperationException("Step-35.0.21 callback handoff has already run in this execution context.");
            ...
            if (!ReferenceEquals(AssemblyLoadContext.GetLoadContext(godotAssembly), context))
                throw new InvalidDataException("Step-35.0.21 callback handoff GodotSharp assembly did not belong to the dedicated Step-35 context.");
            if (!string.Equals(godotAssembly.GetName().FullName, preflight.GodotSharpDiagnostic.AssemblyIdentity, StringComparison.Ordinal) ||
                ...
                throw new InvalidDataException("Step-35.0.21 callback handoff GodotSharp identity/MVID drifted from the verified derivative.");
    ...
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.21 deliberately reflects and invokes one instrumented diagnostic clone of the exact async initialization method after re-verifying the exact transformed source. The dynamic payload is preserved by the physical copy/no-link runtime policy.")]
    [UnconditionalSuppressMessage(
        "Trimming",
        "IL2026",
        Justification = "Step 35.0.21 reflects and invokes only the separately verified diagnostic clone of ExecuteVeryEarly; this derivative is localization evidence and is never exact Step-35 closure evidence.")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (crashCheckpoint is null)
                throw new InvalidOperationException("Step-35.0.21 diagnostic Gate C requires a durable launcher-owned checkpoint callback; refusing to execute an instrumented clone without in-method telemetry.");
            ...
            if (DiagnosticMode == Step35DiagnosticMode.GodotCoreCallbackHandoff)
            {
                var handoff = _callbackHandoff ?? throw new InvalidOperationException("Step-35.0.21 GodotCoreCallbackHandoff mode requires the controlled NativeFuncs.Initialize handoff after Gate B and before Gate C.");
                if (context.ManagedResolverRequests.Count != handoff.ManagedResolverRequests ||
                    context.HostLoads.Count != handoff.HostLoads ||
                    context.PrivateLoads.Count != handoff.PrivateLoads ||
                    context.InitializerBearingRequests.Count != handoff.InitializerBearingRequests ||
                    context.RejectedManagedRequests.Count != handoff.RejectedManagedRequests ||
                    context.NativeLoadAttempts.Count != handoff.NativeLoadAttempts)
                    throw new InvalidDataException("Step-35 resolver state changed after the callback handoff and before Gate C invocation. " + context.FormatResolverState());
                if (handoff.InitializerBearingRequests != 0 || handoff.RejectedManagedRequests != 0 || handoff.NativeLoadAttempts != 0)
                    throw new InvalidDataException("Step-35 callback handoff baseline contains forbidden resolver/native activity.");
                Checkpoint(crashCheckpoint, $"C_RESOLVER_PRECHECK_PASS — callback-handoff baseline is intact immediately before target binding; tableBytes={handoff.CallbackTableSizeBytes}; {context.FormatResolverState()}.");
            }
            else
            {
                if (context.ManagedResolverRequests.Count != 0 || context.PrivateLoads.Count != 0 ||
            ...
            if (!ReferenceEquals(targetType.Assembly, admission.Assembly))
                throw new InvalidDataException("Step-35.0.21 target type did not bind from the admitted diagnostic sts2 clone.");
            ...
            if (!ReferenceEquals(method.DeclaringType, targetType) || !method.IsStatic || method.ReturnType != typeof(Task) || method.GetParameters().Length != 0)
                throw new InvalidDataException("Step-35.0.21 reflected diagnostic ExecuteVeryEarly identity/signature drifted from the exact static parameterless System.Threading.Tasks.Task target.");
            ...
            if (method.MetadataToken != unchecked((int)preflight.DiagnosticMethodToken))
                throw new InvalidDataException($"Step-35.0.21 reflected instrumented ExecuteVeryEarly token drifted: 0x{method.MetadataToken:X8} != preflight diagnostic 0x{preflight.DiagnosticMethodToken:X8}.");
            ...
            stage = "Step-35.0.21 in-method checkpoint bridge arm";
            ...
            if (bridgeField.FieldType != typeof(Action<string>))
                throw new InvalidDataException($"Step-35.0.21 diagnostic bridge field type drifted: {bridgeField.FieldType.FullName}.");
            ...
            try
            {
                Checkpoint(crashCheckpoint, "C_INVOKE_START — entering the first and only MethodInfo.Invoke(null, null) for the Step-35.0.21 instrumented ExecuteVeryEarly diagnostic clone.");
            ...
            catch (TargetInvocationException ex)
            {
                ...
                throw new InvalidOperationException(
                    "Step-35.0.21 instrumented ExecuteVeryEarly threw synchronously during the first controlled invocation. " +
            ...
            {
                throw new InvalidOperationException(
                    "Step-35.0.21 diagnostic-clone ExecuteVeryEarly Task faulted during the controlled await. " +
            ...
            return Pass(gate,
                "STEP-35.0.21 DIAGNOSTIC-CLONE EXECUTEVERYEARLY INVOCATION/AWAIT COMPLETED NORMALLY; THIS IS LOCALIZATION EVIDENCE, NOT EXACT STEP-35 CLOSURE.\n" +
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!diagnosticSha256.Equals(preflight.DiagnosticSha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step-35.0.21 instrumented diagnostic clone changed during ExecuteVeryEarly execution.");
            ...
            if (!godotDiagnosticSha256.Equals(preflight.GodotSharpDiagnostic.Sha256, StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic clone changed during ExecuteVeryEarly execution.");
            ...
            if (matches.Length != 1 || !ReferenceEquals(matches[0], admission.Assembly) || !ReferenceEquals(AssemblyLoadContext.GetLoadContext(admission.Assembly), context))
                throw new InvalidDataException("Step-35.0.21 diagnostic-clone CLR residency/context ownership drifted during final audit.");
            if (execution.MethodToken != unchecked((int)preflight.DiagnosticMethodToken))
                throw new InvalidDataException("Step-35.0.21 execution snapshot diagnostic ExecuteVeryEarly token drifted during final audit.");
            ...
            return Pass(gate,
                "STEP-35.0.21 DIAGNOSTIC-CLONE FINAL ISOLATION AUDIT PASSED; THIS DOES NOT CLOSE EXACT STEP 35.\n" +
                ...
                "Resident sts2 load input: Step-35.0.21 instrumented diagnostic clone derived from the reverified exact Step-32 transformed image\n" +
        ...
        MethodDefinition? commandLineHelperTryGetValue = null)
    {
        ...
        if (nullPlatformConstructor is not null)
        {
            ...
            lines.Add("Step 35.0.21 dynamic constructor markers use the exact-source CALLSITE ordinals below; the direct base-constructor call is intentionally not wrapped.");
    ...
    private static DiagnosticCloneSnapshot CreateInstrumentedDiagnosticClone(string exactTransformedPath, string diagnosticPath, Step35DiagnosticMode diagnosticMode)
    {
               ...
               }))
        {
            if (resolver.Requests.Count != 0)
                throw new InvalidDataException("Step-35.0.21 diagnostic deferred-open unexpectedly resolved a dependency before the bounded writer resolver was configured.");
            ...
            if (EnumerateTypes(module.Types).Any(type => type.FullName == DiagnosticBridgeTypeFullName))
                throw new InvalidDataException("Step-35.0.21 diagnostic bridge type already exists in the exact transformed image.");
            ...
            var systemRuntime = module.AssemblyReferences
                ...
                .FirstOrDefault()
                ?? throw new InvalidDataException("Step-35.0.21 diagnostic clone requires the existing System.Runtime metadata scope.");
            ...
            foreach (var item in markers)
            {
                var type = EnumerateTypes(module.Types).SingleOrDefault(candidate => candidate.FullName == item.TypeName)
                    ?? throw new MissingMemberException($"Step-35.0.21 diagnostic marker target type missing: {item.TypeName}.");
                var methods = type.Methods.Where(method => method.FullName == item.MethodFullName && method.HasBody).ToArray();
                if (methods.Length != 1)
                    throw new MissingMethodException($"Step-35.0.21 expected exactly one managed-IL marker target {item.MethodFullName}, found {methods.Length}.");
            ...
            {
                var type = EnumerateTypes(module.Types).SingleOrDefault(candidate => candidate.FullName == item.TypeName)
                    ?? throw new MissingMemberException($"Step-35.0.21 diagnostic callsite target type missing: {item.TypeName}.");
                var method = type.Methods.SingleOrDefault(candidate => candidate.FullName == item.MethodFullName && candidate.HasBody)
                    ?? throw new MissingMethodException($"Step-35.0.21 diagnostic callsite target method missing: {item.MethodFullName}.");
            ...
            var nullPlatformType = EnumerateTypes(module.Types).SingleOrDefault(candidate => candidate.FullName == NullPlatformTypeFullName)
                ?? throw new MissingMemberException($"Step-35.0.21 NullPlatform callsite-sweep type missing: {NullPlatformTypeFullName}.");
            var nullPlatformConstructor = nullPlatformType.Methods.SingleOrDefault(candidate => candidate.FullName == NullPlatformConstructorFullName && candidate.HasBody)
                ?? throw new MissingMethodException($"Step-35.0.21 NullPlatform callsite-sweep constructor missing: {NullPlatformConstructorFullName}.");
            ...
            var commandLineType = EnumerateTypes(module.Types).SingleOrDefault(candidate => candidate.FullName == CommandLineHelperTypeFullName)
                ?? throw new MissingMemberException($"Step-35.0.21 CommandLineHelper sweep type missing: {CommandLineHelperTypeFullName}.");
            var commandLineCctor = commandLineType.Methods.SingleOrDefault(candidate => candidate.Name == ".cctor" && candidate.IsStatic && candidate.HasBody)
                ?? throw new MissingMethodException($"Step-35.0.21 CommandLineHelper cctor missing: {CommandLineHelperTypeFullName}..cctor.");
            var commandLineTryGetValue = commandLineType.Methods.SingleOrDefault(candidate => candidate.FullName == CommandLineHelperTryGetValueFullName && candidate.HasBody)
                ?? throw new MissingMethodException($"Step-35.0.21 CommandLineHelper method missing: {CommandLineHelperTryGetValueFullName}.");
            ...
            // construction before _args assignment. Step 35.0.21 keeps the exact-source map and markers.
            ...
            if (diagnosticMode is Step35DiagnosticMode.ManagedDictionaryCompatibility or Step35DiagnosticMode.ManagedCommandLineCompatibility)
            {
                ...
                if (commandLineManagedDictionaryRewrite.SubstitutionCount != 4)
                    throw new InvalidDataException($"Step-35.0.21 expected exactly four CommandLine managed-dictionary compatibility substitutions; observed {commandLineManagedDictionaryRewrite.SubstitutionCount}.");
            ...
            if (diagnosticMode == Step35DiagnosticMode.ManagedCommandLineCompatibility)
            {
                ...
                if (providerRewrite.SubstitutionCount != 1)
                    throw new InvalidDataException($"Step-35.0.21 expected exactly one managed command-line provider substitution; observed {providerRewrite.SubstitutionCount}.");
            ...
            if (commandLineCctorDiagnosticMaxStack != commandLineCctorOriginalMaxStack)
                throw new InvalidDataException($"Step-35.0.21 stack-neutral CommandLineHelper cctor unexpectedly changed MaxStack before serialization: original={commandLineCctorOriginalMaxStack}, diagnostic={commandLineCctorDiagnosticMaxStack}.");
        ...
        if (verifyResolver.Requests.Count != 0)
            throw new InvalidDataException("Step-35.0.21 diagnostic clone verification unexpectedly resolved a dependency.");
        ...
        if (!verifiedConstantMetadataSha256.Equals(expectedConstantMetadataSha256, StringComparison.OrdinalIgnoreCase))
            throw new InvalidDataException("Step-35.0.21 diagnostic clone changed the exact transformed image's constant metadata semantics during Cecil serialization.");
        if (verifyModule.Assembly?.Name.FullName != TransformedRealStS2AssemblyAdmission.ClosedStep32AssemblyIdentity ||
            ...
            throw new InvalidDataException("Step-35.0.21 diagnostic clone changed assembly identity or MVID.");
        ...
        {
            throw new InvalidDataException("Step-35.0.21 diagnostic bridge field/method signature drifted after serialization.");
        }
        ...
        {
            throw new InvalidDataException("Step-35.0.21 diagnostic bridge Invoke MemberRef is not encoded as Action<string>::Invoke(!0).");
        }
        ...
        foreach (var item in GetDiagnosticMarkerTargets())
        {
            var type = EnumerateTypes(verifyModule.Types).SingleOrDefault(candidate => candidate.FullName == item.TypeName)
                ?? throw new MissingMemberException($"Step-35.0.21 diagnostic verification target type missing: {item.TypeName}.");
            var method = type.Methods.SingleOrDefault(candidate => candidate.FullName == item.MethodFullName && candidate.HasBody)
                ?? throw new MissingMethodException($"Step-35.0.21 diagnostic verification target method missing: {item.MethodFullName}.");
            if (!HasInjectedEntryMarkerAtStart(method, item.Marker))
                throw new InvalidDataException($"Step-35.0.21 marker is not the first stack-neutral checkpoint in {item.MethodFullName}: {item.Marker}.");
            expectedMarkerCount++;
            ...
            if (cctor is not null && verifiedCctorTypes.Add(item.TypeName))
            {
                ...
                if (!HasInjectedEntryMarkerAtStart(cctor, cctorMarker))
                    throw new InvalidDataException($"Step-35.0.21 cctor marker is not the first stack-neutral checkpoint in {item.TypeName}..cctor.");
                expectedMarkerCount++;
            }
        }
        ...
        {
            var type = EnumerateTypes(verifyModule.Types).SingleOrDefault(candidate => candidate.FullName == item.TypeName)
                ?? throw new MissingMemberException($"Step-35.0.21 diagnostic callsite verification type missing: {item.TypeName}.");
            var method = type.Methods.SingleOrDefault(candidate => candidate.FullName == item.MethodFullName && candidate.HasBody)
                ?? throw new MissingMethodException($"Step-35.0.21 diagnostic callsite verification method missing: {item.MethodFullName}.");
            if (!HasInjectedCallsiteMarkers(method, item.CalleeFullName, item.BeforeMarker, item.AfterMarker))
                throw new InvalidDataException($"Step-35.0.21 callsite markers did not serialize immediately around {item.CalleeFullName} in {item.MethodFullName}.");
        ...
        var verifiedNullPlatformType = EnumerateTypes(verifyModule.Types).SingleOrDefault(candidate => candidate.FullName == NullPlatformTypeFullName)
            ?? throw new MissingMemberException($"Step-35.0.21 serialized NullPlatform sweep type missing: {NullPlatformTypeFullName}.");
        var verifiedNullPlatformConstructor = verifiedNullPlatformType.Methods.SingleOrDefault(candidate => candidate.FullName == NullPlatformConstructorFullName && candidate.HasBody)
            ?? throw new MissingMethodException($"Step-35.0.21 serialized NullPlatform sweep constructor missing: {NullPlatformConstructorFullName}.");
        foreach (var entry in nullPlatformCallsitePlan)
        {
            if (!HasInjectedDiagnosticCallsiteMarkers(verifiedNullPlatformConstructor, entry))
                throw new InvalidDataException($"Step-35.0.21 serialized NullPlatform CALLSITE#{entry.CallsiteOrdinal:D3} marker pair drifted around {entry.CalleeFullName}.");
            expectedMarkerCount += 2;
        }
        ...
        var verifiedCommandLineType = EnumerateTypes(verifyModule.Types).SingleOrDefault(candidate => candidate.FullName == CommandLineHelperTypeFullName)
            ?? throw new MissingMemberException($"Step-35.0.21 serialized CommandLineHelper sweep type missing: {CommandLineHelperTypeFullName}.");
        var verifiedCommandLineCctor = verifiedCommandLineType.Methods.SingleOrDefault(candidate => candidate.Name == ".cctor" && candidate.IsStatic && candidate.HasBody)
            ?? throw new MissingMethodException($"Step-35.0.21 serialized CommandLineHelper cctor missing: {CommandLineHelperTypeFullName}..cctor.");
        ...
        {
            throw new InvalidDataException($"Step-35.0.21 serialized stack-neutral CommandLineHelper cctor MaxStack drifted: original={commandLineCctorOriginalMaxStack}, observed={verifiedCommandLineCctor.Body.MaxStackSize}.");
        }
        if (!HasCommandLineHelperCriticalBoundaryMarkers(verifiedCommandLineCctor))
            throw new InvalidDataException("Step-35.0.21 serialized CommandLineHelper cctor critical stack-neutral markers drifted.");
        ...
        var verifiedCommandLineTryGetValue = verifiedCommandLineType.Methods.SingleOrDefault(candidate => candidate.FullName == CommandLineHelperTryGetValueFullName && candidate.HasBody)
            ?? throw new MissingMethodException($"Step-35.0.21 serialized CommandLineHelper TryGetValue missing: {CommandLineHelperTryGetValueFullName}.");
        ...
        {
            throw new InvalidDataException($"Step-35.0.21 diagnostic mode is unsupported: {diagnosticMode}.");
        }
        ...
        if (markerCountVerified != expectedMarkerCount)
            throw new InvalidDataException($"Step-35.0.21 diagnostic clone marker count drifted after serialization: expected {expectedMarkerCount}, observed {markerCountVerified}.");
    ...
    internal static GodotSharpDiagnosticCloneSnapshot CreateInstrumentedGodotSharpDiagnosticClone(string exactPreparedPath, string diagnosticPath)
    {
        ...
        {
            if (resolver.Requests.Count != 0)
                throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic deferred-open unexpectedly resolved a dependency before writer configuration.");
            sourceIdentity = module.Assembly?.Name.FullName ?? throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic source has no assembly identity.");
            ...
            if (EnumerateTypes(module.Types).Any(type => type.FullName == GodotSharpDiagnosticBridgeTypeFullName))
                throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic bridge type already exists in the exact prepared image.");
            ...
            var systemRuntime = module.AssemblyReferences
                ...
                .OrderByDescending(reference => reference.Version)
                .FirstOrDefault()
                ?? throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic clone requires the existing System.Runtime metadata scope.");
            var (actionStringType, invoke) = CreateDiagnosticActionStringInvokeReference(module, systemRuntime);
            var callbackField = new FieldDefinition(
                GodotSharpDiagnosticBridgeCallbackFieldName,
                Mono.Cecil.FieldAttributes.Public | Mono.Cecil.FieldAttributes.Static,
            ...
            if (markerPlan.Count < 6)
                throw new InvalidDataException($"Step-35.0.21 GodotSharp diagnostic marker plan is unexpectedly sparse: {markerPlan.Count} method(s).");
            foreach (var item in markerPlan)
            {
                var method = EnumerateTypes(module.Types)
                    ...
                    ?? throw new MissingMethodException($"Step-35.0.21 GodotSharp diagnostic marker target disappeared: {item.MethodFullName}.");
        ...
        if (verifyResolver.Requests.Count != 0)
            throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic verification unexpectedly resolved a dependency.");
        if ((verifyModule.Assembly?.Name.FullName ?? string.Empty) != sourceIdentity || verifyModule.Mvid != sourceMvid)
            throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic clone changed assembly identity or MVID.");
        ...
        if (callback.FieldType.FullName != "System.Action`1<System.String>" || bridgeEmit.Parameters.Count != 1 || bridgeEmit.Parameters[0].ParameterType.FullName != "System.String")
            throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic bridge signature drifted after serialization.");
        ...
        if (invokeSites.Length != 1 || invokeSites[0].Operand is not MethodReference bridgeInvoke ||
            ...
            bridgeInvokeParameter.Type != GenericParameterType.Type || bridgeInvokeParameter.Position != 0)
        {
            throw new InvalidDataException("Step-35.0.21 GodotSharp diagnostic bridge Invoke MemberRef is not encoded as Action<string>::Invoke(!0).");
        }
        ...
        foreach (var item in markerPlan)
        {
            var method = EnumerateTypes(verifyModule.Types)
                .SelectMany(type => type.Methods)
                .SingleOrDefault(method => method.FullName == item.MethodFullName && method.HasBody)
                ?? throw new MissingMethodException($"Step-35.0.21 serialized GodotSharp marker target missing: {item.MethodFullName}.");
            if (!HasInjectedEntryMarkerAtStart(method, item.Marker, GodotSharpDiagnosticBridgeTypeFullName))
                throw new InvalidDataException($"Step-35.0.21 serialized GodotSharp marker is not first in {item.MethodFullName}: {item.Marker}.");
        }
        ...
        if (markerCount != markerPlan.Count)
            throw new InvalidDataException($"Step-35.0.21 GodotSharp diagnostic marker count drifted: expected {markerPlan.Count}, observed {markerCount}.");
    ...
    private static IReadOnlyList<GodotSharpDiagnosticMarker> BuildGodotSharpDiagnosticMarkerPlan(ModuleDefinition module)
    {
        ...
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.Collections.Dictionary`2" && method.Name == ".ctor"))
            throw new MissingMethodException("Step-35.0.21 GodotSharp diagnostic requires at least one Godot.Collections.Dictionary`2 constructor.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.OS" && method.Name == "GetCmdlineArgs"))
            throw new MissingMethodException("Step-35.0.21 GodotSharp diagnostic requires Godot.OS.GetCmdlineArgs.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeCalls" && method.Name == "godot_icall_0_108"))
            throw new MissingMethodException("Step-35.0.21 GodotSharp diagnostic requires Godot.NativeCalls.godot_icall_0_108.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeInterop.InteropUtils" && method.Name == "EngineGetSingleton"))
            throw new MissingMethodException("Step-35.0.21 singleton localization requires Godot.NativeInterop.InteropUtils.EngineGetSingleton.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeInterop.InteropUtils" && method.Name == "UnmanagedGetManaged"))
            throw new MissingMethodException("Step-35.0.21 singleton localization requires Godot.NativeInterop.InteropUtils.UnmanagedGetManaged.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeInterop.NativeFuncs" && method.Name == "godotsharp_engine_get_singleton"))
            throw new MissingMethodException("Step-35.0.21 singleton localization requires NativeFuncs.godotsharp_engine_get_singleton.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeInterop.NativeFuncs" && method.Name == "godotsharp_internal_unmanaged_get_script_instance_managed"))
            throw new MissingMethodException("Step-35.0.21 singleton localization requires NativeFuncs.godotsharp_internal_unmanaged_get_script_instance_managed.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeInterop.NativeFuncs" && method.Name == "godotsharp_internal_unmanaged_get_instance_binding_managed"))
            throw new MissingMethodException("Step-35.0.21 singleton localization requires NativeFuncs.godotsharp_internal_unmanaged_get_instance_binding_managed.");
        if (!selected.Values.Any(method => method.DeclaringType.FullName == "Godot.NativeInterop.NativeFuncs" && method.Name == "godotsharp_internal_unmanaged_instance_binding_create_managed"))
            throw new MissingMethodException("Step-35.0.21 singleton localization requires NativeFuncs.godotsharp_internal_unmanaged_instance_binding_create_managed.");
        ...
        MethodDefinition tryGetValue)
    {
        ...
        if (cctor.DeclaringType.FullName != CommandLineHelperTypeFullName || cctor.Name != ".cctor" || !cctor.IsStatic || !cctor.HasBody)
            throw new InvalidDataException($"Step-35.0.21 managed CommandLine dictionary rewrite refuses unexpected cctor: {cctor.FullName}.");
        if (tryGetValue.FullName != CommandLineHelperTryGetValueFullName || !tryGetValue.HasBody || !ReferenceEquals(cctor.DeclaringType, tryGetValue.DeclaringType))
            throw new InvalidDataException($"Step-35.0.21 managed CommandLine dictionary rewrite refuses unexpected TryGetValue method: {tryGetValue.FullName}.");
        ...
        if (systemCollectionsScopes.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 requires exactly one existing System.Collections AssemblyRef in sts2; found {systemCollectionsScopes.Length}.");
        ...
        if (argsFields.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 expected exactly one static CommandLineHelper._args field; found {argsFields.Length}.");
        ...
        if (!argsField.FieldType.FullName.StartsWith("Godot.Collections.Dictionary`2<System.String,System.String>", StringComparison.Ordinal))
            throw new InvalidDataException($"Step-35.0.21 expected CommandLineHelper._args to use the exact Godot string dictionary before compatibility rewriting; observed {argsField.FieldType.FullName}.");
        ...
        if (godotDictionaryCtorSites.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 expected exactly one Godot string-dictionary constructor in CommandLineHelper..cctor; found {godotDictionaryCtorSites.Length}.");
        ...
        if (godotDictionarySetterSites.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 expected exactly one Godot string-dictionary set_Item in CommandLineHelper..cctor; found {godotDictionarySetterSites.Length}.");
        ...
        if (godotDictionaryTryGetValueSites.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 expected exactly one Godot string-dictionary TryGetValue in CommandLineHelper.TryGetValue; found {godotDictionaryTryGetValueSites.Length}.");
        ...
        if (naturalGetCmdlineArgsCount != 1)
            throw new InvalidDataException($"Step-35.0.21 requires the natural Godot.OS.GetCmdlineArgs call to remain exactly once after the dictionary-only compatibility rewrite; observed {naturalGetCmdlineArgsCount}.");
        ...
        bool requireNaturalGetCmdlineArgs = true)
    {
        var argsField = commandLineType.Fields.SingleOrDefault(field => field.Name == "_args" && field.IsStatic)
            ?? throw new InvalidDataException("Step-35.0.21 serialized CommandLineHelper._args field is missing.");
        if (argsField.FieldType.FullName != ManagedStringDictionaryFullName)
            throw new InvalidDataException($"Step-35.0.21 serialized CommandLineHelper._args field is not the managed string dictionary: {argsField.FieldType.FullName}.");
        if (argsField.FieldType.Scope is not AssemblyNameReference argsScope || argsScope.Name != "System.Collections")
            throw new InvalidDataException($"Step-35.0.21 serialized managed CommandLineHelper._args field scope drifted from the existing System.Collections contract: {argsField.FieldType.Scope}.");
        ...
        static MethodReference RequireOne(MethodDefinition method, string name)
        {
            var matches = method.Body.Instructions
                ...
                .ToArray();
            if (matches.Length != 1)
                throw new InvalidDataException($"Step-35.0.21 serialized managed CommandLine rewrite expected exactly one {ManagedStringDictionaryFullName}::{name} in {method.FullName}; found {matches.Length}.");
            return matches[0];
        }

        var ctor = RequireOne(cctor, ".ctor");
        if (ctor.Parameters.Count != 0 || !ctor.HasThis)
            throw new InvalidDataException("Step-35.0.21 serialized managed Dictionary<string,string> constructor MemberRef drifted.");
        ...
        {
            throw new InvalidDataException("Step-35.0.21 serialized managed Dictionary<string,string>.set_Item MemberRef is not encoded as set_Item(!0,!1).");
        }
        ...
        {
            throw new InvalidDataException("Step-35.0.21 serialized managed Dictionary<string,string>.TryGetValue MemberRef is not encoded as TryGetValue(!0,!1&).");
        }
        ...
        if (residualGodotDictionaryReferences.Length != 0)
            throw new InvalidDataException($"Step-35.0.21 serialized CommandLine methods retained {residualGodotDictionaryReferences.Length} Godot string-dictionary call reference(s).");
        ...
        if (naturalGetCmdlineArgsCount != expectedNaturalGetCmdlineArgsCount)
            throw new InvalidDataException($"Step-35.0.21 serialized managed dictionary clone Godot.OS.GetCmdlineArgs call count drifted: expected {expectedNaturalGetCmdlineArgsCount}, observed {naturalGetCmdlineArgsCount}.");
    }
        ...
        MethodReference managedProvider)
    {
        ...
        if (cctor.DeclaringType.FullName != CommandLineHelperTypeFullName || cctor.Name != ".cctor" || !cctor.IsStatic || !cctor.HasBody)
            throw new InvalidDataException($"Step-35.0.21 managed command-line provider rewrite refuses unexpected cctor: {cctor.FullName}.");
        ...
        {
            throw new InvalidDataException($"Step-35.0.21 managed command-line provider has unexpected signature: {managedProvider.FullName}.");
        }
        ...
        if (naturalSites.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 expected exactly one natural Godot.OS.GetCmdlineArgs call before the bounded provider rewrite; found {naturalSites.Length}.");
        ...
        MethodDefinition cctor)
    {
        ...
        var naturalCount = cctor.Body.Instructions.Count(instruction =>
            ...
            callee.Name == "GetCmdlineArgs");
        if (naturalCount != 0)
            throw new InvalidDataException($"Step-35.0.21 managed command-line compatibility clone retained {naturalCount} natural Godot.OS.GetCmdlineArgs call(s).");
        ...
        if (provider.Parameters.Count != 0 || provider.ReturnType.FullName != "System.String[]")
            throw new InvalidDataException("Step-35.0.21 serialized managed command-line provider signature drifted.");
        ...
        {
            throw new InvalidDataException("Step-35.0.21 serialized managed command-line provider is not exactly new string[0].");
        }
        ...
        if (providerCallCount != 1)
            throw new InvalidDataException($"Step-35.0.21 serialized managed command-line provider call count drifted: {providerCallCount}.");
    }
    ...
    {
        var argsField = commandLineType.Fields.SingleOrDefault(field => field.Name == "_args" && field.IsStatic)
            ?? throw new InvalidDataException("Step-35.0.21 natural-recon CommandLineHelper._args field is missing.");
        if (!argsField.FieldType.FullName.StartsWith("Godot.Collections.Dictionary`2<System.String,System.String>", StringComparison.Ordinal))
            throw new InvalidDataException($"Step-35.0.21 natural-recon CommandLineHelper._args no longer uses the exact Godot string dictionary: {argsField.FieldType.FullName}.");
        ...
        if (ctorCount != 1 || setterCount != 1 || tryCount != 1)
            throw new InvalidDataException($"Step-35.0.21 natural-recon Godot string-dictionary references drifted: ctor={ctorCount}, set_Item={setterCount}, TryGetValue={tryCount}.");
        ...
        if (naturalGetCmdlineArgsCount != 1)
            throw new InvalidDataException($"Step-35.0.21 natural-recon clone changed the Godot.OS.GetCmdlineArgs call count: {naturalGetCmdlineArgsCount}.");
    }
    ...
    internal static void InsertCommandLineHelperCriticalBoundaryMarkers(MethodDefinition method, MethodReference emitReference)
    {
        if (method.DeclaringType.FullName != CommandLineHelperTypeFullName || method.Name != ".cctor" || !method.IsStatic || !method.HasBody)
            throw new InvalidDataException($"Step-35.0.21 critical CommandLineHelper cctor markers refuse unexpected method: {method.FullName}.");
        ...
        var dictionaryCtor = original.SingleOrDefault(instruction =>
            ...
            ?? throw new InvalidDataException("Step-35.0.21 could not locate the exact CommandLineHelper _args dictionary constructor.");
        ...
        {
            throw new InvalidDataException("Step-35.0.21 requires the CommandLineHelper _args stsfld immediately after its dictionary constructor.");
        }
        ...
        var getCmdlineArgs = original.SingleOrDefault(instruction =>
            ...
            callee.DeclaringType.FullName == "Godot.OS" &&
            callee.Name == "GetCmdlineArgs")
            ?? throw new InvalidDataException("Step-35.0.21 could not locate Godot.OS.GetCmdlineArgs in CommandLineHelper..cctor.");
        ...
        if (cmdlineStore is null || cmdlineStore.OpCode.Code is not (Code.Stloc or Code.Stloc_0 or Code.Stloc_1 or Code.Stloc_2 or Code.Stloc_3 or Code.Stloc_S))
            throw new InvalidDataException("Step-35.0.21 requires Godot.OS.GetCmdlineArgs to store its result immediately before the stack-neutral POST marker.");
        ...
        foreach (var critical in new[] { dictionaryCtor, dictionaryStore, getCmdlineArgs, cmdlineStore })
        {
            if (IsInstructionBranchTarget(method, critical))
                throw new InvalidDataException($"Step-35.0.21 refuses a critical CommandLineHelper marker at branch target IL_{critical.Offset:X4}.");
        }
        ...
        MethodReference emitReference)
    {
        if (method.FullName != NullPlatformConstructorFullName)
            throw new InvalidDataException($"Step-35.0.21 NullPlatform callsite sweep refuses unexpected method: {method.FullName}.");
        ...
        MethodReference emitReference)
    {
        if (method.DeclaringType.FullName != CommandLineHelperTypeFullName || method.Name != ".cctor" || !method.IsStatic)
            throw new InvalidDataException($"Step-35.0.21 CommandLineHelper cctor sweep refuses unexpected method: {method.FullName}.");
        return InsertDiagnosticCallsiteSweepMarkers(
            method,
            emitReference,
            "INMETHOD_CL",
        ...
        MethodReference emitReference)
    {
        if (method.FullName != CommandLineHelperTryGetValueFullName)
            throw new InvalidDataException($"Step-35.0.21 CommandLineHelper TryGetValue sweep refuses unexpected method: {method.FullName}.");
        return InsertDiagnosticCallsiteSweepMarkers(
            method,
            emitReference,
            "INMETHOD_CLTV",
        ...
        bool failOnBranchTarget)
    {
        ...
        foreach (var instruction in originalInstructions)
        {
            ...
            if (isBranchTarget)
            {
                if (failOnBranchTarget)
                    throw new InvalidDataException($"Step-35.0.21 refuses to sweep branch-target CALLSITE#{callsiteOrdinal:D3} ({callee.FullName}) in {method.FullName}.");
                continue;
            }
        ...
        if (result.Count == 0)
            throw new InvalidDataException($"Step-35.0.21 diagnostic callsite sweep found no eligible managed call/newobj target in {method.FullName}.");
        ...
        string afterMarker)
    {
        ...
        var matches = method.Body.Instructions
            ...
            .ToArray();
        if (matches.Length != 1)
            throw new InvalidDataException($"Step-35.0.21 expected exactly one callsite for {calleeFullName} in {method.FullName}; found {matches.Length}.");

        var callsite = matches[0];
        var isBranchTarget = IsInstructionBranchTarget(method, callsite);
        if (isBranchTarget)
            throw new InvalidDataException($"Step-35.0.21 refuses to place a pre-call marker on branch-target callsite {calleeFullName} in {method.FullName}.");
    ...
    private static Dictionary<DiagnosticExternalConstantTypeKey, TypeCode> CollectDiagnosticExternalConstantTypeRequirements(ModuleDefinition module)
    {
        ...
        void Add(TypeReference declaredType, object? constant, string provider)
        {
            ...
            if (leaf.Scope is not AssemblyNameReference assemblyReference)
                throw new InvalidDataException($"Step-35.0.21 constant provider '{provider}' has unsupported metadata scope '{leaf.Scope?.MetadataScopeType}'.");

            var typeCode = Type.GetTypeCode(constant.GetType());
            if (!IsSupportedDiagnosticConstantTypeCode(typeCode))
                throw new InvalidDataException($"Step-35.0.21 constant provider '{provider}' has unsupported constant storage type {constant.GetType().FullName}.");
            ...
            if (requirements.TryGetValue(key, out var prior) && prior != typeCode)
                throw new InvalidDataException($"Step-35.0.21 external constant type '{leaf.FullName}' has inconsistent storage types {prior} and {typeCode}.");
    ...
    private static TypeReference GetDiagnosticPrimitiveConstantType(ModuleDefinition sourceModule, TypeCode typeCode)
        => typeCode switch
        {
            ...
            _ => throw new InvalidDataException($"Unsupported Step-35.0.21 constant storage type {typeCode}."),
        };
    ...
    private sealed class SelfAuditingConstantMetadataWriteResolver : IAssemblyResolver
    {
        ...
        {
            if (_configured)
                throw new InvalidOperationException("The Step-35.0.21 self-auditing GodotSharp constant-metadata resolver was already configured.");
            _configured = true;
            ...
            foreach (var identity in requirements.Keys.Select(key => key.AssemblyFullName).Distinct(StringComparer.Ordinal))
            {
                ...
                if (matches.Length != 1)
                    throw new InvalidDataException($"Step-35.0.21 GodotSharp source must contain exactly one AssemblyRef for self-audited constant scope {identity}; found {matches.Length}.");
            ...
            foreach (var scopeGroup in requirements.GroupBy(pair => pair.Key.AssemblyFullName, StringComparer.Ordinal).OrderBy(group => group.Key, StringComparer.Ordinal))
            {
                ...
                foreach (var requirement in scopeGroup.OrderBy(pair => pair.Key.TypeFullName, StringComparer.Ordinal))
                {
                    if (requirement.Key.IsNested)
                        throw new InvalidDataException($"Step-35.0.21 GodotSharp diagnostic does not permit nested external constant type synthesis: {requirement.Key.TypeFullName}.");
        ...
        internal void ValidateWriteRequests()
        {
            ...
            if (unexpected.Length != 0)
                throw new InvalidDataException("Step-35.0.21 GodotSharp Cecil serialization attempted an unapproved assembly resolution: " + string.Join(" | ", unexpected));
        }
    ...
    private sealed class DiagnosticConstantMetadataWriteResolver : IAssemblyResolver
    {
        ...
        {
            if (_configured)
                throw new InvalidOperationException("The Step-35.0.21 constant-metadata write resolver was already configured.");
            _configured = true;
            ...
            foreach (var identity in requirements.Keys.Select(key => key.AssemblyFullName).Distinct(StringComparer.Ordinal))
            {
                var matches = sourceModule.AssemblyReferences.Where(reference => reference.FullName.Equals(identity, StringComparison.Ordinal)).ToArray();
                if (matches.Length != 1)
                    throw new InvalidDataException($"Step-35.0.21 source must contain exactly one AssemblyRef for audited constant-metadata scope {identity}; found {matches.Length}.");
                assemblyReferences.Add(identity, matches[0]);
            }
            ...
            foreach (var scopeGroup in requirements.GroupBy(pair => pair.Key.AssemblyFullName, StringComparer.Ordinal).OrderBy(group => group.Key, StringComparer.Ordinal))
            {
                ...
                {
                    if (requirement.Key.IsNested)
                        throw new InvalidDataException($"Step-35.0.21 does not permit nested external constant type synthesis: {requirement.Key.TypeFullName}.");
                    var separator = requirement.Key.TypeFullName.LastIndexOf('.');
                    var typeNamespace = separator < 0 ? string.Empty : requirement.Key.TypeFullName[..separator];
                    var typeName = separator < 0 ? requirement.Key.TypeFullName : requirement.Key.TypeFullName[(separator + 1)..];
                    var enumBase = new TypeReference("System", "Enum", surrogate.MainModule, surrogateName);
        ...
        private static void ValidateAuditedRequirementSet(IReadOnlyDictionary<DiagnosticExternalConstantTypeKey, TypeCode> actual)
        {
            ...
            throw new InvalidDataException("Step-35.0.21 external constant-metadata requirement set drifted from the physically proven Step-32 audit; " + string.Join("; ", detail));
        }
        ...
        internal void ValidateWriteRequests()
        {
            if (_requests.Count == 0)
                throw new InvalidDataException("Step-35.0.21 expected Cecil serialization to use at least one bounded constant-metadata surrogate, but no write-time resolution request occurred.");
            var unexpected = _requests.Where(value => !_surrogates.ContainsKey(value)).Distinct(StringComparer.Ordinal).OrderBy(value => value, StringComparer.Ordinal).ToArray();
            if (unexpected.Length != 0)
                throw new InvalidDataException("Step-35.0.21 Cecil serialization attempted an unapproved assembly resolution: " + string.Join(" | ", unexpected));
        }
