...
public sealed class TransformedRealStS2VeryEarlyInitialization : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (DiagnosticMode == Step35DiagnosticMode.GodotCoreCallbackHandoff)
            {
                var handoff = _callbackHandoff ?? throw new InvalidOperationException("Step-35.0.22 GodotCoreCallbackHandoff mode requires the controlled NativeFuncs.Initialize handoff after Gate B and before Gate C.");
