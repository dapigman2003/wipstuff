...
public sealed class SceneDelegate : UIWindowSceneDelegate
{
    ...
    {
        Console.WriteLine("Step 03: SceneDelegate.WillConnect");

        if (scene is not UIWindowScene windowScene)
        {
            Console.Error.WriteLine(
                $"Step 03: expected UIWindowScene, received {scene.GetType().FullName}");
            return;
        }
        ...
        try
        {
            ...
            Console.WriteLine("Step 03: UIWindow is key and visible");
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"Step 03 startup exception: {ex}");
